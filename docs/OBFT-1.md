...
| Late re-flood within Phase 2 (≤2 honest miss V at `T_commit`, V arrives via late gossipsub before `T_accept_max`) | Defer-state operators σ-emit on V upon arrival; σ-quorum reaches by end of Phase 2. ✓ in ~650ms (incl. 250ms Phase 3). With recommended Δ_2 ≥ 2(D + δ), receiver acceptance window past `T_commit` is `D + δ ≈ 150ms`. | Same — within R=1's envelope. ✓ in ~500ms. | Round 1: PREPARE-pool short; timeout (~2s). Round 2: new leader re-fetches + proposes; succeeds in ~750ms. ✓ in ~2.75s. |
...
| 4. Fake encrypted-presence (k > 0) | Delayed (post-decryption) | **Conditional on slot progressing past prior layers' NR-quorum** — sealed if slot misses early | Very low when surfaced |
| 5. Fake plaintext σ at L_0 | Immediate (partial vs retained V check) | Always when retained-V receivers gossip evidence (MUST-gossip rule) | Very low |
...
| (slack) | 300ms | slot_start + 1.50s | Bundles broadcast at boundary propagate; receivers first-observe through `T_accept_max = 1.65s` (covers receiver acceptance into Phase 2's late-σ-emit window) |
...
| (slack) | 300ms | slot_start + 1.50s | Same propagation/acceptance slack as K=4 |
...
- **OBFT-1's effective propagation envelope is ~`2(D+δ)`** (= 300ms at Config A) — propagation absorption from leader broadcast through receiver acceptance horizon. OBFT(R=2)'s effective envelope is `2D`. Substantially closer than the simpler "`D` vs `2D`" framing — OBFT-1's recommended `Δ_2 = 2(D+δ)` widening recovers most of the partition tolerance OBFT(R=2) provides via round-2 re-flood. The remaining gap is structural: OBFT(R=2) has explicit re-flood between rounds (more robust against gossipsub mesh failures); OBFT-1 relies on continuous gossipsub propagation throughout one long Phase 2.
