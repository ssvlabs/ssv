...
_As landed (see Status): the cache + translation were extracted to a shared `protocol/v2/ssv/runner/obft/proposersig.Cache` so the bare-OBFT and 2abOBFT proposer signers share one copy rather than the hand-aligned per-package sketch above; each adapter injects only its package-local candidate decoders + error prefix._
