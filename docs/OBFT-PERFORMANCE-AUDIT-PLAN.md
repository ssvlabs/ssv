...
- Implementation: **all of Tier-1 (F1-F4) + F5 from Tier-2 landed.** Summary:
  ...
  - **F2** — `proposerSigner.srCache` (map[[32]byte][]byte keyed by sha256(V)) + double-checked `sync.RWMutex` mirror of F3's pattern. Mirrors in both `runner/obft/proposer_signer.go` and `runner/obft/twoab/proposer_signer.go`. B2 re-bench at 17 KB block shows the cache-hit path at **~5.6 µs / 0 allocs** vs the cold path at ~101 µs / 336 allocs — ~18× faster and full alloc elimination. Per-slot saving on the runner-side path (where the same proposerSigner persists across slot calls): **~7-8 ms/slot wall-clock + ~26K allocs/slot avoided**, close to the original audit estimate. Caveat: the validation-layer `Verifier` is constructed per-incoming-envelope ([obft_validation.go:128](message/validation/obft_validation.go:128); same for twoab) — its proposerSigner cache is single-use per envelope, so F2 doesn't help the ~60-100 cold validation-side calls per slot. That separate inefficiency would need to be addressed at the validation-layer construction site (share-cached Verifier), not on the signer itself; tracked as informal followup.
- Remaining:
  ...
  - **Validation-layer Verifier caching** (separate from the F-list) — the per-envelope `NewVerifierFromShare` construction at [obft_validation.go:128](message/validation/obft_validation.go:128) and [twoab_validation.go:93](message/validation/twoab_validation.go:93) re-parses the share's pubkeys + builds a fresh proposerSigner on every inbound message. Cache the Verifier per (validator pubkey, beacon fork) and reuse across envelopes to unlock the F2 win (and the F3 KyberSigner pub-cache, and the BLSSigner share-parse cost) on the validation-layer hot path. Probably a Tier-1-sized fix; not in the original audit because the per-envelope construction wasn't probed until F2's lifetime survey.
