...
- Implementation: **F1 + F3 + F4 + F5 landed (Tier-1 minus F2, plus F5 from Tier-2).** Summary:
...
- Remaining:
  - **F2** (Tier-1, ~8 ms/slot wall-clock + ~27K-allocs/slot GC pressure cut) — cache signingRoot per V on `proposerSigner`. Next-up per the audit's revised sequencing. Would also close the per-msg redundancy in F4's `proposerSigner.VerifyPartialBatch` wrapper when σ-walk batches share a single V (see [OBFT-F4-IMPLEMENTATION-PLAN.md](OBFT-F4-IMPLEMENTATION-PLAN.md) §Risks).
