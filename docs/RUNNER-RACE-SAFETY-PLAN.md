...
- 2abOBFT runner (`protocol/v2/ssv/runner/obft/twoab/`) same (n, K) matrix. Existing fixtures support K=2 only; commit 4 extends to the full K range.
...
4. **Commit 4 — 2abOBFT fixture extension to full matrix**. Extend `buildSmokeCluster` + 2abOBFT schedule helpers to support n ∈ {4, 7} × K ∈ {f+1..N}. Verify 2abOBFT runner works at K > 2 (the protocol supports it but tests have never exercised it). New schedule helper analogous to OBFT base's `compressedTestScheduleForK`. Existing `TestRunProposerSlot_Healthy_n4` + `RealBLS_Healthy_n4` + `RealBLS_SilentL0Leader_NRFallThrough` get matrix-parameterized as part of this commit (they're the existing 2abOBFT scenarios that survive convergence — LateCommit comes in commit 5).

5. **Commit 5 — 2abOBFT LateCommit scenario** (convergence work). Build the 2abOBFT-side equivalent of OBFT base's LateCommit scenario. The 2abOBFT runner has observer-mode Phase-3 (verified by checking the scheduler code); the new scenario asserts that property end-to-end. LateCommit delays a (n, qV)-parameterized subset of Phase-2 NR-Commit messages past the 2abOBFT-equivalent RoundEndOffset. Mirrors OBFT base's existing scenario using 2abOBFT's `smokeBus` + delay predicates.

6. **Commit 6 — 2abOBFT bridge** (full matrix × 3 scenarios). New `race_safety_bridge_test.go` in `protocol/v2/ssv/runner/obft/twoab/`. Same architecture as commit 1 — `recordingSmokeBus` adapted for 2abOBFT's bus shape, `recordValueMsgWire` / `recordNoValueMsgWire` / `recordCommitWire` translators (mirroring `consensustest/twoab/events.go`'s recorders), bridge wraps all 3 scenarios × 8 cells = 24 2abOBFT sub-tests. **Full coverage complete: 48 sub-tests across both protocols.**
...
8. **Commit 8 — Adversarial timing injection** (Coyote-lite). Layer optional artificial pauses inside the `broadcastBus`/`smokeBus` delivery paths to amplify specific race-window classes (e.g., delay-cert-vs-resolve). Bounded scope; ~50 lines per bus. Helps surface specific race classes that random scheduling under `-race` doesn't reliably hit.
