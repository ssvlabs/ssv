...
- Variant C: D has no Phase-1 σ_V. D issues a Phase-2a verdict (σV(V) | σV(V') | NR | silent) and a Phase-2b action. Outcomes by D's behavior:
  ...
  - **D defects (verdict σV(V) + Phase-2b NR-emission OR Phase-2b silent abstention)**: `verdict_pool[V] = 3 ≥ qV` (counted from D's verdict). A, B σ-emit; D withholds σ contribution. σ-pool actual = 2 (A + B); NR-pool = 1 (C) + ≤ 1 (D: 1 if NR-emit, 0 if silent) ≤ 2 < qEnc. **Slot misses at L_0.** Cluster does not fall through (NR-pool short). D's verdict-vs-action mismatch is slashable evidence (Rule 6) under NR-emit; under silent abstention only behavioral evidence (verdict envelope on the wire while D's Phase-2b commit lacks the L_0 partial). Either way, evidence does not save the slot. **Strictly worse than OBFT base, which would have succeeded.**
...
- **2-1 equivocation byz-defect grief** — strictly worse than bare OBFT (regression). Byz that controls 1 vote out of 4, equivocates V/V', delivers V to 2 honest, V' to 1 honest, verdict-claims σV(V), then withholds σ contribution at Phase-2b — either by NR-emit or by silent abstention. Slot misses cleanly either way; Rule-6 evidence is on the wire under NR-emit, only behavioral under silent abstention (no Phase-2b partial to pair with the verdict). Bare OBFT would have succeeded via Phase-1 σ_V lock. The rational-byzantine deterrent absorbs this across many slots — but per-slot, an adversarial byzantine ignoring the deterrent can grief more reliably than under bare OBFT.
