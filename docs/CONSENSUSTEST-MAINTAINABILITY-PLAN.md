...
- **Safety mode — dropped.** A "collect violations, fail at end" stress mode would be additive only; fail-fast `SafetyPanic` (with its seed + trace repro) is sufficient, so this was cut rather than deferred.

**Reviewer-flagged cleanups (assessed):**
- *Committed trace golden* — **DONE** (`TestTraceGolden` + `testdata/trace_*.golden`). Pins the exact event sequence + decided outcome for a representative scenario per protocol (direct mode), plus one mesh case for the desim transport sequence. Stronger net than the determinism test (which only proves run-to-run reproducibility): behavior-preserving-but-different drift now fails against a committed baseline. Regenerate intentional changes via `UPDATE_GOLDEN=1 go test … -run TestTraceGolden`. (Added after the refactor settled — the earlier mid-refactor brittleness objection no longer applies.)
