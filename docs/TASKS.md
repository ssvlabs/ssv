...
- [ ] **Add `drand/tlock` to `go.mod`.** Deferred to after protocol core (Phase 2) is testable on the stub. The stub's interface contract is stable enough that the real impl can be dropped in without protocol-level refactoring.
- [ ] **Verify Option A composes cryptographically.** Specifically: a herumi/bls-eth-go-binary partial signature on a tag (produced with an SSV operator share) must be aggregatable + usable as a `tlock` decryption key for a ciphertext encrypted to the same tag under the validator's pubkey. If not, fall back to Option B (separate DKG).
