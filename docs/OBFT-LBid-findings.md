...
`OBFT.md` references use **section names** (durable across edits, no line numbers to drift). `.go` and `2abOBFT.md` references are line numbers against the current tree (those files are unchanged by this remediation). The remediation analyzed here has since landed in `OBFT.md`; this doc was originally written against commit `0911dc631`.
...
Throughout: `n = 3f+1`, `qV = qEnc = 2f+1`; running example `n=4, f=1, qV=qEnc=3`. "Witness" = the `σ_L^V` re-inclusion section of `KindCommit` (`OBFT.md §Phase 2 (wire format)`, §Phase 2 / Wire format).
...
| F6 | **[Independent]** Recommended-`K` contradiction (`OBFT.md §Failure modes` vs five other sites) | Confirmed | Neutral | Neutral | Documentation |
...
1. **`verdict_quorum` forms on `V_X`.** `verdict_pool[V_X] ≥ qV` (`OBFT.md §Convergence rule`) with all verdicts from honest operators means **≥ qV honest verdicted `V_X`**. To verdict `V_X` an operator must have `V_X ∈ bid_set` — i.e. retained-and-host-validated `V_X` by `T_verdict` (`OBFT.md §Mini-consensus`). Retention holds to slot end, and host-validity is *locked* at first-observation (cannot flip; `OBFT.md §Mini-consensus`). So at Phase 2 those ≥ qV honest each satisfy the L_Bid σ predicate (`verdict_quorum ∧ retain V_X ∧ valid`; `OBFT.md §Phase 2 (L_Bid)`) → **≥ qV plaintext σ on `V_X` → σ-quorum → success.** (An honest operator that received `V_X` only in `(T_verdict, T_commit]` verdicted null but still σ's at Phase 2 — the permitted "null verdict + Phase-2 σ" case, `OBFT.md §Slashing-evidence rules` — so it can only add to the pool.)
...
**The single all-honest regression** is aggressive sizing's partial verdict propagation (`Δ_verdict < 1·BTT`; `OBFT.md §Liveness residuals`): all-honest mesh jitter can split the verdict pool 2-2 so neither σ-quorum nor NR-quorum forms, and the no-fall-through gate (`OBFT.md §Phase 3 (L_Bid)`) turns that into a miss bare OBFT would not have. **Therefore Goal B requires excluding aggressive sizing (floor `Δ_verdict ≥ 1·BTT`).** This costs nothing on MEV-fetch at default RefloodDelay (all sizings shift = 0; `OBFT.md §Broadcast-deadline tightening`), and standard sizing is free even at RefloodDelay=0 (`OBFT.md §Broadcast-deadline tightening`).
...
*Source: review item #1. Includes the independent `OBFT.md §Safety (leader-counting)` inconsistency.*

**Claim.** The 2-1-byz-defect residual counts `σ-pool[V_X] = 2 (majority)` (`OBFT.md §Liveness residuals`) and treats candidate-withholding as deadlocking. But the byzantine rotation leader's own Phase-1 partial `σ_B^V(V_X)` is a valid, value-bound partial that is unavoidably leaked, and counting it makes `σ-pool[V_X] = 3 = qV` → success. So the **candidate-withholding-without-equivocation** variant need not deadlock.
...
**Claim (2a).** Verdict-equivocation is harmless standalone: if honest agree on the bid winner, qV honest verdicts on `V_X` already form `verdict_quorum`, and the byz can only *add* to a pool (Pigeonhole on verdicts, `OBFT.md §Convergence rule`), never subtract honest verdicts. So `OBFT.md §Liveness residuals`'s "some see quorum, some don't" requires a pre-existing honest verdict split — i.e. bid divergence, which requires the byz to be a winning rotation leader. The "available to any byz operator every slot" framing (`OBFT.md §Liveness (trigger frequency)`) conflates *action available* with *deadlock-causing*.
...
**Claim (2b).** The real new mechanism is **promotion via bid inflation**. In bare OBFT a byz *deep-layer* leader that equivocates is harmless: the honest L_0 σ-quorums first and the deep layer stays encrypted (`OBFT.md §Phase 3 (L_Bid)` walk halts at first σ-quorum). Under L_Bid, the same byz can inflate `bid_value` to promote its equivocated candidate to the **outermost gating layer** L_Bid, then equivocate with tight timing → L_Bid deadlocks → the honest L_0 below stays sealed under `nr_tag_LBid` → **miss**. This raises the equivocation-miss surface from **1/n** (byz must be the L_0 leader) to **K/n** (byz any rotation leader + bid lie).

**Verdict: correct, and the sharpest point in the review.** The outermost-gate design (`OBFT.md §Why L_Bid is the outermost gate`) is exactly what lets a deep-layer byz "reach up" and block the whole onion. The 1/n → K/n escalation is the concrete no-fall-through cost.

**Attestation is liveness-load-bearing.** Relay/builder attestation (`OBFT.md §Optional extension`) caps `bid_value` at what a recognized relay committed to, so the byz cannot promote a low-real-value candidate by inflation — it can only reach L_Bid when it genuinely holds the top bid. That bounds *promotion frequency*, i.e. it is a **liveness** mechanism, not only the MEV-honesty mechanism the doc files it as (`OBFT.md §Bid-value honesty`). Without attestation (institutional-honesty deployments), the byz gets the full K/n promotion surface.
...
- Add the attestation→promotion-frequency coupling to §Liveness (currently only in §bid-value honesty), cross-referencing `OBFT.md §Optional extension`.

**Safety (Goal A).** No change — these are documentation corrections.

**Open question.** Is there a design lever (short of relay-attestation) that lets L_Bid *fall through* on detected equivocation instead of deadlocking? This trades against the no-fall-through gate (`OBFT.md §Why L_Bid is the outermost gate`) and bid-routing priority; worth a dedicated analysis if adversarial-liveness is to be pushed further (secondary goal).
...
**(5a)** Bid-value honesty is a genuinely new trust assumption (`OBFT.md §Bid-value honesty`) and — per F2 — attestation is also liveness-load-bearing. The doc already says attestation is "recommended for SSV proposer duty" (`OBFT.md §Bid-value honesty`); the gap is a missing *liveness* cross-reference, not absent guidance. **Action: strengthen the cross-ref (point §Liveness at attestation), don't re-argue.**

**(5b)** "Zero MEV-fetch cost under default RefloodDelay" (`OBFT.md §Slot timing`) is a true *budget* statement, but it reuses the reflood buffer as mini-consensus headroom — "free" assumes the buffer is idle (healthy mesh). Under degraded mesh the cost is paid as **L_Bid success rate** (more fall-through to vanilla L_0), which is already documented as the success-rate gradient (`OBFT.md §Per-layer windows`). The only issue is the "essentially free" headline (`OBFT.md §Deployment envelope`) not cross-referencing it. **Action: cross-ref the success-rate gradient at the "free" headline; no new caveat.**
...
`OBFT.md §Failure modes` calls **`K = n = 4`** the "recommended OBFT default for proposer duty," directly contradicting the **`K = f+1` (=2)** default stated in five other places (`OBFT.md §Setting` and `OBFT.md §Application` (five sites)). Likely stale text in the §Failure modes backup-cascade paragraph (where K=n maximizes fall-through depth, which may have leaked into a "default" phrasing). This is exactly the ambiguity F4 probes, so it should be resolved in the same pass.

**Action.** Reconcile `OBFT.md §Failure modes` to the `K=f+1` default (or, if K=n is genuinely intended for proposer duty, fix the other five sites — but the five-vs-one weight and the F4 analysis both favor `K=f+1`).
...
**The inconsistency.** Rule 8 (`OBFT.md §Slashing-evidence rules`) makes "broadcast `KindBidVerdict(σV(V_X))` **and** emit Phase-2 NR on `nr_tag_LBid`" **self-contained slashable**. But the L_Bid commitment rule (`OBFT.md §Phase 2 (L_Bid)`) says an operator σ's only if it *observes* `verdict_quorum`, **else → NR**. So an operator that verdicts `V_X` but does not observe `verdict_quorum` (the cluster didn't converge) takes the honest fall-through (NR) — which is exactly the Rule-8 pattern. **Honest operators in a benign verdict-split emit self-incriminating evidence a byzantine can harvest and submit.**
...
**Proposed fix (required for Goal B).** Make verdicts **non-binding predictions**: `OBFT.md §Phase 2 (L_Bid)` (verdict_quorum-gated σ, else NR) is the single commitment rule. **Narrow Rule 8 to its double-verdict arm only** (two distinct `KindBidVerdict` envelopes = slashable); drop the verdict-vs-action arm. By symmetry with the already-permitted reverse pattern (null-verdict→σ, `OBFT.md §Slashing-evidence rules`), verdict-`V_X`→NR must also be permitted.
...
**Safety (Goal A).** Unaffected — verdicts bind no threshold partials (`OBFT.md §Safety (L_Bid)`); this changes only slashing-rule scope and the σ/NR commitment wording.
...
- Its recovery is toward **σ-quorum** — V-drop operators stay harvest-eligible and *upgrade* to σ on the majority value (`OBFT.md §A.2 (2abOBFT comparison)`). But the promotion attack makes operators *observe* the equivocation, and the equivocation rule forces NR — they cannot upgrade to σ on `V_X`.
...
**Recommendation (Goal-B-secondary): do not add a defer state to L_Bid.** It cannot close the residual at the running config and re-imports the wire-kind + dynamic-Phase-2b cascade OBFT deliberately sheds (`OBFT.md §A.2 (2abOBFT comparison)`). Manage the residual with F1 (closes half 1), relay-attestation (bounds *frequency* — the byz reaches L_Bid only with a genuine top bid, F2), and slashing + assumption 4 (deterrence). The L_Bid-specific amplification — the no-fall-through gate seals the *whole onion* rather than one layer — is the deliberate bid-routing-priority trade-off (`OBFT.md §Why L_Bid is the outermost gate`); F1 narrows how often the cluster reaches the sealed state. (At larger `n`/`f` the cohort algebra loosens, so a defer state could recover more — but the running and recommended config is `f=1, n=4`, where it cannot.)
...
The appendix's safety basis (`OBFT.md §Safety (L_Bid)`) is sound and unchanged by these findings: the mini-consensus binds no threshold partials (verdicts and bid-metadata contribute 0 to σ/NR pools, `OBFT.md §Safety (L_Bid)`); `nr_tag_LBid` is a deeper chained gate under Pigeonhole 3's induction (`OBFT.md §Safety (L_Bid)`).
...
3. **[Goal A-consistency + adversarial, high — impl change] Specify the L_Bid σ-pool harvest** (F1): value_root-keyed witness for the winner, with the safety paragraph. Resolves the `OBFT.md §Safety (leader-counting)` inconsistency and closes the no-equivocation 2-1-byz-defect **at f=1** (partial mitigation at f≥2 — see §Generalization). This is an *implementation* change (the codebase σ-pool is layer-keyed), to be carried when L_Bid is built.
...
5. **[Docs, trivial — ship now] Resolve the `K` default contradiction** (F6): reconcile `OBFT.md §Failure modes` to the `K=f+1` default. One-line factual fix — **don't gate it on F4's analysis** (unbundled).
6. **[Doc-correctness, medium] Add the `K > f+1` caveat to "When to use it"** (F4): `OBFT.md §When to use it` currently sells L_Bid's MEV upside without noting it is near-vacuous at the recommended `K=f+1`. Pick a stance on `qBid` at small K and state it *there*, not only in this plan.
