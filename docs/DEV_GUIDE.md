...
1) As a base64-encoded private key:
    ...
    ```
   This private key can be generated using the CLI command ./bin/ssvnode generate-operator-keys which generates the RSA public key and private key in base64 format.
   However, for a more secure approach, it's recommended to use an encrypted PEM file with a password. This adds an extra layer of security as the private key is not only encoded but also encrypted.

2) As a Keystore PEM file encrypted with a password:
    ```yaml
    KeyStore:
