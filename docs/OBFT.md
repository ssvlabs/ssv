...
**2abOBFT remains OBFT's natural recovery-scope extension** at +1 RTT cost for the patterns OBFT does not close — equivocation σ-locked split (1-1-1 at L_0), validity-divergence beyond the host's stabilization window, and mesh-flakiness deadlocks under cross-phase exclusivity. (h_V=1 selective Phase-1 delivery, previously in this list, is now closed in bare OBFT in-protocol via the §Phase 2 peer-reflood-V mechanism under healthy mesh; the degraded-mesh residual remains a 2abOBFT-exclusive recovery.) For deployments where these residual patterns matter (high-MEV proposer slots, small adversarial-byz clusters, frequent re-org rates, mesh-flakiness conditions), 2abOBFT closes them in-protocol rather than relying on assumption 4 alone.
