...
| Recovery scope vs OBFTR(R=2) | **OBFT recovers via K-layer fall-through to deeper backup leaders (loses MEV freshness per fall-through); OBFTR(R=2) recovers via round-2 re-flood at L_0 (preserves MEV) at +1200ms latency.** Per-layer staggered budgets give OBFT narrower absorption at the deepest layer (~1100ms at L_3 = `B_3 + slack`) than OBFTR(R=2)'s ~1500ms uniform cross-round retention — OBFT trades absorption width for spec simplicity. All other failure modes (byzantine grief, view-divergence) are R-invariant — same exposure. |
