...
**Δ_2 sizing.** `Δ_2 ≥ 1 BTT` minimum (propagation budget for `KindCommit` messages emitted by `T_commit` to reach all honest by start of Phase 3 in the worst-case synchronous-fallback scenario). **Recommended for production: `Δ_2 = 2 BTT`** to absorb mesh-jitter and per-operator processing variance — one full propagation cycle of slack on top of the P99 propagation budget. At Config A (P99=150ms, δ=50ms): minimum `Δ_2 = 200ms`, recommended `Δ_2 = 400ms`. Concrete tables and downstream timing throughout this document use the recommended sizing. (`Δ_2`'s widening absorbs propagation jitter for the worst-case `T_commit`-anchored emit — distinct purpose from Phase 1's per-layer convergence buffer inside `B_k`; see [§Setting](#setting).)

**Δ_3 sizing.** `Δ_3 ≥ ε_3` (BLS aggregation + IBE decryption walk + certificate construction). At Config A: `Δ_3 ≈ 50ms`. Phase 3 begins when all expected `KindCommit` messages have arrived (i.e., at `T_commit + Δ_2`); reconstruction is local-CPU work. The slot's hard wall is the relay submission deadline `T_relay_cutoff − T_submit`, not a fixed Phase-3-end deadline — a slow operator's reconstruction can spill into submission slack, and `KindCertificate` gossip from a faster peer (see §Final-certificate gossip) lets an operator that hasn't completed local reconstruction submit `(V, S)` directly. (At the §Application max-MEV anchor `T_commit = Relay_cutoff − 3 BTT = 3400ms`, the post-`T_commit` budget of 600ms = 3 BTT decomposes as `Δ_2` (400ms) + `Δ_3` (50ms) + `header_submit_headroom` (100ms) + ~50ms residual jitter buffer between Phase-3 completion and cert/submit start. The jitter buffer is the slack between the optimistic Phase-3-complete time and the latest cert-broadcast start that still meets `Relay_cutoff`.)
...
### Phase 3 — Local decryption and reconstruction (target completion `T_commit + Δ_2 + Δ_3`; opportunistic Resolve from `T_commit` onward)
...
| Adversarial scheduling — adversary delays V at all K layers beyond their respective per-layer absorption | **Out of envelope** (Class A). ✗ Slot misses. The deepest layer's `B_{K-1} = T_commit = 3400ms` at K=4 SSV operating point is the cluster-wide tolerance ceiling. | If delay ≤ OBFTR(R=2)'s cross-round retention (~1500ms at this operating point): in envelope at R=2; round 2 re-flood may resolve at L_0. ✓ in ~1.8s. Else: out of envelope. ✗ | Round 1: PREPARE delayed → timeout (~2s). Round 2: new leader; if adversary delays again → round 2 timeout. ✗ Within 4s, QBFT tolerates ≤ 1 round of adversarial delay. |
| Sustained partition (real propagation > all layers' absorption) | OBFT deepest-layer absorption `B_{K-1} = T_commit ≈ 3400ms` at K=4 SSV operating point; exceeded → ✗ slot misses. Safety holds. | OBFTR(R=2) cross-round retention ~1500ms at this operating point (recovers at L_0 via re-flood, preserves MEV); exceeded → ✗ slot misses. Safety holds. | QBFT round-budget × RT exceeded; slot misses. ✗ Safety holds. |
...
- **OBFT-family > QBFT in latency and multi-leader-failure**: OBFT's healthy path is ~600ms (vs ~1600ms QBFT-SSV at recommended sizing); K-layer parallel fall-through is in-round (vs QBFT's serial round-change at ~3.6s per round-change cycle, exceeding the 4s budget at K-1=3 silent leaders).
...
| Termination (output guaranteed) | Conditional. **One-liner: consensus expected to complete by `slot_start + 3.85s` at SSV proposer-duty operating point (n = 4, f = 1, BTT = 200ms, K = 4, T_commit = 3.40s, recommended Δ_2 = 2 BTT = 400ms, Δ_3 = ε_3 ≈ 50ms), with submission slack to `slot_start + 4.00s` for relay submit (`header_submit_headroom = 100ms` plus ~50ms additional jitter buffer); under conditions: (a) ≤ f operators byzantine/offline, (b) real propagation from leader L_k broadcast to any honest first-observation ≤ that layer's per-layer budget `B_k` (reflood-aware staggered: `B_k_shallow = (k+2)·BTT + RefloodDelay`, `B_3 = T_commit` at K=4 — see §Setting), (c) host validity unanimous at decision time (assumption 3), (d) `K ≥ 3` (late-leader resilience — the SSV default `K = n = 4` already satisfies this).** Single-round protocol; per-layer staggered budgets give the deepest layer the entire slot's commit budget for absorption (`B_{K-1} = T_commit`, leader broadcasts at BFT_start), with shallower layers tightening for MEV freshness. |
...
**Operating point.** `BTT = 200ms`, `header_submit_headroom = 100ms`, `Δ_2 = 2 BTT = 400ms` recommended (KindCommit propagation + jitter), `Δ_3 = ε_3 ≈ 50ms` local CPU.

**Derived anchors.** `T_commit = Relay_cutoff − 3 BTT = 3400ms`. (The 3 BTT post-`T_commit` budget = `Δ_2 + Δ_3 + header_submit_headroom + ~50ms jitter buffer` ≈ 600ms; the `T_commit` anchor is BTT-rounded for design simplicity, leaving ~50ms unallocated as additional submit slack.)

The 600ms after `T_commit` decomposes as: `Δ_2` (2 BTT = 400ms; **scales with BTT** — propagation budget for KindCommit fan-out) + `Δ_3` (`ε_3 ≈ 50ms`; **absolute** — local CPU work, doesn't scale with BTT) + `header_submit_headroom` (100ms; **absolute** — cert broadcast + relay HTTP submit, doesn't scale with BTT) + ~50ms residual jitter buffer. At higher BTT the post-`T_commit` budget is dominated by `Δ_2` and the absolute components matter less in BTT terms. Sizing recommendation: keep `Δ_3` and `header_submit_headroom` as absolute quantities and let `Δ_2` scale.
...
| 1900 | `V_2` broadcast (`T_commit − (4·BTT + RD) = 3400 − 1500`) | Targets up to **1500ms** (4·BTT + RD); MEV-fetch budget **~1750ms** |
| 2100 | `V_1` broadcast (`T_commit − (3·BTT + RD) = 3400 − 1300`) | Targets up to **1300ms** (3·BTT + RD); MEV-fetch budget **~1950ms** |
| 2300 | `V_0` broadcast (`T_commit − (2·BTT + RD) = 3400 − 1100`) | Targets up to **1100ms** (2·BTT + RD); MEV-fetch budget **~2150ms** (freshest relay-bundle) |
| 3400 | `T_commit` (= `Relay_cutoff − 3 BTT`) | View-fix deadline; receivers stop counting Phase-1 bundles. `KindCommit` worst-case emit (fallback when `T_L_0_observed > T_commit`, i.e., silent `L_0`); typical-healthy emit fires earlier at `T_L_0_observed ≈ 3400ms − 1·BTT` per §Phase 2 emission-timing |
...
| 3850 | Phase 3 complete | Local IBE-walk + BLS aggregation + certificate; `Δ_3 ≈ 50ms`. Worst-case anchor; healthy case completes earlier |
| 4000 | `Relay_cutoff` | Cert broadcast + relay submit fit in `header_submit_headroom = 100ms`; ~50ms residual jitter buffer |

**Recovery scope.** Within Phase 3's single reconstruction walk: silent V_0 → fall through to V_1; silent V_0 + V_1 → V_2; silent V_0 + V_1 + V_2 → V_3 (always honest at f=1 by pigeonhole). For `h_V=1` selective-Phase-1-delivery at any layer, **the slot misses at that layer with no fall-through** — bare OBFT has no in-protocol recovery for this pattern; the rational-byzantine deterrent (Assumption 4) bounds it across slots. Per-layer budgets `B_k` (= the broadcast offset before `T_commit` shown in the rows above): V_0 covers real propagation up to 1100ms (2·BTT + RD), V_1 up to 1300ms (3·BTT + RD), V_2 up to 1500ms (4·BTT + RD), V_3 up to 3400ms (the entire commit budget). Beyond V_3's budget — i.e., real propagation > T_commit — is out-of-envelope and slot-misses cleanly.

**MEV-fetch-budget asymmetry.** V_0's ~2150ms (at default RefloodDelay=700ms) is the freshest budget; deeper layers trade fetch time for wider propagation budget. Per-leader budgets at this operating point: `[V_3: ~0ms (slot start), V_2: ~1750ms, V_1: ~1950ms, V_0: ~2150ms]`. At RefloodDelay=0 (fully-meshed cluster opt-out) the budgets shift to `[V_3: ~0ms, V_2: ~2450ms, V_1: ~2650ms, V_0: ~2850ms]`. The staggered design lets V_0 capture maximum MEV under healthy propagation while deeper backups absorb tails when V_0's bundle doesn't reach in time; the deepest layer fetches at slot start (deepest-confirmed parent) and broadcasts immediately, giving the cluster the entire commit budget for that bundle's propagation.
...
| 2 | OBFT V_0 | **~2150ms** | **2850ms** | Best BFT-consensus leader for proposer duty |
| 3 | OBFT V_1 | ~1950ms | 2650ms | |
| 4 | OBFT V_2 | ~1750ms | 2450ms | |
...
**Comparison vs partial-sigs floor**: OBFT V_0 pays a **~1200ms BFT-consensus tax** at default RefloodDelay (3350 − ~2150ms = ~6 BTT), or a **~500ms tax** at RD=0 (3350 − 2850 = 2.5 BTT). The tax decomposes as: **`B_0` + per-layer fetch overhead** = `(2·BTT + RefloodDelay) + ~150ms RANDAO-anchor`. The 2·BTT shallow base covers 1 BTT P99 leader-broadcast propagation + 1 BTT IWANT round-trip; RefloodDelay covers one full IHAVE/IWANT cycle for mesh-flaky receivers. Both protocols at recommended 2 BTT per Phase-2 emission. QBFT-SSV R1 pays a **3200ms tax** (>2.5× larger even vs OBFT at default RD), structurally constrained by needing PROPOSE_1 to fire early enough that consensus + post-consensus + R2 retry (with RT=2s) fit the slot.

**Comparison OBFT vs QBFT**: at default RefloodDelay=700ms, OBFT V_0's ~2150ms MEV-fetch budget matches QBFT R2's 2150ms — but OBFT V_0 is the *primary* leader (always tried first; no round-timeout gap), while QBFT R2 is only reachable after R1 fails (paying the ~2s round-change cost). At RD=0 (fully-meshed opt-out), OBFT V_0 captures **700ms more** MEV-fresh fetch time than QBFT R2 (2850 vs 2150ms). All four OBFT leaders (V_0..V_3) beat QBFT-SSV R1 by ≥1.6s at either RefloodDelay sizing. The MEV-fetch asymmetry is structural: OBFT's K-layer fall-through avoids the round-timeout gap that gates QBFT's R2 fetch.
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. The deadlines:
   ...
   - **`Δ_3 ≥ ε_3`**: Phase 3 is purely local reconstruction processing (BLS aggregation, IBE decryption walk, certificate construction); ≈ 50ms at Config A.
...
| Partial-synchrony envelope (Phase-2 KindCommit propagation) | `Δ_2 ≥ 1 BTT` per round (recommended `Δ_2 = 2 BTT` for jitter absorption) | `Δ_2 ≥ 1 BTT` (recommended `Δ_2 = 2 BTT` — same shape as OBFTR per round). Phase-2 fan-out propagation budget is comparable across the family at recommended sizing |
...
> **Note on values:** This appendix's concrete timing numbers (`B_0 = 1 BTT`, `3050ms` MEV-fetch, `3200ms` broadcast deadline, etc.) reflect the pre-resize bare OBFT schedule. Under the reflood-aware schedule (`B_0 = 2·BTT + RefloodDelay`), all derived numbers shift accordingly. The L_Bid extension's *structural* properties — `B_0_LBid = 0.5·BTT` typical-mesh-only at the bid layer, mini-consensus window, broadcast-deadline-tightening framing — are unchanged; the concrete deltas (vs bare OBFT) need re-derivation under the new schedule. Pending follow-up.
...
The table below shows L_0 broadcast deadline (= MEV-fetch budget at `slot_start = 0`) across BTT regimes (`T_relay_cutoff = 4.0s`, `submit_headroom = 100ms`, `Δ_3 = ε_3 = 50ms`, `Δ_2 = 2 BTT`, `B_0 = 1 BTT` for bare OBFT, `B_0_LBid = 0.5 BTT` for L_Bid). Bare OBFT row included for comparison:
...
> **Note on values:** This appendix's concrete timing numbers reference `B_0 = 1 BTT` from the pre-resize schedule. Under the reflood-aware schedule (`B_0 = 2·BTT + RefloodDelay`), Defer's `T_accept_max` absorption-window framing still applies but the relative magnitudes shift. Pending follow-up.
