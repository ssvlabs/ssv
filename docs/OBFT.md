...
- **K layers** (`max(2, f+1) ≤ K ≤ n`, configurable) with deterministically-derived leaders: layer 0 with **primary leader** `L_0`, layer 1 with **backup leader** `L_1`, ..., layer `K-1` with `L_{K-1}`. Leaders must be distinct (`L_i ≠ L_j` for `i ≠ j`). For `K = n`, every cluster member is a leader at exactly one layer per slot. The lower bound `f+1` ensures **at least one layer has an honest leader** (by pigeonhole over the f-byzantine bound) — necessary for BFT liveness, since at K < f+1 all leaders could be byzantine and no σ-quorum reaches at any layer. At `f = 1` the minimum is `K = 2` (matches baseline TBFT); at `f = 2, n = 7` the minimum is `K = 3`; at `f = 3, n = 10` the minimum is `K = 4`. The K choice within the valid range is per-duty: `K = f+1` for proposer duty (smallest K that satisfies the bound, fits 4s relay cutoff with margin); `K ≥ f+2` for duties with longer budgets (attestation, sync committee) or where non-byzantine multi-failure tolerance is desired.
