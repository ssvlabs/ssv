...
Three failure modes at the bid layer, all resulting in slot-miss without fall-through (chained encryption to L_0 stays sealed when L_Bid has neither σ-quorum nor NR-quorum). The first two are **Class B** (permitted byzantine grief within the f-bound; same taxonomy as bare OBFT's [§Failure modes](#failure-modes)) — slot-miss per-slot, slashable evidence on the wire, deterred across slots by [assumption 4](#implications-of-the-rational-byzantine-deterrent-assumption-4). The third is **Class A** (assumption 3 violation) — same hard algebraic limit as bare OBFT's L_0 validity-divergence:

- **[Class B — cryptographically slashable] 2-1-byz-defect.** Byz engineers a `verdict_quorum` on `V` that some honest do not retain at Phase 2 emit, casts its own verdict on `V` to make the quorum, then defects to NR partial at Phase 2. Two trigger patterns reach the same algebraic shape:
  - **Bid-equivocation trigger.** Byz sends `V` to majority-honest and `V'` to minority-honest. Slashable via Rule 7 (bid-equivocation) AND Rule 8 (verdict-vs-action equivocation) — both cryptographically self-contained.
  - **Bid-withholding trigger.** Byz sends `V` to majority-honest and withholds from minority-honest. Minority-honest still has `|bid_set_i| = n − f`, verdicts on its own non-`V` argmax. Slashable via Rule 8 (verdict-vs-action equivocation) cryptographically; the bid-silence component is behavioral-pattern only (no second envelope, so Rule 7 does not fire) — see [§Slashing-evidence rules / Evidence quality](#slashing-evidence-rules) for the silence-grief evidence character.

  At f=1, n=4 either trigger: σ-pool[V] = 2 (majority-honest who retain V and σ-emit on it); NR-pool = 1 (minority honest, did not retain V) + 1 (byz NR) = 2 < `qEnc`. Deadlock.
- **[Class B — cryptographically slashable] Verdict equivocation at L_Bid.** Byz issues different verdicts to different peers, fragmenting per-peer verdict-pool views. Some honest see `qV`-quorum on `V_X` (and σ-bind); others don't (and NR). σ-pool < `qV`; NR-pool < `qEnc`. Same algebraic deadlock, same slot-miss-without-fall-through. Slashable via Rule 8.
...
| L_Bid liveness — 2-1-byz-defect (bid-equivocation or bid-withholding trigger) | n/a | **Class B** (slashable Rule 8 always; Rule 7 also under bid-equivocation trigger; assumption-4 deterred across slots): slot-miss-without-fall-through |
