...
- **[Class B — closed by design]** **Byzantine selective-delivery grief at end of Phase 2 (h_V = 1 deadlock) — NOT a failure mode in current OBFT.** Earlier OBFT-family designs included a Defer state with a "no-V fallback" rule (receivers without V deferred their NR-decision based on observed peer σ-claims). That rule was the structural enabler of the h_V=1 selective-delivery attack: a byzantine L_0 could withhold Phase-1 from all honest, emit an auth-signed σ-claim, selectively deliver V to exactly one honest, and engineer a deadlock where σ-pool = 2 < qV and NR-pool = 2 < qEnc. **The current OBFT spec removes the Defer state**: receivers without V at `T_commit` immediately NR per the silent-leader rule, so the byzantine's withholding pattern produces NR-quorum at L_0 and clean fall-through to L_1 instead of a deadlock. This is one of the structural improvements bought by Defer removal at small clusters where partition recovery is rarely needed.
...
| h_V=1 selective-delivery deadlock | **Closed by design** (no Defer state, no no-V fallback rule — see §Failure modes Class B closed-by-design entry) | **Closed** via Phase-2a verdict pool |
...
- **h_V=1 deadlock is not relevant for current OBFT.** That attack required the Defer-state's no-V fallback rule, which the current spec does not have (see §Failure modes "Closed by design"). Re-broadcast is therefore not a defense against any failure mode the protocol still exposes.
