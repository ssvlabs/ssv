...
- **Per-layer leader broadcast deadlines `T_broadcast_max_k`** — OBFT uses **asymmetric per-layer broadcast budgets**: the primary `L_0` broadcasts latest with the smallest propagation budget (= freshest MEV); each backup broadcasts progressively earlier with a progressively wider propagation budget. The cluster falls through to whichever layer's bundle actually arrived by `T_commit`.
  ...
  **`B_k` is a target absorption budget, not a hard runtime cap on broadcast time.** The only runtime acceptance gate is `T_commit`: receivers admit bundles first-observed in `[slot_start, T_commit]` regardless of which layer they came from. `B_k` parameterizes the *staggered* design — it tells each leader *when to aim to broadcast by* so that, under partial-synchrony assumptions, the bundle reaches all honest first-observation within the layer's per-layer budget. A leader that cannot meet `T_broadcast_max_k` does best-effort: broadcast as soon as the bundle is ready. The cluster still benefits from any in-envelope propagation that happens to complete before `T_commit`.
  - **Clamp at 0 when `B_k ≥ T_commit`.** The `max(0, ·)` form means the layer's broadcast target is `T_broadcast_max_k = 0` — leader broadcasts at slot start, best effort. This is the *default* configuration for the deepest layer (the recommended `B_{K-1} = T_commit`): the deepest leader fetches from a deepest-confirmed parent at slot start and broadcasts immediately, giving the cluster the entire commit budget for that bundle's propagation. Custom schedules with `B_k > T_commit` (a pure target overshoot) hit the same clamp; the layer's effective absorption window is `min(B_k, T_commit) = T_commit`. Configurations where two-or-more layers all clamp to 0 are rejected by validation (no fall-through depth remains: per-layer fetch times must be strictly decreasing in `k`, and two layers clamped to fetch at slot start collide).

  **Sizing intuition.** `L_0`'s budget covers the optimistic case (nominal propagation, maximum MEV fetch); deeper backups absorb progressively wider propagation tails. Concrete K=4 sizing for SSV proposer duty (in §Application): `B_0 = 1 BTT`, `B_1 = 1.5 BTT`, `B_2 = 2.5 BTT`, `B_3 = T_commit` (= 3400ms at Config A, deepest "earliest possible" — leader broadcasts at slot start). The trade-off: `B_0`'s tighter budget gives the primary the longest fetch window (best MEV) but the least propagation safety margin; if real propagation from L_0's broadcast exceeds `B_0` (= bundle arrives past `T_commit`), the cluster falls through to L_1 (whose `B_1` is wider), and so on, until the deepest layer whose budget covers the entire slot.
...
1. **Phase 1**: each leader `L_k` broadcasts its Phase-1 bundle by its per-layer deadline `T_broadcast_max_k = max(0, T_commit − B_k)` (with `B_k` ordered `B_0 < B_1 < ... < B_{K-1}` so deeper layers have wider propagation budgets — see §Setting). Receivers accept bundles first-observed in `[slot_start, T_commit]`.
...
| Partial-synchrony absorption | Per-layer staggered: `B_k` for layer L_k. At K=4 Config A: `B_0 = 1 BTT = 200ms` for primary L_0 (tightest, freshest MEV) up to `B_{K-1} = T_commit = 3400ms` for deepest L_3 (earliest possible — broadcasts at slot start, last-resort tail covering the entire slot). Cluster falls through layers based on which one's bundle actually arrived by `T_commit`. |
...
| `T_broadcast_max_k` | per-layer leader broadcast target: `max(0, T_commit − B_k)`. Deeper layers broadcast earlier (wider `B_k`) to absorb wider propagation tails. Target, not hard runtime cap — see §Setting |
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. The deadlines:
   ...
   - **`T_broadcast_max_k = max(0, T_commit − B_k)`** per-layer: leader L_k aims to finish broadcasting by this target so its bundle propagates to all honest before `T_commit` under that layer's per-layer propagation budget `B_k`. See §Setting for the staggered-budget design (`B_0 < B_1 < ... < B_{K-1}`; deepest `B_{K-1} = T_commit` clamps target to slot start).
...
| Partial-synchrony envelope (Phase-1 bundle propagation) | `R · P99` cross-round retention (e.g., `2·P99` at R=2 — bundles arriving in `(P99, R·P99]` get a fresh chance to be σ-emitted on in round 2) | per-layer staggered `B_k` from `1 BTT` (primary) up to `T_commit` at K=4 deepest backup (full commit budget, leader broadcasts at slot start); cluster falls through to whichever layer's bundle arrived in time. No cross-round retention (single round) |
