...
- **Leader broadcast deadline** `T_broadcast_max = T_commit − 2(D + δ)` where `D` is the propagation P99/P999 budget and `δ` is the cluster's clock-skew bound. Each layer's leader must finish broadcasting by this time so that under worst-case propagation, all honest first-observe by `T_commit − (D + δ)` (with one `D + δ` of clock-skew margin before `T_commit`). Per-layer fetch windows are sized to fit: `T_k + Δ_1 ≤ T_broadcast_max` for each leader `L_k`. Bundles whose first-observation time is past `T_commit` at any honest receiver are simply not counted by that receiver toward σ-quorum at this layer; the cluster relies on K-layer fall-through (a deeper backup layer's bundle propagating in time) for the slot to succeed.
...
**Phase 2 timing.** Each operator emits exactly one `KindCommit` message at `T_commit`. The Phase 2 window `[T_commit, T_commit + Δ_2]` is sized for that message to propagate to all honest peers before Phase 3 reconstruction begins.
...
**Δ_3 sizing.** `Δ_3 ≥ ε_3` (BLS aggregation + IBE decryption walk + certificate construction). At Config A: `Δ_3 ≈ 100ms`. Phase 3 begins when all expected `KindCommit` messages have arrived (i.e., at `T_commit + Δ_2`); reconstruction is local-CPU work.
...
**`T_round_end`** is the cutoff by which the operator runs the reconstruction walk. Practically, `T_round_end = T_commit + Δ_2 + Δ_3` where `Δ_2 ≥ D + δ` (KindCommit propagation) and `Δ_3 ≥ ε_3` (reconstruction processing). For SSV proposer duty, T_round_end is a planning target rather than a hard wall; submit slack between T_round_end and the relay's submission deadline lets a still-running reconstruction spill over (see Application section).
...
1. **Phase 1**: K leaders broadcast their Phase-1 bundles per their per-layer fetch windows (`[T_{K-1}, T_{K-1} + Δ_1]`, ..., `[T_0, T_0 + Δ_1]`), with `T_0 + Δ_1 ≤ T_broadcast_max = T_commit − 2(D + δ)`. Receivers accept bundles first-observed in `[slot_start, T_commit]`.
...
**Slot timing**: `T_round_end = T_commit + Δ_2 + Δ_3`. Phase 1 fetch occupies `[slot_start, T_commit]`. The slot's consensus budget (Phase 2 + Phase 3) is `Δ_2 + Δ_3 ≈ (D + δ) + ε_3` ≈ 250ms at Config A.
...
- **Partial synchrony for liveness**: messages eventually deliver within bounded delay `D` (propagation P99/P999) and clock skew `δ`. The leader broadcast deadline is `T_broadcast_max = T_commit − 2(D + δ)`; bundles arriving past `T_commit` are not counted. Phase 3's reconstruction deadline is `T_round_end = T_commit + Δ_2 + Δ_3`. Safety holds against arbitrary network adversaries; only liveness depends on synchrony.

  **OBFT's effective absorption window** for a single layer: `D + δ` (≈ 150ms at Config A). Bundles arriving past `T_commit` at any honest receiver are not counted by that receiver toward σ-quorum at this layer; the cluster relies on K-layer fall-through to a deeper backup whose bundle did propagate in time. Multi-round OBFTR (R ≥ 2) extends absorption via cross-round retention; OBFT trades that for spec simplicity at small clusters where asymmetric propagation is rare.
...
If propagation between honest operators stays bounded by `D + δ`, the protocol terminates with a V signature on some layer (the first layer where σ-quorum reaches under cluster-wide V receipt by `T_commit`, or the deepest layer reachable via NR fall-through with a valid backup leader). If propagation exceeds `D + δ` for any single layer, the cluster falls through to a deeper backup. If all K layers fail to propagate in time, the slot misses. **Safety holds in either case.**
...
**Adversarial scheduling within partial synchrony**: the network adversary can delay messages by up to `D + δ`.
...
For wider adversarial-scheduling tolerance (delays beyond `D + δ` at any single layer), use [OBFTR(R=2)](OBFTR.md).
...
|---|---|---|---|
| Healthy (all honest receive V_{L_0}) | σ-quorum reaches in 2 RTTs. ✓ at L_0 in ~250ms (Phase 2 + Phase 3 minimum). | Same. ✓ at L_0 in ~500ms. | PROPOSE→PREPARE→COMMIT (3 RTTs) + post-consensus (1 RTT). ~750ms. ✓ |
| Byzantine leader silent | 0 σ-emits → all honest NR (silent-leader rule) → NR-quorum at L_0 reaches in Phase 2 → automatic fall-through to L_1 in Phase 3 walk; σ-quorum at L_1 if leader honest. ✓ in ~250ms. | Same. ✓ in ~500ms. | Round 1: no PROPOSE arrives; round timeout (~2s). Round 2: new leader proposes; succeeds in ~750ms. ✓ in ~2.75s. |
| Asymmetric propagation (≤1 of 3 honest miss V at T_commit) | Other 2 honest σ-emit on time; σ-pool = 2 + leader = qV. ✓ at L_0 in ~250ms. The miss-honest's NR partial is unused. | Same (within OBFT's absorption). ✓ in ~500ms. | Round 1: PREPARE-pool short; timeout (~2s). Round 2: re-fetch + propose; succeeds in ~750ms. ✓ in ~2.75s. |
| Asymmetric propagation (≥2 of 3 honest miss V at T_commit) | σ-pool < qV at L_0; cluster falls through to L_1 (whose bundle did propagate in time). ✓ at L_1 in ~250ms (assuming L_1 honest and propagation OK). | Within OBFTR(R=2)'s wider absorption: round 2 re-flood may deliver V to the miss-honest; σ-quorum at L_0 reaches in round 2. ✓ in ~1.15s. | Round 1: timeout. Round 2: new leader; succeeds in ~750ms. ✓ in ~2.75s. |
| Byzantine leader equivocates, 2-1 split where one V already has 2 honest + leader-σ_L^V | σ-pool on the 2-honest-V = 2 + leader = 3 = qV; reaches naturally. ✓ in ~250ms. | Same. ✓ in ~500ms. | Round 1: PREPARE-pool split; timeout. Round 2: new leader proposes; succeeds. ✓ in ~2.75s. |
...
- **OBFT fits at higher D where OBFTR(R=2) doesn't.** At D = 500ms: `T_broadcast_max = 1.5s − 2·550ms = 0.40s` (very tight fetch budget; needs T_commit moved later); `Δ_2 = D + δ = 550ms`; Phase 2 + Phase 3 = 550 + 100 ≈ 650ms. With T_commit pushed to slot_start + 1.5s (allowing 1.5s fetch budget): consensus ends at slot_start + 2.15s with 1.85s submission headroom. OBFTR(R=2) at the same D fails to fit the 4s relay cutoff. **High-D networks running proposer duty should use OBFT, with T_commit anchor adjusted accordingly.**
...
- `Δ_2 ≥ D + δ` (`KindCommit` propagation budget).
- `Δ_3 ≥ ε_3` for local reconstruction processing.
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. The deadlines:
   ...
   - **`T_broadcast_max = T_commit − 2(D + δ)`**: leader broadcast deadline. Each layer's leader must finish broadcasting by this time so that under worst-case propagation, all honest first-observe by `T_commit − (D + δ)`.
   ...
   - **`Δ_2 ≥ D + δ`**: `KindCommit` propagation budget — operators emit `KindCommit` at `T_commit`, peers must receive it before Phase 3.
...
- Use **OBFT** when the cluster's gossipsub propagation is reliably within `D + δ` (typically n=4 or n=7 where the mesh is dense enough that asymmetric propagation past `T_commit` is rare); or when the deployment runs at higher D (≈ 300–500ms) where OBFTR(R=2) doesn't fit; or when spec/EKM simplicity and submission headroom outweigh OBFTR(R=2)'s wider absorption.
...
- **All-honest-NR equivocation recovery.** When byz delivers V's early enough for re-flood to spread conflicts before T_commit, all 3 honest retain ≥ 2 V's and emit NR per the equivocation rule; NR-quorum at L_0 → fall-through to L_1. Same recovery as QBFT but in single round (~250ms vs ~2.75s).
...
Phase 3 reconstruction starts at `T_accept_max + (D+δ)` (after late emissions propagate). The cluster relies on K-layer fall-through if neither σ-quorum nor NR-quorum reaches at any layer.
...
- **Wider Phase-2 window**: `Δ_2 = W + (D + δ)` instead of bare OBFT's `Δ_2 = D + δ`. Phase 3 starts at `T_accept_max + (D+δ)` instead of `T_commit + (D+δ)`.
...
- **Bare OBFT**: `T_round_end = T_commit + (D+δ) + Δ_3`. Phase 3 starts at `T_commit + (D+δ)` (after KindCommit propagates) and completes at `T_round_end`.
- **OBFT+Defer**: `T_round_end = T_accept_max + (D+δ) + Δ_3 = T_commit + W + (D+δ) + Δ_3`. Phase 3 starts at `T_accept_max + (D+δ)` (after late emissions propagate) and completes at `T_round_end`.
...
#### MEV impact (T_broadcast_max — leader fetch deadline)

**Defer does not compress MEV.** Under fixed `T_round_end`:

- **Bare OBFT**: `T_broadcast_max = T_commit − 2(D+δ) = T_round_end − 3(D+δ) − Δ_3`.
- **OBFT+Defer**: `T_broadcast_max = T_accept_max − 2(D+δ) = T_round_end − 3(D+δ) − Δ_3`.

Same `T_broadcast_max`. The `2(D+δ)` leader-broadcast safety margin applies relative to `T_accept_max` instead of `T_commit`, but `T_broadcast_max` relative to `T_round_end` is unchanged.

The intuition that "Defer adds W to `T_broadcast_max ↔ T_commit`" treats `T_commit` as the broadcast deadline. With Defer, `T_commit` is the early-signal point; `T_accept_max` is the broadcast deadline. With Defer, `T_commit` shifts earlier by W to fit the absorption window, but `T_broadcast_max` is anchored to `T_accept_max` (the late horizon), which is anchored to `T_round_end − (D+δ) − Δ_3` either way.
...
- Larger Phase-2 window: `Δ_2 = W + (D+δ)` vs bare OBFT's `Δ_2 = D + δ`.
...
- **MEV is not a primary driver**: since `T_broadcast_max` is invariant to Defer, MEV isn't the trade-off lever; the lever is recovery scope vs spec/wire complexity vs Variant A exposure.
...
2. **The MEV cost intuition is a misread.** Defer doesn't compress `T_broadcast_max` — under the binding `T_broadcast_max = T_round_end − 3(D+δ) − Δ_3` equation, the leader fetch deadline is invariant to Defer. The `2(D+δ)` safety margin is anchored to `T_accept_max` (with Defer) or `T_commit` (without), but `T_round_end` minus that anchor is the same in both cases. What does shift is `T_commit` (W earlier with Defer, to make room for the absorption window before Phase 3 starts).
...
5. **OBFT+Defer vs bare OBFT trade summary**:
   ...
   - **=** Same `T_broadcast_max` (leader MEV fetch deadline) under fixed `T_round_end`.
   - **=** Same healthy-path completion time (= `T_round_end`).
