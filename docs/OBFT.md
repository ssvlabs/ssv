...
4. **Persistent operator set with rational-byzantine deterrent.** OBFT operates within a stable SSV cluster running protocol instances over many slots. The deterrent is the same one that already disciplines an offline operator under SSV's network-wide threat model: per-validator operator fees flow continuously to all cluster operators regardless of per-slot contribution (the remaining `n − f` honest carry the work at zero ops cost to the silent/byzantine), and stakers — who can observe per-cluster slot-miss rates — are expected to migrate validators away from underperforming clusters, collapsing the silent/byzantine operator's fee accrual to zero. SSV is already designed for the operator-down case ("the cluster and stakers deal with it"); the rational-byzantine claim is that a byzantine operator gains nothing an offline operator wouldn't already get, and has reputation (persistent across slots) to lose.
   ...
   **Asymmetry — Byzantine vs Down — and what restores equivalence.** With QBFT, `Byzantine ≡ Down` automatically: round-change rotates past silent or malformed PROPOSE/PREPARE/COMMIT, so the worst a byzantine can do per-slot is silently going offline. OBFT has no round-change escape valve, so byzantine is *significantly worse on latency than Down* — equivocation σ-locked splits, fake-encrypted-presence, and behavioral σ-refusal can engineer per-slot grief above what equivalent offline behavior would produce. (`h_V=1` selective-delivery, historically a grief vector at this asymmetry, is now closed by the Phase-2.5 NR-flip mechanism — see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock).) The expected mitigation is **manual blacklisting**: the cluster's surviving `n − f` operators agree out-of-band on the misbehaving operator's identity, push a config-file update to their nodes treating that operator's messages as silent for subsequent slots, and the byzantine's effective contribution becomes identical to offline — restoring the `Byzantine ≡ Down` guarantee. **The blacklist mechanism is a planned protocol extension; the current OBFT spec does not specify it.** Until added, the byzantine's per-slot grief surface above offline behavior is bounded only by stakers eventually migrating validators away from the cluster.
...
**Byzantine ≡ Down in QBFT, significantly worse on latency than Down in OBFT-family — manual blacklist is the equalizer.** QBFT's round-change makes any byzantine deviation functionally indistinguishable from operator silence: round 1 times out, round 2 succeeds with a different leader, byzantine pays the round-1-timeout latency and nothing more. OBFT-family has no round-change escape valve; byzantine grief vectors at L_0 (equivocation σ-locked splits, fake-encrypted-presence, behavioral σ-refusal) can engineer reliable per-slot slot-miss when the byzantine is L_0 — typically ~25% of slots at f=1 n=4 with uniform leader rotation. (`h_V=1` selective-delivery is no longer in this list as of the Phase-2.5 NR-flip mechanism — see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock).) The grief above offline behavior is the residual the deterrent must absorb.
...
**An asymmetry to keep in mind: the deterrent is weakest where per-slot grief is most damaging.** Faults with cryptographically self-contained evidence (equivocation, cross-signing, cross-onion partial-sig equivocation, fake plaintext σ at L_0) are single-observation blacklistable, but these are also the faults the protocol *already* defends against at the slot level (natural σ-recovery on equivocation 2-1 splits; detect-and-reject for fake plaintext σ). Faults with only behavioral evidence (byz σ-refusal coordinated with mesh-flakiness, validity-divergence with byz passivity) are the *most damaging* per-slot grief vectors — they reliably miss the slot at L_0 when an adversarial byz exercises them — and they're the slowest to blacklist with confidence. (`h_V=1` selective-delivery used to be in this list; Phase-2.5 NR-flip closes it within the slot.)
...
**Why both signatures.** Including `σ_{L_k}^V(V_{L_k})` in the Phase-1 bundle gives the cluster a *head start* of one real threshold partial on `V_{L_k}` as soon as Phase 1 succeeds anywhere. Combined with honest threshold partials produced in Phase 2 by operators who received the leader's bundle, the cluster reaches `qV` real partials on `V_{L_k}` whenever ≥ qV-1 honest received the bundle (= the typical byzantine-leader case). The remaining algebraic gap — when *exactly* one honest received (h_V=1) — is closed by the Phase-2.5 NR-flip mechanism via fall-through (see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock) and "Fault tolerance / Liveness").
...
The Phase-2.5 NR-flip mechanism (see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock)) does **not** require an additional sub-window in `Δ_2`: NR-flip is a Phase-3 reconstruction sub-mechanism, emitted by σ-ers detecting deadlock during their local reconstruction at `T_commit + Δ_2`. The emitter includes their own NR-flip partial in their local pool aggregation immediately and reconstructs locally — propagation to other operators is best-effort (via gossipsub + final-certificate gossip) and not load-bearing for cluster slot success.
...
- **Leader σ_L^V witness section.** For every Phase-1 bundle the operator has retained at this point (per §Phase 1's retention rules — typically one bundle per layer, two per layer in the equivocation-observed case), a `(layer k, value_root, σ_{L_k}^V(V_{L_k}))` triple extracted from the bundle. The `value_root` is a 32-byte identifier (e.g., `sha256(V)`) sufficient for the σ-pool to cross-reference against any V the receiver has retained from Phase-1 receipt; the witness does NOT carry the full V. These are byte-for-byte copies of the leader's σ partial as `i` observed it — **not** new signings by `i` (no EKM event, no new signing obligation, no new cryptographic primitive). The section provides redundancy against σ_V-drop at peer receivers who DID receive V from Phase 1: a peer can harvest σ_L^V from `i`'s witness section into the layer-`k` σ-pool (subject to the layer's chained-decryption gate when `k > 0`). Bandwidth is small: per witness ≈ 4 + 8 + 32 + 96 + length-prefix overhead ≈ 145 bytes (Layer + Leader + ValueRoot + σ partial); cluster-wide at K=4, n=4 ≈ 16 witnesses × 145 ≈ 2.3 KB.

  The mechanism does **not** address V-drop — receivers without V locally cannot use a witnessed σ_L^V (σ_L^V verifies against the leader's signing target, which requires V; receivers with only `value_root` lack the bytes needed to reproduce the verification). V-drop receivers rely on `KindCertificate` gossip (see §Final-certificate gossip) for V recovery — a faster peer who reconstructed `(V, S)` gossips the cert, and the V-drop receiver consumes it directly. See [Appendix C](#appendix-c--message-re-broadcast-considerations) for full-bundle re-broadcast that would address V-drop additionally at higher bandwidth cost (optional defensive engineering, not core).

Auth-envelope binding: `(protocol_tag = "OBFT-v1", message_kind = "commit", cluster_id, slot, operator_id i, onion_payload, nr_partials, sigma_L_witnesses)` signed by `i`'s operator-identity key. Emitted at most once per operator per slot. Receivers reject any `KindCommit` whose envelope auth fails verification. (An operator may additionally emit at most one `KindNRFlip` per `(slot, layer)` during Phase 2.5 if the per-layer deadlock predicate is satisfied — see §Phase 2.5 for the wire format.)
...
**Per-operator commitment is exclusive across phases.** OBFT enforces cross-phase exclusivity. The commitment is *one decision per operator per layer, spanning Phase 1 and Phase 2*, with one exception (the Phase-2.5 NR-flip — see below):

- An operator who emitted `σ_i^V(V_{L_k})` at layer `k` on any value `V` has σ-side committed at this layer; they may **not** σ on a different `V'` at the same `(slot, layer)` — see Pigeonhole 2 in "Fault tolerance / Safety". They may emit NR on `nr_tag_k` **only via the Phase-2.5 NR-flip mechanism with valid trigger evidence** (see §Phase 2.5 below); without trigger evidence, NR-after-σ is grief / Rule-1 cross-signing.
- An operator who emitted an NR/NV partial on `nr_tag_k` has NR-side committed at this layer; they may **not** subsequently emit σ on any V at L_k. (σ-after-NR remains strictly forbidden — there is no symmetric "σ-flip" mechanism; see §Phase 2.5 for the asymmetry rationale.)
- The layer-`k` **leader**'s Phase-1 σ_V counts as their σ-side commitment at layer `k`. They may emit NR/NV on `nr_tag_k` only via the Phase-2.5 NR-flip mechanism with valid trigger evidence.
...
**Mechanism.** At `T_commit + Δ_2`, each operator computes the per-layer pool snapshot from observed KindCommits. For each layer `k`, the operator evaluates the **deadlock predicate**:

```
Deadlocked(k) ≡ |nr_pool[k]| ≥ f+1                         AND
                ∀V ∈ retained_V_at_k:
                    |honest_σ_pool[k][V]| + f < qV          (= 2f+1)
```

The first condition (`≥ f+1` NR partials) ensures **at least one honest operator has emitted NR at this layer** (under the f-byz bound, byzantine alone can contribute at most f NR partials; ≥ f+1 implies an honest contribution). The second condition ensures σ-quorum on any retained V is **algebraically unreachable** even if all f byzantine operators contribute σ later — so fall-through is the only path forward.

If `Deadlocked(k)` holds AND the operator has σ-side committed at `k` (= they emitted `σ_i^V(V_{L_k})` at layer k), they emit a `KindNRFlip` message and immediately include their own NR-flip partial in their local pool aggregation:

```
KindNRFlip(slot, layer k, σ_i^IBE(nr_tag_k), trigger_evidence)
```

Where `trigger_evidence` is byte-for-byte copies of `f+1` distinct, auth-verified NR partials at `(slot, k)` from distinct operators that the emitter observed in peer KindCommits. The NR-flip's IBE partial:

- Is a cryptographically valid threshold-IBE share on `nr_tag_k` — verifiable against the cluster's IBE keypair like any NR partial.
- Contributes to `nr_pool[k]` at reconstruction time, advancing it toward `qEnc`.
- Does **not** contribute to or alter `σ_pool[k]` (the operator's σ partial on `V_{L_k}` remains in their KindCommit and counts toward σ-pool unchanged).
- Is paired with the σ partial in the operator's slashing-protection log under the (slot, k) entry. Both partials are co-published; an attestation that the NR-flip's `trigger_evidence` is *valid* is what distinguishes this from grief cross-signing.

**Local-reconstruction sufficiency at n=4, f=1 (= why no separate Δ_flip budget at this size).** Once the σ-er emits NR-flip, they include it in their **own** local pool aggregation immediately — no waiting for propagation. At `n=4, f=1`, the deadlock case has `k=1` honest σ-er and `h_NR=2` honest NR-partials in the σ-er's local view (the 2 honest who didn't retain). After the own flip: `2 + 1 = 3 = qEnc` locally — **sufficient to derive `nr_tag_k`'s decryption key, decrypt L_{k+1} onion contents from peer KindCommits, and reconstruct `V_{L_{k+1}}` if σ-quorum reaches at `k+1`**, all without waiting for NR-flip propagation. The emitter then submits `(V_{L_{k+1}}, S)` directly to the relay AND broadcasts a `KindCertificate` (see §Final-certificate gossip) for redundancy. At `n=4`, this lone-reconstructor path is verified correct (see [docs/OBFT-formal-verif.md §7](OBFT-formal-verif.md)); cluster slot success is independent of NR-flip propagation timing.

**Caveat at n ≥ 7.** The "local-only suffices" argument is **n=4-specific**. The general algebraic condition for an emitter to unlock locally without waiting on peer NR-flips is `h_NR + 1 ≥ qEnc`, i.e., `h_NR ≥ 2f`. In deadlock, `h_NR = |Honest| − k = 2f+1 − k`, so local-only suffices iff `k ≤ 1`. At n=4 (where the only deadlock is k=1), this always holds. At `n=7, f=2`, the `k=2` deadlock case (= 2 honest received V, 3 honest NR'd) leaves each σ-er with `h_NR = 3` locally; after own flip = 4 < qEnc=5. Each σ-er must additionally receive the *other* σ-er's NR-flip (or any byz NR-flip if the byz also flipped under non-grief) before they can unlock — adding up to 1 BTT of σ-er-to-σ-er propagation latency in the worst case at this size. The same pattern recurs at higher n with more `k > 1` deadlock cases. Deployments at `n ≥ 7` should account for this in `Δ_2` sizing if they rely on within-Δ_2 reconstruction completion: one practical option is `Δ_2 ≥ 3 BTT` (= 1 BTT KindCommit + ≤ 1 BTT inter-σ-er NR-flip + jitter slack) at production sizing for n ≥ 7. The alternative is the `KindCertificate` gossip fallback: even when σ-ers can't unlock within `Δ_2`, the slowest-path reconstruction completes within Phase 3 + cert gossip, with deployment-sizing adjustments accordingly. **Concrete tables and timing values throughout this document are at `n=4, f=1` Config A and the zero-MEV-cost claim is verified at that size; n ≥ 7 deployments require independent timing analysis.** The protocol mechanics (commitment lattice, slashing rules, EKM amendment) are unchanged across cluster sizes.

**EKM amendment.** The cross-phase exclusivity rule of [§Phase 2](#phase-2--onion-broadcast-t_commit-t_commit--%CE%94_2) is amended to allow NR-after-σ at `(slot, layer)` iff the operator has accumulated valid `trigger_evidence` locally (= observed `f+1` valid NR partials at this layer satisfying `Deadlocked(k)`'s second clause). The slashing-protection log records both partials with the trigger evidence as proof of the legitimate flip; subsequent independent observers can verify the evidence and confirm the flip is protocol-allowed (see §Slashing-evidence Rule 1 amendment).
...
2. Verifying each NR partial in `trigger_evidence` against its claimed operator's IBE-keypair share.
3. Confirming `trigger_evidence` contains f+1 distinct operators' NR partials.
4. Confirming the receiver's local σ_pool snapshot at this `(slot, layer)` satisfies the second clause of `Deadlocked(k)` — i.e., σ-quorum is algebraically unreachable for any V the receiver has retained.

If all four checks pass, the NR-flip's IBE partial enters `nr_pool[k]` for aggregation. If any check fails, the NR-flip is dropped (and surfaces as Rule-1 cross-signing evidence — see §Slashing evidence).

**Healthy-slot cost: zero.** NR-flip emission is conditional on the deadlock predicate, evaluated locally during Phase 3. Healthy slots (where σ-quorum reaches at some layer) elapse Phase 3 without any operator emitting `KindNRFlip` and without any `Δ_2` extension. The mechanism is lazy: it costs nothing on healthy paths and only fires when a deadlock would otherwise miss the slot.
...
A participant that hasn't received `j`'s `KindCommit` at decryption time treats `j` as not having contributed at any layer: no σ partial, no NR partial. Standard threshold cryptography — only signed messages count. Within Phase 2's hard cluster-wide deadline (`Δ_2 ≥ 1 BTT`), gossipsub propagation is expected to deliver all honest `KindCommit` messages to all honest receivers before Phase 3 starts. Phase-2.5 `KindNRFlip` emissions (issued by σ-ers detecting deadlock during Phase 3) propagate best-effort: receivers that observe an NR-flip in time include it in their own pool aggregation; receivers that don't, fall back to `KindCertificate` gossip from the emitter for cluster-level slot success (see §Phase 2.5 / §Final-certificate gossip).
...
3. **Phase 3** (from `T_commit + Δ_2`): each operator runs the K-layer reconstruction walk. σ-ers detecting per-layer deadlock (per [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock)) emit `KindNRFlip` and immediately include their own NR-flip in their local pool aggregation, reconstructing locally. If σ-quorum reaches on some V at any layer, output the V; halt. If NR-quorum reaches (with or without NR-flip contributions) at some layer `L_C < K-1`, advance L_C and continue the walk. If neither, the slot misses (re-running may incorporate late `KindCommit` / `KindNRFlip` arrivals — see §Phase 3).

**Slot timing**: Phase 1 fetch occupies `[slot_start, T_commit]`. The slot's consensus budget (Phase 2 + Phase 3) is `Δ_2 + Δ_3 ≈ 1 BTT + ε_3` ≈ 250ms at Config A (recommended sizing `Δ_2 = 2 BTT`); consensus is expected to complete at `T_commit + Δ_2 + Δ_3`, leaving the rest of the slot as submission slack to `T_relay_cutoff`. Phase-2.5 NR-flip is a Phase-3 sub-mechanism with no separate budget; the emitter's local reconstruction completes within `Δ_3`.
...
- **Each operator commits σ-or-NR exclusively per (slot, layer), across phases — with one Phase-2.5 exception.** Honest who include σ on any V at layer `k` may not σ on a different V' at the same layer (single-σ-V per operator per layer); honest who broadcast NR/NV may **not** subsequently include σ at L_k (σ-after-NR remains strictly forbidden). The asymmetric exception: honest who σ at L_k may emit a `KindNRFlip` (NR partial on `nr_tag_k`) under valid Phase-2.5 trigger evidence — see §Phase 2.5. Each layer's leader's Phase-1 σ counts as their σ-side commitment — they may also emit NR-flip under trigger evidence but never an unconstrained NR/NV. EKM enforces this cross-phase + single-V exclusivity by coordinating across the operator's V-signing and IBE-signing shares (distinct keys, but slashing-protection log keys on (slot, layer)): an unconstrained NR/NV partial on `nr_tag_k` is rejected if the same operator has previously signed σ at L_k (only the trigger-validated NR-flip path bypasses this); a σ partial on V' at L_k is rejected if the same operator has previously signed σ on V ≠ V' at L_k or any IBE partial (NR or NR-flip) at L_k. Pigeonhole 1 and 2 below rely on these rules.
...
**Log schema.** One row per signing event: `(slot, layer, side, value_root, trigger_evidence)` where `side ∈ {"σ", "NR", "NR-flip"}`; `value_root` is set on σ-side entries, null on NR-side; `trigger_evidence` is set only on `"NR-flip"` entries (= the f+1 NR-partial copies that justify the flip per §Phase 2.5). No round dimension (single-round protocol).

**Per-request checks.**

- **Sign σ on V at (slot, layer)** (V-keypair share): rejected if any prior `(slot, layer, "σ", _, _)` or `(slot, layer, "NR", _, _)` or `(slot, layer, "NR-flip", _, _)` row exists. On success, log `(slot, layer, "σ", value_root(V), null)`. The single-σ-V rule means a second signing attempt with `V' ≠ V` at the same `(slot, layer)` is rejected even though the side matches — the existing row already contains `value_root(V)`.
- **Sign NR on `nr_tag_k` at (slot, layer)** (IBE-keypair share, primary path): rejected if any prior `(slot, layer, _, _, _)` row exists. On success, log `(slot, layer, "NR", null, null)`.
- **Sign NR-flip on `nr_tag_k` at (slot, layer)** (IBE-keypair share, Phase-2.5 path): permitted iff the operator has a prior `(slot, layer, "σ", _, _)` row (= operator is σ-side committed) AND no prior `(slot, layer, "NR", _, _)` or `(slot, layer, "NR-flip", _, _)` row exists AND the request includes valid `trigger_evidence` (= f+1 distinct, auth-verified NR partials at the same (slot, layer); EKM checks signatures against operator pubkeys cached at cluster init). On success, log `(slot, layer, "NR-flip", null, trigger_evidence)`. **This is the only path that emits an IBE partial after a same-layer σ**; the strict NR rule above blocks all other NR-after-σ requests.

The single-round model means no cross-round atomicity, no persistent partial-sig cache, and no deterministic re-signing fallback are required: each (slot, layer, operator) signing event happens at most once per side (σ, NR, or NR-flip), and the σ + NR-flip pair is the only legitimate co-occurrence at the same (slot, layer).
...
- **σ-pool sub-qV but ≥ 1 honest σ-er and ≥ f+1 honest NR-ers** (= the `h_V=1`-style selective-Phase-1-delivery case where σ-quorum is algebraically unreachable but NR-pool from honest-only is also short of qEnc): **the σ-er(s) emit Phase-2.5 NR-flips with valid trigger evidence** (see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock)), pushing nr-pool past qEnc. Chain unlocks to the next layer, where σ-quorum reaches naturally if the next layer's leader is honest and within budget. The deadlock layer itself does NOT reconstruct (σ-pool stays sub-qV by construction — bounded by the trigger condition); cluster output is from the deeper layer.
- **σ-pool sub-qV with all honest NR (= 0 honest σ-ers, leader silent or fully partitioned away)**: NR-quorum reaches at the affected layer naturally (3 honest NR ≥ qEnc at f=1); chain unlocks via the standard NR-quorum path, no Phase-2.5 NR-flip needed.

Phase-2.5 NR-flip closes what was historically OBFT's `h_V=1` algebraic limit (see [Appendix A.2](#a2--comparison-with-2abobftmd) and [Appendix C](#appendix-c--message-re-broadcast-considerations) for the limit's original framing); Class A closure now holds across selective-delivery cases at small clusters, mechanically verified at `n=4, f=1, K=2` (see [docs/OBFT-formal-verif.md §7](OBFT-formal-verif.md)).
...
- *Liveness — adversary delays V to ≥ 2 honest past `T_commit`.* σ-pool < qV at this layer. **Two sub-cases by NR-pool:** (a) all 3 honest delayed (= leader-effectively-silent from cluster view): NR-pool = 3 honest = qEnc, naturally reaches; chain unlocks via standard fall-through. (b) Exactly 2 honest delayed (= "h_V=1" selective-delivery shape): NR-pool = 2 honest < qEnc; the 1 σ-er emits Phase-2.5 NR-flip with valid trigger evidence; nr-pool reaches qEnc; chain unlocks. In both sub-cases, slot succeeds at the deeper backup (assuming its bundle propagated in time and its leader is honest). See [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock).
...
| Asymmetric propagation (≥2 of 3 honest miss V at T_commit, h_V=1 selective-Phase-1-delivery shape) | σ-pool < qV at L_0; the σ-locked recipient emits Phase-2.5 NR-flip during Phase 3 reconstruction, includes their own NR-flip in local pool aggregation, unlocks chain locally, reconstructs `(V_1, S_1)` from L_1 onion contents, and submits + gossips KindCertificate. ✓ at L_1 in ~600ms (lone-reconstructor path; same latency as healthy). The deadlock layer L_0 itself does not reconstruct. Other operators reconstruct locally if they receive the NR-flip in time, otherwise they rely on the certificate gossip for downstream submission. | Within OBFTR(R=2)'s wider absorption: round 2 re-flood may deliver V to the miss-honest; σ-quorum at L_0 reaches in round 2. ✓ in ~2.4s. | Round 1: timeout. Round 2: new leader; succeeds in ~1.6s. ✓ in ~3.6s. |
...
**Phase-2.5 NR-flip exemption.** When the IBE partial is a `KindNRFlip` carrying valid `trigger_evidence` per [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock), the cross-signing is protocol-allowed and **not slashable**. Receivers verifying a (σ, IBE-on-nr_tag_k) pair MUST first check for an accompanying `KindNRFlip` with valid evidence at the same `(slot, layer)` from the same emitter; only if no such valid flip exists does the pair surface as Rule-1 slashable evidence.
...
- **Self-contradiction (σ + NR/NV at the same layer, without valid Phase-2.5 trigger evidence).** If operator `i` emitted `σ_i^V(V)` at some layer AND `σ_i^{IBE}(nr_tag_k)` at the same layer, the dual partials are slashable evidence (cross-signing). **Exemption:** if the IBE partial is a `KindNRFlip` carrying valid trigger evidence per [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock) — at least f+1 distinct, auth-verified NR partials at the same `(slot, layer)` from distinct operators, AND the receiver's local σ-pool snapshot at this `(slot, layer)` satisfies the deadlock predicate (σ-quorum algebraically unreachable for any retained V) — the cross-signing is protocol-allowed and **not slashable**. Receivers verifying the dual partials MUST first check for a paired `KindNRFlip` with valid evidence before treating the pair as slashable. The exemption applies per (slot, layer); cross-signings at any *other* (slot, layer) without valid trigger remain Rule-1 slashable.
...
- **Slot misses at L_0** (no quorum reaches): σ-locked split equivocation, validity-divergence deadlock — each leaves the chained encryption sealed. Compounded with byz fake-presence at deeper layers, byz gets two grief actions per detection. (`h_V=1` selective-delivery is no longer in this list — it now triggers Phase-2.5 NR-flip and falls through to a deeper layer where σ-quorum reaches, unlocking the chained encryption en route. Rule 4 evidence at deeper layers is therefore surface-able in `h_V=1` cases that previously sealed it.)
...
- **[Closed by Phase-2.5 NR-flip]** ~~**Byzantine selective Phase-1 delivery.**~~ A byzantine that broadcasts Phase-1 to exactly one honest historically created an algebraic deadlock at f=1, n=4: σ-pool = 2 (recipient + byz σ_V) and NR-pool = 2 (the two no-V honest; byz and recipient σ-locked, can't NR), with neither reaching qV/qEnc=3. The recipient (σ-locked) now emits a Phase-2.5 `KindNRFlip` with valid trigger evidence (= the f+1 NR partials from the no-V honest) during their Phase-3 reconstruction; they include their own NR-flip in their local pool aggregation, unlock the chain locally, and reconstruct `(V_1, S_1)` from the deeper layer. See [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock). At n=4, the byzantine's grief surface here is **closed in-slot at zero latency cost** (Phase-2.5 fits within `Δ_3`'s local-CPU budget; no extension to `Δ_2`); the recipient submits `(V_1, S_1)` as a lone reconstructor, with `KindCertificate` gossip propagating to other operators for redundancy. (At n ≥ 7, the closure mechanism is the same but `k ≥ 2` deadlock cases may add up to 1 BTT for σ-er-to-σ-er NR-flip propagation — see §Phase 2.5 caveat.) Rational-byzantine deterrent still applies (per-slot cost) since selective-delivery is a behavioral-pattern detection, but the slot DOES produce a V signature.
...
| Byzantine-leader-grief resistance | **Improved with Phase-2.5 NR-flip; still partial under adversarial byz.** `h_V=1` selective-Phase-1-delivery is closed in-slot via Phase-2.5 at zero latency cost at n=4 (no `Δ_2` extension; lone-reconstructor path inside `Δ_3`'s budget); for n ≥ 7, the closure mechanism is the same but absorbing the worst-case `k ≥ 2` propagation delay may need `Δ_2` extension (see §Phase 2.5 caveat). Recovery via "natural" σ-quorum patterns (leader's σ_L^V completes a 2-of-3-honest pool at n=4) handles symmetric splits. **Adversarial byzantine still reliably engineers slot-miss when L_0** via σ-locked split equivocation (1-1-1, 1-1-NR — Phase-2.5 trigger doesn't fire because all honest are σ-locked, NR-pool < f+1). At f=1 n=4 with uniform leader rotation, an adversarial byz primary can deterministically grief ~25% of slots (whenever they're L_0) via residual equivocation patterns; the rational-byzantine deterrent (assumption 4) is the only protocol-level defense for those, and it works *across slots in expectation*, not per-slot. Effective deterrent strength is deployment-specific (stake-to-grief-value ratio, governance responsiveness, slashability evidence quality — see [§Implications of the rational-byzantine deterrent](#implications-of-the-rational-byzantine-deterrent-assumption-4)). |
...
| 3800 | `T_commit + Δ_2` | Hard cluster-wide deadline; σ/NR pools stable; Phase 3 begins (incl. any in-Phase-3 Phase-2.5 NR-flip emissions for deadlocked layers — see §Phase 2.5) |
...
**Recovery scope.** Within Phase 3's single reconstruction walk: silent V_0 → fall through to V_1; silent V_0 + V_1 → V_2; silent V_0 + V_1 + V_2 → V_3 (always honest at f=1 by pigeonhole). For `h_V=1` selective-Phase-1-delivery at any layer, the σ-er(s) at the deadlock layer emit Phase-2.5 `KindNRFlip` during their local reconstruction, include the partial in their own pool aggregation, and reconstruct the deeper-layer V locally — independent of NR-flip propagation timing (see §Phase 2.5 for the local-reconstruction-sufficiency argument). Per-layer budgets `B_k` (= the broadcast offset before `T_commit` shown in the rows above): V_0 covers real propagation up to 200ms (1 BTT), V_1 up to 300ms (1.5 BTT), V_2 up to 500ms (2.5 BTT), V_3 up to 1100ms (5.5 BTT). Beyond V_3's budget is out-of-envelope and slot-misses cleanly.
...
**Phase-2.5 has zero MEV cost.** Phase-2.5 NR-flip is a Phase-3 sub-mechanism, not a separate phase with its own time budget. The σ-er detecting deadlock emits NR-flip and reconstructs locally within `Δ_3`'s existing CPU budget; no extension to `Δ_2` or shift in `T_commit` is needed. V_0's MEV-fetch budget remains 3050ms (unchanged from bare-OBFT-pre-Phase-2.5). Other operators that don't receive the NR-flip in time fall back to `KindCertificate` gossip from the emitter — the existing OBFT lone-reconstructor mechanism for the slot's cluster-level success.
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. The deadlines:
   ...
   - **`Δ_3 ≥ ε_3`**: Phase 3 is purely local reconstruction processing (BLS aggregation, IBE decryption walk, certificate construction, plus any Phase-2.5 NR-flip emission + local pool re-aggregation); ≈ 100ms at Config A.
...
**Phase 2.5 NR-flip — minimal closure of the `h_V=1` selective-delivery limit.** The original simplification accepted the `h_V=1` algebraic limit (selective Phase-1 delivery to one honest, slot-misses at f=1, n=4) as part of the trade. Subsequent formal verification (`tla/BareOBFT_Liveness.tla`, see [docs/OBFT-formal-verif.md §7](OBFT-formal-verif.md)) surfaced this as a Class A leak under within-budget partial-synchrony; closure was added via the Phase-2.5 NR-flip mechanism (§Phase 2.5). The mechanism:
- adds **one new message kind** (`KindNRFlip`) carrying an IBE partial + trigger evidence;
- adds **one EKM amendment** (NR-after-σ allowed at (slot, layer) iff trigger evidence is valid);
- adds **one slashing-rule amendment** (Rule 1 cross-signing exempts trigger-validated NR-flips);
- adds **zero slot-budget items**: NR-flip emission and local re-aggregation fit within `Δ_3`'s existing local-CPU budget; no `Δ_2` extension needed (the emitter reconstructs locally; propagation to other operators is best-effort via gossipsub + KindCertificate fallback).

Phase-2.5 is asymmetric — NR-after-σ allowed under trigger; σ-after-NR remains strictly forbidden under any condition — preserving Pigeonhole 1 cryptographically (σ pool at deadlock layer ≤ 2f < qV). Both safety (Pigeonholes 1, 2, 3) and liveness (Class A closure) are mechanically verified at n=4, f=1, K=2.

The remaining cost is no within-slot partition recovery for *equivocation σ-locked splits or validity-divergence* — bundles past `T_commit` are not counted at the affected layer, the cluster relies on K-layer fall-through to a deeper backup whose bundle did propagate in time, and equivocation grief is bounded by assumption 4 (rational-byzantine deterrent). This is suited for small clusters where the gossipsub mesh is effectively full and equivocation grief is acceptable per the deterrent; the trade-off buys a smaller spec, a simpler EKM, more submission headroom, and a wider deployment envelope at high P99 than 2abOBFT (which closes equivocation in-protocol at +1 RTT cost).
...
| h_V=1 selective-delivery deadlock | **Closed** via Phase-2.5 NR-flip mechanism (see §Phase 2.5) — σ-locked recipient emits `KindNRFlip` with trigger evidence during Phase 3 reconstruction; includes own NR-flip in local pool; unlocks chain locally; reconstructs deeper-layer V; submits as lone reconstructor. Zero `Δ_2` extension (mechanism fits in `Δ_3`'s CPU budget). Both withhold-then-fake-σ (closed as side-effect of Defer removal) and selective Phase-1 delivery (closed via Phase-2.5) variants are in-slot recovered. | **Closed** via Phase-2a verdict pool |
...
- **No additional defense against h_V=1 byz selective-delivery beyond what Phase-2.5 NR-flip already provides**. Historically, the withhold-then-fake-σ variant was closed by the absence of the Defer state, while selective-Phase-1-delivery remained an algebraic limit at f=1, n=4. **The selective-Phase-1-delivery variant is now closed by [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock)** — the σ-locked recipient emits `KindNRFlip` with valid trigger evidence, pushing nr-pool to qEnc and unlocking chain to a deeper layer. Phase-2.5 closes the limit at the σ-commitment layer (not propagation layer); explicit Phase-1 bundle re-broadcast doesn't help further with this case (byz delivered to only one honest, and Phase-2.5 already recovers from there).
...
3. **Explicit re-broadcast addresses gossipsub-layer issues** (mesh sparsity, score-pruning, multi-hop latency) but does not address OBFT's residual adversarial-byz failure modes (σ-locked equivocation, validity-divergence). Those are structural and not fixable at the propagation level. (`h_V=1` selective-delivery used to be in this list; Phase-2.5 NR-flip closes it at the σ-commitment layer.)
...
- OBFT: σ-locked equivocation 1-1-1, validity-divergence — slot-miss patterns at any layer. (`h_V=1` selective-delivery is closed by Phase-2.5 NR-flip — see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock).)
- OBFT-replenish: **identical exposure to bare OBFT (incl. Phase-2.5 closure carrying over).** New layers introduced in later rounds are subject to the same residual byzantine patterns (equivocation, validity-divergence). If byz exercises σ-locked split at L_0 in round 1, the L_0 σ-locks block fall-through; later layers' chained encryption stays sealed under L_0/L_1's nr_tags. Phase-2.5 NR-flip applies per-round and closes h_V=1-style cases identically.
...
4. **Adversarial-byz exposure is unchanged from bare OBFT (incl. Phase-2.5 NR-flip closure of h_V=1)**. Replenish does not provide additional protection against the residual patterns σ-locked equivocation and validity-divergence; closing those would require structural changes orthogonal to replenishment. Phase-2.5 NR-flip carries over to OBFT-replenish per-round and closes selective-delivery deadlocks identically.
...
Variant B (selective Phase-1 delivery — byz broadcasts Phase-1 to exactly one honest, not withholding) was historically an algebraic limit at f=1, n=4 that neither Defer nor Defer-removal addresses at the σ-commitment-timing layer. **Variant B is now closed** at the post-σ-commitment layer via the Phase-2.5 NR-flip mechanism (see [§Phase 2.5](#phase-25--nr-flip-recovery-on-observed-deadlock)) — the σ-locked recipient emits a `KindNRFlip` with valid trigger evidence, enabling fall-through to a deeper layer. So bare OBFT (Defer-removed + Phase-2.5 added) closes both Variant A (via Defer removal) and Variant B (via Phase-2.5) of the h_V=1 family.
