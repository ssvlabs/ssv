...
**Default RefloodDelay** = 700ms (matches SSV's gossipsub HeartbeatInterval per [network/topics/params/gossipsub.go](../network/topics/params/gossipsub.go)). Schedule rows below assume this default; lower RefloodDelay for fully-meshed clusters recovers MEV-fetch headroom (table values shift later by `default − chosen = ΔRefloodDelay`, until the recovery floor saturates the V_0 broadcast at `T_commit − 3·BTT` for `RefloodDelay ≤ 1·BTT` — see §Setting "Recovery margin").
...
- **Under default RefloodDelay=700ms**, every named sizing matches bare OBFT V_0's broadcast deadline across the BTT range where bare OBFT itself fits (BTT ≤ 800ms — the reflood-aware `B_0 = 2·BTT + 700ms`, and the recovery floor `max(RefloodDelay, 1·BTT)` for BTT ≥ 800ms, consume the pre-`T_commit` budget at higher BTT). The buffer-beyond-typical absorbs `Δ_minicon` regardless of BTT — pick `Δ_minicon` purely on L_Bid success-rate vs implementation-complexity grounds, not MEV-fetch.
- **At RefloodDelay=0** (fully-meshed opt-out): with the recovery floor, bare OBFT fits to BTT ≤ 800ms (same as default RD — the floor dominates RefloodDelay for BTT ≥ 800ms), and every sizing remains free on MEV-fetch (the floored `3·BTT` budget absorbs `Δ_minicon`). At BTT ≥ 1000ms bare OBFT's primary clamps to BFT_start so L_Bid is moot.
...
At **RefloodDelay = 0** (fully-meshed opt-out) the recovery floor binds (RD < 1·BTT), holding the L_0 budget at `3·BTT`, so the buffer-beyond-typical is `2.5·BTT = 500ms` — wide enough that **no named sizing overshoots**. All match floored bare OBFT V_0 at `T_commit − 3·BTT = 3000ms` (MEV-fetch ~2850ms):
...
> **Recovery-floor note — the RefloodDelay=0 figures in this appendix predate it.** The recovery margin floor `max(RefloodDelay, 1·BTT)` (see [§Setting](#setting), "Recovery margin") changes the RefloodDelay=0 analysis below. At RD=0 the L_0 budget is floored to `3·BTT` (not `2·BTT`), so: **(i)** bare OBFT V_0's broadcast moves 1·BTT earlier (BTT=200: 3200→**3000ms**; MEV-fetch ~3050→**~2850ms**); **(ii)** the buffer-beyond-typical is `2.5·BTT` (not `1.5·BTT`), which absorbs every named `Δ_minicon` — so L_Bid imposes **zero MEV-fetch cost at RD=0 too** (conservative no longer overshoots; the per-line "conservative pays 0.5·BTT at RD=0" claims below are reversed); **(iii)** the RD=0 BTT envelope shrinks ~1·BTT (bare OBFT fits to BTT ≈ 800ms, not 1200ms). The **default RefloodDelay=700ms figures are unaffected for BTT ≤ 700ms** (floor dormant); for **BTT ≥ 800ms the floor binds at default RD too** (since `1·BTT > 700ms`), so the default-RD table and envelope below shift as well. (Note: L_Bid is spec-only — not in the consensustest adapter — so unlike §Setting/§Application these figures have no data/test backstop; the RD=0 and high-BTT numbers below have been refreshed by hand for the floor.)
...
The L_0 deadline binds whenever `Δ_minicon + B_0_LBid ≤ max(B_0, 3·BTT)` — equivalently, `Δ_minicon ≤ max(B_0, 3·BTT) − B_0_LBid` (= `1.5·BTT + RefloodDelay` when the floor is dormant, `2.5·BTT` when it binds). At Config A default RefloodDelay=700ms this means every `Δ_minicon ≤ 1000ms` clamps L_Bid's broadcast at bare OBFT's `2500ms` deadline; at RefloodDelay=0 the recovery floor holds the L_0-side reference at `3·BTT`, so the threshold is `2.5·BTT = 500ms` and every named `Δ_minicon` (incl. conservative `400ms`) still binds at L_0 — no overshoot.

In the L_0-binding regime the bundle's *effective* bid-layer propagation budget is `max(B_0, 3·BTT) − Δ_minicon`, which exceeds the spec's nominal `B_0_LBid = 0.5·BTT` — providing additional reflood-tail absorption at the bid layer "for free." Under default RefloodDelay this is 600-1000ms of headroom depending on sizing; under RefloodDelay=0 the recovery floor holds the reference at `3·BTT`, giving `3·BTT − Δ_minicon = 200-500ms`.

**Aggressive sizing's MEV-fetch ceiling.** Under bare OBFT's reflood-aware schedule, L_Bid's leader broadcast is bounded above by bare OBFT V_0's deadline (the L_0 constraint binds at every named sizing under default RefloodDelay). Aggressive L_Bid therefore matches bare OBFT V_0 in broadcast time and MEV-fetch budget; conservative and standard sizings at default RefloodDelay do the same (they fit inside the 1000ms reflood-buffer for free). At `RefloodDelay = 0` the recovery floor holds the buffer at `2.5·BTT = 500ms`; no named sizing overshoots — all (incl. conservative `Δ_minicon = 400ms < 500ms`) remain free, matching floored bare OBFT V_0.
...
At **RefloodDelay = 0** (fully-meshed opt-out) the recovery floor holds the post-broadcast budget at `3·BTT = 600ms` (not the naive `2·BTT`); conservative's `Δ_select + Δ_verdict = 400ms` fits with 200ms free headroom, so all sizings fit free — no broadcast shift, all at floored bare OBFT V_0.
...
Post-`T_commit` timing **matches bare OBFT** since mini-consensus runs pre-`T_commit`. Under the reflood-aware schedule, L_Bid's pre-`T_commit` cost is `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` of MEV-fetch budget — `Δ_minicon`'s overlap with bare OBFT's buffer-beyond-typical (`max(B_0, 3·BTT) − B_0_LBid`, using the recovery-floored L_0 reference) doesn't cost extra; only the excess does. At Config A default RefloodDelay=700ms (BTT=200, floor dormant) the buffer is 1000ms wide — all three named sizings fit free, so the L_Bid extension is zero-cost on MEV-fetch under default RefloodDelay (and, thanks to the floor, at RefloodDelay=0 too).

### Deployment envelope by BTT

`T_commit` is back-end-anchored and invariant across bare OBFT and OBFT+L_Bid. What L_Bid changes is the **L_0..L_{K-1} broadcast deadline**, which shifts earlier by `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` to fit mini-consensus pre-`T_commit`. Under the reflood-aware schedule (with the recovery floor on the L_0 reference), the primary leader's MEV-fetch budget shrinks only by the excess of `Δ_minicon` over that buffer — zero whenever `Δ_minicon ≤ max(B_0, 3·BTT) − B_0_LBid` (= `1.5·BTT + RefloodDelay` when the floor is dormant, `2.5·BTT` when it binds), which holds for every named sizing at both default RefloodDelay and RefloodDelay=0.
...
(L_Bid's broadcast deadline matches bare OBFT V_0 exactly across the board — every named `Δ_minicon` fits free, so L_Bid imposes zero MEV-fetch cost under default RefloodDelay. The recovery floor `max(RefloodDelay, 1·BTT)` (see [§Setting](#setting)) binds for BTT ≥ 800ms — where `1·BTT > RefloodDelay = 700ms` — pulling the broadcast to `T_commit − 3·BTT` there, so bare OBFT's MEV-fresh envelope is **BTT ≤ 800ms** (at BTT ≥ 1000ms the primary clamps to BFT_start). Deployments at BTT ≥ 1000ms must reduce RefloodDelay or accept that the primary loses MEV-freshness. For BTT ≥ 800ms the default-RD and RefloodDelay=0 schedules coincide, since the floor dominates RefloodDelay.)

**At RefloodDelay = 0** (fully-meshed opt-out) the recovery floor binds (`RD < 1·BTT`), holding the L_0 budget at `3·BTT` — so L_Bid still imposes **zero** MEV-fetch cost (every named `Δ_minicon` fits the `2.5·BTT` buffer; all sizings match floored bare OBFT V_0), but every broadcast lands `1·BTT` earlier than the pre-floor `2·BTT` schedule and the envelope shrinks accordingly:
...
(At RefloodDelay=0 the recovery floor holds the buffer at `2.5·BTT`, so the L_Bid loss vs bare OBFT = `max(0, Δ_minicon − 2.5·BTT) = 0` at every named sizing — conservative `Δ_minicon = 2·BTT < 2.5·BTT` no longer overshoots. The floor moves every broadcast `1·BTT` earlier than the naive `2·BTT` schedule, so the RD=0 envelope is BTT ≤ 800ms — identical to the default-RD envelope above, since the floor dominates RefloodDelay for BTT ≥ 800ms. Negative MEV-fetch means the L_0 broadcast deadline is before slot_start; the primary clamps to BFT_start.)

**Net for deployment selection.** At production-typical BTT (200-400ms) with default RefloodDelay, every L_Bid sizing matches bare OBFT V_0's MEV-fetch budget exactly — the trade reduces to "L_Bid success rate vs implementation complexity," not MEV-fetch. The reflood-buffer absorption means L_Bid is essentially free **on MEV-fetch budget** under default RefloodDelay — degraded mesh still pays, but via L_Bid *success rate* (more fall-through to vanilla L_0), not budget (the success-rate gradient, [§Per-layer windows and deadlines](#per-layer-windows-and-deadlines)); conservative gives the highest L_Bid success rate (1·BTT settling) at no MEV cost. Under the tightened per-emission sizing with the recovery floor (see [§Setting](#setting)), bare OBFT's MEV-fresh envelope is BTT ≤ 800ms — the floor `max(RefloodDelay, 1·BTT)` binds for BTT ≥ 800ms (where `1·BTT > 700ms`), pulling the L_0 broadcast to `T_commit − 3·BTT`; at BTT ≥ 1000ms the primary clamps to BFT_start regardless of RefloodDelay (the floor dominates). At RefloodDelay=0 the floor absorbs every named `Δ_minicon`, so all sizings (incl. conservative) remain free on MEV-fetch — the floor subsumes the reflood buffer's role at the opt-out. The L_Bid trade has two knobs: `Δ_minicon` controls L_Bid success rate (larger Δ_minicon → more bundle tail-absorption) and — only when it exceeds the reflood buffer — MEV-fetch budget; `Δ_verdict` controls verdict-propagation safety (≥ 1·BTT for P99 guarantees). Choose both from production telemetry. Bare OBFT (no `Δ_minicon`) remains available when the trade isn't favorable.
...
| MEV-fetch budget (4s cutoff, `header_submit_headroom = 100ms`, §Application's max-MEV anchor) | ~2350ms (V_0; T_commit = 3.60s) at default RefloodDelay; ~2850ms at RefloodDelay=0 (floored) | At default RefloodDelay: ~2350ms (V_X) at every named sizing (= bare OBFT V_0; reflood buffer fully absorbs `Δ_minicon`). At RefloodDelay=0: ~2850ms at every named sizing (= floored bare OBFT V_0; the recovery floor absorbs `Δ_minicon`, so conservative no longer pays) |
...
`T_commit` is the same back-end anchor as bare OBFT (`Relay_cutoff − 2 BTT = 3600ms` at Config A post-tighten). L_Bid_New pays the `max(0, Δ_minicon − 0.5 BTT)` pre-`T_commit` cost only on deep-layer candidate deadlines; the primary L_0 MEV-fetch budget remains `T_broadcast_max_0^bare − slot_start` ≈ 2850ms at RefloodDelay=0 (= floored bare OBFT's primary budget; ≈ 2350ms at default RefloodDelay=700ms). Post-`T_commit` timing matches current L_Bid and bare OBFT.
...
| Primary MEV-fetch budget | ~2850ms at every sizing (= floored bare OBFT V_0; at RefloodDelay=0 the recovery floor absorbs `Δ_minicon`, collapsing the historical per-sizing gradient) | ~2850ms (= floored bare OBFT V_0; no `Δ_minicon` shift on primary at any sizing) |
