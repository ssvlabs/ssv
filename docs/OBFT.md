...
| Termination (output guaranteed) | Conditional. **One-liner: consensus expected to complete by `slot_start + 3.85s` at SSV proposer-duty operating point (n = 4, f = 1, BTT = 200ms, K = 4, T_commit = 3.60s, recommended Δ_2 = 1 BTT = 200ms, ε_3 ≈ 50ms), with submission slack to `slot_start + 4.00s` for relay submit (`header_submit_headroom = 100ms` plus ~50ms additional jitter buffer); under conditions: (a) ≤ f operators byzantine/offline, (b) real propagation from leader L_k broadcast to any honest first-observation ≤ that layer's per-layer budget `B_k` (primary-vs-backup: `B_0 = 2·BTT + RefloodDelay`, `B_1..B_{K-1} = T_commit` — see §Setting), (c) host validity unanimous at decision time (assumption 3), (d) `K ≥ 3` (late-leader resilience — the SSV default `K = n = 4` already satisfies this).** Single-round protocol; backup budgets give every backup layer the entire slot's commit budget for absorption (`B_k = T_commit` for k > 0, backups broadcast at BFT_start with deepest-confirmed-parent fetch); the primary L_0 has a tighter budget for MEV-fetch headroom. |
