...
**Bundle propagation.** Honest receivers re-flood every bundle via standard gossipsub on first observation. In addition, every honest operator re-attaches the full Phase-1 bundle `(V, σ_L^V, σ_L^op)` for every layer they retained (= received and stored before `T_commit`, passing all auth + retention rules) as a `bundle_witnesses` entry in their Phase-2 `KindCommit` (see §Phase 2 / Wire format). This provides cluster-wide V availability under within-budget partial-synchrony: a single `KindCommit` from any operator that retained the bundle carries it to all peers via gossipsub, so operators that didn't retain recover both V and the leader's threshold partial — closing V-drop in addition to σ_L^V-drop. Honest leaders broadcasting by their per-layer deadline `T_broadcast_max_k = T_commit − B_k` reach all honest within partial-synchrony assumptions for that layer's propagation budget `B_k` (see §Setting); if any single honest receives, the bundle is cluster-wide-available by `T_commit + Δ_2` via the re-flood section.
...
- Under byz selective Phase-1 delivery (or honest-leader propagation tail), the leader's bundle may reach only a subset of honest receivers by `T_commit`. Without re-flood, operators that didn't receive the original don't have V locally and can't σ-emit even after gossipsub eventually propagates the bundle past `T_commit`.
- With mandatory re-flood, **a single `KindCommit` from any honest operator that retained the bundle carries it** to all peers via gossipsub. By `T_commit + Δ_2` (the snapshot point), all honest who receive that `KindCommit` have V locally and can:
...
The key design property: **cluster-wide V availability under within-budget honest propagation, given any single honest operator that retained the bundle**. Combined with the f-bound on byz contributions, this makes the algebraic-cardinality mutex argument (Pigeonhole 1) valid for the per-operator-view safety basis.
