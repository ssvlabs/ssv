...
- **Per-layer leader broadcast deadlines `T_broadcast_max_k`** — OBFT uses **primary-vs-backup broadcast budgets**: the primary `L_0` broadcasts latest with a small propagation budget (= freshest MEV); all backups `L_1..L_{K-1}` broadcast at `BFT_start` with the maximally-wide propagation budget (the entire commit window). The cluster falls through to whichever layer's bundle actually arrived by `T_commit`.
  ...
  `SafetyBuffer` is the worst-case gossipsub-lazy-push latency before a retransmission cycle completes — bounded by the cluster's HeartbeatInterval (SSV's default = 700ms at [network/topics/params/gossipsub.go](../network/topics/params/gossipsub.go)). The primary's `2·BTT` base decomposes as 1·BTT P99 initial propagation + 1·BTT IWANT round-trip = minimum reflood-cycle coverage at L_0; the additive SafetyBuffer accommodates one full IHAVE/IWANT cycle when initial eager-push fails to reach mesh-flaky receivers. Deployments on fully-meshed clusters (typically n=4) where eager-push reliably reaches all peers may opt out by setting `SafetyBuffer = 0` (or near-zero); the primary budget collapses to 2·BTT and recovers MEV-fetch headroom.

  **Recovery margin (the `max(SafetyBuffer, 1·BTT)` floor).** `B_0` sizes the primary's *propagation* budget; the primary's broadcast is pulled earlier still by a *recovery* margin so the h_V=1 peer-reflood-V σ-upgrade — a `2·BTT` cascade (leader → an in-time recipient → V-drop op, which harvests V from the recipient's `KindCommit` witness and σ-upgrades) — lands before the `T_commit` view-fix. The primary broadcasts by `T_commit − max(B_0, 3·BTT)`, reserving `max(SafetyBuffer, 1·BTT)` beyond the `2·BTT` cascade. The `1·BTT` **structural floor** guarantees one propagation hop of recovery headroom even at `SafetyBuffer = 0`, so the fully-meshed opt-out still closes the h_V=1 deadlock in-protocol — without it the σ-upgrade lands exactly at `T_commit` and loses the view-fix race. At `SafetyBuffer ≥ 1·BTT` the floor is dormant and the target is the plain `T_commit − B_0`. This is the OBFT analogue of 2abOBFT's `max(SafetyBuffer, 1·BTT)` resolve-window margin (see the cross-protocol note).

  > **Cross-protocol note**: 2abOBFT carries the same `SafetyBuffer` knob (sharing the default value, one HeartbeatInterval — 700ms in SSV), but places it differently in the schedule. OBFT folds it into the leader's pre-Phase-2 broadcast budget — `B_0 = 2·BTT + SafetyBuffer` — absorbing one IHAVE/IWANT cycle BEFORE commit fires, appropriate because OBFT's critical path post-bundle-arrival is one hop (early-commit fires immediately on L0Ready close). 2abOBFT instead widens the σ-ward arm of the post-`TPhase2a` resolve window (`1·BTT + SafetyBuffer`), absorbing σ-pool fill via IHAVE/IWANT recovery when the initial `KindValue` eager-push misses a peer (the NR-ward arm is the genuinely two-hop cascade at `2·BTT`; the window reserves the `max` of the two arms). 2abOBFT's `B_k` stays at the structural minimum `(k+2)·BTT` — no SafetyBuffer term. The shared default produces equivalent total post-broadcast structural budgets across the two protocols. Both also carry the same `max(SafetyBuffer, 1·BTT)` structural recovery floor — OBFT in its broadcast-timing margin (just above), 2abOBFT in its resolve window — so each keeps ≥ 1·BTT of recovery headroom even at zero cushion. See [docs/2abOBFT.md §Timing parameters](2abOBFT.md#timing-parameters) for the cross-protocol placement rationale.
  ...
  **Why backups all broadcast at BFT_start.** Backups L_1..L_{K-1} are last-resort safety nets — the primary L_0 carries the slot's MEV in the vast majority of slots. Trading L_1+ MEV-fetch (small expected value under the rational-byzantine deterrent) for L_1+ absorption width = T_commit substantially improves fall-through reliability under Class A partition tails and degraded mesh. Backups all fetch from a deepest-confirmed parent at slot start (re-org resistant by construction) and broadcast immediately, giving the cluster the entire commit budget for that bundle's propagation.
  ...
  `Δ_2 = 1·BTT` recommended — one synchronous-fallback `KindCommit` propagation cycle; reflood absorption is structurally provided by `B_0` via SafetyBuffer, so `Δ_2` no longer carries a reflood cushion. See §Phase 2.
...
**Timing window.** For peer-reflood-V to fire at receiver j, the sender's early-commit must arrive at j before j's T_commit fallback fires. At Config A under healthy mesh: V-holder early-emits at ~`T_commit − 1·BTT − SafetyBuffer`; peer commit reaches V-drop receivers within 1·BTT — comfortably ahead of T_commit. When the sender's emit is closer to T_commit (degraded mesh) or the V-drop receiver is mesh-flaky on inbound, the receiver falls back to T_commit fallback (NR on L_0) — degraded but graceful (no regression vs pre-§1 behavior).
...
1. **Phase 1**: each leader `L_k` broadcasts its Phase-1 bundle by its per-layer deadline `T_broadcast_max_k = max(BFT_start, T_commit − B_k)` (primary-vs-backup: `B_0 = 2·BTT + SafetyBuffer`; `B_1..B_{K-1} = T_commit` — backups broadcast at BFT_start; see §Setting). Receivers accept bundles first-observed in `[slot_start, T_commit]`.
...
OBFT's liveness is **partial-synchrony-conditional within the slot's relay-submission deadline** (the protocol's slot budget). The recovery profile below is the protocol's behavior as exercised by the reference test catalog (`protocol/v2/consensustest/`, protocol `OBFT`). Running example: `f = 1, n = 4, K = 2`; honest A, B, C; byzantine D. The protocol terminates with a V signature on the first layer whose bundle reaches a σ-quorum within its per-layer budget `B_k` (else falls through via NR-quorum to a deeper backup, else — if every layer's bundle is late — misses cleanly); **safety holds in either case**. View-divergence (equivocation, validity-divergence) is NOT recovered by the protocol — some splits succeed naturally as a side-effect of the leader's `σ_L^V` counting toward whichever V reaches qV first, others slot-miss; equivocation cases are slashable, validity-divergence is not attributable.
...
> - **Reflood-aware per-layer budgets** (`B_0 = 2·BTT + SafetyBuffer`; backups at `B_k = T_commit` — see [§Setting](#setting)) absorb a full gossipsub IHAVE/IWANT cycle inside the per-layer budget, making partial propagation past `B_k` rare at L_0 and exceedingly rare at backups.
...
**Healthy path — decides at L_0, 2 RTTs.** All honest receive `V_{L_0}` within ~`1·BTT` and σ-emit; the leader's Phase-1 `σ_L^V` seeds the σ-pool, so a single honest peer completes `qV = 3`. Slot succeeds in 2 RTTs (Phase 1 + Phase 2). **Success (fastest).**

**Network-flakiness and silent/crash patterns recovered:**
- **Non-leader crash, byzantine σ-refusal (within f), asymmetric propagation to ≤ 1 honest** — L_0's remaining `2f` honest σ partials + the leader's `σ_L^V` still reach `qV` (e.g. 2-of-3 honest + leader = 3 = qV); the miss-honest's NR partial is unused. **Success at L_0.**
- **Primary leader silent or crashed** — no `V_0` reaches the cluster; all honest NR (silent-leader rule); `NR-pool = 3 = qEnc` at L_0 → in the same Phase 3 walk, advance to L_1; the backup leader's value σ-decides. **Success (fall-through).**
- **h_V=1 byzantine selective-delivery** (the leader delivers `V_0` to only one honest operator) — the in-time recipient's early `KindCommit` carries `V` plaintext + the `σ_L^V` witness; the V-drop receivers harvest peer-`V`, host-validate, and σ on it → σ-pool reaches `qV` at L_0. **Success at L_0** under healthy mesh — peer-reflood-V largely addresses the worst (b) sub-case; the degraded-mesh tail (the early commit doesn't reach the V-drop receivers before their `T_commit` fallback) slot-misses, deterred via assumption 4.
- **Late L_0-leader broadcast** (first cluster-observation past `T_commit`) — not recovered at L_0 (receivers already committed at `T_commit`); the cluster **falls through to L_1**.

**Leader equivocation** (byzantine `L_0` broadcasts distinct V's selectively; no protocol-level recovery, outcome depends on byzantine timing):
- **2-1 split where one V already holds 2 honest** (D delivers `V` to {A, B}, `V'` to {C}) — `σ-pool[V]` = A + B + D's `σ_L^V(V)` = 3 = qV. **Success at L_0.** (Symmetric: `V` to {A}, `V'` to {B, C} → `σ-pool[V']` = 3.)
- **All-honest-NR** (D delivers V's early enough that re-flood spreads conflicts before `T_commit`; each honest retains ≥ 2 distinct V's → all 3 emit NR per the equivocation rule) — `σ-pool` per V `< qV`; `NR-pool = 3 = qEnc` at L_0 → fall-through to L_1; if L_1 honest, σ-decides. **Success (fall-through).**
- **σ-locked split** (D delivers `V` to {A}, `V'` to {B}, both/neither to C — `σ-pool[V] = σ-pool[V'] = 2 < qV`, `NR-pool = 1 < qEnc`; or **1-1-1**: a distinct `V_i` to each honest — every `σ-pool[V_i] ≤ 2 < qV`, `NR-pool = 0`) — each honest σ-locks on its own value before observing the conflict, and σ-locked operators cannot pivot to NR. **Miss** at L_0, no fall-through (slashable; the slot-miss is invariant to the byzantine's `σ_L` choices once honest split 1-1-1, which only widen the slashing surface). **An adversarial byzantine reliably picks this class** by delivering near end-of-Phase-1 (insufficient re-flood time); the rational-byzantine deterrent (assumption 4) makes it tolerable across slots, but the evidence is *behavioral* class — single-observation slashing is not credible (see [§Implications of the rational-byzantine deterrent](#implications-of-the-rational-byzantine-deterrent-assumption-4)).

Pigeonhole 2 ensures at most one V can reach `qV` cluster-wide regardless of how honest σ-emissions split — there is no two-output safety violation in any equivocation case above.

**Validity-divergence** (a re-org splits honest host verdicts mid-slot; the leader's Phase-1 `σ_L^V` locks it σ-side regardless of a later host flip):
- **3-σ vs 1-NV** — `σ-pool = 3 = qV`. **Success at L_0.**
- **1-σ vs 3-NV** — the NV operators emit no-σ; `NR-pool = 3 = qEnc`; fall-through. **Success at L_1.**
- **2-σ vs 2-NV (boundary)** — `σ-pool = 2 < qV` and `NR-pool = 2 < qEnc`; neither quorum reaches. **Miss** (the f=1 n=4 algebraic limit).
- **Validity-divergence + passive byzantine** (the honest split is below quorum and the byzantine stays silent or σ-on-V, including a leader whose host flips post-fetch) — neither quorum reaches; **Miss**. The σ-locked-leader limit — the leader's `σ_L^V` can't pivot to NR; see [§Failure modes](#failure-modes).

**Deepest-layer / multi-failure fall-through.** With `K > 2` layers the cluster falls through past multiple silent or non-validating layers **within Phase 3's single reconstruction walk** (sequential local decryption, no per-layer RTT): e.g. at K=4 with L_0, L_1, L_2 silent, NR-quorum reaches at each in Phase 2 and σ-quorum at L_3 if L_3 honest — a structural difference from QBFT (which round-changes once per failed leader). At K = n = 4, pigeonhole guarantees ≥ 3 honest leaders at f=1, the maximum fall-through depth within a single round. If all `K` leaders are silent (or every layer's leader fails non-recoverably), the slot **misses** — at K ≥ f+1, byzantines alone cannot cause this (pigeonhole guarantees ≥ 1 honest leader); it requires `> f` faults or coincident independent failures.

**Adversarial scheduling.** A network adversary delaying messages by up to `1 BTT` cannot affect safety (Pigeonholes 1, 2, 3 are over the cluster-wide signed-message set, not arrival times). Its liveness effect reduces to the (b) split: delaying `V` to ≤ 1 honest still leaves `σ-pool = 2 + leader = qV` (the quorum reaches without the delayed operator); delaying to ≥ 2 honest is recovered only where NR fall-through applies (the chain unlocks, the cluster falls through to a deeper backup — ✗ if all K exceed `B_k`) or, in the `h_V=1` shape, peer-reflood-V lifts L_0 under healthy mesh (degraded-mesh tail slot-misses).

### Liveness comparison: OBFT vs OBFTR(R=2) vs QBFT

The table below puts OBFT, OBFTR(R=2), and QBFT side-by-side per scenario at the standard SSV proposer-duty configuration (n=4, f=1, **K=2 default**, ~4s relay cutoff). Timing assumes the SSV proposer-duty operating point (`BTT = 200ms` = P99=150ms + δ=50ms; primary-vs-backup `B_0 = 2·BTT + SafetyBuffer = 1100ms`, `B_1 = T_commit = 3600ms` at K=2 — see [Timing budget](#timing-budget)). The Multi-failure fall-through row uses K=4 as an up-tier illustration of multi-silent-leader recovery (K=2 has only L_0 → L_1; multi-silent fall-through requires K ≥ 3). All counts at the unified tightened "1 BTT per emission" sizing (OBFT `Δ_2 = 1 BTT`; OBFTR per-round `Δ_2 = 1 BTT`; QBFT 4 BTT R1) — see [docs/BFT-comparison.md / Sizing convention](BFT-comparison.md#sizing-convention). For QBFT-SSV, `RT = 2s = 10 BTT`; QBFT-no-reflood uses per-round RT (R1 = 3·BTT, R≥2 = 4·BTT).
...
| Host-validity divergence (head-change mid-slot, strict host) | Honest-majority recovers (3-σ → L_0; 1-σ-3NV → L_1 via NR fall-through); the 2-2 boundary and validity-divergence-with-passive-byz cases miss (assumption 3 — the host's stabilization workflow makes divergence rare). Same as OBFTR(R=2). | Same. | Round 1: validators with stale head don't PREPARE; timeout. Round 2: new leader fetches at moved head; if quorum agrees, succeeds. ✓ in ~3.6s. **QBFT additionally recovers the boundary cases** the OBFT-family misses, via fresh-V refetch on round-change. |
...
| Partial-synchrony absorption | Primary-vs-backup: `B_0` for the MEV-fresh primary L_0; `B_k = T_commit` for all backups L_1..L_{K-1}. At K=2 Config A with default SafetyBuffer=700ms: `B_0 = 2·BTT + SafetyBuffer = 1100ms`; `B_1 = T_commit = 3600ms` (backup broadcasts at BFT_start, deepest-confirmed-parent fetch). At SafetyBuffer=0 (fully-meshed opt-out): `B_0 = 400ms`. K=4 up-tier extends backup absorption to L_2, L_3 with the same `B_k = T_commit` shape. Cluster falls through layers based on which one's bundle actually arrived by `T_commit`. |
...
For SSV's proposer duty, the recommended OBFT configuration is **`K = f+1` (BFT-liveness minimum)** — at n=4 (f=1) this resolves to **`K = 2`**: one fall-through layer `L_0 → L_1`, motivated by production testing showing K > f+1 doesn't materially improve outcomes once peer-reflood-V closes the healthy-mesh `h_V=1` case at L_0 (see [§Phase 2 / Peer-reflood V via early commit](#phase-2--onion-broadcast-t_emit-t_emit--%CE%B4_2-with-t_emit--t_commit)). Concretely at K=2: **`V_0`** is the slot's designated MEV proposer (fetches the freshest relay-bundle); **`V_1`** is the backup leader fetching a deepest-confirmed-parent vanilla beacon-node payload (lower re-org exposure; see §Head-change handling). K-up-tier (`K = f+2..n`; at n=4 up to K=4 where every cluster member leads exactly one layer with `f+1 = 3` honest leaders guaranteed by pigeonhole) is available for deployments preferring deeper fall-through at the cost of larger onion bandwidth and longer Phase-3 IBE walks. The K choice is per-cluster and deployment-dependent.
...
**Operating point.** `BTT = 200ms`, `header_submit_headroom = 100ms`, `Δ_2 = 1 BTT = 200ms` recommended (KindCommit synchronous-fallback propagation cycle; reflood lives in `B_k` via `SafetyBuffer`), `ε_3 ≈ 50ms` local CPU.
...
**Default SafetyBuffer** = 700ms (matches SSV's gossipsub HeartbeatInterval per [network/topics/params/gossipsub.go](../network/topics/params/gossipsub.go)). Schedule rows below assume this default; lower SafetyBuffer for fully-meshed clusters recovers MEV-fetch headroom (table values shift later by `default − chosen = ΔSafetyBuffer`, until the recovery floor saturates the V_0 broadcast at `T_commit − 3·BTT` for `SafetyBuffer ≤ 1·BTT` — see §Setting "Recovery margin").
...
| 2500 | `V_0` broadcast (`T_commit − (2·BTT + SafetyBuffer) = 3600 − 1100`) | Targets up to **1100ms** (2·BTT + SafetyBuffer); MEV-fetch budget **~2350ms** (freshest relay-bundle) |
...
**Recovery scope.** Within Phase 3's single reconstruction walk: at K=2 default, silent V_0 → fall through to V_1 (the only backup). K=4 up-tier extends fall-through: silent V_0 + V_1 → V_2; silent V_0 + V_1 + V_2 → V_3 (at K=4 every cluster member is a leader; pigeonhole guarantees ≥ 3 honest leaders at f=1). For `h_V=1` selective-Phase-1-delivery at L_0, **the slot recovers at L_0 via the §Phase 2 peer-reflood-V mechanism** under healthy mesh (V-drop receivers σ on V harvested from the in-time recipient's KindCommit + σ_L^V witness; pushes σ-pool past qV at L_0). Under degraded mesh where the in-time recipient's early commit doesn't reach V-drop receivers before their T_commit fallback, the pre-§Phase 2 behavior applies (slot misses at L_0 with no fall-through; the rational-byzantine deterrent — Assumption 4 — bounds it across slots). Per-layer budgets `B_k`: V_0 covers real propagation up to 1100ms (2·BTT + SafetyBuffer); backups (V_1 at K=2; V_1, V_2, V_3 at K=4) all cover up to 3600ms (the entire commit budget — backups broadcast at BFT_start). Beyond the deepest backup's budget — i.e., real propagation > T_commit — is out-of-envelope and slot-misses cleanly.

**MEV-fetch-budget asymmetry.** V_0's ~2350ms (at default SafetyBuffer=700ms) is the only MEV-fresh budget; backups all fetch deepest-confirmed-parent vanilla payloads at slot start. Per-leader budgets at this operating point (K=2 default): `[V_0: ~2350ms (MEV-fresh), V_1: ~0ms (deepest-confirmed parent)]`. K=4 up-tier: `[V_0: ~2350ms (MEV-fresh), V_1/V_2/V_3: ~0ms (deepest-confirmed parents)]`. At SafetyBuffer=0 (fully-meshed cluster opt-out) V_0's budget widens to `~2850ms` (the recovery floor caps the broadcast at `T_commit − 3·BTT = 3000ms`, so the budget saturates 1·BTT short of the naive `~3050ms`); backups unchanged. The primary-vs-backup design lets V_0 capture maximum MEV under healthy propagation while all backups absorb the entire-slot tail when V_0's bundle doesn't reach in time.
...
**MEV-freshness ranking** at this operating point (incl. partial-sigs-on-pre-agreed-V baseline) at tightened sizing throughout. Default SafetyBuffer=700ms is the production schedule; the SafetyBuffer=0 column gives the fully-meshed-cluster opt-out for comparison.

| Rank | Leader | MEV-fetch (default SafetyBuffer=700ms) | MEV-fetch (SafetyBuffer=0 opt-out) | Notes |
|---|---|---|---|---|
...
| 3 | OBFT V_0 | **~2350ms** | **2850ms** | The MEV-fresh primary; the only OBFT layer that competes on MEV-fetch (RD=0 capped by the recovery floor at `T_commit − 3·BTT`) |
...
**Comparison vs partial-sigs floor**: OBFT V_0 pays a **~1200ms BFT-consensus tax** at default SafetyBuffer (3550 − ~2350ms = ~6 BTT), or a **~700ms tax** at SafetyBuffer=0 (3550 − 2850 = 3.5 BTT). The tax decomposes as: **broadcast slack + Δ_2 − partial-sigs post-fetch overhead** = `max(2·BTT + SafetyBuffer, 3·BTT) + 1·BTT − 1.5·BTT` (per-leader broadcast slack — floored at 3·BTT by the recovery margin — plus the 1·BTT slot tail OBFT reserves for KindCommit fan-out, less the 1.5·BTT the partial-sigs floor reserves for emit + submit). At default SafetyBuffer this evaluates to 1100 + 200 − 300 = 1000ms; at SafetyBuffer=0 the floor gives 600 + 200 − 300 = 500ms; the additional ~200ms in each reflects RANDAO timing. SafetyBuffer accounts for one full IHAVE/IWANT cycle for mesh-flaky receivers; the 2·BTT shallow base covers 1 BTT P99 leader-broadcast propagation + 1 BTT IWANT round-trip. QBFT-SSV R1 pays a **2600ms tax** under tightened sizing — still meaningfully larger than OBFT at default SafetyBuffer, but substantially smaller than the 3200ms tax under the older framing.

**Comparison OBFT vs QBFT (post-tighten reranking)**: under the older 2·BTT/emission framing, OBFT V_0 led QBFT R2 (~2350ms vs 2150ms at default SafetyBuffer). Under tightened 1·BTT/emission, QBFT R2 now beats OBFT V_0 at default SafetyBuffer (2950ms vs ~2350ms = 600ms ahead) — but OBFT V_0 is the *primary* leader (always tried first; no round-timeout gap), while QBFT R2 is only reachable after R1 fails (paying the ~2s round-change cost). At SafetyBuffer=0 (fully-meshed opt-out), QBFT R2 still leads OBFT V_0, by 100ms (2950 vs 2850) — the recovery floor pulls V_0's RD=0 broadcast 1·BTT earlier (to `T_commit − 3·BTT`), so OBFT no longer overtakes at the opt-out as it did under the pre-floor framing (3050 vs 2950); see §Setting "Recovery margin". OBFT's backups (V_1..V_3) trade all MEV-fetch for maximally-wide propagation absorption (B_k = T_commit) — they're not MEV-fresh alternatives but safety nets that catch the slot when L_0's bundle doesn't reach in time. The structural tradeoff is unchanged: OBFT's primary always tries first; QBFT's R2 is conditional on R1 failure.
...
**The validity-locking window is per-V, bounded by `B_k`.** Operators accept Phase-1 bundles in `[slot_start, T_commit]`. Each operator locks their verdict per V at first-observation of that V. For V_k, the cluster-wide spread of lock-times is at most `B_k` — the time from earliest possible first-observation (right after `T_broadcast_max_k`) to latest (just before `T_commit`). At the operating point above with default SafetyBuffer=700ms: V_0's window ≈ 1100ms (= 2·BTT + SafetyBuffer); backups (V_1 at K=2 default; V_1, V_2, V_3 at K=4 up-tier) all have window ≈ 3600ms (the entire commit budget — backups broadcast at BFT_start). At SafetyBuffer=0 V_0's window shrinks to 400ms; backups unchanged. **For the practical case where the cluster reconstructs V_0 (healthy path), the relevant window is V_0's `B_0`.** Backups (V_k for k > 0) have the maximum possible validity-locking window (entire commit budget), but those layers are only reached on fall-through (silent/late primary leader), so the divergence-rate × fall-through-rate product is the practical metric — and backups fetch from deepest-confirmed parents which are re-org resistant by construction. A re-org landing inside V_0's window can split honest verdicts; the all-honest rate of validity-divergence slot-misses scales with re-org distribution within V_0's `B_0`. **In adversarial-byz deployments the operational rate is multiplicatively higher** — a byzantine within the f-bound exercising passive f-budget (silence or σ-on-V; neither cryptographically slashable individually) widens the deadlock zone beyond the all-honest case. See [§Failure modes / Validity-divergence deadlock](#failure-modes) for the `re-org rate × byz-passivity-rate` scaling.
...
3. **Choosing K (layer count).** K is per-duty:
   ...
   - **`K = f+1`**: BFT-liveness minimum; **recommended default for SSV proposer duty at n=4** (= K=2). Motivated by production testing showing K > f+1 doesn't materially improve outcomes once peer-reflood-V closes the healthy-mesh `h_V=1` case at L_0 (see [§Phase 2 / Peer-reflood V via early commit](#phase-2--onion-broadcast-t_emit-t_emit--%CE%B4_2-with-t_emit--t_commit)). Trade-off: only one fall-through layer (`L_0 → L_1` at K=2), so the late-deepest-layer-leader-broadcast Class A failure mode at L_{K-1} has no further backstop — acceptable given the deterrent and the (healthy-mesh) closure of the Class B `h_V=1` grief vector; deployments operating with sustained degraded mesh may prefer `K ≥ f+2`.
...
| Partial-synchrony envelope (Phase-1 bundle propagation) | `R · P99` cross-round retention (e.g., `2·P99` at R=2 — bundles arriving in `(P99, R·P99]` get a fresh chance to be σ-emitted on in round 2) | primary-vs-backup `B_k`: `B_0 = 2·BTT + SafetyBuffer` for the MEV-fresh primary; `B_1..B_{K-1} = T_commit` for all backups (full commit budget, backup leaders broadcast at BFT_start); cluster falls through to whichever layer's bundle arrived in time. No cross-round retention (single round) |
...
| MEV-fetch budget for primary leader (BTT=200ms, `header_submit_headroom = 100ms`; B_0 is K-independent) | ~1.45s (T_commit_1 = 2.00s constrained by R1+R2 fit within 4s slot) | **~2.35s** at default SafetyBuffer=700ms (T_commit = 3.60s; B_0 = 2·BTT + SafetyBuffer = 1100ms; single-round, no retry budget needed) — **+0.9s more MEV-fresh fetch**; at SafetyBuffer=0 the budget widens to ~3.05s (+1.6s more) |
...
| Phase-2 shape | single `KindCommit` (σ+NR together); each operator commits once by `T_commit` (early on `T_L_0_observed`) | split: `KindValue` (σ inline) / `KindNoValue` (coordination, **no L_0 lock**) / `KindCommit-NRDirect`; dynamic `KindCommit-NR` (cannot-σ gate); upgrade |
...
| σ-locked-split leader equivocation (f-f) | **Misses at L_0** — V-drop operators NR-lock at `T_commit` and cannot recover | **Recovers at L_0** — `KindNoValue` keeps V-drop operators harvest-eligible, so they upgrade to σ on a forwarded witness |
| Late L_0-leader broadcast | Falls through to L_1 | Decides at L_0 (bundle absorbed in the σ-pool fill window) |
| Transient mesh-flakiness (operator would NR-lock before its mesh recovers) | May miss — cross-phase exclusivity locks the flaky NR | **Recovers** — `KindNoValue` no-lock state lets the operator upgrade once its mesh delivers V |
| 1-1-1 leader equivocation | **Misses** (σ-locked, no NR pivot) | **Misses** (σ-locked, no NR pivot) — same |
| h_V=1 selective-delivery | Largely addressed at L_0 via peer-reflood-V (recovers under healthy mesh; degraded-mesh tail slot-misses, deterred via Assumption 4) | Same mechanism — peer-reflood-V harvest of forwarded witnesses + upgrade (same degraded-mesh tail) |
...
> **Note on values:** This appendix's concrete numbers reflect the reflood-aware bare OBFT schedule (`B_0 = 2·BTT + SafetyBuffer`; see [§Setting](#setting)). The L_Bid extension's *structural* design is unchanged from earlier drafts — `B_0_LBid = 0.5·BTT` typical-mesh-only at the bid layer, mini-consensus window between bundle arrival and `T_commit`, bare OBFT's reflood-buffer reused as mini-consensus headroom. Under the wider reflood-aware `B_0`, L_Bid's MEV-fetch cost collapses to zero at every named sizing under default SafetyBuffer (= 700ms); see the per-sizing derivations in [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening).
>
> **Note on deep-layer deadlines under the primary-vs-backup schedule:** Bare OBFT's current schedule (see [§Setting](#setting)) is primary-vs-backup — `B_0 = 2·BTT + SafetyBuffer` for the MEV-fresh primary; `B_1..B_{K-1} = T_commit` for all backups (broadcast at BFT_start with deepest-confirmed-parent fetch). The L_Bid mini-consensus framework below was originally drafted against an earlier per-layer staggered backup schedule (`B_k_shallow = (k+2)·BTT + SafetyBuffer`) and includes per-layer deadline-shift formulas (`T_broadcast_max_k = T_0_arrival − B_k_LBid` for `k ≥ 1`) that assume deeper-layer deadlines can be moved earlier for mini-consensus headroom. Under the current schedule those deadlines are already pinned at BFT_start and cannot be shifted further; deep-layer bundles broadcast from BFT_start propagate via standard gossipsub and reach receivers well before any reasonable `T_0_arrival` anchor, so the mini-consensus operates over deep-layer bundles that arrived "for free" via standard backup propagation rather than via L_Bid-side deadline tightening. The L_0-side derivations (`B_0_LBid = 0.5·BTT` constraint, reflood-buffer reuse, per-sizing MEV-fetch costs) remain accurate. A full L_Bid re-derivation under the current schedule is a follow-up if/when the extension is implemented.
...
The extension closes three deadlock surfaces that any naive bid-routing extension would expose ([§Background — bid-layer deadlocks](#background--bid-layer-deadlocks)) and adds adversarial-byz residual surfaces at L_Bid (equivocation-promotion and verdict-equivocation; candidate-withholding is closed at f=1 by the value-keyed witness harvest, residual only at f≥2) plus the standard 2-2 validity-divergence hard algebraic limit. Post-`T_commit` latency matches bare OBFT; the leader's broadcast deadline shifts earlier vs bare OBFT by `max(0, Δ_minicon − (B_0 − B_0_LBid))` — i.e., the leader broadcasts at whichever is earlier between L_Bid's mini-consensus requirement and bare OBFT's reflood-aware L_0 requirement. Under Config A with default SafetyBuffer=700ms, `max(B_0, 3·BTT) − B_0_LBid = 1.5·BTT + SafetyBuffer = 1000ms` (floor dormant), large enough to absorb all three named `Δ_minicon` sizings (2·BTT / 1.5·BTT / 0.5·BTT) with zero MEV-fetch cost vs bare OBFT. At SafetyBuffer=0 the recovery floor holds the buffer at `2.5·BTT = 500ms`, which still absorbs every named sizing free (conservative `Δ_minicon = 2·BTT < 2.5·BTT` no longer overshoots). See [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening) for the full derivation. Rotation-layer recovery scope is unchanged; safety is identical to bare OBFT. L_Bid relies on one additional assumption beyond bare OBFT's threat model — **bid-value honesty** (see [§Additional assumption — bid-value honesty](#additional-assumption--bid-value-honesty)).
...
L_Bid introduces one assumption beyond bare OBFT's: **eligible rotation leaders report their bid values truthfully**. Without this, a byzantine leader can claim arbitrary `bid_value` for its Phase-1 candidate and reliably win L_Bid argmax, routing the cluster to sign a low-MEV (potentially self-dealt) block. The grief leaves no on-wire evidence and produces no slot-miss, so none of [assumption 4's](#assumed) deterrent mechanisms (slashable evidence, slot-miss visibility to stakers, behavioral patterns visible to honest peers) discipline it per-slot. Safety is unaffected (Pigeonholes hold regardless), and *bid-lying alone* causes no slot-miss — it only breaks L_Bid's MEV-value-capture motivation. But bid-lying is also what lets a byzantine **promote** an equivocated candidate to the outermost gate (see [§Liveness / equivocation-promotion](#new-residual-failure-modes-at-l_bid)), so relay/builder attestation — which enforces this assumption — is additionally **liveness-load-bearing** there, bounding the equivocation-promotion frequency. For adversarial deployments it is therefore closer to required than optional.
...
**Suited for**: deployments where MEV bid-routing upside among eligible rotation-layer candidates justifies (a) the `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` MEV-fetch budget reduction — zero at every named sizing under both Config A default SafetyBuffer=700ms and SafetyBuffer=0 because the buffer-beyond-typical (`1.5·BTT + SafetyBuffer = 1000ms` default; `2.5·BTT = 500ms` floored at the opt-out) absorbs all three sizings free — see [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening), and (b) the new adversarial-byz residual surfaces at L_Bid. For SSV proposer duty under Config A: high-MEV slots where bid-routed block value capture exceeds the slot-loss-rate cost from the new L_Bid failure modes.

**Requires `K > f+1` to be worthwhile.** L_Bid ranks the `K` rotation-layer candidates, so its value scales with `K`. At the recommended bare-OBFT default `K = f+1` (K=2 at n=4), `qBid = K − f = 1` — an operator predicts L_Bid from a *single* observed bid over a 2-candidate universe, so verdicts diverge easily and most slots fall through to L_0 with little realized bid-routing. L_Bid therefore presumes a deliberately higher-`K` deployment (`f+2 ≤ K ≤ n`); taken at `K = f+1` it loads the new residual surfaces for near-zero bid-routing benefit. (Note this trades against bare OBFT's own `K = f+1` recommendation — adopting L_Bid means choosing a higher `K` than bare OBFT would.)

**Higher `K` alone isn't enough — and attestation is an *adversarial*-only requirement.** The `K` rotation leaders must each fetch a genuinely *competing* bid (e.g. different relays/builders): the default primary-vs-backup fetch (L_0 MEV, L_1.. vanilla deepest-confirmed-parent at bid ≈ 0) gives L_Bid nothing to rank, so it just reproduces L_0 regardless of `K`. A deployment must supply multiple real bid sources for L_Bid to do any work. Separately, **relay/builder attestation is not needed for all-honest operation** — it only constrains a *byzantine* operator (no bid-inflation → no equivocation-promotion to the gate, no self-dealt low-MEV routing), so all-honest or institutionally-vetted operator sets can run L_Bid without it (the institutional-honesty path of [§Additional assumption — bid-value honesty](#additional-assumption--bid-value-honesty)). It becomes *effectively required* only for **adversarial** deployments — most so at small `n`, where the value-bearing `K = n` makes a byzantine a rotation leader every slot, so attestation is what holds the equivocation-promotion surface bounded.

**BTT regime guidance** (see [§Deployment envelope by BTT](#deployment-envelope-by-btt) for full table):
- **Under default SafetyBuffer=700ms**, every named sizing matches bare OBFT V_0's broadcast deadline across the BTT range where bare OBFT itself fits (BTT ≤ 800ms — the reflood-aware `B_0 = 2·BTT + 700ms`, and the recovery floor `max(SafetyBuffer, 1·BTT)` for BTT ≥ 800ms, consume the pre-`T_commit` budget at higher BTT). The buffer-beyond-typical absorbs `Δ_minicon` regardless of BTT — pick `Δ_minicon` purely on L_Bid success-rate vs implementation-complexity grounds, not MEV-fetch.
- **At SafetyBuffer=0** (fully-meshed opt-out): with the recovery floor, bare OBFT fits to BTT ≤ 800ms (same as default RD — the floor dominates SafetyBuffer for BTT ≥ 800ms), and every sizing remains free on MEV-fetch (the floored `3·BTT` budget absorbs `Δ_minicon`). At BTT ≥ 1000ms bare OBFT's primary clamps to BFT_start so L_Bid is moot.
...
| `T_broadcast_max_0^bare` | Bare OBFT primary broadcast deadline = `T_commit − B_0` (= `T_commit − (2·BTT + SafetyBuffer)` = `T_commit − 1100ms` at Config A with default SafetyBuffer=700ms; `T_commit − 400ms` at SafetyBuffer=0). L_Bid_New preserves this deadline for bid_1. |
...
- **Mini-consensus window** `Δ_minicon`: `Δ_minicon = T_commit − T_0_arrival`. Mini-consensus starts at `T_0_arrival`, ends at `T_commit`, and all L_Bid timing derives from that interval. `T_0_broadcast_max = T_0_arrival − B_0_LBid` is the L_Bid-side constraint; the actual broadcast time is `T_commit − max(Δ_minicon + B_0_LBid, B_0)` — the leader broadcasts at whichever is earlier between L_Bid's mini-consensus requirement and bare OBFT's L_0 reflood-absorption requirement (so the same bundle remains in-envelope for L_0 fall-through if L_Bid mini-consensus fails). L_Bid uses tighter per-layer budgets `B_k_LBid` (typical-mesh propagation only, no reflood-tail budget; e.g., `B_0_LBid = 0.5 BTT` at Config A vs bare OBFT's `B_0 = 2·BTT + SafetyBuffer = 1100ms` at default SafetyBuffer); the rationale is that L_Bid is opportunistic and doesn't need bare OBFT's reflood-absorption guarantee at the bid layer — reflood-tail bundles miss `bid_set_i` but the same bundle still arrives in time for L_0 σ-pool aggregation via bare OBFT's wider `B_0` (see [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening) below). The L_0..L_{K-1} broadcast deadlines shift earlier vs bare OBFT by `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` (zero at every named sizing under both Config A default SafetyBuffer and SafetyBuffer=0, where the recovery floor holds the buffer at `2.5·BTT`). `T_commit` itself stays back-end-anchored to `T_relay_cutoff − submit_headroom − ε_3 − Δ_2` and is unchanged from bare OBFT.
...
- **Conservative** `Δ_minicon = 2 BTT`, `Δ_verdict = 1 BTT`, `Δ_select = 1 BTT`: highest L_Bid success rate. P99 verdict propagation + 1 BTT post-`T_0_arrival` bid-set settling for late-arriving bundles. Under bare OBFT's reflood-aware schedule when L_0 reflood-absorption binds the broadcast, the bid_set effectively absorbs bundles in `[broadcast, T_verdict]` = `B_0 − Δ_verdict` post-broadcast — at Config A default SafetyBuffer that's 900ms total absorption window (=  100ms typical-mesh propagation + 600ms pre-`T_0_arrival` reflood-buffer headroom + 200ms `Δ_select` post-`T_0_arrival`). At SafetyBuffer=0 the recovery floor binds (not the L_Bid constraint; see [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening)), so the broadcast stays at floored bare OBFT V_0 (no shift) and the bid_set window is 400ms = 100ms typical-mesh + 100ms pre-`T_0_arrival` headroom + 200ms `Δ_select`.
- **Standard** `Δ_minicon = 1.5 BTT`, `Δ_verdict = 1 BTT`, `Δ_select = 0.5 BTT`: medium L_Bid success rate. P99 verdict propagation + 0.5 BTT post-`T_0_arrival` bid-set settling. Under L_0-binding (default SafetyBuffer): bid_set window = `B_0 − Δ_verdict` = 900ms (= 100ms typical-mesh + 700ms pre-`T_0_arrival` headroom + 100ms `Δ_select`). At SafetyBuffer=0 (floor binds): bid_set window = 400ms (= 100ms typical-mesh + 200ms pre-`T_0_arrival` headroom + 100ms `Δ_select`).
- **Aggressive** `Δ_minicon = 0.5 BTT`, `Δ_verdict = 0.5 BTT`, `Δ_select = 0`: lowest L_Bid success rate. Sub-P99 verdict propagation (verdicts may not all arrive by `T_commit` under tail jitter — partial-propagation deadlock becomes a Class A residual, see [§New residual failure modes](#new-residual-failure-modes-at-l_bid)) + zero `Δ_select` post-`T_0_arrival` settling. Under L_0-binding (default SafetyBuffer): bid_set window = `B_0 − Δ_verdict` = 1000ms — counterintuitively wider than conservative/standard because tighter `Δ_verdict` cedes 100ms back to bid_set. Suitable only when production telemetry shows mesh propagation tighter than the standard P99 assumption. Matches bare OBFT V_0's broadcast deadline exactly under both default SafetyBuffer and SafetyBuffer=0.
...
**Parity floor (normative).** `Δ_verdict ≥ 1·BTT` (standard or conservative) is required for all-honest liveness parity with bare OBFT. At `Δ_verdict < 1·BTT` (aggressive), all-honest mesh jitter can leave L_Bid with neither σ-quorum nor NR-quorum — an all-honest slot-miss bare OBFT does not have (Class A; see [§Liveness / All-honest liveness parity](#all-honest-liveness-parity)). Aggressive sizing is therefore **non-parity**, admissible only when production telemetry shows mesh propagation tighter than the standard P99 assumption.
...
What L_Bid changes is the **L_0..L_{K-1} broadcast deadlines** and the **MEV-fetch budget**: the L_Bid-side constraint is `T_0_broadcast_max = T_0_arrival − B_0_LBid = T_commit − Δ_minicon − B_0_LBid` (with tighter `B_0_LBid = 0.5 BTT` at Config A vs bare OBFT's `B_0 = 2·BTT + SafetyBuffer = 1100ms` at default SafetyBuffer; L_Bid budgets only typical-mesh propagation at the bid layer, leaving the reflood-tail to L_0 fall-through). The bundle must ALSO remain in-envelope for L_0 σ-pool aggregation if mini-consensus fails — so the leader broadcasts at the EARLIER of L_Bid's deadline and bare OBFT's `T_commit − B_0` deadline. Net broadcast-deadline shift vs bare OBFT is `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` — the L_0-side reference is the recovery-floored `max(B_0, 3·BTT)` (see [§Setting](#setting), "Recovery margin"). Bare OBFT's buffer beyond typical-mesh (`max(B_0, 3·BTT) − B_0_LBid`; = `1.5·BTT + SafetyBuffer = 1000ms` at Config A default SafetyBuffer where the floor is dormant, = `2.5·BTT = 500ms` at SafetyBuffer=0 where the floor binds) is reused as mini-consensus headroom for free, and any excess of `Δ_minicon` over that buffer is the actual MEV-fetch cost L_Bid pays. At Config A with **default SafetyBuffer = 700ms** (BTT ≤ 700, floor dormant) the buffer is 1000ms — wide enough to absorb every named sizing free:
...
At **SafetyBuffer = 0** (fully-meshed opt-out) the recovery floor binds (RD < 1·BTT), holding the L_0 budget at `3·BTT`, so the buffer-beyond-typical is `2.5·BTT = 500ms` — wide enough that **no named sizing overshoots**. All match floored bare OBFT V_0 at `T_commit − 3·BTT = 3000ms` (MEV-fetch ~2850ms):

- **Conservative** (`Δ_minicon = 2 BTT = 400ms`): shift = 0 (400 < 500). L_0 broadcast at `3000ms` (= bare OBFT V_0, floored); MEV-fetch ~2850ms.
- **Standard** (`Δ_minicon = 1.5 BTT = 300ms`): shift = 0. L_0 broadcast at `3000ms`; MEV-fetch ~2850ms.
- **Aggressive** (`Δ_minicon = 0.5 BTT = 100ms`): shift = 0. L_0 broadcast at `3000ms`; MEV-fetch ~2850ms.

### L_Bid broadcast-deadline tightening

> **Recovery-floor note — the SafetyBuffer=0 figures in this appendix predate it.** The recovery margin floor `max(SafetyBuffer, 1·BTT)` (see [§Setting](#setting), "Recovery margin") changes the SafetyBuffer=0 analysis below. At RD=0 the L_0 budget is floored to `3·BTT` (not `2·BTT`), so: **(i)** bare OBFT V_0's broadcast moves 1·BTT earlier (BTT=200: 3200→**3000ms**; MEV-fetch ~3050→**~2850ms**); **(ii)** the buffer-beyond-typical is `2.5·BTT` (not `1.5·BTT`), which absorbs every named `Δ_minicon` — so L_Bid imposes **zero MEV-fetch cost at RD=0 too** (conservative no longer overshoots; the per-line "conservative pays 0.5·BTT at RD=0" claims below are reversed); **(iii)** the RD=0 BTT envelope shrinks ~1·BTT (bare OBFT fits to BTT ≈ 800ms, not 1200ms). The **default SafetyBuffer=700ms figures are unaffected for BTT ≤ 700ms** (floor dormant); for **BTT ≥ 800ms the floor binds at default RD too** (since `1·BTT > 700ms`), so the default-RD table and envelope below shift as well. (Note: L_Bid is spec-only — not in the consensustest adapter — so unlike §Setting/§Application these figures have no data/test backstop; the RD=0 and high-BTT numbers below have been refreshed by hand for the floor.)

Bare OBFT's reflood-aware `B_0 = 2·BTT + SafetyBuffer` (= 1100ms at Config A default SafetyBuffer=700ms) decomposes as ~0.5 BTT typical-mesh propagation plus a wider **reflood-absorption buffer** — extra time between expected typical-mesh arrival and `T_commit` covering one full IHAVE/IWANT lazy-push cycle when initial eager-push fails to reach all honest peers (see [§Setting](#setting)). The reflood buffer is `B_0 − 0.5·BTT = 1.5·BTT + SafetyBuffer = 1000ms` at default SafetyBuffer; at `SafetyBuffer = 0` (fully-meshed opt-out) it collapses to `1.5·BTT = 300ms`. The reflood buffer is **required at every layer** in bare OBFT because σ-or-NR commit is mandatory: each layer must reach σ-quorum or NR-quorum at `T_commit` for the cluster to make progress, so without it the reflood tail would leak past `T_commit` and operators could wrongly NR-emit on layers where the bundle was actually in flight.
...
- If no `verdict_quorum` forms, the cluster falls through to L_0 — which is bare OBFT's L_0 with its full reflood-aware `B_0 = 2·BTT + SafetyBuffer`. So the bid layer doesn't need its own reflood-absorption; bare OBFT's L_0 covers the case.
...
The L_0 deadline binds whenever `Δ_minicon + B_0_LBid ≤ max(B_0, 3·BTT)` — equivalently, `Δ_minicon ≤ max(B_0, 3·BTT) − B_0_LBid` (= `1.5·BTT + SafetyBuffer` when the floor is dormant, `2.5·BTT` when it binds). At Config A default SafetyBuffer=700ms this means every `Δ_minicon ≤ 1000ms` clamps L_Bid's broadcast at bare OBFT's `2500ms` deadline; at SafetyBuffer=0 the recovery floor holds the L_0-side reference at `3·BTT`, so the threshold is `2.5·BTT = 500ms` and every named `Δ_minicon` (incl. conservative `400ms`) still binds at L_0 — no overshoot.

In the L_0-binding regime the bundle's *effective* bid-layer propagation budget is `max(B_0, 3·BTT) − Δ_minicon`, which exceeds the spec's nominal `B_0_LBid = 0.5·BTT` — providing additional reflood-tail absorption at the bid layer "for free." Under default SafetyBuffer this is 600-1000ms of headroom depending on sizing; under SafetyBuffer=0 the recovery floor holds the reference at `3·BTT`, giving `3·BTT − Δ_minicon = 200-500ms`.

**Aggressive sizing's MEV-fetch ceiling.** Under bare OBFT's reflood-aware schedule, L_Bid's leader broadcast is bounded above by bare OBFT V_0's deadline (the L_0 constraint binds at every named sizing under default SafetyBuffer). Aggressive L_Bid therefore matches bare OBFT V_0 in broadcast time and MEV-fetch budget; conservative and standard sizings at default SafetyBuffer do the same (they fit inside the 1000ms reflood-buffer for free). At `SafetyBuffer = 0` the recovery floor holds the buffer at `2.5·BTT = 500ms`; no named sizing overshoots — all (incl. conservative `Δ_minicon = 400ms < 500ms`) remain free, matching floored bare OBFT V_0.

**Timeline comparison** (Config A, BTT=200ms, **default SafetyBuffer=700ms**, L_0 layer; deeper layers broadcast earlier and are off-table):
...
Post-broadcast budget allocation (broadcast → `T_commit`, at default SafetyBuffer=700ms; total = `B_0 = 1100ms` across all variants since L_0 constraint binds):

| Variant | Total | Breakdown |
|---|---|---|
| Bare OBFT | 1100ms (= 2·BTT + SafetyBuffer) | 100ms typical-mesh propagation + 1000ms reflood-absorption buffer (idle in healthy case) |
...
At **SafetyBuffer = 0** (fully-meshed opt-out) the recovery floor holds the post-broadcast budget at `3·BTT = 600ms` (not the naive `2·BTT`); conservative's `Δ_select + Δ_verdict = 400ms` fits with 200ms free headroom, so all sizings fit free — no broadcast shift, all at floored bare OBFT V_0.

Reading the gradient column-by-column (default SafetyBuffer):
...
- **Standard** also matches bare OBFT V_0 in broadcast time at default SafetyBuffer (the reflood buffer absorbs its 300ms `Δ_minicon` for free). Zero MEV-fetch cost vs bare OBFT V_0; medium L_Bid success rate (0.5·BTT settling + P99 verdict propagation).
- **Conservative** matches bare OBFT V_0 in broadcast time at default SafetyBuffer (the reflood buffer absorbs its 400ms `Δ_minicon` for free). Zero MEV-fetch cost vs bare OBFT V_0; highest L_Bid success rate (1·BTT settling + P99 verdict propagation). At SafetyBuffer=0 the recovery floor absorbs its `Δ_minicon` too — zero MEV-fetch cost at every sizing (the floor's `3·BTT` budget subsumes the reflood buffer's role here).
...
- **L_Bid**:
  ...
  - The operator's own `KindBidVerdict` does **not** bind this choice: verdicts are non-binding predictions (see [Rule 8](#slashing-evidence-rules)), and the commitment is determined solely by the two conditions above. An operator that verdicted `V_X` but whose view did not reach `verdict_quorum` emits NR here without fault.
...
1. **L_Bid**: σ-pool plaintext on `V_X` (assembled per the harvest rule below). If `|σ-pool[V_X]| ≥ qV`: reconstruct `(V_X, S)`; halt and broadcast `KindCertificate`.
...
**L_Bid σ-pool harvest — value-keyed for the promoted winner.** L_Bid has no Phase-1 leader of its own; its winner `V_X` is a rotation candidate promoted by the bid. So the L_Bid σ-pool on `V_X` is assembled **by `value_root(V_X)`** — from the plaintext `σ_j^V(V_X)` emitted at L_Bid by σ-state operators, **plus the originating rotation leader's `σ_{L_h}^V(V_X)` witness** (§Phase 2 / Wire format), matched by `value_root` and BLS-verified against that leader's pubshare even though it is tagged at rotation layer `h` — deduplicated per operator. This is the one deviation from bare OBFT's layer-keyed harvest: the leader's partial follows `V_X` up to L_Bid instead of being stranded at the (sealed) rotation layer. (The witness is plaintext at every layer, being a byte-for-byte copy of a Phase-1 partial.)

This counts a partial the byzantine cannot suppress: a leader that wins L_Bid by bid and then withholds σ at Phase 2 still had to broadcast `V_X` with `σ_{L_h}^V` to ≥ `qV − f` honest to form `verdict_quorum`. At **f=1** that closes the candidate-withholding-without-equivocation 2-1-byz-defect (2 honest σ + 1 witness = `qV`); at `f ≥ 2` the deficit is `f > 1`, so the single witness narrows but does not close it, and the equivocation variant is not closed at any `f` (see [New residual failure modes](#new-residual-failure-modes-at-l_bid)).

**Safety — unchanged from bare OBFT.** Pigeonholes hold: only BLS-verified `σ^V(V_X)` partials count (forgeries rejected at verify time); the witness adds ≤ 1 partial (one leader per value, deduped per operator); and L_Bid σ-reconstruct vs fall-through stay mutually exclusive at all `n = 3f+1` — if `σ-pool = e_h + 1 ≥ qV = 2f+1` then honest σ-emitters `e_h ≥ 2f`, so honest NR-emitters `≤ 1` and `NR-pool ≤ 1 + f < qEnc`; conversely NR-quorum forces `e_h ≤ f`, so `σ-pool ≤ f+1 < qV`. The witness may be counted at both L_Bid and origin layer `h`, but the walk halts at the first σ-quorum layer (L_Bid is outermost), so no second `V` reconstructs. (The reference σ-pool is layer-keyed `sigmaPool[layer][value_root][op]`; this value-keyed harvest for the winner is the one deviation an L_Bid implementation must add — a build-time note, as L_Bid is not yet implemented.)
...
- **Cost — no fall-through if L_Bid deadlocks.** Adversarial-byz patterns at L_Bid (equivocation-promotion, verdict-equivocation, and candidate-withholding at f≥2; see [§New residual failure modes](#new-residual-failure-modes-at-l_bid)) can produce σ-pool < `qV` AND NR-pool < `qEnc` simultaneously. Without NR-quorum, the rotation-layer chained encryption stays sealed, and rotation-layer σ-pools — even if they would otherwise reach `qV` — cannot reconstruct. The slot misses.
...
- **Rule 8 — Verdict equivocation.** Operator `i` broadcasts two distinct `KindBidVerdict` envelopes for the same slot. Self-contained slashable evidence — both signed envelopes exist on the wire. **Verdicts are non-binding predictions, not σ-commitments**: an operator's Phase-2 commitment is governed solely by the L_Bid commitment rule (`verdict_quorum` observed ∧ retain ∧ valid → σ, else NR). So a verdict-vs-action mismatch in *either* direction — verdict `V_X` then Phase-2 NR (the operator's own view never reached `verdict_quorum`), or null verdict then Phase-2 σ on `V_X` (late re-flood completed the view) — is permitted honest behavior under partial verdict propagation, **not** a fault. (Making verdict-vs-action slashable would be unsound: at `Δ_verdict ≥ 1·BTT` a benign all-honest verdict split would otherwise force honest operators to emit self-incriminating evidence. The deterrence lost is negligible — a byz that verdicts `V_X` to pad `verdict_quorum` and then defects gets the identical deadlock from the *silent* variant, which leaves no verdict-vs-action evidence regardless, so the dropped arm only ever caught an irrational byz.)
...
|---|---|---|
| Phase-1 bundle equivocation, bid-metadata equivocation, verdict equivocation | Cryptographic, self-contained — single signed message-pair conclusively demonstrates the action | Very low |
...
#### All-honest liveness parity

At `Δ_verdict ≥ 1·BTT`, the all-honest case (no byzantine) matches bare OBFT — L_Bid adds no all-honest slot-miss (the argument is f-agnostic; holds at all `n = 3f+1`). Either a value reaches `verdict_quorum` (its `≥ qV` honest verdicters retain and σ on it → σ-quorum → output), or none does (every honest NRs at L_Bid → NR-quorum → clean fall-through to L_0, bare OBFT's L_0 unchanged). So L_Bid is success-or-clean-fall-through at its own layer, and its all-honest miss set is a **subset** of bare OBFT's.

The only residual all-honest miss is the **shared** L_0 2-2 validity floor (assumption 3 — reached whenever `V_{L_0}` is itself 2-2 validity-split; bare OBFT misses it identically at L_0, see [§Implications of validity-divergence](#implications-of-validity-divergence-not-being-recovered-assumption-3)). On 2-2 validity L_Bid's own layer falls through (verdict-gating forces unanimous NR); the miss is *inherited* at L_0, never added — and L_Bid can occasionally *recover* it when a valid alternative wins the bid and routes around the split `V_{L_0}`. The lone all-honest regression is aggressive sizing's partial verdict propagation (`Δ_verdict < 1·BTT`), which the parity floor excludes (Class A, below). The adversarial 2-2 case (a byz withholds NR to block L_Bid fall-through) is a separate residual, also below. Safety is identical to bare OBFT throughout (see [§Safety](#safety)).

#### Recovery scope at L_Bid

The mini-consensus addresses the three deadlock classes from [§Background — bid-layer deadlocks](#background--bid-layer-deadlocks). C3 closes cleanly (3-of-4 majority); C1 closes when no V reaches `verdict_quorum` and — via the value-keyed witness harvest — at **f=1** even under minority-withholding (residual at f≥2); C2 closes when no V reaches `verdict_quorum`, else folds into [equivocation-promotion](#new-residual-failure-modes-at-l_bid):

- **C1 — Selective candidate withholding.** Byz withholds its L_Bid-eligible Phase-1 bundle from a subset of honest peers. **Closed when no V reaches `verdict_quorum`** (affected peers verdict null or on non-byz candidates → pool fragments below `qV` → fall-through). When the byz instead withholds from exactly the minority to force `verdict_quorum` on `V_X` and then withholds σ at Phase 2, the **value-keyed witness harvest ([§Phase 3](#phase-3--reconstruction-walk)) closes it at f=1**: forming the quorum requires delivering `V_X` (with `σ_{L_h}^V`) to ≥ `qV − f` honest, and that leaked partial completes σ-quorum. **At `f ≥ 2` the residual remains** (deficit `f > 1`) and folds into [2-1-byz-defect](#new-residual-failure-modes-at-l_bid).
- **C2 — Candidate / bid equivocation.** Byz sends conflicting signed Phase-1 bundle views to different peers. **Closed when no V reaches `verdict_quorum`** (verdicts split widely → fall-through). **Not closed when the byz reveals the conflict to enough honest to fragment the σ-side** while a passive byz keeps the NR-side short — detected equivocation forces those honest to NR, so the witness harvest cannot complete σ-quorum. This is the genuinely-new residual (folds into [equivocation-promotion](#new-residual-failure-modes-at-l_bid)); slashable (base leader-equivocation / Rule 7).
...
With the value-keyed witness harvest ([§Phase 3](#phase-3--reconstruction-walk)) and `Δ_verdict ≥ 1·BTT`, the L_Bid residuals are narrower than a naive bid-routing extension's. All below are slot-miss without fall-through (chained encryption to L_0 stays sealed when L_Bid reaches neither σ-quorum nor NR-quorum). Each is labeled **Class B** (permitted byzantine grief within the f-bound, deterred across slots by [assumption 4](#implications-of-the-rational-byzantine-deterrent-assumption-4)) or **Class A** (assumption violation):

- **[Class B] Candidate-withholding 2-1-byz-defect — closed at f=1.** Byz wins `verdict_quorum` on its `V_X` by delivering it to ≥ `qV − f` honest (the minimum needed) while withholding from the rest, then withholds σ at Phase 2 (NR-emit or silent). The value-keyed witness harvest ([§Phase 3](#phase-3--reconstruction-walk)) completes σ-quorum at **f=1** (2 honest σ + 1 leader witness = `qV`) → **success, not a miss**: the byz cannot suppress the `σ_{L_h}^V` it had to leak to form the quorum. **Residual only at `f ≥ 2`**, where the deficit `f` exceeds the single witness — `σ-pool = (f+1) + 1 < qV`, `NR-pool = f + (≤ f byz) < qEnc` → deadlock. Evidence is behavioral (candidate-silence) for either Phase-2 action; the narrowed Rule 8 does not fire on verdict-then-NR (verdicts are non-binding).
- **[Class B] Equivocation-promotion — the genuinely-new L_Bid cost.** In bare OBFT a byz *deep-layer* leader that equivocates is harmless: the honest L_0 σ-quorums first and the deep layer stays encrypted. L_Bid makes the bid layer the **outermost gate**, so a byz that is *any* rotation leader can inflate its `bid_value` to promote its equivocated candidate to L_Bid, then equivocate with tight timing — revealing the conflict to enough honest to fragment the σ-side below `qV` while a passive byz keeps the NR-side below `qEnc` → L_Bid deadlocks, sealing the honest L_0 below → miss. This raises the equivocation-miss surface from **`1/n`** (bare OBFT: byz must be the L_0 leader) to **`K/n`** (byz any rotation leader + a bid lie). The witness harvest does **not** close it (detected equivocation forces honest to NR). At `f=1` it bottoms out at Pigeonhole 1's algebraic limit (1 honest σ-locked + 1 passive byz: `σ-pool = 1 (honest) + 1 (witness) < qV`, `NR-pool = 2 < qEnc`) — the same wall bare OBFT and 2abOBFT hit at L_0 (1-1-1 equivocation; validity + passive byz). Evidence: base leader-equivocation / Rule 7 (cryptographic). **Relay/builder attestation ([§Optional extension](#optional-extension--relaybuilder-attestation-verification)) is the structural mitigation** — capping `bid_value` at a recognized relay's commitment bars promotion-by-inflation, so a byz reaches the gate only with a *genuine* top bid, returning the frequency toward bare OBFT's `1/n`. Attestation is therefore **liveness-load-bearing** here, not only an MEV-honesty measure.

  **No defer/no-lock state.** A 2abOBFT-style `KindNoValue` (deferring the σ/NR commitment) is the usual lever for σ-locked-split equivocation, but it does not rescue these residuals at `f=1`: its recovery is toward σ-quorum (harvest-eligible operators *upgrade* to σ on the majority value), yet the promotion attack makes operators *observe* the equivocation — which forces NR; and un-σ-locking a value-holder to feed NR-quorum would let one operator feed both pools, breaking Pigeonhole 1 (safety). So the equivocation residual is irreducible at `f=1` without discarding the eager-σ head-start that defines the design. Manage it with the witness harvest (closes candidate-withholding), attestation (bounds promotion frequency), and assumption 4 — not added wire machinery. (At larger `n`/`f` the cohort algebra is looser, so whether a defer state recovers any of this residual is an open question — moot at the recommended `f=1, n=4`, and relevant only if `n ≥ 7` adversarial liveness becomes a hard requirement.)
- **[Class B — slashable] Verdict-equivocation — a rider, not a standalone deadlock.** Issuing different `KindBidVerdict`s to different peers fragments per-peer verdict-pool views. It is **not** a standalone deadlock: if honest agree on the bid winner, their `≥ qV` matching verdicts form `verdict_quorum` and a byz can only *add* to a pool, never subtract honest verdicts ([Pigeonhole on verdicts](#convergence-rule-for-l_bid)). It bites only as a **rider on a pre-existing honest verdict split** (bid-view divergence), tipping some honest over `qV` while others stay under — the same σ-locked-split / Pigeonhole-1 limit as equivocation-promotion. Rule 8 fires on the two-distinct-verdict pair regardless of Phase-2 action — cryptographic.
...
- **[Class A — assumption 3, byz-blocked fall-through] 2-2 validity-divergence.** When honest split 2-2 on `V_X` validity, L_Bid's verdict-gating makes the **all-honest** case fall through cleanly to L_0 — the miss, if any, is bare OBFT's *inherited* L_0 floor (see [All-honest liveness parity](#all-honest-liveness-parity)). The L_Bid-*layer* deadlock requires a **byz to withhold NR**, keeping the NR-pool below `qEnc` while the σ-side is already below `qV`, blocking fall-through. Bare OBFT faces the same 2-2 validity limit at L_0; re-orgs are real-world, so no slashing applies. (See [§Implications of validity-divergence not being recovered (assumption 3)](#implications-of-validity-divergence-not-being-recovered-assumption-3).)

#### Trigger frequency

Candidate-withholding (residual only at `f ≥ 2`) and equivocation-promotion require the byz to be one of the slot's `K` rotation leaders — roughly `K/n` slots under uniform selection (every slot at `K = n`); with `f` byz, `P(≥1 byz is a leader) = 1 − C(n−f, K)/C(n, K) ≥ K/n`. Verdict-equivocation is available every slot (every operator broadcasts a `KindBidVerdict`) but bites only as a rider on bid divergence (above). Compared with bare OBFT's `1/n` L_0-only equivocation surface, L_Bid's outermost-gate raises the equivocation-miss frequency toward `K/n` — which **relay attestation bounds back toward `1/n`** by barring promotion-by-inflation.
...
| L_Bid equivocation-promotion, verdict-equivocation, partial-propagation, or f≥2 candidate-withholding deadlock | slot misses | Deadlock at L_Bid blocks fall-through |

Post-`T_commit` timing **matches bare OBFT** since mini-consensus runs pre-`T_commit`. Under the reflood-aware schedule, L_Bid's pre-`T_commit` cost is `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` of MEV-fetch budget — `Δ_minicon`'s overlap with bare OBFT's buffer-beyond-typical (`max(B_0, 3·BTT) − B_0_LBid`, using the recovery-floored L_0 reference) doesn't cost extra; only the excess does. At Config A default SafetyBuffer=700ms (BTT=200, floor dormant) the buffer is 1000ms wide — all three named sizings fit free, so the L_Bid extension is zero-cost on MEV-fetch under default SafetyBuffer (and, thanks to the floor, at SafetyBuffer=0 too).

### Deployment envelope by BTT

`T_commit` is back-end-anchored and invariant across bare OBFT and OBFT+L_Bid. What L_Bid changes is the **L_0..L_{K-1} broadcast deadline**, which shifts earlier by `max(0, Δ_minicon − (max(B_0, 3·BTT) − B_0_LBid))` to fit mini-consensus pre-`T_commit`. Under the reflood-aware schedule (with the recovery floor on the L_0 reference), the primary leader's MEV-fetch budget shrinks only by the excess of `Δ_minicon` over that buffer — zero whenever `Δ_minicon ≤ max(B_0, 3·BTT) − B_0_LBid` (= `1.5·BTT + SafetyBuffer` when the floor is dormant, `2.5·BTT` when it binds), which holds for every named sizing at both default SafetyBuffer and SafetyBuffer=0.

The table below shows L_0 broadcast deadline (= MEV-fetch budget at `slot_start = 0`) across BTT regimes at **default SafetyBuffer = 700ms** (`T_relay_cutoff = 4.0s`, `submit_headroom = 100ms`, `ε_3 = 50ms`, tightened `Δ_2 = 1·BTT`, `B_0 = 2·BTT + 700ms` for bare OBFT, `B_0_LBid = 0.5 BTT` for L_Bid). `T_commit` scales with BTT as `≈ 3800 − BTT` (= `Relay_cutoff − Δ_2 − ε_3 − submit_headroom − ~50ms jitter`); `T_broadcast_max_0` for bare OBFT = `T_commit − B_0 = 3800 − 3·BTT − SafetyBuffer`. Bare OBFT row included for comparison:
...
| 800ms | 600ms ✓ tight | 600ms ✓ tight | 600ms ✓ tight | 600ms ✓ tight |
| 1000ms | **−200ms ✗** | **−200ms ✗** | **−200ms ✗** | **−200ms ✗** |
| 1200ms | **−1000ms ✗** | **−1000ms ✗** | **−1000ms ✗** | **−1000ms ✗** |

(L_Bid's broadcast deadline matches bare OBFT V_0 exactly across the board — every named `Δ_minicon` fits free, so L_Bid imposes zero MEV-fetch cost under default SafetyBuffer. The recovery floor `max(SafetyBuffer, 1·BTT)` (see [§Setting](#setting)) binds for BTT ≥ 800ms — where `1·BTT > SafetyBuffer = 700ms` — pulling the broadcast to `T_commit − 3·BTT` there, so bare OBFT's MEV-fresh envelope is **BTT ≤ 800ms** (at BTT ≥ 1000ms the primary clamps to BFT_start). Deployments at BTT ≥ 1000ms must reduce SafetyBuffer or accept that the primary loses MEV-freshness. For BTT ≥ 800ms the default-RD and SafetyBuffer=0 schedules coincide, since the floor dominates SafetyBuffer.)

**At SafetyBuffer = 0** (fully-meshed opt-out) the recovery floor binds (`RD < 1·BTT`), holding the L_0 budget at `3·BTT` — so L_Bid still imposes **zero** MEV-fetch cost (every named `Δ_minicon` fits the `2.5·BTT` buffer; all sizings match floored bare OBFT V_0), but every broadcast lands `1·BTT` earlier than the pre-floor `2·BTT` schedule and the envelope shrinks accordingly:

| BTT | Bare OBFT | Δ_minicon=2 BTT (conservative) | Δ_minicon=1.5 BTT (standard) | Δ_minicon=0.5 BTT (aggressive) |
|---|---|---|---|---|
| 200ms | 3000ms ✓ | 3000ms ✓ | 3000ms ✓ | 3000ms ✓ |
| 400ms | 2200ms ✓ | 2200ms ✓ | 2200ms ✓ | 2200ms ✓ |
| 600ms | 1400ms ✓ | 1400ms ✓ | 1400ms ✓ | 1400ms ✓ |
| 800ms | 600ms ✓ tight | 600ms ✓ tight | 600ms ✓ tight | 600ms ✓ tight |
| 1000ms | **−200ms ✗** | **−200ms ✗** | **−200ms ✗** | **−200ms ✗** |
| 1200ms | **−1000ms ✗** | **−1000ms ✗** | **−1000ms ✗** | **−1000ms ✗** |

(At SafetyBuffer=0 the recovery floor holds the buffer at `2.5·BTT`, so the L_Bid loss vs bare OBFT = `max(0, Δ_minicon − 2.5·BTT) = 0` at every named sizing — conservative `Δ_minicon = 2·BTT < 2.5·BTT` no longer overshoots. The floor moves every broadcast `1·BTT` earlier than the naive `2·BTT` schedule, so the RD=0 envelope is BTT ≤ 800ms — identical to the default-RD envelope above, since the floor dominates SafetyBuffer for BTT ≥ 800ms. Negative MEV-fetch means the L_0 broadcast deadline is before slot_start; the primary clamps to BFT_start.)

**Net for deployment selection.** At production-typical BTT (200-400ms) with default SafetyBuffer, every L_Bid sizing matches bare OBFT V_0's MEV-fetch budget exactly — the trade reduces to "L_Bid success rate vs implementation complexity," not MEV-fetch. The reflood-buffer absorption means L_Bid is essentially free **on MEV-fetch budget** under default SafetyBuffer — degraded mesh still pays, but via L_Bid *success rate* (more fall-through to vanilla L_0), not budget (the success-rate gradient, [§Per-layer windows and deadlines](#per-layer-windows-and-deadlines)); conservative gives the highest L_Bid success rate (1·BTT settling) at no MEV cost. Under the tightened per-emission sizing with the recovery floor (see [§Setting](#setting)), bare OBFT's MEV-fresh envelope is BTT ≤ 800ms — the floor `max(SafetyBuffer, 1·BTT)` binds for BTT ≥ 800ms (where `1·BTT > 700ms`), pulling the L_0 broadcast to `T_commit − 3·BTT`; at BTT ≥ 1000ms the primary clamps to BFT_start regardless of SafetyBuffer (the floor dominates). At SafetyBuffer=0 the floor absorbs every named `Δ_minicon`, so all sizings (incl. conservative) remain free on MEV-fetch — the floor subsumes the reflood buffer's role at the opt-out. The L_Bid trade has two knobs: `Δ_minicon` controls L_Bid success rate (larger Δ_minicon → more bundle tail-absorption) and — only when it exceeds the reflood buffer — MEV-fetch budget; `Δ_verdict` controls verdict-propagation safety (≥ 1·BTT for P99 guarantees). Choose both from production telemetry. Bare OBFT (no `Δ_minicon`) remains available when the trade isn't favorable.
...
- **Selective candidate withholding** (C1) — attestation does not address it; the value-keyed witness harvest ([§Phase 3](#phase-3--reconstruction-walk)) does, closing it at **f=1** (residual at `f ≥ 2`; see [§Recovery scope at L_Bid](#recovery-scope-at-l_bid)). Falls through cleanly when no V reaches `verdict_quorum`.
...
| Slashing-evidence rules | 5 | 7 (+ Rule 7 bid-metadata equivocation, + Rule 8 verdict equivocation) |
...
| L_0 broadcast deadline | `T_broadcast_max_0 = T_commit − B_0` (e.g., 2500ms at Config A with tightened Δ_2 = 1·BTT and `B_0 = 2·BTT + SafetyBuffer = 1100ms` at default SafetyBuffer=700ms) | `T_0_broadcast_max = T_commit − max(Δ_minicon + B_0_LBid, B_0)` — at default SafetyBuffer the L_0 constraint binds, so deadline coincides with bare OBFT V_0 (2500ms at Config A) for every named `Δ_minicon`. See [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening). |
| MEV-fetch budget (4s cutoff, `header_submit_headroom = 100ms`, §Application's max-MEV anchor) | ~2350ms (V_0; T_commit = 3.60s) at default SafetyBuffer; ~2850ms at SafetyBuffer=0 (floored) | At default SafetyBuffer: ~2350ms (V_X) at every named sizing (= bare OBFT V_0; reflood buffer fully absorbs `Δ_minicon`). At SafetyBuffer=0: ~2850ms at every named sizing (= floored bare OBFT V_0; the recovery floor absorbs `Δ_minicon`, so conservative no longer pays) |
...
| L_Bid liveness — C1 selective candidate withholding | n/a (no bid layer) | **Closed**: fall-through when no verdict-quorum forms; when the byz forces the quorum and defects, the value-keyed witness harvest closes it **at f=1** (residual at f≥2, Class B). |
| L_Bid liveness — C2 candidate / bid equivocation | n/a | **Conditional**: fall-through when no verdict-quorum forms; otherwise the genuinely-new equivocation-promotion residual (Class B, `K/n`, bounded by relay attestation). |
| L_Bid liveness — C3 validity-divergence majority (3-of-4) | n/a | **Closed** (verdict-quorum reaches on majority; minority NR) |
| L_Bid liveness — candidate-withholding / equivocation-promotion | n/a | **Class B**: candidate-withholding **closed at f=1** by the witness harvest (residual at f≥2, behavioral evidence); equivocation-promotion is the genuinely-new residual (`K/n`, base leader-equivocation / Rule 7, frequency bounded by relay attestation); assumption-4 deterred across slots |
| L_Bid liveness — verdict-equivocation | n/a | **Class B** (rider on bid divergence, not standalone; slashable Rule 8; assumption-4 deterred): slot-miss-without-fall-through |
...
| L_Bid liveness — 2-2 validity split | shared hard algebraic limit at L_0 (BFT-theoretical, not protocol-specific) | **Class A** (assumption 3): all-honest falls through to the *inherited* L_0 floor; an L_Bid-*layer* deadlock needs a byz to block NR. Same hard limit either way; not attributable, no slashing |
...
>
> **Note on stale primary-MEV-fetch comparison:** The motivational claim below — that L_Bid_New gains 300ms (conservative) / 200ms (standard) / 0 (aggressive) of primary MEV-fetch over current L_Bid — reflects an older L_Bid sizing where Δ_minicon directly compressed the primary's broadcast deadline. Under bare OBFT's current reflood-aware schedule (see [§L_Bid broadcast-deadline tightening](#l_bid-broadcast-deadline-tightening)), current L_Bid's reflood-buffer absorbs Δ_minicon for free at every named sizing under default SafetyBuffer (= 700ms), so current L_Bid *already* matches bare OBFT V_0 (~2350ms) — leaving L_Bid_New with **zero** primary-MEV gain to offer at default SafetyBuffer. At SafetyBuffer=0 the recovery floor (see [§Setting](#setting)) makes current L_Bid free at every sizing too (the floored `3·BTT` L_0 budget absorbs `Δ_minicon`), so L_Bid_New's primary-MEV gain is **zero at SafetyBuffer=0 as well**. Treat the 200-300ms figures throughout this appendix as historical motivation, not as current operating-point numbers.
...
The structural trade vs current L_Bid: L_Bid_New preserves bare OBFT V_0's primary MEV-fetch budget (~2350ms at default SafetyBuffer, ~2850ms at SafetyBuffer=0 — the floored bare OBFT V_0) at every sizing by excluding bid_1 from mini-consensus, at the cost of exposing bid_1 to bare-OBFT-style asymmetric-delivery patterns at the bid layer (where current L_Bid uses mini-consensus to converge on bid_1's status). The primary-MEV-fetch gain vs current L_Bid is sizing-dependent: 300ms at conservative, 200ms at standard, **0 at aggressive** (where current L_Bid already matches bare OBFT V_0 because `Δ_minicon = 0.5 BTT`). Post-`T_commit` latency is the same order as current L_Bid because both variants run mini-consensus before `T_commit`. The timing distinction is pre-`T_commit`: current L_Bid starts mini-consensus at `T_0_arrival` and shifts every eligible rotation-layer deadline earlier; L_Bid_New starts mini-consensus at `T_deep_arrival` and shifts only deep-layer deadlines earlier.
...
`T_commit` is the same back-end anchor as bare OBFT (`Relay_cutoff − 2 BTT = 3600ms` at Config A post-tighten). L_Bid_New pays the `max(0, Δ_minicon − 0.5 BTT)` pre-`T_commit` cost only on deep-layer candidate deadlines; the primary L_0 MEV-fetch budget remains `T_broadcast_max_0^bare − slot_start` ≈ 2850ms at SafetyBuffer=0 (= floored bare OBFT's primary budget; ≈ 2350ms at default SafetyBuffer=700ms). Post-`T_commit` timing matches current L_Bid and bare OBFT.
...
| Primary MEV-fetch budget | ~2850ms at every sizing (= floored bare OBFT V_0; at SafetyBuffer=0 the recovery floor absorbs `Δ_minicon`, collapsing the historical per-sizing gradient) | ~2850ms (= floored bare OBFT V_0; no `Δ_minicon` shift on primary at any sizing) |
...
| Adversarial-byz exposure surface at the bid layer | C1 closed at f=1 (witness harvest); equivocation-promotion, verdict-equivocation, f≥2 candidate-withholding, sub-1-BTT `Δ_verdict` residual | bare-OBFT-style L_0 exposure for bid_1 + deep mini-consensus residuals + verdict-equivocation on deep bids |
