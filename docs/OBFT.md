...
- **[Class A]** **Asymmetric propagation past `T_commit` (real propagation from L_k's broadcast to first-observation > `B_k` at layer L_k)** — violates assumption 2 (partial synchrony) for that layer. Honest who first-observe V past `T_commit` treat the leader as silent and NR. If the resulting σ-pool falls below qV at this layer, the cluster falls through to a deeper backup whose own `B_{k+1}` is wider (per-layer budgets are staggered: `B_0 = 1 BTT` for the primary up to `B_{K-1} = T_commit` for the deepest at K=4 Config A; see §Setting). If propagation also exceeds the deepest layer's absorption ceiling (= the full slot's commit budget), slot misses cleanly. **No safety violation.**
...
| Termination (output guaranteed) | Conditional. **One-liner: consensus expected to complete by `slot_start + 3.85s` at SSV proposer-duty operating point (n = 4, f = 1, BTT = 200ms, K = 4, T_commit = 3.40s, recommended Δ_2 = 2 BTT = 400ms, Δ_3 = ε_3 ≈ 50ms), with submission slack to `slot_start + 4.00s` for relay submit (`header_submit_headroom = 100ms` plus ~50ms additional jitter buffer); under conditions: (a) ≤ f operators byzantine/offline, (b) real propagation from leader L_k broadcast to any honest first-observation ≤ that layer's per-layer budget `B_k` (staggered: B_0 = 1 BTT, ..., B_3 = T_commit at K=4 — see §Setting), (c) host validity unanimous at decision time (assumption 3), (d) `K ≥ 3` (late-leader resilience — the SSV default `K = n = 4` already satisfies this).** Single-round protocol; per-layer staggered budgets give the deepest layer the entire slot's commit budget for absorption (`B_{K-1} = T_commit`, leader broadcasts at BFT_start), with shallower layers tightening for MEV freshness. |
...
| Partial-synchrony absorption | Per-layer staggered: `B_k` for layer L_k. At K=4 Config A: `B_0 = 1 BTT = 200ms` for primary L_0 (tightest, freshest MEV) up to `B_{K-1} = T_commit = 3400ms` for deepest L_3 (earliest possible — broadcasts at BFT_start, last-resort tail covering the entire slot). Cluster falls through layers based on which one's bundle actually arrived by `T_commit`. |
...
**MEV-fetch-budget asymmetry.** V_0's 3050ms is the freshest budget; deeper layers trade fetch time for wider propagation budget. Per-leader budgets at this operating point: `[V_3: ~0ms (slot start), V_2: 2750ms, V_1: 2950ms, V_0: 3050ms]`. The staggered design lets V_0 capture maximum MEV under healthy propagation while deeper backups absorb tails when V_0's bundle doesn't reach in time; the deepest layer fetches at slot start (deepest-confirmed parent) and broadcasts immediately, giving the cluster the entire commit budget for that bundle's propagation.
