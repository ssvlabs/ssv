...
**This is a TBFT-family limitation, not specific to OBFT.** [TBFT.md](TBFT.md) "Application-validity-divergence — known liveness limit" documents the same algebraic deadlock with the same root cause: per-operator independent validity verdict + leader's Phase-1 σ_V locked + cross-phase exclusivity. OBFT inherits all three from TBFT and adds nothing that recovers from it — R-round retry doesn't help (verdicts are locked at acceptance), Defer state doesn't help (Defer is for partition recovery, not verdict reconciliation), L_C consensus doesn't help (it coordinates the frontier layer, not the validity verdict).
...
- **σ-emit (start of window)**: operators in σ-state at T_commit_r emit their σ partials (Phase-2 onion) immediately at T_commit_r. Operators who become σ-eligible *during* the window (e.g., late re-flood delivers V, equivocation evidence resolves) emit σ as soon as eligibility is determined.
...
**Late σ-emit limitation.** A σ-emit at time `T_commit_r + t` (where `t` is during Phase 2) propagates to other honest by `T_commit_r + t + D + δ`. For other honest to observe it before their NR/Defer decision at `T_commit_r + Δ_2`, we need `t + D + δ ≤ Δ_2`, i.e., `t ≤ Δ_2 − (D + δ)`. With `Δ_2 = D + δ` (the minimum), this requires `t = 0` — only σ-emits at the very start of Phase 2 are observable in time. Late σ-emits (operators receiving V near end of Phase 2) won't propagate before NR-decision and are effectively invisible for that round's commitment. This is part of the byzantine selective-delivery grief surface — see "Failure modes / Selective-delivery grief". Setting `Δ_2 ≥ 2(D + δ)` widens the late-σ-emit window at proportional latency cost.
...
- **OBFT-strict-superset cases** (OBFT recovers in fewer RTTs or more cleanly): healthy, byzantine-leader-silent (in-round NR fall-through vs. round-change), aggressive marginal, all-Defer equivocation (NR-fall-through to L_1 in round R when byz delivers V's early enough that re-flood spreads conflicts before σ-emit), multi-leader-failure (K-layer parallel fall-through within one round vs. K-1 serial round-changes), most adversarial-scheduling patterns within R · D. These are OBFT's wins because (a) NR fall-through is in-protocol within a single round, no round-change needed; (b) per-round overhead (~250ms at Config A) is much smaller than QBFT's RT (~2s).
...
- **[Class A]** **Late deepest-layer leader broadcast at K=2 (NR-lock at backup layer).** A backup leader L_{K-1} (= L_1 at K=2) whose Phase-1 bundle's first cluster-observation arrives after `T_candidate_accept_1` — e.g., the leader's fetch loop overruns Δ_1 due to slow beacon node, MEV-relay timeout, or head-change refresh — gets auth-only-retained for round 2 acceptance. *But* by round-1 NR-decision time at L_1, no honest has V_{L_1} retained → no honest is σ-eligible at L_1 → no peer σ-emit observable cluster-wide (per Phase 2 sub-phasing) → silent-leader rule fires → all 3 honest NR-emit at L_1 → cross-round exclusivity locks them. Round 2 accepts the bundle, but no honest can σ-emit (NR-locked from round 1). NR-pool at L_1 reaches qEnc → fall-through to L_2, which doesn't exist at K=2. **Slot misses** despite the bundle eventually being accepted. (At K ≥ 3 this falls through harmlessly to L_2 — the NR-lock at L_1 advances the L_C frontier and L_2's σ-quorum can still reach. K=2-specific failure.)

  **Why Class A?** None of the six explicit assumptions is violated: BFT trust bound holds, partial synchrony bounds *gossipsub* propagation but not the leader's *application-internal fetch latency*, validity is orthogonal, EKM and DKGs are orthogonal. The failure relies on an *implicit* operational assumption — that honest leaders broadcast within their Phase-1 fetch window plus the per-round widening slack. When this implicit assumption fails (legitimate operational delay, no byzantine action), the protocol's per-round acceptance widening (which advertises late-arrival recovery via `T_candidate_accept_2`) collides with cross-round σ-or-NR exclusivity (which locks NR from round 1).

  **Concrete budget at Configuration A** (T_commit_1 = 2s, D + δ = 150ms): for at least one honest to σ-emit observably at L_1 before round-1 NR-decision, L_1 must broadcast by `T_commit_1 − 2(D + δ) = 1.7s`. With L_1's nominal schedule at `T_1 + Δ_1 ≈ 1.1s`, that's ~600ms of leader-fetch slack. Anything longer NR-locks the backup layer.
...
- **[Class B — gossipsub-pattern slashable, weaker accountability]** **Byzantine σ-refusal coordinated with honest mesh-bounded observation / transient flakiness (cross-round NR-lock).** The Defer rule's spec wording says "peer σ-emit observed cluster-wide", but operationally each operator only observes via their own gossipsub mesh. An honest operator with poor mesh visibility (few peers, high mesh-hop latency, transient connectivity glitch, EL-CL desync, etc.) can fail to observe peer σ-emits within their NR-decision window even when peers have σ-emitted — the Defer rule's intent (cluster-wide observation) is not what the implementation can deliver (mesh-bounded observation). This is a real design fragility, not a purely adversarial edge case: under realistic gossipsub conditions, individual operators occasionally hit propagation outliers.
  ...
  **Mitigations:**
  - **Phase 2a/2b** ([Path forward](#future-improvement--phase-2a2b-for-full-equivocation-recovery)) — defers commitment past mesh-visibility outliers; if honest's mesh visibility recovers before Phase-2b, they σ instead of NR-locking.
  ...
  - **Larger Δ_2** — extends the σ-emit observation window to absorb mesh-hop latency variance. At minimum `Δ_2 = D + δ`, only direct-mesh paths propagate before NR-decision; `Δ_2 ≥ 2(D + δ)` allows multi-hop paths to complete (see §Timing budget for the late-σ-emit propagation analysis).
...
For an SSV cluster proposing an Ethereum block, the recommended OBFT configuration is **`K = 3, R = 2`** — `K = 3 = f+2` at f=1 satisfies both the BFT-liveness minimum (`K ≥ f+1`) and the late-leader-resilience recommendation (`K ≥ f+2`; see §Setting and §Failure modes / Late deepest-layer leader broadcast). One recovery round absorbs partition cases and equivocation patterns where ≥1 honest is in Defer at evidence-detection time, within the 4s relay cutoff. Onion bandwidth at K=3 is ~3KB per onion (vs ~1KB at K=2) — within practical SSV bandwidth budgets.

**`K = 2, R = 2` is the BFT-minimum configuration** — matches baseline TBFT's onion structure exactly, ~1KB per onion. Acceptable when the deployment can guarantee tight host-side fetch deadlines (see §Failure modes / Late deepest-layer leader broadcast for the host-side hard-deadline mitigation), but fragile to legitimate operational delay in the L_1 fetch loop. Use only when bandwidth is the binding constraint.

Out-of-scope cases (host-validity divergence, 1-1-1 equivocation splits) are addressed by [Assumptions and implications](#assumptions-and-implications) — host stabilizes the validity verdict at Phase-1 acceptance; equivocation 1-1-1 falls back on the reputation deterrent.
...
**Late-σ-emit propagation (informational).** Per the [Phase 2 sub-phasing](#phase-2--onion-broadcast-t_commit_r-t_commit_r--Δ_2-per-round-r), `Δ_2 ≥ D + δ` is the minimum constraint and is what the configurations below use — but it only allows σ-emits at `t = 0` of Phase 2 to propagate before the NR/Defer decision at `T_commit_r + Δ_2`. A σ-emit at time `T_commit_r + t` (for `t > 0`) reaches other honest at `T_commit_r + t + D + δ`, which must satisfy `t + D + δ ≤ Δ_2` for observability — i.e., `t ≤ Δ_2 − (D + δ)`. At minimum `Δ_2 = D + δ` this allows only `t = 0`. **Late σ-emits during Phase 2 do not propagate before the same round's NR/Defer decision** — they only contribute to the next round's σ-pool. Operators who become σ-eligible during Phase 2 (e.g., late re-flood delivers V mid-window) effectively σ for round `r+1` rather than round `r`. This is part of the byzantine selective-delivery grief surface — see §Failure modes.

To widen the late-σ-emit window, set `Δ_2 ≥ 2(D + δ)` at the cost of doubling Phase 2's contribution to per-round latency. At Configuration A this is small (300ms vs 250ms — fits comfortably). At Configuration B (D = 500ms), `2(D + δ) = 1100ms` for Phase 2 alone — incompatible with even R = 1's tight 400ms-headroom analysis. **The configurations below use minimum `Δ_2`; deployments choosing `Δ_2 ≥ 2(D + δ)` should re-validate the slot-budget fit, especially at higher D where the cascade through round 1 and round 2 windows can push past the 3.75s consensus deadline.**

**Bursty-mesh propagation at K ≥ 3 (informational).** The `D` parameter is defined as cluster gossipsub propagation P99/P999, which captures single-message propagation under typical conditions. At K ≥ 3, Phase 3's reconstruction walk does sequential NR-quorum unlocks (L_0 → L_1 → ... → L_{K-1}), and operators emit NR partials at *multiple layers simultaneously* in Phase 2 — a burst of `O(K · n)` partials cluster-wide. Bursty mesh conditions (gossipsub topic congestion, simultaneous re-flood from multiple sources) can extend cluster-wide reception time beyond the steady-state P99 D used to size `Δ_2`. Deployments with K ≥ 3 should measure D under burst-load conditions (multiple simultaneous broadcasts per operator, cross-layer onion + NR partials in flight together), not just point-to-point single-message latency, when sizing `Δ_2`. K = 2 (proposer-duty default) avoids this — only L_0's plaintext σ partials in Phase 2; no multi-layer parallel NR fall-through.
...
**Migration path**: a cluster running baseline TBFT can adopt OBFT incrementally by enabling the optional features: (1) Defer-state rule (decision-time only; no wire change at K=2 since layer-0 σ is plaintext); (2) round-2 retry; (3) `KindLCClaim` message kind for round-transition acceleration. Each is independently useful. For full equivocation coverage, **Phase 2a/2b** (TBFTR-style Phase-2 split) is needed — costs +1 RTT per round (total ~R·(D+δ) per slot) but preserves cryptographic safety AND f-tolerant liveness. The Phase-1 protocol-tag (`OBFT-v1` vs `TBFT-v1`) provides envelope domain separation.
