...
OBFT does not provide an in-protocol equivocation recovery mechanism. Some patterns reach qV naturally (when honest happen to split such that 2-of-3 σ-commit on the same V, leader's σ_L^V completes the pool). Other patterns — including 1-1-1 split (each honest σ-commits on a distinct V on receipt before equivocation evidence reaches them), 1-1-with-Defer-C, and asymmetric-retention patterns under byzantine flooding of 3+ V's — do not naturally reach qV. The protocol does not pick a winner cluster-wide. Slot misses; equivocation evidence is slashable.

The reputation deterrent (assumption 4) is what makes this a tolerable failure mode in expectation: a byzantine that equivocation-griefs in slot N pays for it in slot N+1 onward via stake slashing and cluster kick-out. Phase 2a/2b (future improvement) closes the recovery gap without relying on the deterrent.
...
Local protocol response: **honest stay in their current commitment state** at the equivocated layer. Already σ-committed honest stay σ-committed (cross-phase exclusivity binds them). Defer-state honest stay in Defer (hoping re-flood eventually delivers a single auth-valid bundle the host validates) and force-NR at round R per the final-round rule. The protocol does not attempt to "pick a winner" cluster-wide — under f=1 byzantine and OBFT's retention bound (2 distinct V's per `(slot, layer, leader_id)`), no rule based on per-operator local state can guarantee cluster-wide convergence on a single V (different operators may retain different V-pairs under adversarial gossipsub-ordered delivery; see [Implications of equivocation not being recovered](#implications-of-equivocation-not-being-recovered)).
...
- **σ (sign-on-V)**: the operator received the leader's bundle on time, both protocol-level and application-level checks passed, and the operator did not observe equivocation evidence at acceptance time. Materializes as a σ partial in the Phase-2 onion (or as the leader's Phase-1 σ for the layer's own leader). Once σ-emitted, the operator is **σ-locked** at this layer for the entire slot (cross-phase + cross-round exclusivity).
...
- **Defer (uncommitted)**: cutoff for the current round passed, peer σ-emit on this layer is observed cluster-wide (so the leader is *not* silent — some V exists somewhere in the cluster), but the operator does not have an auth-valid Phase-1 bundle locally that the host validates AND has not observed equivocation evidence. The operator stays uncommitted at this layer through this round, hoping re-flood will deliver such a bundle before the next round's cutoff. Defer is **not visible on the wire** (no message is broadcast for Defer state — it's pure local state). At round R (final round), all Defer operators force-transition to either σ (if V received and host validates and no equivocation observed) or NR (silent-leader rule). Operators who observe equivocation while in Defer also force-NR at round R rather than picking a winner — see "Equivocation handling — detect and slash".
...
**Defer is the key OBFT addition.** It lets the cluster distinguish "the leader is silent (commit NR fast, fall through)" from "I just haven't received V yet (wait, V might still arrive)". The discriminator is **observed peer σ-emit cluster-wide** (visible at layer 0 as plaintext σ partials, visible at deeper layers via the same encrypted-partial broadcast presence — see "Peer σ-emit observability" below). If any honest peer's σ-emit is observed on this layer, the cluster knows V exists, so an operator without V locally should defer rather than NR-emit. This rule preserves baseline TBFT's fast L_0-silent fall-through (no peer σ-emit ⇒ NR-emit immediately) while enabling partition recovery (peer σ-emit observed ⇒ Defer until re-flood completes in next round).

**Validity-gate on observed σ-emit (L_0 specifically).** At L_0, σ partials are plaintext, so receivers can verify the partial against the cluster's V-keypair pubkey on a retained V — i.e., check that the σ partial is a valid signature on a V the operator has (or has access to via re-flood). The Defer rule at L_0 requires this validity check: only **valid σ partials** count as "peer σ-emit observed". An auth-valid `KindOnion` carrying a plaintext `σ_partial` that does not verify against any retained V at L_0 is treated as if no σ-emit happened (it does NOT push the operator into Defer). This prevents byzantine garbage at L_0 from forcing honest into Defer when the legitimate response would be NR-silent fall-through. At deeper layers (k > 0), the σ partial is encrypted and validity isn't checkable until decryption — encrypted-presence alone is sufficient for Defer (with the fake-encrypted-presence slashing rule as the post-decryption attribution backstop; see "Slashing evidence").
...
- Every operator signs each layer's V_{L_k} they consider valid (host returns `valid`) in their Phase-2 onion, provided no equivocation evidence is observed at that layer. The Defer state (no σ, no NR) means the operator hasn't decided yet — they may still σ-emit in a later round if a single auth-valid V is received via re-flood and the host validates it. See "Phase 1 / Equivocation handling — detect and slash".
...
**Equivocation handling (no protocol-level recovery; some patterns succeed naturally)**: byzantine `L_0` equivocates, broadcasting distinct V's selectively. Case analysis at `f = 1, n = 4` (3 honest A, B, C; byz D = leader):

Naturally-recovered patterns (succeed as a side-effect of leader-σ-V counting toward whichever V already has 2-of-3 honest support):
...
Not recovered (slot misses; equivocation slashable, leader pays stake-based penalty):

- **D delivers V to {A}, V' to {B}, both to C (or neither).** A σ-locked on V; B σ-locked on V'; C in Defer with equivocation evidence (or no V received). σ-pool on V = A + leader = 2 < qV. σ-pool on V' = B + leader = 2 < qV. C cannot rescue: the protocol does not pick a winner. C stays in Defer, force-NR at round R; NR-pool = 1 < qEnc = 3 → no fall-through. Slot misses.
...
- **0 honest σ-committed initially, asymmetric retention under 3+-V flood.** byz floods V_1, V_2, V_3, V_4 with adversarially-ordered gossipsub delivery. Each honest's first-2-distinct retention may differ. No honest σ-commits because each observes equivocation while in Defer; no winner-pick rule. Round R force-NR; NR-pool may or may not reach qEnc depending on byz NR contribution. Slot likely misses.
...
| Byzantine leader equivocates, no V has 2 honest support (1-1-1 split, 1-1-with-Defer-C, asymmetric retention) | All splits fail to naturally reach qV; OBFT does not pick a winner. **✗ slot misses;** equivocation slashable; reputation deterrent kicks byz out for future slots (see [Assumptions](#assumed)). | Round 1: PREPARE split; no quorum; timeout. Round 2: new leader proposes a fresh V (not one of the equivocated V_i); honest converge; succeeds. ✓ in ~2.75s. **QBFT recovers what OBFT doesn't** via "new leader proposes fresh V" mode. |
...
2. If still uncommitted (Defer), stay in Defer; force-NR at round R per the final-round rule. The protocol does not pick a winner cluster-wide.
...
- **Equivocation patterns that don't naturally reach qV** (1-1-1 split, 1-1-with-Defer-C, asymmetric retention under 3+-V flood) — see [Implications of equivocation not being recovered](#implications-of-equivocation-not-being-recovered). Equivocation evidence is slashable; the persistent reputation deterrent (assumption 4) makes this tolerable across many slots.
...
7. **Equivocation is permitted, not recovered.** OBFT does not provide an in-protocol equivocation recovery mechanism. Some patterns reach qV naturally (when 2-of-3 honest happen to σ-commit on the same V at f=1 n=4; leader's σ_L^V completes the qV-quorum). Other patterns slot-miss. Pigeonhole 2 ensures at most one V reaches qV cluster-wide regardless of split — there is no two-output safety failure. Equivocation is treated as a slashable byzantine fault; the persistent reputation deterrent (assumption 4) makes this tolerable across many slots. The TBFTR-style **Phase 2a/2b split** (broadcast-only Phase-2a, then σ-emit on a deterministically-chosen V in Phase-2b after Phase-2a observation completes) is the structurally correct full fix; +1 RTT every slot, preserves cryptographic safety AND f-tolerant liveness. Documented as future improvement, not in current OBFT.
...
An earlier draft of OBFT included a **winner-completion rule** for equivocation recovery: Defer-state honest who observed equivocation σ-emitted on the V with the lowest `value_root` hash among retained Phase-1 bundles. The rule was removed after analysis showed: (a) under the retention bound (2 distinct V's per `(slot, layer, leader)`), byzantine flooding of 3+ V's with adversarial gossipsub-ordered delivery can produce asymmetric retention sets where no V is the argmin in 2+ honest's local sets — divergent winners, no σ-quorum, slot misses; (b) the recovery scope was already partial (only Defer-state honest, not σ-locked honest) — patterns like 1-1-1 split (each honest σ-commits on receipt of a different V) were never covered. Rather than retain a partial mechanism with caveats, OBFT now treats equivocation as a permitted-but-slashable pattern: some splits succeed naturally (when 2-of-3 honest happen to σ-commit on the same V), others slot-miss. The persistent reputation deterrent (assumption 4) makes this tolerable across many slots.

An even earlier draft included a **skip mechanism** (separate `skip_tag_k` IBE tags, chained-OR encryption, persistent-divergence trigger) intended to also recover validity-divergence under strict host policy. That mechanism was removed after audit found it relied on an "effective σ-pool" honest-aggregation rule rather than a cryptographic property — a byzantine offline aggregator could ignore the rule and produce a second V signature from raw σ partials. Validity-divergence recovery within a slot is no longer an OBFT goal — the design assumes the host stabilizes the verdict at Phase-1 acceptance time as a best-effort host-side mechanism (assumption 3 in [Assumptions](#assumed)).

**Future improvement — Phase 2a/2b for full equivocation recovery.** The TBFTR-style Phase-2 split (broadcast-only Phase-2a where operators re-flood retained Phase-1 bundles without σ-emitting, then Phase-2b where σ-emits happen on a deterministically-chosen V after Phase-2a observation completes) is the structurally correct fix for OBFT's equivocation recovery limit: by deferring σ-emit until *after* operators observe the cluster's σ_V set, no honest carries an "initial" σ partial on a non-winner V; single-σ-V exclusivity stays intact; Pigeonhole 2 holds; at most one V reaches qV cluster-wide *regardless of equivocation pattern* (1-1-1, 2-1, 1-2, all-Defer). Costs +1 RTT every slot. Preserves cryptographic safety AND f-tolerant liveness — strictly better than the qV-bump alternative which trades f-tolerance for safety. Documented here as future improvement, not in current OBFT.

The result is a protocol that **strictly improves on baseline TBFT's recovery scope** for partition cases (Defer state + R-round retry) at TBFT's healthy-path latency (2 RTTs). It does **not match QBFT's full recovery scope** — view-divergence (validity-divergence and equivocation patterns that don't naturally reach qV) is out of scope, handled by assumptions 3 (host validity stabilization) and 4 (persistent reputation deterrent) respectively. The R (round count) parameter is the partial-synchrony tolerance knob; K (layer count) is the multi-failure-fall-through depth knob; Phase 2a/2b (future) is the equivocation-coverage knob.
...
OBFT is a **strict superset** of baseline TBFT: at `R=1, K=2`, OBFT reduces exactly to TBFT. The added machinery (Defer state, R-round retry, L_C consensus) extends partition-recovery scope without affecting baseline's properties.
...
**Migration path**: a cluster running baseline TBFT can adopt OBFT incrementally by enabling the optional features: (1) Defer-state rule (decision-time only; no wire change at K=2 since layer-0 σ is plaintext); (2) round-2 retry; (3) `KindLCClaim` message kind for round-transition acceleration. Each is independently useful. For full equivocation coverage, **Phase 2a/2b** (TBFTR-style Phase-2 split) is needed — costs +1 RTT every slot but preserves cryptographic safety AND f-tolerant liveness. The Phase-1 protocol-tag (`OBFT-v1` vs `TBFT-v1`) provides envelope domain separation.
...
**OBFT replaces TBFTR's Phase-2-split with rounds**, achieving similar recovery scope for partition cases with cleaner spec structure. The R-round structure subsumes Phase-2b's "late σ-emit" via the same mechanism (re-flood + Defer transition to σ across rounds). For full equivocation recovery, OBFT can adopt the TBFTR-style Phase-2 split as a future improvement.
...
OBFT's K-generic structure composes cleanly with **host-supplied leader-determination extensions** — selecting which operator gets which layer based on application-supplied criteria (rather than just a deterministic rotation table). The extensions live outside the OBFT core: they affect *which* leaders get assigned to layers L_0, L_1, ..., L_{K-1}, while OBFT's safety/liveness machinery (Defer state, R-round retry, L_C consensus, chained encryption) operates uniformly across whatever leader assignment the host supplies.
