...
OBFTR's liveness is **partial-synchrony-conditional within the slot's relay-submission deadline** (the protocol's slot budget, spread across R rounds). Running example: `f = 1, n = 4, K = 2`; honest A, B, C; byzantine D. The protocol terminates with a V signature on the first layer whose bundle reaches a σ-quorum within the cluster's `R · P99` absorption window (else falls through via NR-quorum to a deeper backup, else — past `R · P99` — misses cleanly); **safety holds in either case**. View-divergence is recovered only where an honest majority resolves it: some equivocation splits reach σ-quorum naturally and honest-majority validity-divergence decides, but adversarially-timed equivocation and the 2-2 validity boundary slot-miss — equivocation is slashable, validity-divergence is not attributable.

> ### ⚠️ OBFTR's liveness is conditional on two requirements per slot:
>
> **(a) [Assumption 2](#assumptions-and-implications) (within-budget partial-synchrony) holds for the slot.** Honest `KindCommit_r` messages emitted by each round's `T_commit_r` reach a 2f+1 quorum within that round's `Δ_2` (recommended `Δ_2 = 1 BTT`), and late bundles re-flood across rounds within the cumulative `R · P99` absorption window. This is the same partial-synchrony assumption QBFT operates under for its `PREPARE` / `COMMIT` phases, extended across R rounds.
>
> **(b) At each layer `L_k`, in each round, the leader's Phase-1 bundle must EITHER reach a 2f+1 quorum by `T_commit_r` (σ-quorum forms — output V), OR fail to reach by a wide-enough margin that the remaining ≥ qEnc operators NR-quorum (chain falls through to `L_{k+1}`).** Partial propagation in between — `n − qEnc < receivers < qV`, at f=1 n=4 exactly `r = 2` (the `h_V=1` shape) — deadlocks the layer *in that round*: σ-pool < qV, NR-pool < qEnc, no in-protocol flip. **Unlike single-round bare [OBFT](OBFT.md), OBFTR recovers the *propagation* case across rounds** — per-round commitments are independent (no cross-round σ-or-NR exclusivity), so a round-`r` operator that lacked V σ-emits in round `r+1` once gossipsub re-floods V (cross-round retention). The (b) deadlock is terminal only for **sustained partition** (real propagation > `R · P99`) and **adversarial selective-delivery** (a byzantine re-targets the partial delivery every round — R-invariant; see Leader equivocation below).
>
> **Caveat at f=1, n=4 with a dormant byzantine** (byz silent / offline): the NR-fall-through branch is capacity-limited — only 2 of 4 ops are honest non-leaders, so NR-pool ≤ 2 < qEnc = 3 and a layer can't fall through on honest NR alone. Within the `R · P99` window cross-round retention still delivers V to the honest non-leaders for σ-quorum; past it, any layer's partial propagation collapses the slot.
>
> **Practical causes of (b) failing in a round:** late leader broadcast, mesh propagation tail past `T_commit_r`, clock skew beyond `δ`. OBFTR absorbs these across rounds via cross-round retention (its structural mesh-jitter absorber) rather than per-layer reflood budgets.
>
> **Comparison to QBFT.** QBFT round-changes — re-proposing a *fresh* V — when a round fails; OBFTR re-floods the *same* V across rounds (no fresh-V refetch). OBFTR's R-round retry recovers the propagation tails QBFT also recovers, but not the view-divergence patterns (adversarial equivocation, the 2-2 validity boundary) QBFT's fresh-V round-change handles.

**Healthy path — decides at L_0, round 1.** All honest receive `V_{L_0}` within ~`1·BTT` and σ-emit; the leader's Phase-1 `σ_L^V` seeds the σ-pool, so a single honest peer completes `qV = 3`. Slot succeeds in round 1 (Phase 1 + Phase 2 + Phase 2.5 ≈ 5 BTT ≈ 1000ms at Config A). **Success (fastest).**

**Network-flakiness and silent/crash patterns recovered:**
- **Round-2 propagation recovery — the OBFTR win over bare OBFT.** 1 honest received V before round-1's `T_commit_1`, 2 didn't → the 2 NR at round 1; σ-pool = 1 + leader = 2 < qV, NR-pool = 2 < qEnc → no quorum; round 2 fires. By `T_commit_2` gossipsub has re-flooded the bundle; the 2 missing honest σ-emit in round 2 (per-round commitments independent — round-1 NR doesn't lock them out) → σ-pool = 3 + leader = 4 ≥ qV. **Success in round 2** (~3 RTTs).
- **Primary leader silent / crashed** — all honest NR (silent-leader rule) → NR-quorum at L_0 → Phase-3 walk falls through to L_1; the backup leader's value σ-decides if L_1 honest. **Success (fall-through).**
- **h_V=1 selective-delivery** — **miss.** OBFTR's witness section re-broadcasts σ_L^V but *not* V, so a V-drop receiver can't recover from the witness, and an adversarial byz re-targets the single delivery each round (R-invariant). Bare OBFT and 2abOBFT close this via peer-reflood-V; OBFTR does not.
- **Late deepest-layer leader broadcast (K=f+1)** — NR-quorum reaches but there is no deeper layer to fall through to → **miss** at K=f+1; `K ≥ f+2` recovers via a deeper honest leader (see [§Failure modes](#failure-modes)).

**Leader equivocation** (byzantine `L_0` broadcasts distinct V's selectively; no protocol-level recovery — outcome depends on byzantine timing). At `f=1, n=4` (honest A, B, C; byz D = leader):
- **2-1 split — recovers via σ-quorum at L_0** (2-of-3 honest hold the same V; the leader's `σ_L^V` completes the pool): D delivers V to {A, B}, V' to {C} → σ-pool[V] = A + B + D's σ_L^V(V) = 3 = qV. **Success in round 1.**
- **All-honest-NR — recovers via fall-through** (D delivers early; re-flood spreads conflicts before σ-emit → each honest retains ≥ 2 V's → all NR) → NR-quorum at L_0 → fall-through to L_1; if L_1 honest, **success at L_1.**
- **σ-locked split / 1-1-1 / 1-NR-NR — miss, no fall-through** (slashable): a distinct V delivered to ≤ f honest each (and/or asymmetric retention with non-overlapping sets) leaves every σ-pool < qV and NR-pool < qEnc, with no cluster-wide winner derivable from per-operator local retention. **Miss at L_0.**

**An adversarial byzantine reliably picks the slot-miss class, and R-rounds don't help** (R-invariant — byz re-targets the delivery each round): delivering near end-of-Phase-1 leaves insufficient re-flood time → σ-locked split → miss. The natural-recovery cases above fire only under non-adversarial timing. The rational-byzantine deterrent (assumption 4) makes this tolerable across slots, but the evidence is *behavioral* (single-observation slashing not credible — see [§Implications of the rational-byzantine deterrent](#implications-of-the-rational-byzantine-deterrent-assumption-4)). Pigeonhole 2 ensures at most one V reaches qV cluster-wide regardless of the split — no two-output safety violation.

**Validity-divergence** (a re-org splits honest host verdicts mid-slot; the leader's Phase-1 `σ_L^V` locks it σ-side regardless of a later flip): **honest-majority recovers** (3-σ vs 1-NV → σ-quorum at L_0; 1-σ vs 3-NV → NR-quorum → fall-through to L_1, same as bare OBFT); the **2-2 boundary** and validity-divergence-with-passive-byz cases deadlock and miss. R-round retry does not help (verdicts are locked at acceptance — see [Implications of validity-divergence not being recovered](#implications-of-validity-divergence-not-being-recovered-assumption-3)). Not attributable; relies on assumption 3.

**Deepest-layer / multi-failure fall-through (K ≥ 3).** With `K > 2` layers the cluster falls through past multiple silent or non-validating layers **within a single round** — Phase 3's reconstruction walk processes layers sequentially via local decryption (no per-layer RTT): e.g. at K=4 with L_0, L_1, L_2 silent, NR-quorum reaches at each in Phase 2 and σ-quorum at L_3 if L_3 honest. Recovery at K layers needs no round-change overhead — a structural difference from QBFT (which round-changes once per failed leader). If every layer's leader fails non-recoverably the slot misses — at K ≥ f+1 byzantines alone can't cause this (pigeonhole guarantees ≥ 1 honest leader); it requires `> f` faults or coincident independent failures.

**Adversarial scheduling.** A network adversary delaying messages within the synchrony bound cannot affect safety (Pigeonholes 1, 2, 3 are over the cluster-wide signed-message set, not arrival times); its liveness leverage scales with how many operators it keeps without V through R rounds. Delaying V to ≤ 1 honest leaves `σ-pool = 2 + leader = qV` (quorum reaches without the delayed operator). Delaying to 2 honest deadlocks round 1, but round-2 re-flood (from gossipsub peers, not just the leader) recovers — unless the adversary sustains the delay against many sources through round R. Delaying to all 3 honest yields NR-quorum at L_0 → fall-through to L_1 (σ-decides if L_1 honest and reachable). The cumulative `R · P99` budget is the per-operator delivery-delay tolerance; within it the cluster σ-recovers at L_0 or NR-falls-through to L_1, outside it the slot misses cleanly.

**OBFTR recovers strictly more than bare [OBFT](OBFT.md) (single-round)** for partition cases (R-round retry with re-flood; per-round-independent commitments), at the same 2-RTT healthy-path latency. It does **not match QBFT's full recovery scope** — adversarial equivocation and the 2-2 validity boundary are out of scope, handled by assumptions 3 (host-validity stabilization) and 4 (rational-byzantine deterrent).
...
OBFTR does not provide in-protocol equivocation recovery. Some equivocation patterns naturally reach qV on a single V (when honest happen to split such that 2-of-3 σ-emit on the same V; leader's σ_L^V on that V completes the qV-quorum). Other patterns slot-miss. Pigeonhole 2 ensures at most one V reaches qV cluster-wide regardless of split — there is no two-output safety failure. See "Liveness / Leader equivocation" for the case analysis. Equivocation is treated as a slashable byzantine fault (Phase-1 bundles signed by leader's key are self-contained slashing evidence — see "Slashing evidence"); the rational-byzantine deterrent (assumption 4) is what makes the slot-miss tolerable across many slots.
