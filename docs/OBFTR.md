...
- **Partial synchrony for liveness**: messages eventually deliver within bounded delay `P99` (propagation P99/P999) and clock skew `δ`. Two distinct per-round cutoffs operationalize this: `T_broadcast_max_r = T_commit_r − 2 BTT` (leader broadcast deadline, round 1 only) and `T_accept_max_r = T_commit_r + Δ_2 − 1 BTT` (per-round receiver acceptance horizon). Round `r` boundary is at `T_commit_r + Δ_2 + Δ_2.5 + ε_3`; the slot's hard wall is the relay-submission deadline `T_relay_cutoff − T_submit`. Safety holds against arbitrary network adversaries; only liveness depends on synchrony.
  ...
  **OBFTR's R-round absorption window** = `T_accept_max_R − T_broadcast_max_1`:
  ...
  - At R=2 Config A with recommended Δ_2: total absorption ≈ `7 BTT + ε_3` ≈ 1450ms — roughly 2.5× a single round's window.
...
| `T_broadcast_max_1` | round 1 leader broadcast deadline — `T_commit_1 − 2 BTT` (= 1.20s at P99=150ms, δ=50ms); per-layer fetch windows are `[T_k_1, T_broadcast_max_1]` with `T_{K-1} < ... < T_0 ≤ T_broadcast_max_1`. Bundles broadcast at this deadline have `1 BTT` of propagation slack to all-honest first-observation by `T_commit_1 − 1 BTT`. |
