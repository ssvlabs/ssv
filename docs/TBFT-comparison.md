...
At n=4 the operator picks between TBFT (lean, covers partial synchrony + moderate marginal — 1 of 3 honest missing re-flood) and TBFTR(K=2) (fuller, also covers aggressive marginal — up to 2 of 3 honest missing). At n ≥ 7 only TBFTR is supported; the leaner TBFT-shape protocol's marginal-synchrony coverage caps at "1 honest missing re-flood" regardless of `f`, missing slots in the rest of the f-honest-missing band once `f ≥ 2`.
...
At n=4 the SSV operator has a choice between the leaner [TBFT](TBFT.md) and the full [TBFTR](TBFTR.md) (configured with K=2). Both are cryptographically safe; they differ in aggressive-marginal robustness (2-of-3-honest-missing-reflood) vs bandwidth/latency premium. Comparing all three apples-to-apples:
...
| **6c** | Same as 6b, plus aggressive-marginal breakdown (re-flood misses ≥2 of 3 honest) | ~3.0 s / 27 KB ² | miss ⁴ | **~400 ms** / 30 KB ⁵ |
...
- **Scenario 6c is where TBFTR's secondary closure earns its premium.** TBFT's marginal-synchrony coverage caps at "1 of 3 honest missing re-flood" (one slow link in the mesh) — the leader-σ head-start gives σ count = 2 honest + 1 leader = qV = 3. When 2 of 3 honest miss re-flood (gossipsub propagation between *multiple* honest pairs is exceeding budget — not just one slow link), TBFT's single-window Phase 2 commits to NR before recovery can complete; TBFTR's Phase-2 split defers that commit, letting peer-onion V-recovery + Phase-2b late σ close the gap. The price is the bandwidth (V plaintext, ~9 KB extra at n=4) and the latency (`Δ_2b` ≈ 100–200 ms). At n=4 the gap (1-of-3 → 2-of-3 honest missing) is narrow; worth it if production observes the 2-of-3 case at non-trivial frequency, not worth it otherwise.
...
- **TBFTR (K=3, full-V) ~325 KB** — roughly K× bandwidth (each layer carries full V plaintext); enables the secondary closure (Phase-2a peer-onion V-recovery + Phase-2b late σ) that extends robustness into the *aggressive-marginal* band — where Phase-1 re-flooding misses ≥2 of 5 honest (gossipsub propagation between multiple honest pairs exceeding budget, not just one slow link) but Phase-2a propagation still completes within its (longer) window.

Both variants are cryptographically safe; they differ in marginal-synchrony liveness vs bandwidth. Under partial synchrony with the cutoff enforced — and through the moderate-marginal band (1 honest missing re-flood) — scenarios 1–6b complete identically in both variants (primary closure + leader-σ head-start suffice). Scenario 6c — aggressive-marginal breakdown (≥2 honest missing re-flood) — is where the variants diverge.
...
| **6b** | Byzantine layer-0 leader actively griefs (selective delivery + dark on votes) | ~3.0 s / 76 KB ² | **~400 ms** / 108 KB ³ | **~400 ms** / 325 KB ³ |
| **6c** | Same as 6b, plus aggressive-marginal breakdown (Phase-1 re-flood misses ≥2 of 5 honest) | ~3.0 s / 76 KB ² | miss ⁴ | **~400 ms** / 325 KB ⁵ |
| **7** | More than f failures (beyond byzantine bound) | miss | miss | miss |
...
- **TBFTR's flat ~400 ms across all in-bound failure modes (scenarios 1–6b)** is the headline. The +Δ_2b window (~150 ms) over a TBFT-shape protocol pays for the byzantine-leader grief closure under partial synchrony. QBFT pays per round change — passive failures cost ~3.0 s, the worst-case 6a costs ~5.0 s.
...
- **Scenario 6b (active byzantine grief) is closed at n=7 in both variants under partial synchrony**, via gossipsub re-flooding. QBFT also recovers via round change, at higher latency.
...
- **Bandwidth: TBFTR(hash) is consistently larger** than QBFT in healthy-case (108 KB vs 37 KB) but stays flat across failures while QBFT scales toward 115 KB in worst-case 6a. TBFTR(full-V) is much larger (~325 KB) and only earns its premium in the aggressive-marginal band. The gap in failure scenarios narrows; the gap in healthy case stays open.
- **Scenario 1 still favors QBFT on bandwidth.** TBFTR's V-plaintext + composition premium is real. The trade is paying for guaranteed flat performance across the failure modes (both variants) plus optional aggressive-marginal robustness (full-V only).
...
|---|---|---|
| n=4 | 1 | **TBFT** as default (~21 KB) — primary closure via leader-σ + gossipsub re-flooding; covers up to "1 of 3 honest missing re-flood". **TBFTR(K=2, full-V)** as alternative (~30 KB) if aggressive-marginal robustness ("2 of 3 honest missing") is wanted; the secondary closure needs full V plaintext (hash variant reduces to TBFT-shape behavior). |
| n=7 | 2 | **TBFTR (K=3, hash)** for typical deployment (~108 KB) — primary closure + leader-σ head-start covers up to "1 of 5 honest missing re-flood". **TBFTR (K=3, full-V)** (~325 KB) if aggressive-marginal robustness (up to 2 of 5 honest missing) justifies the bandwidth premium. |
