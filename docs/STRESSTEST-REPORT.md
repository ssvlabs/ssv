...
```bash
make stresstest
open ./stresstest-report/index.html
```
...
ITERATIONS=1000 make stresstest

# Different output directory:
REPORT_DIR=/tmp/report-out make stresstest
```
...
└── data.js           ← generated by `make stresstest` (gitignored)
                        contains `window.REPORT_DATA = {...}` as a script tag,
                        loaded directly — no fetch, no local server required
```

**Iteration story.** Edit `app.js` / `styles.css` / `index.html` and refresh the browser — no test rerun. Re-run `make stresstest` only when you want fresh stats. Opens directly via `file://` (data.js is a `<script>` tag, not a `fetch`, so no CORS/server fuss).
...
Then register it in `DefaultSweeps`. Run `make stresstest` — the new sweep appears as a new section (with trend layout if >1 point, detail layout if 1 point) automatically.
...
}, "./stresstest-report") // writes <dir>/data.js
```
...
| [`stresstest-report/index.html`](../stresstest-report/index.html) | UI page shell (tracked) |
| [`stresstest-report/styles.css`](../stresstest-report/styles.css) | UI styles (tracked) |
| [`stresstest-report/app.js`](../stresstest-report/app.js) | UI renderer (tracked) — reads `window.REPORT_DATA` |
