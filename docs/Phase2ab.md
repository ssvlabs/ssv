# Phase 2a/2b Split for OBFT-1 — Design Plan

A design plan for the Phase 2a/2b extension to [OBFT-1](OBFT-1.md). This document specifies the protocol, walks through the convergence rule, identifies edge cases, and lists open questions to resolve before implementation. **It is not the implementation itself**; it is the spec/plan to follow when the implementation is undertaken.

The reader is assumed to have read [OBFT-1.md](OBFT-1.md) — this document only describes deltas. Symbols (`f`, `n`, `K`, `qV`, `qEnc`, `Δ_2`, `T_commit`, etc.) carry the meanings defined there.
...
- **Relationship to existing code**: [protocol/v2/tbft/](../protocol/v2/tbft/) implements baseline TBFT. OBFT-1 itself has not been implemented. This plan assumes Phase 2a/2b is built directly (skipping a bare-OBFT-1 intermediate) since the Defer-state machinery OBFT-1 introduces is subsumed by Phase-2a observation here. If a bare-OBFT-1 implementation lands first, several pieces here become drop-in replacements rather than additions.

## What changes vs bare OBFT-1

| Component | OBFT-1 | OBFT-1 + Phase 2a/2b (this) |
|---|---|---|
...
OBFT-1.md describes Phase 2a/2b loosely and points at TBFTR.md. The two specifications diverge on whether Phase-2a carries σ partials. Three coherent design points emerge:
...
Leader signs Phase-1 σ_V (same as OBFT-1 base). Phase-2a is bundle re-flood + verdict broadcast, no σ partials. Phase-2b: each operator σ-or-NR-emits based on Phase-2a verdict observations. Cross-phase exclusivity: leader is σ-locked from Phase-1; non-leaders commit at Phase-2b.
...
**Why Variant C is the right design point.** OBFT-1's strongest claim about Phase 2a/2b — "Recovers the Class A validity-divergence deadlock" ([docs/OBFT-1.md:872](OBFT-1.md)) — is only true under Variant C. The parenthetical hint at [docs/OBFT-1.md:873](OBFT-1.md) ("without a Phase-1 σ_L^V, the leader doesn't pre-lock; Phase-2b's σ-emit is on the post-observation stabilized V") points at exactly this design. The marginal-`h_V` cost is acceptable because L_0 → L_1 fall-through is cheap (no extra RTT), and the recovery scope is genuinely wider.

## Setting

Inherits all of OBFT-1's setting ([docs/OBFT-1.md:23](OBFT-1.md)), with the deltas below.

### Wire kinds

In addition to OBFT-1's `KindOnion`, `KindNR`, `KindCertificate`, `Phase1Bundle`:

- **`KindVerdict`** (new): operator `i`'s per-layer Phase-2a verdict envelope. Structured payload `(protocol_tag = "OBFT-1-2ab-v1", message_kind = "phase2a-verdict", cluster_id, slot, operator_id i, layer k, verdict ∈ {σV, NR, NV}, value_root_or_null)`, signed by `i`'s operator-identity key. The `value_root` field is set when `verdict = σV` (commits `i` to claiming σ-eligibility on a specific `V` whose hash is `value_root`); null when `verdict ∈ {NR, NV}`.

- **`Phase1Bundle`** schema becomes `(V_{L_k}, σ_{L_k}^{op}(envelope))` — the σ_V partial is removed. The auth envelope still binds `(protocol_tag = "OBFT-1-2ab-v1", message_kind = "phase1-bundle", cluster_id, slot, layer k, leader_id, value_root)`. Domain separation matters: bundle envelopes from this protocol must not collide with bare-OBFT-1 (`OBFT-1-v1`) or TBFT (`TBFT-v1`) envelopes. Non-σ_V Phase-1 bundles signed under bare-OBFT-1's tag would be rejected by Phase-2a/2b receivers; cross-protocol mixing is impossible by design.

### Per-layer windows and deadlines

Replace OBFT-1's single `Δ_2` with two sub-windows:
...
- **Recommended**: `Δ_2a = Δ_2b = 2(D + δ)` (mirrors OBFT-1's Δ_2 widening recommendation), absorbing one full `D + δ` of jitter per window.

`Δ_3` keeps OBFT-1's sizing: `Δ_3 ≥ (D + δ) + ε_3`. NR partials are emitted at end of Phase-2b; Phase 3 must absorb their propagation plus reconstruction processing.
...
**Leader broadcast deadline**: `T_broadcast_max = T_commit − 2(D + δ)`. Same as OBFT-1.
...
Submission headroom drops from OBFT-1's 1.95s to 1.65s. Both fit the 4s relay cutoff comfortably.
...
- **σ on V at layer k**: emit a single Phase-2b σ partial. At layer 0, plaintext `σ_i^V(V_{L_k})`. At layer `k > 0`, chained-IBE-encrypted under `nr_tag_0 ∧ ... ∧ nr_tag_{k-1}` (same chained construction as OBFT-1's Phase-2 onion at deeper layers).
- **NR/NV at layer k**: emit a Phase-2b NR partial `σ_i^{IBE}(nr_tag_k)` (only for `k ∈ {0, ..., K-2}`; the last layer has no NR tag).

The operator wraps both σ partials and NR partials into a single auth envelope `KindOnion2b` signed by `i`'s operator-identity key, binding `(protocol_tag, message_kind = "phase2b-onion", cluster_id, slot, operator_id i, per-layer commits)`. This is the same auth envelope shape as OBFT-1's Phase-2 onion, restructured to carry the operator's per-layer Phase-2b commits.
...
**Single signing event per (slot, layer) per operator** — same shape as OBFT-1's EKM, simpler than OBFT(R≥2)'s cross-round atomicity.

### Phase 3 — Local decryption and reconstruction `[T_commit + Δ_2a + Δ_2b, T_round_end]`

Identical to OBFT-1's Phase 3 ([docs/OBFT-1.md:283](OBFT-1.md)) except for the σ-pool source — there is no Phase-1 σ_V to include. The σ-pool at layer `k` is:
...
There is no `Defer` state in the OBFT-1 sense — Phase-2a's window IS the deferral mechanism. By Phase-2a end, the convergence rule resolves every operator to either `σ` or `NR`.

## EKM coordination

Same shape as OBFT-1's EKM ([docs/OBFT-1.md:383](OBFT-1.md)):
...
The EKM is **simpler than OBFT-1's** because there is no Phase-1 σ_V to coordinate with the leader's later Phase-2 σ. A single signing event per (slot, layer) per operator.

## Safety

The three pigeonhole arguments from OBFT-1 ([docs/OBFT-1.md:431](OBFT-1.md)) carry over with minor restatements for the absence of Phase-1 σ_V.
...
The argument is identical to OBFT-1's Pigeonhole 1 except `h_σ` no longer includes the leader's Phase-1 σ_V (because there isn't one). The leader's Phase-2b σ counts the same as any other honest's σ. The bound is unchanged.
...
Same argument as OBFT-1's Pigeonhole 2 with the leader's σ counted at Phase-2b instead of Phase-1.
...
Identical to OBFT-1's Pigeonhole 3 ([docs/OBFT-1.md:458](OBFT-1.md)). Chained encryption gates Phase-2b σ at layer `k > 0` behind NR-quorum at layers `0..k-1`. Pigeonhole 1 then prevents two layers' σ-quorums from co-existing.
...
A byzantine that verdict-claims σV but Phase-2b NR-emits (or vice versa) commits **verdict-vs-action equivocation** — slashable evidence (the verdict envelope contradicts the Phase-2b σ/NR partial), but does not violate Pigeonhole 1/2/3. The slashable evidence is on top of OBFT-1's five rules; see "Slashing-evidence rules" below.

## Liveness analysis

OBFT-1's recovery scope ([docs/OBFT-1.md:471](OBFT-1.md)) extends in three places. We walk through each scenario class.
...
- **Cost vs bare OBFT-1**: bare OBFT-1 with `h_V = 2` has σ-pool = 2 (honest σ) + 1 (leader Phase-1 σ_V) = 3 = qV → succeeds at L_0. Variant C falls through to L_1. One extra reconstruction-walk iteration in Phase 3 (no RTT).
...
- **All sub-cases recover via NR-quorum fall-through.** OBFT-1 base would slot-miss at L_0 here ([docs/OBFT-1.md:639](OBFT-1.md)); Variant C structurally fixes it.

### Equivocation σ-locked split

Byzantine D = leader equivocates at L_0. Patterns from OBFT-1's analysis ([docs/OBFT-1.md:480](OBFT-1.md)). **At 2-1 cases, Variant C regresses vs bare OBFT-1**; documented after the recovered cases.
...
**Either sub-case recovers.** OBFT-1 base 1-1-1 split slot-misses ([docs/OBFT-1.md:495](OBFT-1.md)); Variant C structurally fixes it via the σ-eligibility-quorum-short rule.
...
#### 2-1 split — REGRESSION vs bare OBFT-1

D delivers V to {A, B}, V' to {C}. (One of OBFT-1's "naturally recovered" cases — see [docs/OBFT-1.md:485](OBFT-1.md).)

- Bare OBFT-1: D's Phase-1 σ_V(V) is on the wire. σ-pool on V = A + B + D = 3 = qV. Slot succeeds at L_0 *regardless of D's Phase-2 cooperation* — the Phase-1 σ partial is cryptographically locked-in.
...
- Bare OBFT-1 here: slot misses (leader's Phase-1 σ_V locks them to σ; non-leader σ-eligible can't switch; NR-pool capped at 2 < qEnc). Variant C recovers.
...
### Late-deepest-layer leader broadcast (Class A in OBFT-1)

Bare OBFT-1's failure mode ([docs/OBFT-1.md:643](OBFT-1.md)): deepest-layer leader broadcasts past T_accept_max → all honest treat as silent → NR-quorum at L_{K-1} → walk advances past L_{K-1}, no L_K → slot misses.

In Variant C, late-arriving Phase-1 bundles are auth-only-retained until Phase-2a ends. If the bundle re-floods to all honest before `T_commit + Δ_2a − (D + δ)`, honest can verdict-claim σV on V; verdict-pool reaches qV; Phase-2b σ-emit on the late V. **Slot succeeds where bare OBFT-1 fails.**
...
### Mesh-flakiness coordinated with byz σ-refusal (Class B in OBFT-1)

Bare OBFT-1's failure mode ([docs/OBFT-1.md:706](OBFT-1.md)): mesh-flaky honest A NR-emits early → A is NR-locked → byz refuses σ → σ-pool short → deadlock.
...
This is wider mesh-flakiness mitigation than bare OBFT-1. The Phase-2a observation window is a structural buffer for transient flakiness.

### Sustained partition (Class A — unchanged)

Real propagation > absorption window: bundles don't reach honest in time → all honest NR-pool short → slot misses cleanly. Same as OBFT-1.

Variant C's absorption window is `Δ_2a + (D + δ)` (the Phase-2a-end horizon) — same shape as OBFT-1's `Δ_2 + (D + δ)`. At recommended sizing both are ~450ms at Config A.

### > f operators offline/byzantine (Class A — unchanged)

Standard 3f+1 violation → slot misses. Same as OBFT-1.

## Slashing-evidence rules

OBFT-1's five rules ([docs/OBFT-1.md:567](OBFT-1.md)) carry over unchanged. One new rule covers verdict-vs-action equivocation:

- **Rule 1 — Self-contradiction (σ + NR/NV at the same layer)**. Same as OBFT-1.
- **Rule 2 — Leader equivocation**. Two distinct Phase-1 bundles `(V, σ^{op})` and `(V', σ^{op})` from the same leader at the same (slot, layer). Same as OBFT-1, except the bundle now carries σ^{op} only (no σ_V); evidence is still self-contained.
- **Rule 3 — Cross-onion partial-sig equivocation**. Operator `i` emitting Phase-2b σ partials on different V's at the same layer.
- **Rule 4 — Fake encrypted-presence (post-decryption garbage at k > 0)**. Same as OBFT-1.
- **Rule 5 — Fake plaintext σ at L_0**. Same as OBFT-1.
...
Same Class A / Class B taxonomy as OBFT-1 ([docs/OBFT-1.md:601](OBFT-1.md)). Variant C's recovery scope shifts several classes:

| Failure | OBFT-1 base | OBFT-1 + Phase 2a/2b (Variant C) |
|---|---|---|
| Sustained partition (real propagation > absorption window) | Class A | Class A — same; absorption window is `Δ_2a + (D + δ)` = same shape as OBFT-1's `Δ_2 + (D + δ)` |
...
- **Mitigation**: Rule 6 attribution is best-effort and should not trigger automated slashing without cluster-wide consensus on the evidence. This is consistent with OBFT-1's slashing model (human-supervised coordination).
...
- Same as OBFT-1's Phase-2 onion at deeper layers. No new attack surface, but the implementation must wrap Phase-2b σ partials in chained IBE the same way OBFT-1's Phase-2 onion does.
...
- In bare OBFT-1, this is the "fake plaintext σ at L_0" attack (Rule 5).
...
- **Result**: byz's lone σ-emission has no liveness effect at f=1 n=4 (cluster either reaches qV with honest cooperation or doesn't reach at all). Same outcome as OBFT-1. No new grief surface.
...
- **Rate-limit (anti-amplification rule)**: same as OBFT-1's Rule 5 rate-limit ([docs/OBFT-1.md:579](OBFT-1.md)). Each honest receiver gossips slashable verdict-equivocation evidence at most once per `(slot, layer, operator_id)` tuple. Caps amplification.
...
- **Mitigation**: same as OBFT-1's mesh-flakiness mitigation — `Δ_2a ≥ 2(D + δ)` recommended; mesh-diversity at deployment level.
...
- **Implementation**: EKM operations within a single Phase-2b emission must be atomic — either all per-layer log rows are written or none. Standard transactional database semantics apply. OBFT-1's EKM coordination model ([docs/OBFT-1.md:383](OBFT-1.md)) calls this out; it's unchanged here.
...
- Same as OBFT-1's Phase-2 onion behavior at deeper layers. No new edge case.
...
- OBFT-1's host workflow ([docs/OBFT-1.md:801](OBFT-1.md)): validate against stable head snapshot at Phase-1 acceptance, lock the verdict.
...
- **Two options**:
  1. **Host locks verdict at Phase-1 acceptance** (OBFT-1 style). Phase-2a verdict echoes the locked verdict. Validity-divergence behavior same as OBFT-1: re-org during Phase-1 acceptance window splits operators; `verdict_pool[V]` short of qV → NR fall-through. ✓
...
2. **Verdict equivocation rate-limit**: should honest receivers gossip slashable verdict-equivocation evidence on first detection or wait for cluster confirmation? OBFT-1 Rule 5 uses first-observed gossip with a per-(slot, layer, operator_id) cap; same rule fits here.
...
11. **K = 4 vs K = 3 trade-off**: same as OBFT-1 base. K=4 (= n) has maximum fall-through depth at +3 KB onion bandwidth; K=3 (= f+2) saves bandwidth but is less robust to multi-layer adversarial scenarios. Recommend K = n = 4 for SSV proposer.
...
13. **Migration / co-existence with QBFT**: rollout via per-cluster opt-in (DKG event) or feature flag. Wire-protocol versioning via `protocol_tag` (`OBFT-1-2ab-v1`) prevents cross-protocol message mixing. Operationally: ship behind feature flag, enable per cluster after DKG.

14. **DKG cost**: same as OBFT-1 — one V-keypair DKG (already in SSV) + one IBE-keypair DKG (new, run once at cluster init). Per-cluster setup, not per-slot.
...
- Update `Phase1Bundle` schema to remove `σ_V` partial; auth envelope retains `protocol_tag = "OBFT-1-2ab-v1"`.
...
- Add domain-separation tests confirming OBFT-1-2ab-v1 envelopes don't validate under bare TBFT or bare OBFT-1 envelope handlers.
...
| Aspect | Bare OBFT-1 | OBFT-1 + Phase 2a/2b (this) |
|---|---|---|
...
- **Honest software bugs producing byzantine-equivalent behavior** — same trust posture as OBFT-1 / QBFT (honest-majority cryptographic, not 100% cryptographic).
- **2-1 equivocation byz-defect grief** — strictly worse than bare OBFT-1 (regression). Byz that controls 1 vote out of 4, equivocates V/V', delivers V to 2 honest, V' to 1 honest, verdict-claims σV(V), then defects to NR at Phase-2b. Slot misses cleanly with Rule-6 evidence on the wire. Bare OBFT-1 would have succeeded via Phase-1 σ_V lock. The reputation deterrent absorbs this across many slots — but per-slot, an adversarial byzantine ignoring the deterrent can grief more reliably than under bare OBFT-1.
...
Variant C is the structural extrapolation of OBFT-1's "Phase 2a/2b" prose ([docs/OBFT-1.md:869](OBFT-1.md)) — taking seriously the "without a Phase-1 σ_L^V" hint at [docs/OBFT-1.md:873](OBFT-1.md). TBFTR's spec ([docs/TBFTR.md](TBFTR.md)) is Variant A (Phase-1 σ_V kept, Phase-2a onion carries σ); this design diverges to recover validity-divergence at all `n`/`f`.

The verdict-broadcast mechanism is the load-bearing addition: it makes cluster-wide convergence on σ-eligibility observable before any operator commits a partial, which is the structural fix for OBFT-1's Class A validity-divergence and Class B byzantine-grief patterns. Without verdict broadcasts, a Phase-2a window only gives more time for Phase-1 bundle propagation — equivalent to a wider `Δ_2` in OBFT-1 base.
...
The trade-off vs bare OBFT-1: a healthy-h_V=2 case falls through to L_1 (rather than succeeding at L_0 via the Phase-1 σ_V head-start). At K = n = 4, fall-through is one local-decryption iteration in Phase 3 — no extra RTT, slot still succeeds.

This is the design point for SSV proposer duty under realistic adversarial conditions, per OBFT-1.md's own assessment ([docs/OBFT-1.md:894](OBFT-1.md)): "OBFT-1 + Phase 2a/2b should be considered near-term, not future".
