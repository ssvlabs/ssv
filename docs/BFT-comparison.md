...
- **QBFT** (reflood-aware): `RefloodDelay + 1·BTT` added to each round's timer (R1 = 4·BTT + RefloodDelay, R≥2 = 5·BTT + RefloodDelay) — one gossip-backstop recovery (heartbeat + IWANT round-trip), matching OBFT's `B_0` reflood budget. The realistic mesh-tolerant default; compare against bare OBFT.
...
- **QBFT** (reflood-aware): per-round RT = structural minimum + `RefloodDelay + 1·BTT` (R1 = 4·BTT + RefloodDelay, R≥2 = 5·BTT + RefloodDelay). The realistic mesh-tolerant default — absorbs one gossip-backstop cycle per round like OBFT.
...
- **QBFT** (reflood-aware default): 3-phase consensus (PROPOSE → PREPARE → COMMIT) + post-consensus partial-sig collection = **4 BTT R1** healthy. Per-round RT adds one gossip-backstop cushion (`RefloodDelay + 1·BTT`) over the structural minimum: R1 timer = 4·BTT + RefloodDelay, R≥2 = 5·BTT + RefloodDelay. Absorbs one IHAVE/IWANT recovery per round (like OBFT's `B_0`); R2 fresh-V refetch on round timeout.
...
| QBFT (reflood) | 4 BTT | 9 BTT + RefloodDelay (R2) | 14 BTT + 2·RefloodDelay (R3) |
...
3. **QBFT (with R2 budget)**: closes adversarial-byz via fresh-V refetch on round-change. Bound by R2 fitting within budget — available at BFT_start ≤ 800ms with BTT=200ms for QBFT-SSV; BFT_start ≤ 1800ms with BTT=200ms for QBFT-no-reflood.
