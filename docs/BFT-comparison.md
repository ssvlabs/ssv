...
(At RefloodDelay=0 the recovery floor `max(RefloodDelay, 1·BTT)` pulls V_0's broadcast to `T_commit − 3·BTT = 3000ms` — 1·BTT earlier than the naive `T_commit − B_0 = 3200ms` — so the h_V=1 peer-reflood-V σ-upgrade still lands before `T_commit`; see [OBFT.md §Setting](OBFT.md#setting). Dormant at default RefloodDelay=700ms since `RefloodDelay ≥ 1·BTT`.)
...
| 3 | OBFT V_0 | **~2350ms** | **2850ms** | The only OBFT layer competing on MEV; primary always tried first (RD=0 capped by the recovery floor at `T_commit − 3·BTT`) |
...
- **OBFT V_0 pays a 700ms–1200ms BFT-consensus tax over the partial-sigs floor** (depending on RefloodDelay): 3550 − 2850 = 700ms (3.5·BTT) at RefloodDelay=0, 3550 − ~2350 = ~1200ms (~6·BTT) at default RefloodDelay. The tax decomposes as `broadcast slack + Δ_2 − partial-sigs post-fetch overhead = max(2·BTT + RefloodDelay, 3·BTT) + 1·BTT − 1.5·BTT` (the broadcast slack is floored at 3·BTT by the recovery margin). The 2·BTT shallow base covers 1·BTT P99 leader-broadcast propagation + 1·BTT IWANT round-trip; RefloodDelay covers one full IHAVE/IWANT cycle for mesh-flaky receivers. OBFT runs at tightened `Δ_2 = 1 BTT`; partial-sigs floor reserves 1·BTT emit + 0.5·BTT (100ms) submit = 1.5·BTT post-fetch.
