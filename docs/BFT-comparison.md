...
|---|---|---|---|---|---|
| 200ms | 400ms ✓ | 600ms ✓ | 1200ms ✓ | 1200ms ✓ | 1600ms ✓ |
| 600ms | 1200ms ✓ | 1800ms ✓ | 3600ms ✓ | 3600ms ✓ | **4800ms ✗** |
| 1000ms | 2000ms ✓ | 3000ms ✓ | **6000ms ✗** | **6000ms ✗** | **8000ms ✗** |
...
| 200ms | 400ms ✓ | 600ms ✓ | 1200ms ✓ | 1200ms ✓ | 1600ms ✓ |
| 600ms | 1200ms ✓ | 1800ms ✓ | **3600ms ✗** | **3600ms ✗** | **4800ms ✗** |
| 1000ms | 2000ms ✓ | 3000ms ✓ tight | **6000ms ✗** | **6000ms ✗** | **8000ms ✗** |
...
| 200ms | 400ms ✓ | 600ms ✓ | 1200ms ✓ | 1200ms ✓ | 1600ms ✓ |
| 600ms | 1200ms ✓ | 1800ms ✓ | **3600ms ✗** | **3600ms ✗** | **4800ms ✗** |
| 1000ms | 2000ms ✓ tight | **3000ms ✗** | **6000ms ✗** | **6000ms ✗** | **8000ms ✗** |

### Table 1e — BFT start = 2500ms (late MEV fetch), budget = 1400ms

| BTT | Partial-sigs (1 emission) † | OBFT (single round, K-onion) | OBFTR R1 (1 of R rounds) | 2abOBFT (Phase 2a/2b split) | QBFT R1 (3-phase + post-cons.) |
|---|---|---|---|---|---|
| 200ms | 400ms ✓ | 600ms ✓ | 1200ms ✓ tight | 1200ms ✓ tight | **1600ms ✗** |
| 600ms | 1200ms ✓ tight | **1800ms ✗** | **3600ms ✗** | **3600ms ✗** | **4800ms ✗** |
| 1000ms | **2000ms ✗** | **3000ms ✗** | **6000ms ✗** | **6000ms ✗** | **8000ms ✗** |
...
**Reading Tables 1a–1e:**

- **Partial-sigs floor (V pre-agreed)**: 2 BTT fits at every BFT_start except (2500ms, 1000ms). Sets the absolute floor — BFT consensus protocols pay 1-6 BTT extra to resolve V-disagreement.
- **BTT=200ms** (production-typical healthy mesh): OBFT, OBFTR R1, and 2abOBFT all fit every BFT_start. QBFT R1 (1600ms) fits BFT_start ≤ 1800ms; misses at 2500ms by 200ms.
- **BTT=600ms** (degraded mesh): OBFT (1800ms) fits BFT_start ≤ 1800ms; misses at 2500ms by 400ms. OBFTR R1 (3600ms) and 2abOBFT (3600ms) fit only at BFT_start = 0; miss at 800ms by 500ms. QBFT R1 (4800ms) misses everywhere.
- **BTT=1000ms** (severely degraded): only OBFT (3000ms, smallest BTT count due to staggered model + free Phase 3) fits at BFT_start ≤ 800ms (100ms margin at 800ms — tight). All other protocols miss everywhere.
...
| **h_V=1 selective-delivery deadlock** | n/a | ✓ via R2 | ✗ (Phase-2.5 σ-flip's `snap_NR_nl < f+1` trigger blocked when 2 honest NR; leader-only NR-flip cannot apply when leader is byz; deterred via Assumption 4 across slots) | ✗ (R-invariant) | ✓ convergence rule |
| **R1/R2/R3 narrow Phase-2.5 σ-flip recoveries** (byz σ-equivocation onto V' or byz σ on V' with 2 honest σ + 1 honest NR at f=1, n=4; see [OBFT.md §Phase 2.5](OBFT.md#phase-25--σ-flip--nr-flip-flips)) | n/a | ✓ via R2 (covered by general R2-fresh-V mechanism) | ✓ via Phase-2.5 σ-flip (snap-gated, single-round, in-protocol) | ✗ (OBFTR has no σ-flip; could be ported — see [OBFTR.md §Future work](OBFTR.md#future-work)) | ✓ via convergence rule (broader scope, also closes h_V=1) |
...
**OBFT (K=4)** — staggered per-layer broadcast at `T_broadcast_max_k = T_commit − B_k`, with `B_0 = 1 BTT, B_1 = 1.5 BTT, B_2 = 2.5 BTT, B_3 = 5.5 BTT` (see [OBFT.md §Setting](OBFT.md#setting)) at `T_commit = 3400ms`:
...
- **OBFT V_0 captures 900ms more MEV-fresh fetch time than QBFT R2** (3050 vs 2150ms). Under recommended sizing, QBFT R2 lands at 2150ms — same as OBFT V_3 (deepest backup) but reached only after R1 timeout. All four OBFT leaders beat QBFT R1 (both variants) by ≥1200ms; QBFT-SSV R1 at 150ms is tightest.
- **OBFT V_0 pays a 300ms BFT-consensus tax over the partial-sigs floor** (3050 vs 3350ms). This 300ms = 1.5 BTT: 1 BTT V_0 leader-broadcast propagation (OBFT-only) + 0.5 BTT B_0 convergence buffer (the part of `B_0 = 1 BTT` beyond pure typical-mesh propagation). Both at recommended 2 BTT per emission, the gap shrinks vs the old asymmetric framing.
- **QBFT-SSV R1 is structurally constrained** to 150ms MEV-fetch under the 2-round target — RT=2000ms eats 2000ms of slot budget, leaving the R1 leader essentially no fetch time. QBFT-optimal recovers ~800ms of R1 fetch by tightening RT to 6 BTT, but R2 fetch budget stays at 2150ms either way.
...
4. **Bare OBFT** (new design): closes silent-leader / partition cases via K-layer fall-through. **Closes R1/R2/R3 narrow Phase-2.5 σ-flip recoveries** (snap-gated, single-round; see Table 3 row + [OBFT.md §Phase 2.5](OBFT.md#phase-25--σ-flip--nr-flip-flips)). **Does NOT close**: h_V=1 selective Phase-1 delivery, 1-1-1 σ-locked equivocation, validity-divergence boundary splits — these remain R-invariant Class B grief deterred via Assumption 4 (rational-byzantine deterrent + planned blacklist + staker migration).
5. **Bare OBFTR**: closes silent-leader / partition cases via K-layer fall-through (single round) and partition-tail cases via R-round retry. Adversarial patterns (1-1-1, h_V=1, etc.) are R-invariant slot-misses; rational-byzantine deterrent absorbs across slots. (Phase-2.5 σ-flip / NR-flip could be ported from bare OBFT — see [OBFTR.md §Future work](OBFTR.md#future-work).)
...
- **Bandwidth (small `V`, e.g. attestations ~100 B; cluster-wide healthy path)**: QBFT ~14 KB across 4 emissions per round; OBFT ~28 KB across 1 emission (includes the mandatory `bundle_witnesses` leader-bundle re-flood ~1.5 KB at K=4 n=4); OBFTR ~28-30 KB across 2 emissions per round (R=2 worst case ~52 KB across 4 emissions; OBFTR's `sigma_L_witnesses` per-round ~1.5 KB at K=4 n=4); 2abOBFT ~30 KB across 2 emissions (no σ_L^V witness; 2abOBFT has no Phase-1 σ_L^V). All four +3-5 KB if L_Bid mini-consensus extension is used (see [each doc's Appendix B](OBFT.md#appendix-b--l_bid-mini-consensus-extension)).
- **Bandwidth (blinded-block `V`, OBFT proposer duty, ~5–15 KB typical / ~50 KB worst case at K=4 n=4)**: OBFT's `bundle_witnesses` re-flood scales with `V` size — per-op `KindCommit` ~88 KB typical / ~228 KB worst case, cluster-wide ~352 KB / ~912 KB across 4 emissions at K=4 n=4 (see [OBFT.md §Application: SSV Ethereum proposer duty](OBFT.md#application-ssv-ethereum-proposer-duty) for the derivation). OBFTR's per-round `sigma_L_witnesses` doesn't carry full V — only σ_L^V partials — so OBFTR's blinded-block bandwidth is closer to the small-`V` baseline + R-round overhead. 2abOBFT also doesn't re-flood full V (no Phase-1 σ_L^V). For OBFT-family proposer duty, blinded-block V is mandatory (full-payload V exceeds practical bandwidth budgets — see [OBFT.md §Application](OBFT.md#application-ssv-ethereum-proposer-duty)). Few-large emissions (OBFT) are gentler on the gossipsub mesh than many-small (QBFT) at SSV's KB-range message sizes — per-message overhead (signature verify, dedup, peer-score, mesh forwarding) dominates over per-byte cost.
