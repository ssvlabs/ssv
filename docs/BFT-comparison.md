...
- **Time unit `PC` (propagation cycle)** = `D + δ` — one one-way propagation cycle under partial-synchrony assumptions. Used throughout for time-budget formulas. Concrete: at D=200ms, δ=50ms, `1 PC = 250ms` (treated as ≈ 200ms = 1×D in tabulation, since δ is dropped per above).
...
- **QBFT** (current SSV): 3-PC consensus (PROPOSE → PREPARE → COMMIT) + 1-PC post-consensus partial-sig collection = **4 PC minimum**. R-round retry on round timeout. Recovery via round-change with new leader + potentially fresh V.
- **OBFT**: K-layer onion with chained encryption, single Phase 2 with one `KindCommit` emission per operator at `T_commit` carrying both σ and NR partials (no Defer state, no sub-phasing). Per-layer staggered broadcast deadlines `T_broadcast_max_k`, with deeper layers having wider propagation budgets `B_k`. **3 PC** (Phase 1 + Phase 2 + Phase 3). Recovery via in-round K-layer parallel fall-through (sequential local decryption in Phase 3, no extra PC per layer).
- **OBFTR** (R≥2): same K-layer onion as OBFT, plus R-round retry with re-flood, per-round independent commitments (no cross-round σ-or-NR exclusivity), L_C cluster-consensus signaling. **3 PC per round**, total `3R PC`. Recovers partition tails up to `R · D` via re-flood across rounds — succeeds at L_0 specifically (preserves MEV freshness) at the cost of an extra round.
- **2abOBFT**: K-layer onion with Phase 2a (verdict broadcast) + Phase 2b (σ-or-NR commit driven by convergence rule on Phase-2a verdict pool). **4 PC** (Phase 1 + 2a + 2b + 3). Single-round only.

## Total time to signed output (in PC units)

Numbers below assume **recommended Δ sizing** consistently across all four protocols — each per-emission propagation cycle is sized at `2 PC` for jitter absorption (one full extra propagation cycle of slack on top of P99). Per-protocol totals:

- **OBFT**: `1 PC` broadcast slack [staggered model — `B_0 = 1 PC` for the primary L_0; deeper layers' broadcasts pre-arrived; see [OBFT.md §Setting](OBFT.md)] + `2 PC` Phase 2 + 0 Phase 3 = **3 PC**.
- **OBFTR per round**: `2 PC` broadcast slack [uniform model] + `2 PC` Phase 2 + `1 PC` Phase 2.5 (L_C signaling) + 0 Phase 3 = **5 PC**. Round 2 omits fresh broadcast slack (uses re-flood ≈ `1 PC`) → **4 PC**.
- **2abOBFT**: `2 PC` broadcast slack + `2 PC` Phase 2a + `2 PC` Phase 2b + 0 Phase 3 = **6 PC**.
- **QBFT**: 4 phase emissions (PROPOSE + PREPARE + COMMIT + post-consensus), each sized at `2 PC` for jitter absorption = **8 PC**.

| Protocol | Round 1 healthy | Round 2 (recovery) | Total at R-round failure |
|---|---|---|---|
| QBFT | 8 PC | RT (2s timeout) + 8 PC | 16 PC + 2s |
| OBFT | 3 PC | n/a (single-round) | n/a (slot misses if R1 fails on adversarial pattern; K-layer fall-through is in-round, free) |
| OBFTR(R=2) | 5 PC | 4 PC | 9 PC |
| 2abOBFT | 6 PC | n/a (single-round) | n/a (K-layer fall-through is in-round, free) |

K-layer fall-through (OBFT, OBFTR, 2abOBFT) is sequential local decryption in Phase 3 — no per-layer PC cost. It recovers silent/late leaders within the same round.

**Why OBFT < OBFTR per-round.** The staggered per-layer broadcast model in bare OBFT (see [OBFT.md §Setting](OBFT.md)) lets the primary L_0 broadcast at `T_commit − 1 PC` (1 PC slack) instead of OBFTR's uniform `T_commit_r − 2 PC` (2 PC slack). Plus OBFT has no Phase 2.5 (L_C signaling) since it has no rounds to coordinate. Net: OBFT saves 2 PC vs OBFTR per round at recommended sizing.

**Note on QBFT sizing.** QBFT in SSV production uses round timeout `RT = 2s` and lets phases happen at organic gossipsub speed within `RT` (no explicit per-phase Δ_phase recommendation). The 8 PC above applies the same "2× per emission cycle for jitter absorption" convention used for the OBFT family. Without that convention (i.e., minimum 1 PC per phase), QBFT would be 4 PC minimum — but that under-budgets jitter absorption relative to how OBFT/OBFTR/2abOBFT are sized below. Concretely: at D=600ms, 8 PC = 4.8s exceeds SSV's `RT = 2s`, so QBFT under recommended sizing doesn't actually fit at high D. That mismatch is real — QBFT has weaker jitter absorption than the OBFT family at the same D if we hold the recommended-sizing convention constant.
...
| BFT start, D | Budget | QBFT (8 PC) | OBFT (3 PC) | OBFTR R1 (5 PC) | 2abOBFT (6 PC) |
|---|---|---|---|---|---|
| **0s, D=200ms** | 3.75s | 1.6s ✓ | 0.6s ✓ | 1.0s ✓ | 1.2s ✓ |
| **0s, D=600ms** | 3.75s | **4.8s ✗** | 1.8s ✓ | 3.0s ✓ | 3.6s ✓ tight |
| **0s, D=1000ms** | 3.75s | **8.0s ✗** | 3.0s ✓ | **5.0s ✗** | **6.0s ✗** |
| **400ms, D=200ms** | 3.35s | 1.6s ✓ | 0.6s ✓ | 1.0s ✓ | 1.2s ✓ |
| **400ms, D=600ms** | 3.35s | **4.8s ✗** | 1.8s ✓ | 3.0s ✓ tight | **3.6s ✗** |
| **400ms, D=1000ms** | 3.35s | **8.0s ✗** | 3.0s ✓ tight | **5.0s ✗** | **6.0s ✗** |
| **2.5s, D=200ms** | 1.25s | **1.6s ✗** | 0.6s ✓ | 1.0s ✓ tight | 1.2s ✓ very tight |
| **2.5s, D=600ms** | 1.25s | **4.8s ✗** | **1.8s ✗** | **3.0s ✗** | **3.6s ✗** |
| **2.5s, D=1000ms** | 1.25s | **8.0s ✗** | **3.0s ✗** | **5.0s ✗** | **6.0s ✗** |

**Reading Table 1:**

- **D=200ms** (production-typical for healthy mesh): all four protocols complete healthy at 0s and 400ms BFT start. At 2.5s start, QBFT (1.6s) misses the 1.25s budget while the OBFT family fits.
- **D=600ms** (degraded mesh): QBFT misses everywhere (8 PC = 4.8s exceeds even the 0s budget of 3.75s — under recommended sizing, QBFT can't absorb jitter at this D). OBFT and OBFTR R1 fit 0s/400ms; 2abOBFT fits 0s tightly but misses 400ms.
- **D=1000ms** (severely degraded): only OBFT (3 PC, smallest PC count due to staggered model) fits at 0s and 400ms start. Everything else misses everywhere.
- **OBFT < OBFTR R1 in PC count.** The staggered per-layer broadcast model in OBFT (see [OBFT.md §Setting](OBFT.md)) gives the primary L_0 a tighter 1 PC propagation budget. OBFTR uses uniform 2 PC broadcast slack across all leaders + Phase 2.5 (L_C signaling). Net: OBFTR R1 is 2 PC longer than bare OBFT at recommended sizing.
- **OBFT < OBFTR R1 < 2abOBFT < QBFT** is the healthy-path ordering at recommended sizing. QBFT's 8 PC (4 phases × 2 PC each for jitter absorption) is the slowest under this convention; in practice SSV QBFT relies on the 2s round timeout absorbing variance rather than per-phase 2× sizing, so production QBFT runs faster than 8 PC suggests at low D — but the comparison here holds the convention constant.
...
| BFT start, D | QBFT R2 (RT + 8 PC) | OBFT K-layer fall-through | OBFTR R1+R2 (9 PC) | 2abOBFT K-layer fall-through |
|---|---|---|---|---|
| **0s, D=200ms** | 3.6s ✓ tight | in-round (free) | 1.8s ✓ | in-round (free) |
| **0s, D=600ms** | **6.8s ✗** | in-round (free) | **5.4s ✗** | in-round (free) |
| **0s, D=1000ms** | **10.0s ✗** | in-round (free) | **9.0s ✗** | in-round (free) |
| **400ms, D=200ms** | **3.6s ✗** (250ms over) | in-round (free) | 1.8s ✓ | in-round (free) |
| **400ms, D=600ms** | **6.8s ✗** | n/a (R1 missed) | **5.4s ✗** | in-round (free) |
| **400ms, D=1000ms** | **10.0s ✗** | n/a (R1 missed) | **9.0s ✗** | n/a (R1 missed) |
| **2.5s, D=200ms** | **3.6s ✗** | in-round (free) | **1.8s ✗** | in-round (free) |
| **2.5s, D=600ms** | **6.8s ✗** | n/a | **5.4s ✗** | n/a |
| **2.5s, D=1000ms** | **10.0s ✗** | n/a | **9.0s ✗** | n/a |

"In-round (free)" means the recovery happens within the same single round — no additional time cost. K-layer fall-through is sequential local decryption in Phase 3, processing-bound (~100ms ε_3), not PC-bound.
...
- **OBFTR's R1+R2 retry** at recommended sizing fits only at D=200ms (1.8s vs 3.75s budget at 0s; 1.8s vs 3.35s at 400ms). At D=600ms or above, R1+R2 doesn't fit any start time (5.4s+ exceeds 3.75s budget). **OBFTR R1+R2 fitness is narrow under recommended sizing** — production deployments of OBFTR(R=2) need to either tighten Δ_2 toward minimum (sacrificing jitter absorption) or accept that round-2 recovery only works at low-D.
- **QBFT's round-2 retry** at recommended sizing costs RT (2s) + 8 PC = 3.6s+ — fits tightly at (0s, D=200ms) but misses everywhere else. At (400ms, D=200ms) it's 250ms over the 3.35s budget. **Round-2 QBFT under recommended sizing barely fits production** — a real shift from the minimum-sizing analysis where round-2 QBFT fit at multiple scenarios.
...
- The **structural QBFT-only recovery** (round-2 with fresh-V refetch) is unavailable in adversarial conditions at any reasonable D under recommended sizing — the 2s round timeout + 8 PC phase budget exceeds 4s budget at all but the (0s, D=200ms) edge case.
...
- **OBFT family inherits QBFT's structural disadvantage** at multi-leader-silent (K-1 ≥ 3) patterns: QBFT requires K serial round-changes (each ~RT + 8 PC), exceeding the 4s budget at any K-1 ≥ 3 in production sizing. OBFT/OBFTR/2abOBFT recover within a single round via K-layer fall-through (free in Phase 3).
...
**Late-fetch tolerance (BFT start = 2.5s, budget = 1.25s)**: OBFT comfortably fits at D=200ms (600ms in 1250ms budget); OBFTR R1 at 1.0s is tight; 2abOBFT at 1.2s is very tight (50ms margin); QBFT at 1.6s misses. At D ≥ 600ms, all four miss — late-fetch is incompatible with degraded mesh.

**Degraded-mesh tolerance (D = 1000ms)**: only OBFT (3 PC, smallest PC count due to staggered model) fits at 0s and 400ms BFT start. OBFTR R1 (5 PC), 2abOBFT (6 PC), QBFT (8 PC) all miss everywhere because their consensus time meets or exceeds the 3.35-3.75s budget.

**Mid-D tolerance (D = 600ms)**: bare OBFT (1.8s) fits comfortably; OBFTR R1 (3.0s) fits tightly at 400ms start; 2abOBFT (3.6s) fits at 0s tightly but misses at 400ms; QBFT (4.8s) misses everywhere — under recommended sizing it can't absorb jitter at this D.

**Round-2 retry usefulness**: **OBFTR's R1+R2 (9 PC recommended sizing) fits only at D=200ms** at any reasonable BFT_start; at D≥600ms it doesn't fit any start time. **QBFT's R2 (RT + 8 PC = 3.6s+) fits only at (0s, D=200ms) tightly**; everywhere else it misses. Above those, retry budget is unavailable. The OBFT family's K-layer in-round fall-through is structurally cheaper than either retry mechanism — it doesn't consume additional PC and recovers silent leaders for free.
...
3. **QBFT (with R2 budget)**: closes adversarial-byz via fresh-V refetch on round-change. Bound by RT + 8 PC fitting within budget — under recommended sizing this is rarely available.
...
- **QBFT (current SSV)**: production-mature, recovers more adversarial-byz patterns than bare OBFT/OBFTR when round-2 budget is available. Pays high latency cost (~8 PC healthy under recommended sizing; ~4 PC minimum) and degrades sharply at late BFT-start or high D.
...
- **+2 PC healthy-path latency**: OBFT+L_Bid is **5 PC** (1 PC broadcast slack + 2 PC mini-consensus + 2 PC Phase 2 + 0 Phase 3) vs bare OBFT's 3 PC, at recommended Δ sizing. The mini-consensus phase contributes 2 PC (recommended `Δ_minicon = 2 PC` matching `Δ_2`'s widening for jitter absorption — see [OBFT.md Appendix B](OBFT.md#appendix-b--l_bid-mini-consensus-extension)).
...
|---|---|---|---|---|---|---|
| **0s, D=1000ms** | 3.75s | 8.0s ✗ | 3.0s ✓ | **5.0s ✗** | 5.0s ✗ | 6.0s ✗ |
| **400ms, D=1000ms** | 3.35s | 8.0s ✗ | 3.0s ✓ tight | **5.0s ✗** | 5.0s ✗ | 6.0s ✗ |

OBFT+L_Bid loses bare OBFT's healthy-path advantage at these severely degraded scenarios — its 5-PC structure (vs bare OBFT's 3-PC) joins the protocols that miss budget. At D ≤ 600ms with BFT_start ≤ 400ms, OBFT and OBFT+L_Bid fit equally.

#### Failure-recovery delta — Table 2

**No latency difference.** Both bare OBFT and OBFT+L_Bid recover via in-round K-layer / K'-layer fall-through (sequential local decryption in Phase 3, no per-layer PC cost). OBFT+L_Bid's K' = K + 1 adds one extra layer at the top, giving an additional "first-try" recovery opportunity at no extra time. Recovery profile across all scenarios is identical between bare OBFT and OBFT+L_Bid.
...
OBFT+L_Bid pays:
- **+2 PC healthy-path latency** (loses bare OBFT's advantage at (0s, 1000ms) and (400ms, 1000ms) scenarios where bare OBFT fits and OBFT+L_Bid doesn't; all other scenarios are unaffected at the budget-fit level).
...
The trade is favorable when MEV bid-routing value-capture upside exceeds the combined cost of (a) the new failure modes' slot-loss rate and (b) the +2 PC latency cost. For low-MEV slots or deployments with significant mesh degradation pushing scenarios toward the (0s, 1000ms) or (400ms, 1000ms) borderline, bare OBFT is the better choice.

## Limits of this comparison

- **Numbers are PC-count approximations** (3 PC, 4 PC, etc.). Production has long tails; ε_3 (~100ms local processing) is treated as small relative to D in tabulation. Real implementations may add 50-200ms of constant overhead per round.
