...
- **Three distinct deadlines** (do not conflate):
  ...
  - **Per-layer leader broadcast targets** `T_broadcast_max_k = max(0, T_verdict_start − B_k)`. Each layer `L_k` aims to finish broadcasting its Phase-1 bundle by its per-layer target so all honest first-observe within `B_k` of broadcast under worst-case propagation. Per-layer fetch windows fit: `T_k + Δ_1 ≤ T_broadcast_max_k`. `B_k` is a *target* absorption budget, not a hard runtime cap; the only runtime acceptance gate is `T_accept_max` (below). When `B_k ≥ T_verdict_start` the target clamps to 0 — leader broadcasts at slot start, best effort. This is the *default* configuration for the deepest layer: `B_{K-1} = T_verdict_start` ("earliest possible"). **Default schedule** at K=4 (mirrors OBFT.md §Setting): `B_0 = 1 BTT, B_1 = 1.5 BTT, B_2 = 2.5 BTT, B_3 = T_verdict_start`. `B_k` strictly increasing in k (deeper layers get wider propagation budgets); configurations where two-or-more layers all clamp to 0 are rejected (strict-decreasing FetchAt invariant would collide).
  ...
  **Per-layer `B_k` composes with Phase-2a re-flood absorption.** The Phase-2a witness window adds a *uniform* `Δ_2a + 1·BTT = 3·BTT` of additional absorption past every leader's `T_broadcast_max_k` — bundles first-observed in `(T_broadcast_max_k, T_accept_max]` re-flood through Phase-2a and still feed verdicts. Per-layer `B_k` adds an asymmetric layer on top: a deeper layer's leader broadcasts earlier (wider `B_k`), giving its bundle more time to propagate *before* Phase-2a starts. The two mechanisms compose — they don't substitute. At Config A with the default K=4 schedule: L_0's total effective absorption window is `B_0 + Δ_2a + 1·BTT = 4·BTT = 800ms` (1·BTT pre-Phase-2a + 3·BTT Phase-2a re-flood); L_{K-1}'s is `B_{K-1} + Δ_2a + 1·BTT = T_verdict_start + 3·BTT` ≈ the entire slot's pre-commit budget (deepest broadcasts at slot start, then full Phase-2a re-flood on top). The marginal contribution of per-layer `B_k` beyond Phase-2a is the *MEV-fetch asymmetry* — primary fetches latest, deepest fetches earliest — and an extra pre-Phase-2a propagation margin for deep-layer tails.
...
5. **Coordinated EKM across both keypair shares + persistent Phase-2a state.** The "EKM" referenced throughout this spec is a coordinated signing service spanning the operator's V-keypair share and IBE-keypair share, backed by a single slashing-protection log keyed on `(slot, layer, side, value_root)`. **2abOBFT's EKM is the simplest in the OBFT family**: one signing event per (slot, layer) per operator at Phase-2b — no cross-round atomicity, no Phase-1 σ_V to coordinate with later Phase-2 σ, no persistent partial-sig cache.
   ...
   **However, 2abOBFT requires a separate per-slot persistent state** beyond the EKM log: each operator must durably record (a) any `KindVerdict` envelope they have broadcast at Phase-2a, and (b) the retained `(V, σ_L^op)` tuples per `(slot, layer, leader_id)`. This state is needed because Phase-2a verdicts are **not** EKM-logged (they are op-identity-signed, not threshold partials), but they bind the operator's behavior at Phase-2b: an operator restarting mid-slot whose pre-crash verdict is on the wire but whose retained-V state is lost cannot replay the verdict-broadcast decision (re-broadcasting would be self-equivocation under Rule 6a) and may be forced into an action that mismatches the on-wire verdict (Rule 6b false-positive against the honest operator).
   ...
   The persistent-state requirement is operator-internal — the EKM log alone is insufficient. **Implementations MUST persist** verdict envelopes and retained Phase-1 bundles per `(slot, layer)` to a durable store keyed on slot, with retention until reconstruction halts or the slot is declared missed. This is a correctness requirement, not a performance optimization — a non-persisting honest operator restarting mid-slot produces byzantine-equivalent wire behavior under Rule 6a (self-equivocation if a different verdict is re-broadcast) or Rule 6b (verdict-vs-action mismatch from lost retained-V state). See [§EKM coordination model](#ekm-coordination-model).
...
1. **Phase 1** `[slot_start, T_verdict_start]`: K leaders broadcast their Phase-1 bundles per their per-layer fetch windows (`[T_{K-1}, T_{K-1} + Δ_1]`, ..., `[T_0, T_0 + Δ_1]`), with `T_k + Δ_1 ≤ T_broadcast_max_k = max(0, T_verdict_start − B_k)` per layer (deepest target clamps to 0; see §Setting). **No σ_V partial in Phase-1 bundles** (Variant C). Receivers accept bundles first-observed in `[slot_start, T_accept_max]` where `T_accept_max = T_commit − 1 BTT`.
...
This state lives outside the EKM slashing-protection log (the log only covers σ/NR threshold-partial signings at Phase 2b). The persistent-state requirement on this state is **MUST** (per assumption 5); implementations may colocate it with the EKM database for transactional consistency, or store it in a separate per-slot durable file.
...
- **Partial synchrony for liveness**: messages eventually deliver within bounded delay `P99` (propagation P99/P999) and clock skew `δ`. Three distinct cutoffs operationalize this bound: `T_broadcast_max_k = max(0, T_verdict_start − B_k)` (per-layer leader broadcast target; see §Setting), `T_accept_max = T_commit − 1 BTT` (receiver acceptance horizon, cluster-wide), `T_verdict_max = T_commit − 1 BTT` (verdict broadcast horizon, coincident with `T_accept_max`). Reconstruction is expected to complete at `T_commit + Δ_2b + Δ_3`; the slot's hard wall is the relay-submission deadline `T_relay_cutoff − T_submit`.

  **2abOBFT's per-layer effective absorption window** = `T_accept_max − T_broadcast_max_k = (Δ_2a + 1·BTT) + (B_k − 2·BTT)` = uniform Phase-2a re-flood absorption `Δ_2a + 1·BTT = 3·BTT ≈ 600ms` at Config A (at the mandatory `Δ_2a = 2 BTT` recommended sizing; sub-2-BTT sizings are broken-by-construction with the late-broadcast verdict schedule — see [§Phase 2a](#phase-2a)), *plus* the per-layer pre-Phase-2a margin `B_k − 2·BTT`. At the default K=4 schedule: L_0 = `3·BTT − 1·BTT = 2·BTT` total; L_{K-1} = `3·BTT + (T_verdict_start − 2·BTT) ≈ T_verdict_start + 1·BTT` total (deepest leader broadcasts at slot start; the cluster gets the entire pre-`T_accept_max` window for that bundle's propagation, plus Phase-2a re-flood on top).
...
| `T_broadcast_max_k` | per-layer leader broadcast target — `max(0, T_verdict_start − B_k)`; deeper layers broadcast earlier (wider `B_k`). Default K=4: B_0=1·BTT, B_1=1.5·BTT, B_2=2.5·BTT, B_3=T_verdict_start. Target, not hard runtime cap — see §Setting |
...
|---|---|---|---|
| Phase 1 fetch (per-layer effective) | varies | per-layer | Per-layer staggered broadcast targets `T_broadcast_max_k = max(0, T_verdict_start − B_k)`. Default K=4 schedule: L_3 = 0 (B_3 = T_verdict_start), L_2 = 1.10s (T_verdict_start − 2.5·BTT), L_1 = 1.30s, L_0 = 1.40s |
...
| Phase 2a | 400ms | slot_start + 2.00s = T_commit | `Δ_2a = 2 BTT`; verdict broadcast horizon = `T_commit − 1 BTT = 1.80s`; absorbs late bundles arriving up to 1.80s |
| Phase 2b | 400ms | slot_start + 2.40s | `Δ_2b = 2 BTT`; absorbs Phase-2b emission propagation (operators emit at start of Phase 2b per [§Phase 2b](#phase-2b)'s early-emission discipline) |
| Phase 3 | 50ms | slot_start + 2.45s | `Δ_3 = ε_3 = 50ms`; purely local reconstruction processing |
| Submission | 1550ms | slot_start + 4.00s | ~15× the 100ms minimum — comfortable headroom |

The "Submission" row above is the slack between consensus completion and the relay-submission deadline. This is the **comfortable-headroom anchor** (`T_commit = 2.00s`, 1.55s submission slack) used as the recommended production deployment. A **max-MEV anchor** is also viable — anchoring `T_commit` later (≈ `slot_start + 3.45s`, back-derived from `T_relay_cutoff − T_submit − Δ_2b − Δ_3 = 4.00 − 0.10 − 0.40 − 0.05`) gives L_0 a wider MEV-relay fetch window at the cost of submission slack. The cross-protocol comparison in [docs/BFT-comparison.md](BFT-comparison.md) uses the max-MEV anchor for like-for-like MEV-fetch-budget framing.

Sub-recommended sizings (e.g., `Δ_2a = 1 BTT`) are broken-by-construction with 2abOBFT's late-broadcast verdict schedule — see [§Phase 2a](#phase-2a). Production deployments MUST use `Δ_2a = Δ_2b = 2 BTT` recommended sizing.
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. Three distinct deadlines (do not conflate):
   ...
   - **`T_broadcast_max_k = max(0, T_verdict_start − B_k)`** per-layer: leader L_k aims to broadcast by this target; deepest clamps at 0 (B_{K-1} = T_verdict_start by default — see §Setting).
...
The production-T allocation reflects current deployment choices, which are **not** strictly apples-to-apples for failure-mode coverage — QBFT uses ~3.6s for 2-round consensus + ~0.4s for submission, while 2abOBFT uses ~1200ms for consensus + ~2.8s submission headroom. Comparing failure-mode coverage at production-T can read as "QBFT recovers more failure modes than 2abOBFT" but conflates two different effects:
