...
3. **Host validity is best-effort unanimous at decision time.** The protocol consumes the host application's `valid` / `not-valid` verdict on each `V_{L_k}` at Phase-1 acceptance, at Phase-2a fire time, and at A1-upgrade time. Operators may transiently diverge (e.g., a head change observed by some but not others); the host's job is to make divergence rare via per-operator stabilization.
...
- **`KindNoValue`** — the operator has no `V_0`, or its host says NV. This is a **coordination message, not a commitment**: it carries the operator's deeper-layer entries but **no L_0 payload and no L_0 lock**. The operator stays in the L_0 `coordination` EKM state, free to go either direction later (the A1 upgrade or an NR commit).
...
| `σ-locked(V)` | σ-committed to `V` | leader: signing its Phase-1 `LWitness`; any operator: emitting `KindValue` (incl. A1 upgrade) or a `SigmaChained` deeper-layer entry |
...
2. **Phase 2a** (async, backstop at `TPhase2a = T_0_broadcast + 1·BTT`): each operator fires `KindValue` / `KindNoValue` / `KindCommit-NRDirect` as soon as L_0 is determinable, else at the backstop. The A1 upgrade window stays open afterward.
...
- **Validating `V_{L_k}` at the receiver role.** The host's validity check is invoked at three points: Phase-1 bundle acceptance (drives retention), Phase-2a fire time (drives the `KindValue` vs `KindNoValue` choice), and A1-upgrade time on a harvested value. Each check is independent; a `not-valid` at any point steers the operator away from σ.
...
- **h_V=1 byzantine selective-delivery** (the leader delivers `V_0` to only one honest operator) — the V-drop operators harvest `V_0` + the leader's witness from the recipient's `KindValue` (peer-reflood-V), host-validate, and A1-upgrade to σ. **Success (fastest).**
...
|---|---|---|---|
| Healthy path | ✓ | ✓ | ✓ |
| Network flakiness / σ-refusal / non-leader fault (within f) | ✓ | ✓ | ✓ |
...
| Late L_0-leader broadcast | ✓ fall-through | ✓ at L_0 | ✓ round 2 |
...
2abOBFT's distinctive win over bare OBFT is **σ-locked-split leader equivocation** (bare OBFT misses; 2abOBFT recovers at L_0 via the witness head-start + peer-reflood-V harvest), plus deciding a **late L_0-leader at L_0** instead of falling through. Its distinctive losses relative to QBFT are the patterns a fresh-value round 2 recovers but a single round cannot re-propose: **1-1-1 equivocation, the 2-2 validity boundary, and validity-divergence with a passive byzantine**. Bandwidth (n=4, K=2, healthy) is ~20–22 KB cluster-wide, comparable to bare OBFT and modestly above QBFT.
