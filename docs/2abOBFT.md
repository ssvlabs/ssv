...
> **⚠️ Spec rewrite pending.** This document describes the v1 protocol (`KindVerdict` envelopes at Phase 2a, `KindOnion2b` at Phase 2b, `T_commit` hard wall + 5-row convergence rule). The current implementation in `protocol/v2/obft/twoab/` is the v4 redesign: Phase 2a is a `KindValue` / `KindNoValue` coordination fire-instant; Phase 2b is dynamic (`KindCommit` with side flag) driven by three triggers (σ-eligibility, NR-eligibility with cannot-σ gate, equivocation) — no `T_commit` hard wall. The wire format, evidence rules (Rule 6b dropped), and timing parameters (`TPhase2a` + `SafetyBuffer` replace `TCommit` + `Delta2a` + `Delta2b` + `Eps3`) all differ. Refer to the implementation, `docs/2abOBFT-REDESIGN-PLAN.md`, and the package documentation in `protocol/v2/obft/twoab/` for the current protocol semantics. This spec doc will be rewritten in a separate pass.
...
2abOBFT operates with K configurable layers (`f+1 ≤ K ≤ n`), each layer with its own deterministically-derived leader, falling through within a single Phase-3 reconstruction walk (sequential local decryption, no per-layer RTT). The running example throughout is `n = 4, f = 1, K = 2` (= K=f+1, BFT-liveness minimum at f=1, the recommended default — see §Application); algebra generalizes uniformly across the supported K-range and to higher cluster sizes. K=4 worked examples appear as up-tier illustrations where deeper-fall-through behavior is relevant.
...
**Not suited for:**
- Deployments where every millisecond of submission headroom is critical and the Class B grief patterns 2abOBFT closes are not relevant (e.g., low-stake testnet clusters with cooperative byzantines). [OBFT](OBFT.md) saves ~450ms (~2.25 BTT at Config A: Phase 2a window 250ms at tightened `Δ_2a = 1·BTT + ε_proc` + ε_proc ~50ms on Δ_2b + post-Phase-2b consolidation 150ms) but exposes the closed-here failure modes.
...
- **K layers** (`f+1 ≤ K ≤ n`, configurable) with deterministically-derived leaders: layer 0 with **primary leader** `L_0`, layer 1 with **backup leader** `L_1`, ..., layer `K-1` with `L_{K-1}`. Leaders must be distinct (`L_i ≠ L_j` for `i ≠ j`). For `K = n`, every cluster member is a leader at exactly one layer per slot.
  ...
  **Spec-level K bounds:**
  ...
  Choice of K is **deployment-dependent** and left to the operator. **Recommended default: `K = f+1` (BFT-liveness minimum)** — at n=4 (f=1) this is K=2, motivated by production testing showing K > f+1 doesn't materially improve outcomes once the structural Class B grief patterns are closed in-protocol (see [§Application](#application)). Clusters operating closer to the partial-synchrony tail or treating adversarial-byz tolerance as a hard requirement may prefer `K ≥ f+2` or `K = n` (more fall-through layers, at the cost of deeper chained-encryption and larger Phase-3 IBE walks). The running protocol example below uses `n = 4, f = 1, K = 2` for clarity but the spec applies uniformly across `f+1 ≤ K ≤ n`.
...
- **Three distinct deadlines** (do not conflate):
  ...
  **Per-layer `B_k` composes with Phase-2a re-flood absorption.** Per-layer effective absorption (from `T_broadcast_max_k` to `T_accept_max`) = `B_k + Δ_2a − 1·BTT` — decomposes as `B_k` pre-Phase-2a (leader-broadcast to T_verdict_start propagation budget) plus `Δ_2a − 1·BTT` Phase-2a re-flood absorption (the slice of the Phase-2a window before the `T_accept_max = T_commit − 1·BTT` cutoff). Under the recommended `Δ_2a = 1·BTT + ε_proc`, the Phase-2a contribution `Δ_2a − 1·BTT` collapses to `ε_proc` — late-bundle absorption is structurally shifted to `B_0`'s `RefloodDelay`-baked schedule pre-Phase-2a. Per-layer `B_k` adds an asymmetric pre-Phase-2a layer: a deeper layer's leader broadcasts earlier (wider `B_k`), giving its bundle more time to propagate *before* Phase-2a starts. At Config A with the K=4 reflood-aware schedule (up-tier illustration; K=2 default collapses to `B = [B_0, T_verdict_start]`) (default RefloodDelay=700ms): L_0's total absorption = `(2·BTT + RefloodDelay) + ε_proc = 1150ms` (decomposes as `B_0 = 1100ms` pre-Phase-2a + `Δ_2a − 1·BTT = 50ms` Phase-2a residual); at RefloodDelay=0 the same formula gives `450ms`. L_{K-1}'s is `T_verdict_start + ε_proc ≈ T_accept_max` total ≈ the entire slot's pre-commit budget (deepest broadcasts at BFT_start). The marginal contribution of per-layer `B_k` beyond the ε_proc Phase-2a residual is the *MEV-fetch asymmetry* — primary fetches latest, deepest fetches earliest — and an extra pre-Phase-2a propagation margin for deep-layer tails.

- **Phase-window minimums:**

  - **`Δ_2a ≥ 1·BTT + ε_proc`** (the strict structural minimum, recommended for production) so verdicts propagate before Phase-2a end. Sub-floor sizing is **broken-by-construction** with the late-broadcast verdict schedule — see [§Phase 2a / Verdict propagation budget](#phase-2a). Under the reflood-aware schedule, late-Phase-1-bundle absorption is provided by `B_0 = 2·BTT + RefloodDelay` pre-Phase-2a, so `Δ_2a` no longer carries a separate reflood / jitter cushion — same philosophy as OBFT's tightened `Δ_2 = 1·BTT`.
...
2. **Partial synchrony for liveness.** Messages eventually deliver within bounded propagation `P99` (cluster gossipsub P99/P999) and clock skew `δ`. Safety is unconditional on timing; only liveness depends on this. **2abOBFT's per-layer effective absorption window is `B_k + Δ_2a − 1·BTT = B_k + ε_proc`** (at recommended `Δ_2a = 1·BTT + ε_proc`) — Phase-1-broadcast-to-receiver-first-observation tolerance for late bundles to still drive a Phase-2a verdict; mostly carried by `B_k` under the reflood-aware schedule. At Config A default RefloodDelay=700ms: L_0 = `B_0 + ε_proc = 1150ms`; deeper layers wider (L_{K-1} ≈ T_accept_max). Real propagation that exceeds this window is Class A "sustained partition" — out of scope by definition.
...
**Verdict propagation budget.** Verdicts broadcast at `T_verdict_max − ε_proc` propagate to all honest peers within `1 BTT` under partial synchrony, reaching them by `T_verdict_max + 1 BTT − ε_proc = T_commit − ε_proc`. The arrival-to-Phase-2a-end slack is `ε_proc` — the tail margin between the last honest receiver seeing the verdict and Phase-2a closing, sized to absorb propagation jitter (not a separate cycle).

  **Strict structural minimum: `Δ_2a = 1·BTT + ε_proc`.** Below this floor the schedule is broken-by-construction: at `Δ_2a < 1·BTT + ε_proc`, the late-broadcast time `T_verdict_max − ε_proc < T_verdict_start` — the operator's verdict broadcast is scheduled *before Phase-2a begins*. At exactly `Δ_2a = 1·BTT + ε_proc`, the late-broadcast time coincides with `T_verdict_start` (zero within-Δ_2a slack) and the verdict propagates over the full window. Production deployments MUST size `Δ_2a ≥ 1·BTT + ε_proc`.

  **Recommended: `Δ_2a = 1·BTT + ε_proc`** (the strict minimum). Same philosophy as OBFT's tightened `Δ_2 = 1·BTT` (see [OBFT.md §Phase 2](OBFT.md)): the post-`T_verdict_start` budget covers exactly one propagation cycle for the synchronous-fallback emit. **Reflood absorption is structurally provided by `B_0 = 2·BTT + RefloodDelay` pre-Phase-2a** under the reflood-aware schedule, so `Δ_2a` no longer carries a separate reflood / late-bundle-absorption cushion. Early emit via `T_L_0_observed` (typical case under healthy mesh) gives much more than `1·BTT` of verdict-propagation slack; the synchronous-fallback emit at `T_verdict_max − ε_proc` is the worst case and just fits.
...
**Late-bundle path within Phase-2a.** Under the recommended `Δ_2a = 1·BTT + ε_proc`, the late-bundle window `(T_verdict_start, T_accept_max]` collapses to `ε_proc` width — late Phase-1 bundle absorption is structurally shifted to `B_0`'s pre-Phase-2a `RefloodDelay`-baked budget under the reflood-aware schedule, not the Phase-2a slack. A receiver who first-observes a Phase-1 bundle in the `ε_proc` window may still issue a `σV` verdict if their broadcast lands before `T_verdict_max`, but the window is too tight for this to be a primary recovery path. Bundles arriving past `T_verdict_max` become auth-only retained.
...
- **Partial synchrony for liveness**: messages eventually deliver within bounded delay `P99` (propagation P99/P999) and clock skew `δ`. Three distinct cutoffs operationalize this bound: `T_broadcast_max_k = max(BFT_start, T_verdict_start − B_k)` (per-layer leader broadcast target; see §Setting), `T_accept_max = T_commit − 1 BTT` (receiver acceptance horizon, cluster-wide), `T_verdict_max = T_commit − 1 BTT` (verdict broadcast horizon, coincident with `T_accept_max`). Reconstruction is expected to complete at `T_commit + Δ_2b + ε_3`; the slot's hard wall is the relay-submission deadline `T_relay_cutoff − T_submit`.
  ...
  **2abOBFT's per-layer effective absorption window** = `T_accept_max − T_broadcast_max_k = Δ_2a + B_k − 1·BTT` — decomposes as `Δ_2a − 1·BTT` uniform Phase-2a residual (the slice of the Phase-2a window before `T_accept_max = T_commit − 1·BTT`; at the recommended `Δ_2a = 1·BTT + ε_proc` this collapses to `ε_proc`, sub-floor sizings are broken-by-construction with the late-broadcast verdict schedule — see [§Phase 2a](#phase-2a)) plus the per-layer pre-Phase-2a margin `B_k`. Late-bundle absorption is structurally provided by `B_0`'s `RefloodDelay`-baked schedule pre-Phase-2a, not by Phase-2a slack. At the K=4 reflood-aware schedule (up-tier illustration; K=2 default collapses to `B = [B_0, T_verdict_start]`) with default RefloodDelay=700ms: L_0 = `B_0 + ε_proc = 1150ms` total; at RefloodDelay=0: L_0 = `400 + 50 = 450ms` total. L_{K-1} = `T_verdict_start + ε_proc ≈ T_accept_max` total (deepest leader broadcasts at BFT_start; the cluster gets the entire pre-`T_accept_max` window for that bundle's propagation).
...
Running example: `f = 1, n = 4, K = 2` (default; see §Application). Honest A, B, C; byzantine D. Worked cases below describe behavior at L_0 → L_1 fall-through; K=4 up-tier extends the same convergence-rule logic to additional fall-through layers.
...
The recommended `Δ_2a = 1·BTT + ε_proc` covers the synchronous-fallback verdict-propagation cycle exactly (zero within-Δ_2a slack); typical mesh-jitter absorption shifts to `B_k`'s reflood-aware pre-Phase-2a schedule (`B_0 = 2·BTT + RefloodDelay` carries the reflood cycle for L_0's bundle). For wider mesh outliers, deployment-level mesh-diversity remains relevant.
...
- **[Class A]** **Sustained partition (real propagation > absorption window)** — violates assumption 2 (partial synchrony) under 2abOBFT's framing (per-layer absorption = `Δ_2a + B_k − 1·BTT`; L_0 = B_0 + ε_proc = 1150ms at Config A default; deeper layers wider). Slot misses cleanly. No safety violation.
...
| Mesh-flakiness tolerance | Good — `B_0 = 2·BTT + RefloodDelay` (reflood-aware schedule) absorbs the gossipsub IHAVE/IWANT cycle pre-Phase-2a; at recommended tightened `Δ_2a = 1·BTT + ε_proc` the Phase-2a window itself just covers one verdict-propagation cycle (no extra jitter cushion). Wider outliers fall back through NR-quorum to L_1. |
| Operators reach the same decision | Not necessarily — only the *output* is unique cluster-wide. Same as OBFT-family. |
| Built-in leader fallback | Yes (K-layer fall-through within Phase 3's reconstruction walk; K configurable, **`K = f+1` (BFT-min) recommended default** at SSV's n=4 cluster — see [§Application](#application). K up to `n` available as up-tier for deeper fall-through). |
...
| Partial-synchrony absorption window | Per-layer `Δ_2a + B_k − 1·BTT` (single round) — L_0 = B_0 + ε_proc = 1150ms at Config A default RefloodDelay; deeper layers wider (L_{K-1} ≈ T_accept_max). |
...
| Slot budget cost vs single-Phase-2 ([OBFT](OBFT.md)) | +250ms (= Δ_2a = 1·BTT + ε_proc) at recommended tightened sizing, end-to-end V_0-broadcast to consensus-complete: extra Phase 2a window 250ms. Schedule shapes diverge: OBFT uses primary-vs-backup (`B_0 = 2·BTT + RefloodDelay`; `B_1..B_{K-1} = T_commit` — see [OBFT.md §Setting](OBFT.md)); 2abOBFT retains the K-layer staggered (`B_k_shallow = (k+2)·BTT + RefloodDelay`) since the staggered `B_k` already absorbs per-layer mesh tails. At the same RefloodDelay setting, OBFT's primary `B_0` matches 2abOBFT's `B_0`; OBFT's backups have wider per-layer slack than 2abOBFT's staggered backups. ε_3 identical to OBFT's. Post-`T_commit` budget ~50ms larger in 2abOBFT (Δ_2b = 1·BTT + ε_proc vs OBFT's Δ_2 = 1·BTT) due to the Phase-2b convergence-computation overhead. |
...
For an SSV cluster proposing an Ethereum block, the SSV adapter's default 2abOBFT configuration is **`K = f+1` (BFT-liveness minimum at the cluster's `f`-bound)** — at n=4 (f=1) this resolves to `K=2`, one fall-through layer (`L_0 → L_1`). Motivated by production testing at n=4 showing that K > f+1 doesn't materially improve outcomes once 2abOBFT's verdict-pool mechanism closes Class B grief patterns at L_0 in-protocol (and bare OBFT's peer-reflood-V mechanism does the same for the `h_V=1` sub-case — see [OBFT.md §Phase 2 / Peer-reflood V via early commit](OBFT.md)). The K trade-off: smaller `K` means fewer fall-through layers (K=2 has only L_0 → L_1, so the late-deepest-layer-leader-broadcast Class A failure mode at L_1 has no further backstop); larger `K` adds Phase-3 IBE-walk cost (`~ε_3 × K` at K-1 silent layers). The adapter also supports `K = f+2` (one extra fall-through layer, modest onion-bandwidth increase) and `K = n` (= K=4 at n=4: every cluster member is a leader, maximum K-layer fall-through depth; pigeonhole guarantees ≥ 3 honest leaders at f=1). The K choice is per-cluster and deployment-dependent — see §Setting for the K-bounds discussion.
...
| `T_verdict_start` | Phase-1 / Phase-2a boundary — anchor: ≈ `slot_start + 1.75s` for the configurations below (= `T_commit − Δ_2a` at recommended `Δ_2a = 1·BTT + ε_proc = 250ms`). Phase 2a verdict-broadcast window begins here. |
...
| `T_broadcast_max_k` | per-layer leader broadcast target — `max(BFT_start, T_verdict_start − B_k)`; deeper layers broadcast earlier (wider `B_k`). Default K=2 (reflood-aware): `B_0 = 2·BTT + RefloodDelay`, `B_1 = T_verdict_start` (clamped at BFT_start). At Config A with default RefloodDelay=700ms: B = [1100, T_verdict_start]ms. At RefloodDelay=0 (opt-out): [400, T_verdict_start]ms. Target, not hard runtime cap — see §Setting. (Up-tier K=4: `B_k_shallow = (k+2)·BTT + RefloodDelay`, `B_3 = T_verdict_start`, B = [1100, 1300, 1500, T_verdict_start]ms.) |
...
#### 2abOBFT(n=4, K=2) recommended sizing

| Window | Length | End time | Notes |
|---|---|---|---|
| Phase 1 fetch (per-layer effective) | varies | per-layer | Per-layer staggered broadcast targets `T_broadcast_max_k = max(BFT_start, T_verdict_start − B_k)`. Default K=2 schedule at RefloodDelay=700ms: L_1 = 0 (B_1 = T_verdict_start, clamped at BFT_start), L_0 = 0.65s. At RefloodDelay=0 (opt-out): L_1 = 0, L_0 = 1.35s. |
| Phase-1 propagation slack | 200ms | slot_start + 1.75s = T_verdict_start | L_0's bundle (broadcast latest at 0.65s under default RefloodDelay, 1.35s at RefloodDelay=0) propagates to all honest within `1 BTT` plus the RefloodDelay-sized reflood headroom |
| Phase 2a | 250ms | slot_start + 2.00s = T_commit | `Δ_2a = 1·BTT + ε_proc` (strict structural minimum, recommended); verdict broadcast horizon = `T_commit − 1 BTT = 1.80s` (worst-case fallback). Under L_0-trigger early-emit (see [§Phase 2a / Verdict broadcast timing](#phase-2a)), healthy-mesh `T_emit_verdict ≈ T_verdict_start + ε_proc` — verdicts on the wire much earlier than the fallback. Late-bundle absorption shifts to `B_0`'s pre-Phase-2a `RefloodDelay`-baked schedule (Phase-2a residual window = ε_proc). |
...
Sub-floor sizings (`Δ_2a < 1·BTT + ε_proc`) are broken-by-construction with 2abOBFT's late-broadcast verdict schedule — see [§Phase 2a](#phase-2a). Production deployments MUST use `Δ_2a = 1·BTT + ε_proc` (the strict structural minimum, recommended) and SHOULD use `Δ_2b = 1·BTT + ε_proc` (the post-tighten recommended sizing).
...
2. **Deadline coordination.** Clock skew across operators must be bounded by `δ` and known. Three distinct deadlines (do not conflate):
   ...
   Phase-window minimums: **`Δ_2a ≥ 1·BTT + ε_proc`** (sub-floor sizing is broken-by-construction with the late-broadcast verdict schedule — see [§Phase 2a / Verdict propagation budget](#phase-2a)), `Δ_2b ≥ 1 BTT + ε_proc` (sized for Phase-2b emission propagation under the early-emission discipline at [§Phase 2b](#phase-2b)), `ε_3 ≈ 50ms`. Recommended: `Δ_2a = 1·BTT + ε_proc` (the strict structural minimum; reflood lives in B_k under the reflood-aware schedule) and `Δ_2b = 1·BTT + ε_proc` (the post-tighten minimum coherent value).

3. **Choosing K (layer count).** K is per-duty within `f+1 ≤ K ≤ n` — deployment-dependent (see §Setting). **The SSV adapter's default for proposer duty is `K = f+1` (= K=2 at n=4)** — the BFT-liveness minimum, motivated by production testing showing K > f+1 doesn't materially improve outcomes once structural Class B grief patterns are closed in-protocol. Trade-off: at K=f+1 there's only one fall-through layer (`L_0 → L_1` at K=2), so the late-deepest-layer-leader-broadcast Class A failure mode at L_{K-1} has no further backstop. `K = f+2..n` provides additional fall-through layers within Phase 3's single reconstruction walk for deployments preferring deeper recovery at the cost of larger onion bandwidth and longer Phase-3 IBE walks.
...
8. **Verdict envelope size.** ~100-200 bytes per verdict per layer per operator. Per slot: K × n verdicts × 200 bytes ≈ 1.6 KB at K=2 n=4 (recommended default), 3.2 KB at K=n=4 (up-tier). Within budget.
...
| Bandwidth (healthy, n=4, K=2 default) | ~20 KB across 1 emission (~5 KB/op × 4 ops at K=2 — chained-encryption onion shrinks to ~1 KB at K=2 vs ~3 KB at K=4; includes σ_L^V witness section ≈ +1.2 KB at K=2 n=4) | ~22 KB across 2 emissions (~5 KB Phase-2b onion + ~1.6 KB verdicts; no σ_L^V witness — 2abOBFT has no Phase-1 σ_L^V) |
...
- **OBFTR(R≥2) covers wider partition tails** within `(P99, R · P99]` than 2abOBFT does — multi-round retry extends the absorption envelope, while 2abOBFT is bounded by its per-layer absorption window `Δ_2a + B_k − 1·BTT` (single round; L_0 = B_0 + ε_proc = 1150ms at Config A default).
...
| Bandwidth (n=4, K=2 healthy default) | ~20 KB | ~22 KB | ~14 KB |
