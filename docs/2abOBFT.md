...
- **Transient mesh-flakiness** (honest operators NR-lock before their mesh delivers V) — IHAVE/IWANT recovery within the SafetyBuffer fills the flaky operators' σ-pool before they would NR-commit. **Success (fastest).**
...
| Transient mesh-flakiness (honest ops NR-lock before V) | ✗ (≥2 ops hard-NR-lock at `T_commit` before V arrives → σ-pool stays < qV) | ✓ at L_0 (no-lock — the flaky ops upgrade once their mesh delivers V) | ✓ round 2 |
