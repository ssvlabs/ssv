...
**Equivocation-recovery equivalence within the proposer-duty time budget.** Both protocols share the same fundamental limit on leader-equivocation-with-single-receivers under SSV's 4-second relay cutoff. QBFT recovers via round-change in round 2 (~+2s, with no headroom for round 3); TBFT recovers via the K=2 fall-through structure (`L_1` backup leader). Neither resolves the byzantine-leader-at-cutoff equivocation deadlock fully under sub-partial-synchrony — both hit the same partial-synchrony envelope in which re-flooded equivocation evidence may not reach all honest before they commit (cf. baseline TBFT's "Single-V receiver interaction with leader equivocation" at "Fault tolerance / Equivocation handling"; QBFT's analog is round-1 leader equivocation when round 2 also fails). **TBFT and QBFT are equivalent on this class of liveness limit** — the trade is in *what shape* slot-time gets allocated to recovery (one round-change in QBFT, one fall-through layer in TBFT), not in the limit itself. The bid-routing extension in Appendix B.3 precludes one specific instance — bid-equivocation on the same `V` — by anchoring the bid to a non-equivocable external source (the relay's `SignedBuilderBid`); this protection is unique to the bidder model and has no QBFT analog because QBFT does not carry a bid concept.

**Recovery-scope equivalence within the proposer-duty time budget.** Both protocols get effectively two fresh-V attempts in 4s — TBFT via L_0/L_1 fall-through, QBFT via round 1 / round 2. TBFT's K=2 doesn't buy more recovery slots than QBFT's 2-round budget; what TBFT trades for the K=2 layered structure is **flat failure-mode latency** (fall-through is essentially free latency-wise, vs QBFT's ~2s round-change overhead) at the cost of pre-fetching both V's in Phase 1. The differentiation is concentrated in the latency profile, not in recovery scope.

**Where QBFT wins for proposer duty.** Aspects where QBFT genuinely does better than TBFT, beyond the structural trade-offs above:

- **Application-validity-divergence recovery under strict host-policy.** When honest receivers' validity verdicts diverge mid-slot (head-change between fetch and acceptance), TBFT can deadlock at the divergence layer per "Fault tolerance / Application-validity-divergence — known liveness limit". QBFT round-changes through this — round 2's leader fetches a fresh V at the new head, all honest agree on validity, cluster converges. TBFT's mitigation is host loose-validity (lock the verdict at acceptance), which trades divergence-deadlock for relay-rejection on orphaned-parent V — a *trade*, not a free win. Under strict host-validity, QBFT's recovery here is genuinely better-suited to head-changes-mid-slot semantics.

- **Simpler cryptographic primitive set.** QBFT just needs BLS threshold signatures. TBFT additionally needs threshold IBE / signature-based witness encryption (drand/tlock-style), and B.3 specifically also needs chained IBE encryption. The IBE primitive is more novel and less universally audited than BLS — for a cluster operator deploying SSV, "we use threshold BLS" is a much shorter security argument than "we use threshold IBE plus chained encryption proven safe via Pigeonhole 3". This translates to fewer external trust dependencies for the consensus core (no separate IBE-keypair DKG ceremony, no IBE-primitive-specific assumptions in the safety argument).

- **Cleaner extension to larger clusters (n ≥ 7).** TBFT shape stops working at `f ≥ 2` — the `f+2 = qV` closure breaks. For SSV clusters of size 7, 10, 13, ..., TBFTR is needed instead — different protocol, different DKG ceremonies, different per-cluster config (TBFT and TBFTR share the cryptographic core but diverge on Phase-2 split + V-plaintext machinery). QBFT scales uniformly across cluster sizes with the same protocol shape and round-change machinery. If SSV ever wants a single consensus protocol across all cluster sizes, QBFT is the easier path; TBFT/TBFTR introduces a size-dependent protocol-selection problem.

- **Production maturity.** QBFT is what SSV runs in production today. Implementation is exercised, corner cases have been hit, bug fixes have accumulated. Switching to TBFT means re-deriving that confidence on a new codebase that hasn't seen production traffic. Even if the protocol is sound on paper, deployment risk is real and quantitatively non-trivial — this isn't a permanent disadvantage (TBFT can mature too), but for a "should we switch?" decision it's a meaningful one.
...
**Summary.** Parent-root match doesn't give a useful new routing rule at K=2 *as a top-level ordering criterion*, fragments under the very condition it would purport to address (head disagreement), and is best handled at the application layer by choosing a more re-org-resistant parent for the backup candidate. It's worth understanding as a *non-extension to the commit-tag family* — a direction explored and ruled out — rather than a path forward in B.1's shape.

**Forward reference.** B.3 ([L_Bid-prepended TBFT](#b3--l_bid-prepended-tbft-at-n4)) reuses parent-root in a different role: as a *filter* on the cluster-consistent bidder envelope set, not as a per-operator ordering rule. The filter framing avoids B.2's input-non-cluster-consistency problem because parent_roots are extracted from the signed bidder envelopes (byte-identical for every honest receiver under partial synchrony with re-flooding), not from each operator's local beacon-node head view. Parent-root-as-filter on the bid layer is a productive extension to B.3; parent-root-as-ordering at the layer level is not productive in B.1's shape.
...
**Status: design sketch, not part of the baseline TBFT spec.** Captured because it **preserves baseline TBFT's safety and liveness profile** under three preconditions on the deployment: (a) relay-anchored bid binding (see "Bid binding via relay attestation" below), (b) host loose-validity policy across layers (see "Application: SSV Ethereum proposer duty / Head-change handling"), and (c) tightened candidate-acceptance cutoff `T_candidate_accept = T_commit − (2D + δ)` rather than baseline's `T_commit − (D + δ)`. Given those three preconditions, every slot baseline TBFT completes is also completed here (at the same layer or earlier with higher value capture). Cost is K=3 chained encryption (per Appendix C) + n bid envelopes in Phase 1 + the tightened cutoff (~`D` shaved off the high-MEV fetch window); no extra rounds.

#### Design principle

L_Bid is an *optional opportunistic layer* — it kicks in to capture potentially higher MEV when cluster conditions allow, and otherwise falls through cleanly to baseline TBFT's K=2 rotation-determined structure. **Under no circumstances does the addition of L_Bid compromise the protocol's safety or liveness relative to baseline TBFT.** This is the constraint that shapes every design choice in B.3:

- The σ-eligibility predicate is conditioned on cluster-state-derived inputs (saw-all bids with PR-majority filter, OR saw-`n−1` bids with PR-unanimity), so σ-side honest converge on the same `L_Bid` identity under partial synchrony.
- Chained encryption per Appendix C provides cross-layer safety (Pigeonhole 3): at most one of `V_{L_Bid}`, `V_{L_0_baseline}`, `V_{L_1_baseline}` reconstructs cluster-wide.
- The relay-attestation envelope-acceptance rule (envelopes accepted unconditionally on basic checks; relay-attestation outcome determines *effective bid* via cluster-recognition counting against operators' `trusted_builder_pubkeys` lists) keeps `received_bidders` cluster-consistent under any trusted-builder configuration. Bids from cluster-recognized builders contribute their claimed value to L_Bid argmax; bids from non-recognized builders are demoted to 0. Cluster-recognition is auto-derived from the envelope set itself — no out-of-band cluster trusted-set configuration is required.
- Under sub-partial-synchrony, the layer-assignment predicate fails uniformly → fall through to baseline. Predicate-failure-driven NR-quorum is the same mechanism baseline TBFT itself uses to walk past a failed L_0; B.3 inherits it for L_Bid.
- Uniform host validity policy across all three layers preserves baseline's per-layer behavior; B.3 inherits baseline's known limit at every layer with no novel divergence axis.

Any design choice that would violate this principle in any failure mode — including misconfigurations like trusted-set drift — does not belong in B.3. This principle is the standard against which the open questions below should be evaluated, and the standard against which any future B.3 refinement should be audited.
...
**σ-eligibility at `L_Bid`** is conditional on a cluster-state predicate over received bidder envelopes plus local validation. An operator signs σ on the L_Bid winner if they have enough envelopes to deterministically settle the cluster's *majority parent_root*, and they locally validate the resulting bid-winner. Two cases satisfy the predicate:

- **Saw all `n` bids:** parent-root majority is computed over the full `n` envelopes (with deterministic tiebreak — see "Layer assignment" below). Bidders are filtered to the majority side, then argmax by `(bid desc, op_id asc)` selects `L_Bid`.
- **Saw `n − 1` bids with parent-root unanimity:** the `n − 1` received bidders all share the same `parent_root`. The missing envelope can contribute at most one vote to any parent_root, so the unanimous parent_root has cluster-wide count ≥ `n − 1` — settling majority regardless of the missing bid. Argmax over the `n − 1` received bidders selects `L_Bid`.

Either case yields a cluster-consistent `L_Bid` identity for σ-side honest. If neither holds, or the bid-winner doesn't locally validate: NR_LBid. Layers 1 and 2 follow baseline TBFT's standard σ-or-NR rules unchanged, applied to their respective rotation-determined leaders.

The σ-side population at `L_Bid` is cluster-consistent by construction — every σ-side honest computed `L_Bid` from the same envelope subset (either all `n` or the same `n − 1`). Pigeonhole 1 + 2 hold at `L_Bid` by the same arguments as baseline. Pigeonhole 3 (from K=3 chained encryption per Appendix C) ensures cross-layer safety.

The parent-root data feeding the predicate is intrinsic to each candidate `V`: `V.parent_root` is a field of the proposed block (for SSV proposer duty, the beacon-block header). Since `value_root = hash(V)` and the envelope's `σ^op` signs over `value_root`, parent_root is transitively bound by the bidder's identity-key signature — no separate envelope field is required.
...
- `value_root` (hash of the proposed value `V_{op_id}`; for SSV proposer duty, bound to `header.block_hash` from the relay's `SignedBuilderBid` — see "Bid binding via relay attestation" below)
- `bid` (application-supplied numeric value; for SSV proposer duty, bound to `value` from the relay's `SignedBuilderBid`)
- `relay_attestation` (cryptographic anchor binding `(value_root, bid)` to a non-equivocable external source — for SSV's proposer duty, the relay's `SignedBuilderBid` from MEV-Boost; see "Bid binding via relay attestation" below)
- `trusted_builder_pubkeys` (list of builder pubkeys the operator considers trusted, sourced from their personal MEV-Boost relay configuration; used by receivers to compute *cluster-recognition* of each envelope's builder — see "Bid binding via relay attestation" below)
...
The lower-`op_id`-wins tiebreaker gives a static structural advantage to whichever cluster member has the lowest `op_id` *only if* a byzantine can synthesize a bid value equal to `max_observed_honest_bid` to win the tie deliberately. With relay-anchoring (see "Bid binding via relay attestation" below), byz cannot synthesize bid values — the bid is fixed by whatever the relay's `SignedBuilderBid` sealed at byz's query time. Tie scenarios in practice are limited to (a) cluster-wide relay outage where all bidders fall back to `bid = 0` (vanilla blocks; tiebreak to lowest-op_id is operationally equivalent to a rotation-determined leader for the slot), or (b) coincidental relay-signed value matches (extremely rare given MEV-bid resolution). For applications without relay anchoring, byz can grind to win ties — clusters running B.3 without an external bid anchor should consider a stronger tiebreaker (e.g., a VRF over `(slot, op_id)` rather than static `op_id`) to neutralize this surface, at the cost of additional spec.

**Phase 1 (modified):** each of the `n = 4` operators broadcasts a Phase-1 bundle:

- The candidate `V_{op_id}` (their proposed value).
- The signed envelope binding `(protocol_tag, message_kind, cluster_id, slot, op_id, value_root, bid, relay_attestation, trusted_builder_pubkeys)` (full field list per "Setting" above).
- A σ_V partial signature `σ_{op_id}^V(V_{op_id})` on their own value (locks the bidder to a single `V` for this slot).

**Per-bidder fetch timing.** Each bidder's Phase-1 fetch timing is determined by their rotation role for the slot:

- **L_0_baseline (rotation primary):** fetches at `T_0` (late, MEV-optimized) — same as baseline TBFT in shape, but `T_0` is shifted earlier by `D` due to the tightened cutoff (see below).
- **L_1_baseline (rotation backup):** fetches at `T_1 < T_0` (early, re-org-resistant against a deeper-confirmed parent) — same as baseline TBFT.
- **Other bidders (operators not in `{L_0_baseline, L_1_baseline}` for this slot):** fetch at `T_0` (late, MEV-optimized), contributing late-fetched candidates for L_Bid.

This preserves baseline TBFT's `T_1 < T_0` asymmetry for the rotation layers: `L_1_baseline`'s safe-early-fetched property is unchanged, and head-change resilience for the backup is preserved exactly as in baseline (per "Application: SSV Ethereum proposer duty / Head-change handling"). Non-rotation bidders join the `T_0` cohort to maximize bid-routing's value-capture upside; their candidates are late-fetched MEV-optimized blocks. In healthy operation, L_Bid's winner is typically among the `T_0`-fetched candidates ({L_0_baseline} ∪ {non-rotation bidders}), since L_1_baseline's early-fetched bid is structurally lower-MEV. If `L_1_baseline` does win L_Bid (rare — would require anomalously low MEV on `T_0` fetches), the cluster captures that bid; layer-2 fall-through to L_1_baseline remains available regardless of L_Bid outcome.

**Tightened candidate-acceptance cutoff for B.3.** Whereas baseline TBFT uses `T_candidate_accept = T_commit − (D + δ)`, B.3 requires the tightened version `T_candidate_accept = T_commit − (2D + δ)`. Reason: the additional `D` of headroom guarantees that gossipsub re-flood from any honest receiver completes before every other honest's local cutoff, even in the worst-case clock-skew scenario — closing the byzantine-leader-at-cutoff edge that baseline accepts. Without this, B.3 has a deadlock surface at L_Bid that baseline doesn't have at L_0/L_1 (see "σ-side deviation surface at L_Bid" in "Liveness & attribution" below for the analysis). Cost: shrinks the high-MEV fetch window by `D` (~200 ms typical); acceptable trade for closing the H4 deadlock surface for SSV proposer duty under adversarial byzantine assumptions.
...
# Cluster-recognition: derive cluster trust from the envelope set itself.
# An envelope's builder is cluster-recognized iff ≥ qV operators (across received
# envelopes) include that builder in their trusted_builder_pubkeys list. Bids from
# non-recognized builders are demoted to 0 for L_Bid argmax purposes. See "Bid
# binding via relay attestation" below for the rationale.
for each envelope b in received_bidders:
    builder_b = b.relay_attestation.builder_pubkey      # nil if relay_attestation == nil
    if builder_b == nil:
        b.cluster_recognized = false                    # vanilla envelope; never recognized
    else:
        count_b = |{ envelope f in received_bidders : builder_b ∈ f.trusted_builder_pubkeys }|
        b.cluster_recognized = (count_b ≥ qV = 3)
    b.effective_bid = b.bid if b.cluster_recognized else 0

# L_Bid layer (bid-determined with parent-root awareness, over effective bids):
saw_all_bids   = (|received_bidders| == n)              # n = 4 for this spec
saw_minus_one  = (|received_bidders| == n - 1)

if saw_all_bids:
    # Predicate A: parent-root majority filter over the full n envelopes.
    pr_counts   = histogram(b.V.parent_root for b in received_bidders)
    majority_pr = mode(pr_counts)                       # tiebreak: among tied PRs, pick the
                                                        # one whose lowest-op_id bidder has the
                                                        # globally lowest op_id (deterministic).
    eligible    = { b ∈ received_bidders : b.V.parent_root == majority_pr }
    L_Bid       = argmax_{(effective_bid desc, op_id asc)}(eligible)

elif saw_minus_one and ∃ p such that ∀ b ∈ received_bidders: b.V.parent_root == p:
    # Predicate B: unanimity over the n - 1 received bidders. The single missing
    # envelope can contribute at most 1 vote to any parent_root, so the unanimous
    # parent_root p has cluster-wide count ≥ n - 1 = 3, exceeding ⌈n/2⌉ = 2.
    # Majority is settled at p regardless of the missing envelope.
    L_Bid       = argmax_{(effective_bid desc, op_id asc)}(received_bidders)

else:
    L_Bid       = undefined                             # operator will NR_LBid

V_{L_Bid} = candidate from L_Bid's envelope             # if L_Bid is defined
...
Under partial synchrony with gossipsub re-flooding, every honest operator computes the same `L_Bid` value — either (a) the same parent-root-majority-filtered argmax when all `n` envelopes were received (predicate A), (b) the same argmax over the `n − 1` received bidders when byz withheld and honest are on a stable head with unanimous parent_root (predicate B), or (c) `undefined` when neither predicate holds. The re-flood property collapses byzantine selective-broadcast to a binary "broadcast or don't" choice, so `|received_bidders|` and the parent_root distribution over received bidders resolve to the same values for every honest receiver. `L_0_baseline` / `L_1_baseline` identities are rotation-determined and cluster-consistent unconditionally.

**Phase 2 (modified):** each operator i constructs a 3-layer onion with chained encryption (per Appendix C):

```
layer 0 (L_Bid):
  if L_Bid is defined (per layer-assignment predicate A or B above)
     AND V_{L_Bid} validated by i:
...
**σ-or-NR exclusivity per (slot, layer)** applies at every layer. At `L_Bid`, it's conditional on the combined predicate `(predicate A or predicate B) AND V_{L_Bid} validated`; at `L_0_baseline` and `L_1_baseline`, it's the standard baseline TBFT rule applied to the rotation-determined leader's V.
...
```
# Layer 0 — L_Bid:
sigs_LBid = { σ_{L_Bid}^V(V_{L_Bid}) from L_Bid's Phase-1 envelope, if L_Bid is defined and valid }
...
sigs_L1 = { σ_{L_1_baseline}^V(V_{L_1_baseline}) from Phase-1 envelope, if valid }
        ∪ { σ_j^V(V_{L_1_baseline}) from layer-2 ciphertexts via sequential IBE decryption:
            outer-decrypt with key_L0 to recover E_{nr_tag_LBid}(σ_j^V(V_{L_1_baseline})),
            then inner-decrypt with key_LBid to recover σ_j^V(V_{L_1_baseline}) }
        # deduplicated per operator
...
**Slashing-protection scope (delta from baseline).** Each operator's V-signing share may sign σ_V on up to 4 distinct values per slot in B.3: their own `V_{op_id}` in the Phase-1 bid envelope (which locks the bidder to a single V), plus `V_{L_Bid}`, `V_{L_0_baseline}`, and `V_{L_1_baseline}` in the Phase-2 onion (with dedup collapsing to fewer signings when op_id matches any of those layer roles). EKM permits multi-V signings under existing rules; the slashing-protection log keys on `(slot, value_root)` and accepts multiple distinct value_roots per slot for the same operator. Per-`(slot, value_root)` signing-once enforcement is unchanged from baseline (the equivocation-on-same-V rule).

**Cross-phase exclusivity is per `(slot, layer)`, anchored to that layer's target V.** For each layer k ∈ {`L_Bid`, `L_0_baseline`, `L_1_baseline`}, an operator commits exclusively to either σ-side (a σ_V partial on the layer's target V — `V_{L_Bid}`, `V_{L_0_baseline}`, or `V_{L_1_baseline}` respectively) or NR-side (an `σ^IBE` partial on the layer's `nr_tag` for layers `L_Bid` and `L_0_baseline`; layer `L_1_baseline` has no NR side). EKM enforces this commitment per `(slot, layer)`: a NR partial on the layer's tag is rejected if the operator has signed σ_V on that layer's target V at the same slot, and vice versa.

The asymmetry between baseline and B.3 in *what counts as σ-side commitment* derives from the bidder model: every operator broadcasts a Phase-1 bid envelope containing `σ_V(V_{op_id})`. That Phase-1 σ_V is bid-level signing — it locks the bidder to a single V for this slot but is **not by itself** a layer-level σ-side commitment. It promotes to a layer-level commitment **iff** `V_{op_id}` matches the layer's target V (i.e., `op_id == L_Bid`, or `op_id == L_0_baseline`, or `op_id == L_1_baseline` — at most one match per slot per operator under the rotation). For an operator whose own V matches `V_{L_Bid}` (the L_Bid leader), their Phase-1 σ_V is their σ-side commitment for L_Bid; they cannot subsequently emit NR_LBid (per `σ_V partial dedup rule` and cross-phase exclusivity together). For all other bidders at L_Bid, Phase-1 σ_V on `V_{op_id}` and Phase-2 onion σ_V on `V_{L_Bid}` are distinct partials (different V's), so the σ-side commitment for L_Bid lives entirely in Phase-2 — Phase-1's bid signing is independent. The same logic applies symmetrically at `L_0_baseline` and `L_1_baseline`: only the rotation-determined leader's Phase-1 σ_V contributes to the layer's σ-side commitment via Phase-1; non-leader bidders contribute (or don't) entirely through Phase-2 onion.

#### Bid binding via relay attestation

The bid carried in each Phase-1 envelope is cryptographically anchored to a non-equivocable external source whose attestation is verifiable cluster-wide. For SSV's proposer duty, this is the relay's `SignedBuilderBid` returned by the MEV-Boost `getHeader(slot, parent_hash, validator_pubkey)` query: a `BuilderBid = (header, value, pubkey)` tuple plus a BLS signature from the builder over that bid. The envelope's `relay_attestation` field carries this signature plus enough of the `SignedBuilderBid` payload to verify it. The envelope's `value_root` binds to `header.block_hash`; the envelope's `bid` is the claimed `value`.

Each Phase-1 envelope additionally includes a `trusted_builder_pubkeys` field — the operator's list of builder pubkeys they consider trusted, sourced from their personal MEV-Boost relay configuration. This list is the operator's input to **cluster-recognition**: a per-envelope determination of whether the envelope's bid is recognized for L_Bid argmax.

**Cluster-recognition rule.** An envelope's builder is *cluster-recognized* iff at least `qV = 3` operators (across the received envelopes) include that builder in their `trusted_builder_pubkeys` list. The envelope's *effective bid* for L_Bid argmax is its claimed `bid` value if cluster-recognized; otherwise demoted to 0. Cluster-recognition is computed deterministically from the envelope set itself — no out-of-band cluster trusted-set configuration is required, and cluster trust is auto-derived from the union of operators' personal MEV-Boost configurations with the qV threshold acting as a Byzantine-tolerant majority filter (under f=1, a builder needs ≥ 2 honest operators trusting it to potentially reach the threshold; with byz cooperation it reaches qV = 3, and with 3 honest agreement it reaches qV = 3 unconditionally regardless of byz behavior).

**Decoupling envelope membership from bid valuation.** Envelopes pass *basic acceptance* (envelope-level signature on `(slot, op_id, value_root, bid, relay_attestation, trusted_builder_pubkeys)`, `value_root` binding, parent_root extraction, etc.) and join `received_bidders` regardless of cluster-recognition outcome. **Cluster-recognition determines effective bid valuation, not envelope membership.** This means `received_bidders` stays cluster-consistent under any operator-side trusted-builder configuration; only the *bid valuations* depend on cluster-wide builder agreement. Trusted-builder drift across operators shows up as different effective-bid distributions but never as different envelope membership — so the layer-assignment predicate (saw-all or saw-`n−1`-with-PR-unanimity) operates on a cluster-consistent envelope set in all cases, and its outcome is cluster-consistent across honest receivers under partial synchrony with re-flooding. An envelope that fails *basic* acceptance (malformed envelope-level signature, missing required fields, etc.) is treated as not-received and excluded from `received_bidders`.

**Why this matters for liveness.** Without a non-equivocable bid anchor, a byzantine bidder could equivocate on bids for the same `V` — produce two distinct envelopes carrying the same `value_root` but different `bid` values, each with a valid `σ^op` signature. Single-envelope receivers would see only one of the two and compute argmax over different effective-bid sets, locking σ-side commitments to different `V_{L_Bid}` candidates and fragmenting the σ pool below `qV` (analog of baseline TBFT's "Single-V receiver interaction with leader equivocation" at "Fault tolerance / Equivocation handling", widened from `K=2` rotation leaders to `n` bidders). With relay anchoring, recognized bids are cryptographically bound to `(block_hash, value)` via the builder's signature; producing two valid envelopes with the same `value_root` and different recognized `bid` values would require the builder to sign two distinct values for the same `block_hash`, which the builder protocol does not do. **Same-V bid-equivocation is precluded at the source for cluster-recognized bids.** Different-V envelope equivocation (byz queries the relay multiple times during the slot, gets two `SignedBuilderBid`s with different `block_hash`es) reduces to standard leader-V-equivocation handled by the existing equivocation-→-NR rule. Bids whose builders fail cluster-recognition are uniformly demoted to 0 cluster-wide — there's no recognized bid value to equivocate over.

**Vanilla-block handling.** A vanilla envelope (locally-built payload, no relay queried) carries `relay_attestation = nil` and `bid = 0`. Per the cluster-recognition rule, vanilla envelopes have effective bid = 0 — cluster-recognition is skipped for nil-attestation envelopes (no builder to recognize). The operator's `trusted_builder_pubkeys` list still contributes to cluster-recognition counts for *other* operators' envelopes. If all bidders submit vanilla envelopes (cluster-wide relay outage), all effective bids are 0 and L_Bid resolves via `op_id` tiebreak — operationally rotation-equivalent for that slot, with the chosen V being a vanilla beacon-node-built block carrying no MEV.

**Cluster-internal bid is decoupled from realized MEV value.** The cluster's commit to `V_{L_Bid}` produces a signed blinded header that the validator submits to MEV-Boost. For envelopes carrying valid `relay_attestation`, the relay's `getPayload` flow reveals the full execution payload and pays out whatever real `value` the relay sealed in the original `SignedBuilderBid`. **The cluster's "effective bid" used for L_Bid argmax is an ordering signal distinct from the realized MEV value paid out at proposal time.** Two consequences:

- **L_Bid completion does not require cluster recognition of the realized bid.** A slot where a relay-attested envelope's builder happened to fail cluster-recognition (effective bid demoted to 0) but the envelope nonetheless wins L_Bid via tiebreak still pays out the real bid at proposal time — the relay-attestation is still cryptographically valid; only its cluster-internal recognition was missing. The cluster captures whatever value the chosen block actually carries.
- **Cluster-recognition is an ordering optimization, not a value gate.** Demoting an envelope's effective bid to 0 affects which V the cluster commits to (since argmax operates on effective bids), but does not affect the proposer-flow's ability to realize the chosen V's MEV. So a cluster with limited builder agreement degrades gracefully: the L_Bid argmax may land on a different (recognized) bidder than the genuinely-highest-bid one, but the chosen bidder's real MEV is still captured.

The cluster-internal bid is *purely an ordering signal* for picking which block to commit to during consensus; value capture is realized downstream at proposal time, independent of consensus-internal bid recognition. This decoupling is what makes the L_Bid layer genuinely opportunistic in the design-principle sense: cluster-recognition failures (operators with no trusted-builder overlap, partial drift, byz manipulation of `trusted_builder_pubkeys` lists) only affect *which* block the cluster commits to; they don't reduce the realizable MEV of whatever block ends up chosen.

**Cluster-recognition vs cluster-configured trust.** The cluster does not maintain a configured "trusted builder set" at the protocol level. Instead, each operator's `trusted_builder_pubkeys` field advertises which builders they personally trust (from their own MEV-Boost relay configuration). The cluster-recognition rule auto-derives "cluster trust" from the union of personal trust lists, gated by the qV threshold:

- **No out-of-band cluster configuration** for trusted builders is required — operators run their existing personal MEV-Boost configs unchanged.
- **Heterogeneous personal configs are tolerated**: operators can use different relay sets, as long as ≥ qV of them overlap on the relays whose bids they want to drive the L_Bid argmax.
- **Drift in personal configs is automatically absorbed**: if an operator changes their MEV-Boost relays, the cluster-recognition computation adapts on the next slot (using whichever `trusted_builder_pubkeys` list each operator includes in that slot's envelope). Atomic cluster-wide updates of a shared trusted set are not needed.

Byzantine manipulation of `trusted_builder_pubkeys` is bounded by the qV threshold: a single byzantine operator can lift a builder's count by 1 (cooperating with 2 honest who trust the builder, lifting count to 3 = recognized) or omit the builder (lowering count by 1, potentially preventing recognition if only 2 honest trust it). Neither breaks safety or liveness; both affect value capture (byz can shift L_Bid winner or force fall-through to baseline).

**Trust model.** The bid's integrity depends on the builder's BLS key being non-byzantine. SSV's MEV-Boost flow already trusts the builder's signature for `getPayload` (the actual block-construction step downstream of consensus), so relay-anchoring does not introduce a new external trust assumption beyond what the proposer-duty integration already accepts.
...
**Pigeonhole 2 — two σ-quorums on different V's at the same layer.** At `L_Bid`: only one V can be σ'd — the cluster's unique bid-winner per the layer-assignment predicate. The predicate (parent-root-majority filter over the saw-all input, or argmax over the saw-n-minus-one-with-PR-unanimity input) is a deterministic function of the cluster-consistent envelope set, and every σ-signer used the same envelope subset (either all `n` or the same `n − 1`) under partial synchrony with re-flooding. So σ-signers compute the same `L_Bid` and σ on the same `V`. The σ_LBid pool is on a single V cluster-wide. At `L_0_baseline` / `L_1_baseline`: standard baseline argument (single V per rotation-determined layer, locked by the leader's Phase-1 σ_V).

**Pigeonhole 3 — cross-layer safety via chained encryption.** Per Appendix C, `σ_{L_0_baseline}` partials are wrapped in IBE encryption under `nr_tag_LBid`; `σ_{L_1_baseline}` partials are wrapped in two nested IBE encryptions, the inner under `nr_tag_LBid` and the outer under `nr_tag_L0_baseline` (chain depth 2 — sequential nesting, decrypted outer-first per the Phase-3 walk). To reconstruct `V_{L_0_baseline}`, NR_LBid-quorum must reach (which by Pigeonhole 1 at `L_Bid` prevents σ_LBid-quorum from reaching). To reconstruct `V_{L_1_baseline}`, both NR_LBid- and NR_L0-quorums must reach (which prevents both prior σ-quorums). At most one σ-quorum reconstructs a V cluster-wide.

**Cluster-consistency under sub-partial-synchrony.** Under partial synchrony with gossipsub re-flooding, all honest operators see the same envelope set (re-flooding collapses byzantine selective-broadcast to a binary "broadcast or don't" choice — if any honest peer received an envelope, all honest peers receive it within the propagation bound). So `|received_bidders|` and the parent_root distribution over received bidders resolve identically across all honest receivers. The layer-assignment predicate (saw-all with PR-majority filter, OR saw-n-minus-one with PR-unanimity) is a deterministic function of the cluster-consistent envelope set — every honest σ-side computes the same `L_Bid` from the same input. Beyond partial synchrony, individual operators may evaluate the predicate differently (e.g., one sees `n − 1` while others see `n`, or visible bidders' parent_roots disagree on some honest while unanimous on others) — but the rule shifts those operators to NR-side rather than σ-side on a divergent `L_Bid`. The σ-side population remains cluster-consistent regardless: every honest who is σ-side at `L_Bid` used the same envelope subset to derive `L_Bid`, so they all argmaxed to the same V. **No L-identity-divergence regression** — the σ-eligibility predicate is conditioned on an envelope-set-derived majority, not on local argmax over an arbitrary received subset; this is what makes the cluster-consistency property hold.

This property is about *agreement on which V is L_Bid among σ-signers*, not about *σ-pool size sufficing for qV*. The latter can still fail under sub-partial-synchrony fragmentation: if honest split between σ-side (saw all bids) and NR-side (didn't), the σ-pool may be too small to reach qV while the NR-pool is simultaneously too small to reach qEnc — the deadlock cases analyzed at "σ-side deviation surface at L_Bid" in the Liveness section. The L-identity-divergence property guarantees that *whatever* V_X reaches σ-quorum cluster-wide is uniquely determined; it does not guarantee that any V reaches σ-quorum at all. Both properties matter; this paragraph addresses only the first.

#### Liveness & attribution

**Slot success conditions:** identical to baseline TBFT K=2's *plus* an opportunistic L_Bid layer on top.

- **Optimistic path A (saw-all with PR-majority filter)** — All 3 honest receive all `n = 4` bids by `T_candidate_accept`, the parent-root-majority filter yields a non-empty eligible set, and 3 honest validate `V_{L_Bid}` → σ_LBid quorum reaches → `V_{L_Bid}` wins (highest-bid majority-head block captured). PR-filter excludes minority-head bidders before argmax, avoiding wasted L_Bid attempts on V's that most honest can't validate.
- **Optimistic path B (saw `n − 1` with PR-unanimity)** — Byz withholds their bid; the 3 visible bidders are honest and unanimously share `parent_root` (stable-head case). 3 honest argmax over the `n − 1` and validate the winner → σ_LBid quorum reaches → `V_{L_Bid}` wins (highest *honest* bid this slot captured). The unanimity guarantees the missing envelope cannot overturn the majority parent_root, so σ-side stays cluster-consistent.
- **Fall through to layer 1** — Predicate fails (not saw-all and not saw-n-minus-one-with-unanimity), OR predicate holds but some honest didn't validate `V_{L_Bid}` → those honest NR_LBid → if NR_LBid-quorum reaches, layer 1 unlocks. Then standard baseline TBFT: 3 honest validate `V_{L_0_baseline}` → σ_L0 reaches → `V_{L_0_baseline}` wins.
...
**Baseline-equivalent liveness across all failure modes** under three preconditions: uniform host validity policy across layers (see "Application-validity-divergence inherits baseline's known limit at every layer" below), relay-anchored bid binding (see "Bid binding via relay attestation" above), and the tightened candidate-acceptance cutoff `T_candidate_accept = T_commit − (2D + δ)` (see "Tightened candidate-acceptance cutoff for B.3" above). Given those three, every slot baseline TBFT K=2 completes, L_Bid-prepended TBFT also completes — at the same layer or earlier (via L_Bid). The L_Bid layer adds opportunistic completion at higher value capture; failure to complete at L_Bid trivially falls through to the same K=2 baseline structure that would have run if L_Bid didn't exist. The σ-eligibility predicate (saw all bids AND validate bid-winner) is a strictly stricter precondition than baseline's σ-eligibility (validate the rotation-determined leader's V), so any operator who would σ at L_Bid would also σ at the corresponding rotation layer if needed — sub-partial-synchrony included.
...
**Application-validity-divergence inherits baseline's known limit at every layer.** When honest receivers' application verdicts on `V_{L_k}` diverge — some return `valid` (commit σ), others return `not-valid` (commit NV) — that layer can deadlock under adversarial byzantine, exactly as baseline's `L_0` does (per "Fault tolerance / Application-validity-divergence — known liveness limit"). In B.3 the divergence-deadlock surface is per-layer at `L_Bid`, `L_0_baseline`, and `L_1_baseline`; the per-layer numerics are identical to baseline's `L_0`. The mitigation is identical too: host adopts loose-validity (validate `parent_root` once at acceptance, lock the verdict, no post-acceptance re-validation), trading divergence-deadlock for relay/beacon-rejection on orphaned-parent V — same severity (slot miss), different failure mode. See "Application: SSV Ethereum proposer duty / Head-change handling" for the SSV-specific guidance; the same host-policy choice applies symmetrically across all three of B.3's layers.

**Bid lies as liveness faults** (analogous to B.1's cases, with cleaner fall-through and parent-root-aware filtering, conditional on cluster-recognition):

- Byzantine claims `bid = ∞` via a **cluster-recognized builder** (one trusted by ≥ qV operators), with an honest-looking but suboptimal block on the **majority head**. Their op becomes `L_Bid` candidate (effective bid = `∞`, recognized). If 3 honest validate `V_byz`, σ_LBid reaches and slot completes with `V_byz` (low-MEV). Cluster captures `actual_bid_byz` instead of the highest honest bid. **Loss = `max_honest_bid − actual_bid_byz`.** Attributable post-hoc.
- Byzantine claims `bid = ∞` via a **non-cluster-recognized builder** (one fewer than qV operators trust — e.g., a builder byz fabricated, or an unpopular relay). Per the cluster-recognition rule, byz's effective bid is demoted to 0. Argmax over effective bids ignores byz's claim entirely; L_Bid resolves to the highest-effective-bid bidder among recognized envelopes (typically an honest one). **Bid lie filtered at the cluster-recognition step**, no liveness regression.
- Byzantine claims `bid = ∞` for a block on a **minority head** (e.g., relay-late fetch landing on a re-org'd parent), regardless of cluster-recognition status. The PR-majority filter excludes byz's envelope from the eligible set before argmax (predicate A) — `L_Bid` is the highest-effective-bid candidate among majority-head bidders, typically an honest leader. σ_LBid reaches without honest having to fail validation on byz's V. **Bid lie filtered at envelope-set processing**, no liveness regression, no fall-through wasted on a doomed L_Bid attempt.
- Byzantine claims `bid = ∞` for an *invalid* block (regardless of cluster-recognition or parent-root). Honest reject `V_{L_Bid}` at validation; all 3 honest NR_LBid; NR_LBid-quorum reaches; fall through to layer 1 (`L_0_baseline`). Slot completes at baseline primary. **Bid lie wasted, no slot miss, no value capture loss** — strictly more graceful than B.1's behavior in the same scenario (which would commit to the invalid V_byz at layer 0 and require NR-walk to L_1).
...
**Bid-lie + selective-broadcast DoS is bounded — byz cannot break liveness via bid manipulation alone.** When byz withholds or selectively broadcasts their own bid envelope, three closure paths cover it under partial synchrony:

- **Byz withholds; honest are on a stable head (predicate B fires).** Every honest operator computes `saw_all_bids = false` and `saw_minus_one = true`; the 3 visible bidders are honest and unanimously share parent_root → predicate B fires → 3 honest σ on argmax-of-honest-bids → σ_LBid reaches without byz's contribution → `V_{L_Bid}` wins (highest honest bid). **Strict improvement over a saw-all-only formulation:** L_Bid completes at the highest-bid honest block here rather than falling through to a rotation-determined baseline leader.

- **Byz withholds; honest are split across heads (predicate B fails — no PR-unanimity).** Every honest operator computes `saw_all_bids = false`, `saw_minus_one = true`, but visible bidders' parent_roots are non-unanimous → predicate B fails → all honest NR_LBid → NR_LBid-quorum (3 = qEnc) reaches → fall through to baseline TBFT structure → slot completes at `L_0_baseline` (or `L_1_baseline` if byz happens to also be `L_0_baseline`).

- **Selective-broadcast edge — byz delivers to some honest but not all, with byz as L_Bid candidate.** When byz's bid is the cluster's highest, σ-side honest who saw the bid σ on `V_{L_Bid} = V_byz`; honest who missed it go NR_LBid. The σ_LBid pool count is `(σ-side honest count) + (L_Bid's own Phase-1 σ_V partial)`: byz signed `σ_{L_Bid}^V(V_{L_Bid})` in their Phase-1 envelope (delivered to at least the σ-side honest), and that partial is part of σ_LBid pool cluster-wide per the Phase 3 walk above (`sigs_LBid` includes `σ_{L_Bid}^V(V_{L_Bid})` from L_Bid's Phase-1 envelope). Under partial synchrony with re-flood, at most 1 honest misses the bid (the byzantine-leader-at-cutoff edge described at "Phase 1 / Bundle propagation"); the closure is then `2 honest σ + 1 L_Bid Phase-1 σ = f+2 = qV = 3` exactly. **Same closure mechanism as baseline's byzantine-L_0-at-cutoff edge** (per the table at "Fault tolerance / Liveness"). σ-side honest reconstruct `V_byz` and submit; certificate gossip then propagates the final signature for any honest who couldn't reconstruct locally.

The worst byz can do via bid manipulation is force fall-through to the rotation-fixed baseline, which they have no special control over — baseline TBFT's standard f=1 byz tolerance kicks in from there.
...
**σ-side deviation surface at L_Bid (closed by the recommended-config preconditions).** The σ-eligibility predicate at `L_Bid` (`saw_all_bids AND V_{L_Bid}` valid) is a rule for honest behavior, not a cryptographic constraint — byzantine can σ on a different V than the cluster's bid-winner to fragment the σ pool. The fragmentation deadlocks the slot only when three conditions co-occur: (1) one honest goes NR_LBid (via byz-at-cutoff selective-withhold, sub-partial-synchrony fragmentation, or — under strict host validity policy — validity-divergence at L_Bid); (2) the cluster's bid-winner X happens to be one of the σ-side honest (`X = σ-side self`), causing that operator's Phase-1 σ_V on `V_X` and Phase-2 onion σ on `V_X` to dedup to a single partial per "σ_V partial dedup rule" above; and (3) byz σ-deviates onto a non-winner's V instead of helping `V_X`. When all three fire, σ_LBid pool count = 2 < qV, NR_LBid pool ≤ 2 < qEnc, slot misses without fall-through.

The structural reason this surface exists in B.3 but not in baseline TBFT: in baseline, byz-at-cutoff requires byz to be the rotation leader L_0, and the leader's Phase-1 σ_V is on `V_{L_0}` (= σ-target V) by protocol structure, contributing 1 partial that closes σ-pool to qV via the f+2=qV math. In B.3, byz can be a non-L_Bid bidder; their Phase-1 σ_V is on `V_byz` (≠ `V_{L_Bid}`), so byz's contribution doesn't naturally land on σ-target V. The dedup edge then leaves σ-pool short.

**The three preconditions in the recommended config close all three conditions:**

- **Relay-anchored bid binding** (see "Bid binding via relay attestation") eliminates bid-equivocation as a strategy, removing the equivocation-induced fragmentation sub-case of (1) (the canonical L257 trigger).

- **Host loose-validity policy** (see "Application-validity-divergence inherits baseline's known limit at every layer") eliminates the validity-divergence sub-case of (1).

- **Tightened candidate-acceptance cutoff** (`T_candidate_accept = T_commit − (2D + δ)`, see "Tightened candidate-acceptance cutoff for B.3" above) closes the byz-at-cutoff selective-withhold sub-case of (1). Granting gossipsub re-flood a full `D` of headroom after byz's last possible first-delivery means all 3 honest see all bids before any local cutoff under partial synchrony, so σ-side has 3 honest and σ pool reaches 3 distinct partials regardless of dedup (the X=self dedup collapses one operator's Phase-1+Phase-2 to a single partial, but the other 2 honest still contribute 2 more partials → 3 total = qV). Cost: ~`D` shaved off the high-MEV fetch window. This trade is symmetric to the cutoff-tightening option baseline accepts at L_0/L_1 (see "Phase 1 / Bundle propagation" in baseline) — but B.3 elevates it from optional to required, because the H4 dedup edge needs it.
...
|---|---|---|
| Phase 1 envelope | `(protocol_tag, message_kind, cluster_id, slot, layer, leader_id, value_root)` | + `bid`, `relay_attestation`, `trusted_builder_pubkeys`, `op_id` replaces `(layer, leader_id)` |
...
| Per-operator commitment | σ XOR NR per layer (hedging preserved) | identical — σ XOR NR per layer at each of the 3 layers, with σ-eligibility at L_Bid conditioned on the combined predicate (saw-all with PR-majority filter, OR saw-`n−1` with PR-unanimity) |
| Phase 3 walk | 2-layer fall-through | 3-layer fall-through (Appendix C) |
| Equivocation handling | Equivocation → NR for that layer | Equivocation → NR for that layer (rule extends to all bidders at L_Bid); same-V bid-equivocation precluded by relay-attestation anchoring (see "Bid binding via relay attestation") |
| Leader selection | rotation L_0 + rotation L_1 | bid-determined L_Bid + rotation L_0_baseline + rotation L_1_baseline |
| σ-eligibility at L_Bid | n/a | conditional: (saw-all-bids with PR-majority filter) OR (saw-`n−1`-bids with PR-unanimity), AND V_{L_Bid} validated. Effective bid in argmax = real bid if envelope's builder is cluster-recognized (≥ qV operators include it in their `trusted_builder_pubkeys`), else 0 |
| Relay/builder trust | n/a | Auto-derived from envelope set via cluster-recognition (≥ qV operators trusting a builder); no out-of-band cluster trusted-set configuration required (see "Bid binding via relay attestation" / "Cluster-recognition vs cluster-configured trust") |
| Liveness when byz withholds bid (honest stable head) | n/a | **L_Bid completes via saw-`n−1` PR-unanimity predicate**; cluster captures highest honest bid (better value capture than fall-through to rotation-determined baseline) |
| Liveness when byz withholds bid (honest split across heads) | n/a | predicate B fails (no unanimity) → fall through to baseline structure; slot completes |
| Liveness when byz claims high bid on minority head | n/a (no PR filter) | PR-majority filter excludes byz's envelope from eligible set; L_Bid completes at majority-head argmax (no wasted L_Bid attempt on a V most honest can't validate) |
| Liveness when byz lies on invalid block | n/a | fall through to baseline; bid lie wasted |
| Liveness when byz lies on valid block (majority head) | n/a | V_byz signed (value-capture loss); attributable post-hoc |
| Liveness when honest disagree on V validity at any layer | Layer deadlocks at the divergence layer; mitigated by host loose-validity (see "Application: SSV Ethereum proposer duty / Head-change handling") | Same per-layer behavior at each of L_Bid / L_0_baseline / L_1_baseline; same host-policy mitigation applies symmetrically across all three layers |
| Liveness under sub-partial-synchrony | Bound to per-layer reception | Same — the predicate (saw-all with PR-majority filter, OR saw-`n−1` with PR-unanimity) is a deterministic function of the cluster-consistent envelope set; under partial synchrony with re-flooding it resolves identically across all honest receivers. Sub-partial-synchrony degrades equivalently to baseline. No L-identity-divergence regression: σ-eligibility is conditioned on the envelope-set-derived predicate, not on local argmax over an arbitrary received subset, so σ-side stays cluster-consistent even under partial reception |
| Liveness equivalence to baseline TBFT | n/a | **Identical in every failure mode** under three preconditions: uniform host validity policy + relay-anchored bid binding + tightened candidate-acceptance cutoff (`T_candidate_accept = T_commit − (2D + δ)`); every baseline-completable slot completes here at the same layer or earlier with V_{L_Bid} (see "Application-validity-divergence inherits baseline's known limit at every layer", "Bid binding via relay attestation", and "Tightened candidate-acceptance cutoff for B.3") |
| Candidate-acceptance cutoff | `T_commit − (D + δ)` | `T_commit − (2D + δ)` — tightened by `D` to close the H4 σ-side deviation surface (see "σ-side deviation surface at L_Bid"). Cost: ~`D` shaved off the high-MEV fetch window |
| Bid attribution | n/a | Self-contained envelope + relay-attested bid record per bidder, every slot; bid binding cryptographically anchored to builder's `SignedBuilderBid` (see "Bid binding via relay attestation") |
| Application contract | Application validity only | + bid concept, signed in envelope |
| Phase 1 bandwidth | 2 candidate broadcasts | `n` candidate broadcasts × per-envelope size that includes `bid` + `relay_attestation` (~800 B for SSV's `SignedBuilderBid` payload) + `trusted_builder_pubkeys` (~32 B × number of personally-trusted builders, typically ~100–200 B). At `n = 4`: roughly `4 × (baseline-envelope-size + 1 KB)` cluster-wide. The constant-factor expansion in each envelope is non-trivial alongside the `n` scaling |
| Phase 2-3 bandwidth | baseline | + ~50% (3 layers vs 2; each operator emits one extra σ or NR partial; chained encryption at layer 2). Note: Phase-1 σ_V partials from bidders not in `{L_Bid, L_0_baseline, L_1_baseline}` are retained for attribution but unused for reconstruction (up to `n − 3 = 1` extra partials per slot at n=4) |
| Latency (rounds) | 2 RTTs (Phase 1, Phase 2) | 2 RTTs (no extra rounds) |
| Propagation budget `D` | sized for 2 envelopes per slot | sized for `n` envelopes per slot — gossipsub mesh propagation does not scale O(1) in message count (queueing, processing, and amplification overhead). Real-world `D` may be modestly higher than baseline's; the `T_candidate_accept = T_commit − (D + δ)` budget compresses the relay-cutoff headroom proportionally |

The trade is concentrated in bandwidth and a tighter MEV fetch window: Phase 1 scales as `n` with a constant-factor envelope expansion (relay-attestation payload), Phase 2 onion grows by one chained-encryption layer, and `T_candidate_accept` tightens by `D` (closing the H4 deadlock surface). Latency profile is unchanged in terms of round-trips relative to baseline; safety/liveness profile is unchanged given the three preconditions: uniform host validity policy across layers, relay-anchored bid binding, and the tightened cutoff (see "Bid binding via relay attestation" and "Tightened candidate-acceptance cutoff for B.3").
...
- **Equivocation rule — bidder-level** — extends from "leader equivocation" (baseline) to "bidder equivocation" (B.3). Every operator is a bidder every slot, so the equivocation rule applies cluster-wide every slot. Evidence shape (two distinct envelopes with same `(slot, op_id)`, different bids or value_roots) is unchanged. With relay-attestation anchoring (see "Bid binding via relay attestation"), same-V bid-equivocation is precluded at the source for SSV proposer duty — the only equivocation evidence that can be produced is "two distinct `value_root`s from the same op at the same slot" (different `SignedBuilderBid`s queried at different times, each relay-signed). The evidence-collection surface scales linearly in `n`: at n=4 this is a 2× increase vs baseline (which has L_0 and L_1 leaders capable of equivocation per slot) — manageable at n=4. Each piece of evidence is a self-contained slashable fault proof verifiable in isolation (signed by the offending op's identity key with relay attestation), so attribution doesn't require cluster-wide coordination or modifications to existing per-validator-share slashing-protection logs — the standalone `(σ^op_i, σ^op_j)` pair from the same op at the same slot is the evidence.
- **Post-hoc verification mechanism** — for SSV proposer duty, the relay's `SignedBuilderBid` *is* the bid-truth, so post-hoc bid-lying detection collapses to verifying the relay attestation against the cluster's trusted builder set (already done at envelope acceptance — no separate slashing flow needed for bid-lying). Slashable evidence reduces to envelope equivocation (two distinct envelopes from same op at same slot), handled by the existing equivocation rule. For other applications without a non-equivocable bid anchor, the B.1 flow applies — out of TBFT scope; shared with B.1.
- **Personal MEV-Boost configuration drift across operators** — B.3's cluster-recognition mechanism (see "Bid binding via relay attestation" / "Cluster-recognition rule") auto-derives cluster trust from operators' `trusted_builder_pubkeys` envelope fields with a `qV` threshold. The mechanism is principle-preserving by construction (the slot always either reaches L_Bid σ-quorum, resolves L_Bid via op_id tiebreak, or falls through to baseline; never deadlocks regardless of trusted-builder configuration). Open questions concern *value-capture optimization* rather than correctness:
  - **Builder discovery**: how operators learn which builders are popular enough that ≥ qV others trust them. Operators independently configuring MEV-Boost relays without coordination may produce frequent demote-to-0 outcomes (effective bids = 0, L_Bid resolves via tiebreak rather than bid argmax). Convergence happens passively as operators observe which builders consistently dominate cluster-recognition; explicit cluster-level coordination (off-chain agreement on a shared "default" relay list) accelerates convergence at the cost of coordination overhead.
  - **Builder rotation**: as builders rotate BLS keys or new builders launch, operators' personal configs drift over time. The cluster-recognition mechanism adapts per-slot (each envelope's `trusted_builder_pubkeys` is current at envelope-construction time), so rotation doesn't require atomic cluster-wide updates — but each operator's update timing affects which builders reach cluster-recognition in the slots around the rotation. A builder that ≥ qV operators are about to migrate away from has natural cluster-recognition during the migration window; one that only some operators have migrated *to* may not yet meet the threshold.
  - **Privacy considerations**: operators' `trusted_builder_pubkeys` lists are exposed cluster-wide via Phase-1 envelopes. This isn't a security concern (the lists are MEV-Boost relay configurations, not cryptographic secrets) but is competitive-information disclosure — operators with sophisticated relay setups reveal their configurations to other cluster members. Mitigation: hash-or-Bloom-filter the list (with verifiable membership proofs) to obscure exact contents while preserving cluster-recognition. Out of TBFT scope; deployment-dependent.
  - **Lower-bound for L_Bid value-capture**: when fewer than qV operators agree on any builder (extreme drift, e.g., 1-1-1-1 fully disjoint personal configs), no envelope is cluster-recognized, all effective bids demote to 0, and L_Bid resolves via op_id tiebreak — operationally rotation-equivalent for that slot. The slot still completes (principle preserved), but the L_Bid layer adds no value over baseline TBFT in this regime. Whether this is acceptable is a deployment-level question; clusters expecting heterogeneous relay configurations may want to coordinate on at least a small shared "popular builders" list to ensure most slots reach effective-bid recognition.

  None of these concerns affect safety or liveness — the design principle holds unconditionally for B.3 regardless of trusted-builder configuration. They affect *value-capture frequency* (how often the L_Bid layer realizes its bid-routing upside vs degrading to tiebreak-resolution).
...
- **PR-aware rotation (`L_0_PR` / `L_1_PR`)** — extend parent-root awareness from L_Bid into the rotation tiers. Idea: in re-org scenarios where `rotation_primary` or `rotation_backup` happens to be on a minority head, walk the rotation order and use the first / second op on the majority head as `L_0_PR` / `L_1_PR` instead of the standard rotation_primary / rotation_backup. The plurality parent_root is computed deterministically from the cluster-consistent envelope set (with tiebreak), so the substitution is cluster-consistent. The benefit: avoid wasted Phase-2 attempts on rotation leaders whose V most honest can't validate. Two design questions to resolve before specifying: (a) ensuring `L_0_PR ≠ L_1_PR` even when the majority head has only 1 or 2 ops (rotation walk could collapse), and (b) edge case where the plurality head has only 1 bidder (no second op available for L_1_PR — fallback to plain rotation? or accept that L_1_PR doesn't exist for that slot?). Resolvable but not in current spec; deferred until practical re-org rates justify the additional spec surface.
- **Bid hiding (anti-grinding)** — for SSV proposer duty, relay-anchoring (see "Bid binding via relay attestation") already precludes the canonical anti-grinding concern of "byz fabricates a bid value after observing honest bids": byz can only carry whatever bid value the relay's `SignedBuilderBid` sealed at query time, not a synthesized value. The residual anti-grinding question reduces to query-timing — "byz queries the relay later than honest, gets an updated `SignedBuilderBid`, includes that one" — a different attack vector that anti-grinding-via-bid-hiding wouldn't address either. For non-relay applications without an external bid anchor, an alternative is to reuse TBFT's existing IBE primitive to tlock-encrypt bids in Phase-1 envelopes under a `(slot, "phase1-cutoff")` tag, with each bidder's envelope including a partial signature `σ_{op}^{IBE}((slot, "phase1-cutoff"))`. At cutoff, qEnc partials aggregate into a decryption key, all bids reveal simultaneously cluster-wide. Doesn't add rounds. Caveat: at f=1, this gives **no anti-grinding** against a rational byzantine — byz withholds their own IBE partial, waits for `qEnc = 3` honest partials to broadcast (which they do, having no reason not to), aggregates the 3 honest partials locally to derive the decryption key, and reveals all bids before crafting their own. Full mitigation requires an external time-released randomness beacon (drand-style) that produces unpredictable randomness at the cutoff time independent of any cluster-internal aggregation. Adopting drand-style randomness brings its own setup, trust model, and liveness assumptions (drand has its own DKG, threshold of independent operators, and partial-synchrony assumptions) — these would need separate analysis as an external dependency, not as part of TBFT proper. The bottom line: for non-relay-anchored applications, anti-grinding is conditional on an unspecified external dependency; the IBE-only construction does not provide it on its own.
...
**Empirical caveat: bid variance across operators within a cluster.** B.3's value-capture upside depends on per-slot bid variance across operators in the cluster. In practice, SSV clusters often share relay/builder configurations (Flashbots, BloXroute, etc.) for reliability reasons; operators querying the same builder set at similar times typically receive correlated bids, reducing realized variance. The bid-routing benefit is most pronounced when operators have meaningfully different relay/searcher pipelines or orderflow access — empirical to verify per cluster. With relay-anchoring in effect (see "Bid binding via relay attestation"), correlated bids degrade gracefully: when all bids tie, the `op_id` tiebreak applies and L_Bid resolves deterministically (effectively rotation-determined for that slot), the same outcome as baseline. The K=3 spec surface and `n × baseline` Phase-1 bandwidth are paid every slot regardless of whether the realized variance produces a meaningful L_Bid winner; clusters with low intra-cluster bid variance get the bandwidth cost without proportional value-capture upside.

**Relay rate-limiting and per-bidder fetch failure.** With `n` operators querying potentially-shared relay endpoints at `T_0` simultaneously, relay rate-limiting and individual fetch failures become more likely than baseline's 2-broadcast Phase 1. The vanilla-block fallback (per "Bid binding via relay attestation") handles this gracefully: a bidder whose relay query fails broadcasts an envelope with `bid = 0` and `relay_attestation = nil` rather than skipping the broadcast, preserving `saw_all_bids = true` cluster-wide. The L_Bid winner is then determined among the bidders who did successfully fetch; if all bidders' relays fail (catastrophic outage), all bid 0, L_Bid degenerates to `op_id` tiebreak. The optimistic L_Bid path therefore activates whenever any honest bidder has a non-zero relay-anchored bid — not conditional on all `n` operators successfully fetching.

Without those signals, baseline TBFT is simpler — every operator runs the same rotation table, no per-slot Phase-1 bandwidth blow-up, no per-operator fetch work. **L_Bid-prepended TBFT is an additive optimization on top of baseline; given the three preconditions (uniform host validity policy, relay-anchored bid binding, and the tightened candidate-acceptance cutoff), it doesn't compromise any baseline property to add the L_Bid layer**, so the question is purely "is the value-capture upside worth the cost (bandwidth + ~`D` of fetch window)?"

If pursued, this is the right shape for bid-routing in TBFT — preserves per-operator hedging across layers (where B.1 doesn't) and accommodates relay-anchored bid binding cleanly, at the cost of K=3 spec surface (chained encryption per Appendix C, third onion layer), n-bid-envelope Phase-1 bandwidth, and ~`D` shaved off the MEV fetch window (tightened cutoff). B.3 vs B.1 is a Pareto trade rather than a total ordering: B.1 stays at K=2 with commit-tag routing but sacrifices hedging and introduces a K=2 fragmentation regression; B.3 keeps hedging and inherits baseline's per-layer equivocation/validity-divergence surfaces at the L_Bid layer (mitigated by relay-anchoring + host loose-validity + cutoff tightening, per the sections above). The K=3 spec cost vs baseline is linear and tractable at n=4; the ~`D` fetch-window cost is the price of closing the H4 σ-side deviation surface.
...
**Effect on Appendix B.3 (L_Bid-prepended TBFT).** B.3 is the one variant in this appendix family that *requires* K=3 and the chained-encryption machinery described above. Its layer 0 is bid-determined (`L_Bid`); layers 1 and 2 are baseline TBFT's rotation-determined `L_0` and `L_1`. The cross-layer safety argument is exactly Pigeonhole 3 from this appendix: σ_{L_0_baseline} partials are wrapped in IBE encryption under `nr_tag_LBid`; σ_{L_1_baseline} partials are wrapped in two nested IBE encryptions, the inner under `nr_tag_LBid` and the outer under `nr_tag_L0_baseline` (chain depth 2 — sequential nesting, decrypted outer-first per the Phase-3 walk). At K=3 the chain depth is 2 — small, but non-zero spec surface.

**Audit-cost delta vs baseline.** The spec-surface delta isn't only the chained ciphertext format. The safety proof for B.3 requires Pigeonhole 3 (cross-layer safety against the offline-aggregating adversary model, where a byzantine offline aggregator with public partials can attempt to combine across layers) in addition to the per-layer Pigeonhole 1 + 2 that K=2 baseline TBFT relies on. Auditing B.3 therefore needs the auditor to verify all three pigeonholes plus the offline-aggregation adversary model — meaningfully more than auditing K=2 baseline. The chained-encryption machinery itself is mechanical once Pigeonhole 3 is established, but the proof obligation is real spec surface.

B.3 is the design point where committing to K=3 actually buys something (an opportunistic bid-routing layer atop the K=2 baseline structure, with no liveness regression) — unlike a plain K=3 baseline TBFT, which by the bullet points above doesn't buy anything in the standard byzantine model at n=4.
