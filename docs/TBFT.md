...
- The relay-attestation envelope-acceptance rule (reject envelopes whose attestation fails verification against the operator's stored trusted set) produces predicate failure under cluster-inconsistent trusted sets, which drives fall-through to baseline TBFT's K=2 structure. Predicate-failure-driven NR-quorum is the same mechanism baseline TBFT itself uses to walk past a failed L_0; B.3 inherits it for L_Bid.
...
- The signed envelope binding `(protocol_tag, message_kind, cluster_id, slot, op_id, value_root, bid, relay_attestation)` (full field list per "Setting" above).
...
# L_Bid layer (bid-determined with parent-root awareness):
...
if saw_all_bids:
    ...
    L_Bid       = argmax_{(bid desc, op_id asc)}(eligible)
    ...
    L_Bid       = argmax_{(bid desc, op_id asc)}(received_bidders)
...
The bid carried in each Phase-1 envelope must be cryptographically anchored to a non-equivocable external source. For SSV's proposer duty, this is the relay's `SignedBuilderBid` returned by the MEV-Boost `getHeader(slot, parent_hash, validator_pubkey)` query: a `BuilderBid = (header, value, pubkey)` tuple plus a BLS signature from the builder over that bid. The envelope's `relay_attestation` field carries this signature plus enough of the `SignedBuilderBid` payload to verify it. The envelope's `value_root` binds to `header.block_hash`; the envelope's `bid` binds to `value`.

Receivers verify `relay_attestation` against the cluster's known set of trusted builder pubkeys before accepting the envelope. An envelope without a valid `relay_attestation` is treated as not-received (operationally identical to a cryptographically malformed envelope).

**Why this matters for liveness.** Without a non-equivocable bid anchor, a byzantine bidder can equivocate on bids for the same `V` — i.e., produce two distinct envelopes carrying the same `value_root` but different `bid` values, each with a valid `σ^op` signature. Single-envelope receivers see only one of the two and compute argmax over different bid sets, locking σ-side commitments to different `V_{L_Bid}` candidates and fragmenting the σ pool below `qV` (analog of baseline TBFT's "Single-V receiver interaction with leader equivocation" at "Fault tolerance / Equivocation handling", widened from `K=2` rotation leaders to `n` bidders). With relay anchoring, the bid is cryptographically bound to `(block_hash, value)` via the builder's signature; producing two valid envelopes with the same `value_root` and different `bid` would require the builder to sign two distinct values for the same `block_hash`, which the builder protocol does not do. **Same-V bid-equivocation is precluded at the source.** Different-V envelope equivocation (byz queries the relay multiple times during the slot, gets two `SignedBuilderBid`s with different `block_hash`es) reduces to standard leader-V-equivocation handled by the existing equivocation-→-NR rule.

**Vanilla-block handling.** When a bidder cannot or does not query a relay (relay timeout, fallback to a locally-built vanilla beacon-node payload), the envelope carries `relay_attestation = nil` and `bid = 0`. Receivers skip relay-attestation verification for `bid = 0` envelopes; standard envelope checks apply. Byzantine cannot weaponize vanilla bids since `bid = 0` cannot become `L_Bid` if any honest bidder has a non-zero relay-anchored bid; if all bidders submit `bid = 0` (cluster-wide relay outage), the L_Bid winner is determined by `op_id` tiebreak — operationally equivalent to a rotation-determined leader for that slot.

**Cluster-internal bid is decoupled from realized MEV value.** The cluster's commit to `V_{L_Bid}` produces a signed blinded header that the validator submits to MEV-Boost. The relay's `getPayload` flow then reveals the full execution payload and pays out whatever real `value` the relay sealed in the original `SignedBuilderBid`. **The cluster's "effective bid" used for L_Bid argmax is an ordering signal distinct from the realized MEV value paid out at proposal time.** Two consequences:

- **L_Bid completion does not require cluster recognition of the realized bid.** A slot that resolves L_Bid via `op_id` tiebreak (e.g., all bidders submitted vanilla envelopes with `bid = 0` because the cluster trusted set is empty, or because cluster-wide relay outage forced vanilla fallback) still pays out the chosen V's real bid at proposal time. The cluster captures whatever value the chosen block actually carries; it just used a tiebreak rather than a bid comparison to pick which block to sign.
- **Trusted-set drift cannot block value capture.** When operators have drifted to inconsistent stored copies of the trusted set → `received_bidders` differs across operators → predicate fails asymmetrically → all honest converge to NR_LBid → fall through to baseline → `V_{L_0_baseline}` wins. The chosen V at L_0_baseline may itself be a relay-supplied block carrying real MEV value; that value is paid out at proposal time. The cluster captures whatever the chosen block carries, regardless of how internal bid-ordering resolved during consensus.

The cluster-internal bid is *purely an ordering signal* for picking which block to commit to during consensus; value capture is realized downstream at proposal time, independent of consensus-internal bid recognition. This decoupling is what makes the L_Bid layer genuinely opportunistic in the design-principle sense: even when bid-routing fails to capture the highest bid through cluster-internal recognition (relay outages, trusted-set drift, mixed bidder configs), the slot's eventual block proposal still realizes whatever MEV the chosen block carries — so falling through to baseline is a value-loss only relative to "the cluster recognized the highest bid", not a value-loss in absolute terms relative to baseline TBFT (which doesn't recognize bids at all).

**Trusted builder set.** The cluster needs agreement on which builder pubkeys are trusted. Practical options: (a) cluster config lists trusted builders explicitly, derived from the intersection of operators' MEV-Boost relay configurations; (b) operators submit a per-cluster trusted-builder roster at cluster init and the union is trusted with attribution metadata (so a misbehaving builder is attributable). Option (a) is simpler if operators coordinate on a shared relay set; option (b) is more flexible across heterogeneous relay configurations.
...
**Bid lies as liveness faults** (analogous to B.1's cases, with cleaner fall-through and parent-root-aware filtering):

- Byzantine claims `bid = ∞` for an honest-looking but suboptimal block on the **majority head**. Their op becomes `L_Bid` candidate. If 3 honest validate `V_byz`, σ_LBid reaches and slot completes with `V_byz` (low-MEV). Cluster captures `actual_bid_byz` instead of the highest honest bid. **Loss = `max_honest_bid − actual_bid_byz`.** Attributable post-hoc.
- Byzantine claims `bid = ∞` for a block on a **minority head** (e.g., relay-late fetch landing on a re-org'd parent). The PR-majority filter excludes byz's envelope from the eligible set before argmax (predicate A) — `L_Bid` is the highest-bid candidate among majority-head bidders, typically an honest leader. σ_LBid reaches without honest having to fail validation on byz's V. **Bid lie filtered at envelope-set processing**, no liveness regression, no fall-through wasted on a doomed L_Bid attempt.
- Byzantine claims `bid = ∞` for an *invalid* block (regardless of parent-root). Honest reject `V_{L_Bid}` at validation; all 3 honest NR_LBid; NR_LBid-quorum reaches; fall through to layer 1 (`L_0_baseline`). Slot completes at baseline primary. **Bid lie wasted, no slot miss, no value capture loss** — strictly more graceful than B.1's behavior in the same scenario (which would commit to the invalid V_byz at layer 0 and require NR-walk to L_1).
...
| σ-eligibility at L_Bid | n/a | conditional: (saw-all-bids with PR-majority filter) OR (saw-`n−1`-bids with PR-unanimity), AND V_{L_Bid} validated |
...
| Application contract | Application validity only | + bid concept, signed in envelope |
| Phase 1 bandwidth | 2 candidate broadcasts | `n` candidate broadcasts × per-envelope size that includes `bid` + `relay_attestation` (~800 B for SSV's `SignedBuilderBid` payload). At `n = 4`: roughly `4 × (baseline-envelope-size + 800 B)` cluster-wide, not `4 × baseline-envelope-size` — the constant-factor expansion in each envelope is non-trivial alongside the `n` scaling |
...
- **Trusted-builder-set distribution, rotation, and disagreement detection** — B.3's "Bid binding via relay attestation" requires every honest operator to apply a trusted-builder pubkey set at envelope acceptance. The body's "Trusted builder set" paragraph sketches options (a) intersection-at-cluster-init and (b) union-with-attribution as practical resolutions, but real operational details are unspecified:
  - **Pubkey discovery**: how operators learn that the cluster wants to trust a new builder (off-chain governance, on-chain registry, default registry with rotation). Different choices have different coordination overheads and trust assumptions.
  - **Pubkey rotation**: builders rotate BLS keys periodically; the cluster's trusted set must update atomically across operators without producing mid-slot disagreement (a stale operator who hasn't yet rotated their local copy will reject envelopes signed by the new key).
  - **Operator-local-vs-cluster drift**: each operator's MEV-Boost configuration may evolve independently of the cluster's trusted set; without periodic reconciliation, drift is silent until it produces a cluster-consistency violation. A stale-config operator rejects envelopes other operators accept, fragmenting `received_bidders` cluster-wide.
  - **Drift detection**: the spec does not currently mandate a drift-detection mechanism — candidates include a slot-by-slot trusted-set hash gossiped alongside Phase-1 envelopes, a periodic cluster-wide reconciliation message, or out-of-band monitoring with alerting.
  - **Trusted-set agreement vs disagreement edge cases**: under the reject rule (envelope is rejected if `relay_attestation` fails verification against the operator's stored trusted-set copy), two structurally-distinct outcomes emerge:
    1. **Cluster-consistent trusted set (option a or b applied uniformly across operators)**: every honest operator either accepts or rejects the same envelopes uniformly → `received_bidders` is cluster-consistent → predicate A or B fires consistently across honest → L_Bid resolves consistently. If the cluster's trusted set is empty (e.g., disjoint personal relay sets, intersection = ∅), bidders must fall back to vanilla (`bid = 0, relay_attestation = nil`); predicate A then fires over `n` vanilla envelopes; argmax tiebreak resolves L_Bid via `op_id` (operationally rotation-equivalent for that slot). The chosen V's real MEV value is captured at proposal time regardless (see "Cluster-internal bid is decoupled from realized MEV value").
    2. **Cluster-inconsistent trusted set (operators have drifted to different stored copies)**: different operators reject different subsets of envelopes → `received_bidders` differs across operators → predicate A and B fail asymmetrically → all honest NR_LBid → NR_LBid-quorum reaches → fall through to baseline TBFT structure → slot completes at `L_0_baseline` (or `L_1_baseline`). The chosen V's real MEV value is captured at proposal time.

    Neither case deadlocks the protocol; both progress to slot completion. **Cluster-inconsistent trusted set is the operational hazard worth detecting** — not because it breaks safety or liveness (it doesn't, under the reject rule's predicate-failure-driven fall-through, per the design principle above), but because it silently bypasses the L_Bid value-capture upside without raising any protocol-level alarm. Drift detection is therefore an *optimization* concern (catch drift to restore L_Bid value-capture), not a *correctness* concern (the protocol stays correct under drift).

  These are SSV-operational concerns rather than TBFT-protocol concerns. Safety and liveness already hold under the reject rule regardless of trusted-set agreement state; the open question is purely about how the cluster maintains trusted-set agreement well enough to realize the L_Bid value-capture upside often. Resolution may need a companion "trusted-set sync protocol" or an explicit assumption that the cluster runs out-of-band coordination machinery for trusted-set lifecycle management.
