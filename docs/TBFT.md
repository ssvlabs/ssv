...
- **L_0_baseline (rotation primary):** fetches at `T_0` (late, MEV-optimized) — same as baseline TBFT.
...
**σ-side deviation surface at L_Bid (residual L257-equivalent edge).** The σ-eligibility predicate at `L_Bid` ((predicate A or predicate B) AND `V_{L_Bid}` valid) is a rule for honest behavior, not a cryptographic constraint — byzantine can σ on a different V than the cluster's bid-winner to fragment the σ pool. The fragmentation deadlocks the slot only when three conditions co-occur: (1) one honest goes NR_LBid (via byz-at-cutoff selective-withhold, sub-partial-synchrony fragmentation, or — under strict host validity policy — validity-divergence at L_Bid); (2) the cluster's bid-winner X happens to be one of the σ-side honest (`X = σ-side self`), causing that operator's Phase-1 σ_V on `V_X` and Phase-2 onion σ on `V_X` to dedup to a single partial per "σ_V partial dedup rule" above; and (3) byz σ-deviates onto a non-winner's V instead of helping `V_X`. When all three fire, σ_LBid pool count = 2 < qV, NR_LBid pool ≤ 2 < qEnc, slot misses without fall-through. This is the same shape as baseline TBFT's "Single-V receiver interaction with leader equivocation" (per "Fault tolerance / Equivocation handling"), generalized from rotation leaders to all bidders. Mitigations:

- **Recommended config: relay-anchoring + host loose-validity** (per "Bid binding via relay attestation" and "Application-validity-divergence inherits baseline's known limit at every layer"). Relay-anchoring eliminates bid-equivocation as a strategy, removing the equivocation-induced fragmentation sub-case (the canonical L257 trigger). Host loose-validity eliminates the validity-divergence sub-case. Together they reduce condition (1) to "byz-at-cutoff selective-withhold or genuine sub-partial-synchrony fragmentation".

- **Cutoff tightening** (`T_candidate_accept = T_commit − (2D + δ)`, optional cluster choice — same trade as documented at "Phase 1 / Bundle propagation"). Closes the remaining byz-at-cutoff edge by granting gossipsub re-flood a full propagation budget after byz's last possible first-delivery; under partial synchrony all honest see all bids before their cutoff, so σ-side has 3 honest and σ pool reaches 3 partials regardless of dedup (3 honest each contribute 1 distinct partial; the X=self dedup collapses one operator's Phase-1+Phase-2 to a single partial but doesn't reduce the cluster-wide count below qV). Cost: `D` shaved off the high-MEV fetch window. This trade is symmetric to baseline's L257 cutoff-tightening option — not B.3-specific.

- **Phase-2 broadcast deferral** is the standard L257 mitigation in baseline (per [TBFT.md L257](#L257)) for *equivocation* cases — observing the second equivocating envelope before σ-commit, then NR'ing per the equivocation rule. With relay-anchoring precluding bid-equivocation, this mitigation no longer applies to B.3's bid-equivocation sub-case (since there's no second envelope to detect). Deferral has *no effect* on the selective-withhold sub-case, since `T_candidate_accept` precedes Phase-2 broadcast and late-arriving bundles are protocol-rejected for σ-eligibility regardless of when the Phase-2 onion is broadcast.

Without cutoff tightening, the residual H4 surface under recommended config (relay-anchoring + loose-validity) is bounded by the byz-at-cutoff probability — the same partial-synchrony envelope baseline accepts at L_0/L_1, just generalized to all bidders. **No new fundamental attack vector beyond what baseline already documents at "Single-V receiver interaction with leader equivocation"**; the dedup edge adds a bid-distribution-dependent probability multiplier on top of the L257 trigger (the X=σ-side-self condition's likelihood depends on which operators happen to be the slot's highest bidders — quantitatively cluster-specific, not protocol-uniform), but the underlying byz-at-cutoff edge is the same one baseline lives with at the rotation layers.
...
| Liveness equivalence to baseline TBFT | n/a | **Identical in every failure mode** under uniform host validity policy + relay-anchored bid binding — every baseline-completable slot completes here at the same layer or earlier with V_{L_Bid} (see "Application-validity-divergence inherits baseline's known limit at every layer" and "Bid binding via relay attestation") |
...
The trade is concentrated in bandwidth: Phase 1 scales as `n` with a constant-factor envelope expansion (relay-attestation payload), Phase 2 onion grows by one chained-encryption layer. Latency profile is unchanged relative to baseline; safety/liveness profile is unchanged given uniform host validity policy across layers and relay-anchored bid binding (see "Bid binding via relay attestation").
...
Without those signals, baseline TBFT is simpler — every operator runs the same rotation table, no per-slot Phase-1 bandwidth blow-up, no per-operator fetch work. **L_Bid-prepended TBFT is an additive optimization on top of baseline; given uniform host validity policy and relay-anchored bid binding, it doesn't compromise any baseline property to add the L_Bid layer**, so the question is purely "is the value-capture upside worth the bandwidth cost?"

If pursued, this is the right shape for bid-routing in TBFT — preserves per-operator hedging across layers (where B.1 doesn't) and accommodates relay-anchored bid binding cleanly, at the cost of K=3 spec surface (chained encryption per Appendix C, third onion layer) and n-bid-envelope Phase-1 bandwidth. B.3 vs B.1 is a Pareto trade rather than a total ordering: B.1 stays at K=2 with commit-tag routing but sacrifices hedging and introduces a K=2 fragmentation regression; B.3 keeps hedging and inherits baseline's per-layer equivocation/validity-divergence surfaces at the L_Bid layer (mitigated by relay-anchoring + host loose-validity, per the sections above). The K=3 cost vs baseline is linear and tractable at n=4.
