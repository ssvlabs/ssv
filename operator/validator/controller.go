...
import (
	...
	"github.com/bloxapp/ssv/protocol/v2/ssv/valuechecker"
...
func SetupRunners(ctx context.Context, logger *zap.Logger, options validator.Options) runner.DutyRunners {
	...
	for _, role := range runnersType {
		...
		case spectypes.BNRoleAttester:
			qbftCtrl := buildController(spectypes.BNRoleAttester, vc.AttesterValueCheckF)
			runners[role] = runner.NewAttesterRunnner(options.BeaconNetwork.GetBeaconNetwork(), &options.SSVShare.Share, qbftCtrl, options.Beacon, options.Network, options.Signer, vc.AttesterValueCheckF, 0)
		case spectypes.BNRoleProposer:
			qbftCtrl := buildController(spectypes.BNRoleProposer, vc.ProposerValueCheckF)
			runners[role] = runner.NewProposerRunner(options.BeaconNetwork.GetBeaconNetwork(), &options.SSVShare.Share, qbftCtrl, options.Beacon, options.Network, options.Signer, vc.ProposerValueCheckF, 0)
			runners[role].(*runner.ProposerRunner).ProducesBlindedBlocks = options.BuilderProposals // apply blinded block flag
		case spectypes.BNRoleAggregator:
			qbftCtrl := buildController(spectypes.BNRoleAggregator, vc.AggregatorValueCheckF)
			runners[role] = runner.NewAggregatorRunner(options.BeaconNetwork.GetBeaconNetwork(), &options.SSVShare.Share, qbftCtrl, options.Beacon, options.Network, options.Signer, vc.AggregatorValueCheckF, 0)
		case spectypes.BNRoleSyncCommittee:
			qbftCtrl := buildController(spectypes.BNRoleSyncCommittee, vc.SyncCommitteeValueCheckF)
			runners[role] = runner.NewSyncCommitteeRunner(options.BeaconNetwork.GetBeaconNetwork(), &options.SSVShare.Share, qbftCtrl, options.Beacon, options.Network, options.Signer, vc.SyncCommitteeValueCheckF, 0)
		case spectypes.BNRoleSyncCommitteeContribution:
			qbftCtrl := buildController(spectypes.BNRoleSyncCommitteeContribution, vc.SyncCommitteeContributionValueCheckF)
			runners[role] = runner.NewSyncCommitteeAggregatorRunner(options.BeaconNetwork.GetBeaconNetwork(), &options.SSVShare.Share, qbftCtrl, options.Beacon, options.Network, options.Signer, vc.SyncCommitteeValueCheckF, 0)
		case spectypes.BNRoleValidatorRegistration:
			qbftCtrl := buildController(spectypes.BNRoleValidatorRegistration, vc.ValidatorRegistrationValueCheckF)
