...
func ShareFromValidatorEvent(
	...
	validatorShare.Quorum, validatorShare.PartialQuorum = types.ComputeQuorumAndPartialQuorum(len(committee))
...
func LoadLocalEvents(logger *zap.Logger, handler eth1.SyncEventHandler, path string) error {
	...
	for _, ev := range parsedData {
		...
		errs := eth1.HandleEventResult(logger, *ev, logFields, err, false, nil)
		if len(errs) > 0 {
