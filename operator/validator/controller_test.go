...
func TestNewController(t *testing.T) {
	...
	registryStorage, newStorageErr := storage.NewNodeStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), logger, db)
	require.NoError(t, newStorageErr)

	controllerOptions := ControllerOptions{
		NetworkConfig:     networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig),
		Beacon:            bc,
		FullNode:          true,
		Network:           network,
...
func TestHandleNonCommitteeMessages(t *testing.T) {
	...
	identifier := spectypes.NewMsgID(networkconfig.TestSSVConfig.DomainType, []byte("pk"), spectypes.RoleCommittee)
...
func setupController(t *testing.T, logger *zap.Logger, opts MockControllerOptions) controller {
	...
	if opts.networkConfig == nil {
		opts.networkConfig = networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig)
	}
...
func setupCommonTestComponents(t *testing.T, operatorPrivKey keys.OperatorPrivateKey) (*gomock.Controller, *zap.Logger, *mocks.MockSharesStorage, *mocks.MockP2PNetwork, ekm.KeyManager, *mocks.MockRecipients, *beacon.MockBeaconNode) {
	...
	db, err := getBaseStorage(logger)
	require.NoError(t, err)
	km, err := ekm.NewLocalKeyManager(logger, db, networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), operatorPrivKey)
	require.NoError(t, err)
