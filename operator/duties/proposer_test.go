...
func setupDutiesMockProposer(s *Scheduler, dutiesMap *hashmap.Map[phase0.Epoch, []*eth2apiv1.ProposerDuty]) (chan struct{}, chan []*spectypes.ValidatorDuty) {
...
func expectedExecutedDutiesProposer(handler *ProposerHandler, duties []*eth2apiv1.ProposerDuty) []*spectypes.ValidatorDuty {
...
func TestScheduler_Proposer_Same_Slot(t *testing.T) {
	...
	expected := expectedExecutedDutiesProposer(handler, duties)
...
func TestScheduler_Proposer_Indices_Changed(t *testing.T) {
	...
	duties, _ := dutiesMap.Get(phase0.Epoch(0))
	expected := expectedExecutedDutiesProposer(handler, []*eth2apiv1.ProposerDuty{duties[2]})
	setExecuteDutyFunc(scheduler, executeDutiesCall, len(expected))
...
func TestScheduler_Proposer_Multiple_Indices_Changed_Same_Slot(t *testing.T) {
	...
	currentSlot.Set(phase0.Slot(2))
	duties, _ = dutiesMap.Get(phase0.Epoch(0))
	expected := expectedExecutedDutiesProposer(handler, []*eth2apiv1.ProposerDuty{duties[0]})
	setExecuteDutyFunc(scheduler, executeDutiesCall, len(expected))
	...
	currentSlot.Set(phase0.Slot(3))
	duties, _ = dutiesMap.Get(phase0.Epoch(0))
	expected = expectedExecutedDutiesProposer(handler, []*eth2apiv1.ProposerDuty{duties[1]})
	setExecuteDutyFunc(scheduler, executeDutiesCall, len(expected))
...
func TestScheduler_Proposer_On_Fork(t *testing.T) {
	...
	currentSlot.Set(phase0.Slot(2))
	duties, _ := dutiesMap.Get(phase0.Epoch(0))
	expectedGenesis := expectedExecutedDutiesProposerGenesis(handler, duties)
	setExecuteGenesisDutyFunc(scheduler, executeDutiesCallGenesis, len(expectedGenesis))

	ticker.Send(currentSlot.Get())
	waitForDutiesExecutionGenesis(t, logger, fetchDutiesCallGenesis, executeDutiesCallGenesis, timeout, expectedGenesis)
