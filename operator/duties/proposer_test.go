...
func TestScheduler_Proposer_Same_Slot(t *testing.T) {
	...
	dutiesMap[baseEpoch] = []*v1.ProposerDuty{
		{
			PubKey:         pubKey,
			Slot:           0,
	...
	s, mockTicker, logger, executeDutiesCall, cancel, schedulerPool := setupSchedulerAndMocks(t, NewProposerHandler(), currentSlot.GetSlot())
...
func TestScheduler_Proposer_Indices_Changed(t *testing.T) {
	...
	currentSlot.SetSlot(phase0.Slot(1))
	dutiesMap[currentEpoch] = append(dutiesMap[currentEpoch], &v1.ProposerDuty{
