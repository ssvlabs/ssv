...
func setupSyncCommitteeGenesisDutiesMock(
	...
	s.network.(*networkconfig.MockInterface).EXPECT().LastSlotOfSyncPeriod(gomock.Any()).DoAndReturn(
	...
	s.network.(*networkconfig.MockInterface).EXPECT().GetEpochFirstSlot(gomock.Any()).DoAndReturn(
...
func expectedExecutedGenesisSyncCommitteeDuties(handler *SyncCommitteeHandler, duties []*v1.SyncCommitteeDuty, slot phase0.Slot) []*genesisspectypes.Duty {
	...
	for _, d := range duties {
		...
		expectedDuties = append(expectedDuties, handler.toGenesisSpecDuty(d, slot, genesisspectypes.BNRoleSyncCommitteeContribution))
	}
