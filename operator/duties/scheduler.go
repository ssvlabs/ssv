...
func (s *Scheduler) SlotTicker(ctx context.Context) {
	for {
		...
		case <-s.ticker.Next():
			...
			finalTime := s.network.BeaconConfig.GetSlotStartTime(slot).Add(delay)
...
func (s *Scheduler) HandleHeadEvent(logger *zap.Logger) func(event *eth2apiv1.Event) {
	return func(event *eth2apiv1.Event) {
		...
		if data.Slot != s.network.BeaconConfig.EstimatedCurrentSlot() {
			return
		}

		// check for reorg
		epoch := s.network.BeaconConfig.EstimatedEpochAtSlot(data.Slot)
		...
		delay := s.network.SlotDuration() / casts.DurationFromUint64(goclient.IntervalsPerSlot) /* a third of the slot duration */
		slotStartTimeWithDelay := s.network.BeaconConfig.GetSlotStartTime(data.Slot).Add(delay)
...
func (s *Scheduler) ExecuteGenesisDuties(logger *zap.Logger, duties []*genesisspectypes.Duty) {
	for _, duty := range duties {
		...
		slotDelay := time.Since(s.network.BeaconConfig.GetSlotStartTime(duty.Slot))
...
func (s *Scheduler) ExecuteCommitteeDuties(logger *zap.Logger, duties committeeDutiesMap) {
	for _, committee := range duties {
		...
		dutyEpoch := s.network.BeaconConfig.EstimatedEpochAtSlot(duty.Slot)
...
func (s *Scheduler) loggerWithGenesisDutyContext(logger *zap.Logger, duty *genesisspectypes.Duty) *zap.Logger {
	return logger.
		...
		With(fields.CurrentSlot(s.network.BeaconConfig.EstimatedCurrentSlot())).
		With(fields.Slot(duty.Slot)).
		With(fields.Epoch(s.network.BeaconConfig.EstimatedEpochAtSlot(duty.Slot))).
		With(fields.PubKey(duty.PubKey[:])).
		With(fields.StartTimeUnixMilli(s.network.BeaconConfig.GetSlotStartTime(duty.Slot)))
}
