...
func TestValidatorDuty(t *testing.T) {
	...
	collector := New(context.TODO(), logger, validators, nil, nil, networkconfig.TestNetwork.Beacon.GetBeaconNetwork(), true)
	...
	{ // TC 1 - PartialSig - Aggregator - pre-consensus
		...
		collector.Collect(context.Background(), partSigMsg)
	...
	{ // TC - 3 - Proposal
		...
		collector.Collect(context.Background(), proposalMsg)
	...
	{ // TC - 4 - Prepare
		...
		collector.Collect(context.Background(), prepareMsg)
	...
	{ // TC - 5 - Decided
		...
		collector.Collect(context.Background(), decidedMsg)
	...
	{ // TC - 7 - RoundChange
		...
		collector.Collect(context.Background(), roundChangeMsg)
...
func TestCommitteeDuty(t *testing.T) {
	...
	tracer := New(context.TODO(), logger, validators, nil, nil, networkconfig.TestNetwork.Beacon.GetBeaconNetwork(), true)
	...
	{ // TC 1 - process partial sig messages
		...
		partSigMsg := buildPartialSigMessage(identifier, partSigMessagesData)
		tracer.Collect(context.Background(), partSigMsg)
	...
	{ // TC 4 - Decided
		...
		tracer.Collect(context.Background(), decided)
	...
	{ // TC 6 - RoundChange
		...
		tracer.Collect(context.Background(), roundChangeMsg1)
	...
	{ // TC 7 - Second RoundChange
		...
		tracer.Collect(context.Background(), roundChangeMsg2)
...
func TestDutyTracer_SyncCommitteeRoots(t *testing.T) {
	collector := New(context.TODO(), zap.NewNop(), nil, mockclient{}, nil, networkconfig.TestNetwork.Beacon.GetBeaconNetwork(), true)
