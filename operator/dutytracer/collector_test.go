...
func TestValidatorDuty(t *testing.T) {
	...
	collector := New(t.Context(), logger, validators, nil, nil, networkconfig.TestBeaconConfig)
...
func TestCommitteeDuty(t *testing.T) {
	...
	tracer := New(t.Context(), logger, validators, nil, dutyStore, networkconfig.TestBeaconConfig)
...
func TestCollector_GetCommitteeDuty(t *testing.T) {
	...
	collector := New(t.Context(), zap.NewNop(), vstore, nil, dutyStore, networkconfig.TestBeaconConfig)
	committeeID := spectypes.CommitteeID{1}
	slot := phase0.Slot(10)
...
func TestDutyTracer_SyncCommitteeRoots(t *testing.T) {
	collector := New(t.Context(), zap.NewNop(), nil, mockclient{}, nil, networkconfig.TestBeaconConfig)
...
func TestCollector_getOrCreateCommitteeTrace(t *testing.T) {
	...
	_, vstore, _ := storage.NewSharesStorage(nil, db, nil)
	...
	t.Run("slot <= last evicted", func(t *testing.T) {
		...
		t.Run("getCommitteeDutyFromDisk errors", func(t *testing.T) {
			...
			collector := New(t.Context(), zap.NewNop(), vstore, nil, dutyTraceStore, networkconfig.TestBeaconConfig)
			collector.lastEvictedSlot.Store(uint64(evictionSlot))
...
func TestCollector_saveLateValidatorToCommiteeLinks(t *testing.T) {
	...
	t.Run("save late validator to committee links error", func(t *testing.T) {
		...
		logger := zap.New(core)
		collector := New(t.Context(), logger, vstore, nil, dutyStore, networkconfig.TestBeaconConfig)
