...
func TestHandleUnknownQuery(t *testing.T) {
	...
	nm := api2.NetworkMessage{
		Msg: api2.Message{
			Type:   "unknown_type",
			Filter: api2.MessageFilter{},
		},
		Err:  nil,
		Conn: nil,
	}
...
func TestHandleOperatorsQuery(t *testing.T) {
	...
	t.Run("query by index", func(t *testing.T) {
		nm := api2.NetworkMessage{
			Msg: api2.Message{
				Type:   api2.TypeOperator,
				Filter: api2.MessageFilter{From: 0, To: 1},
			},
			Err:  nil,
			Conn: nil,
		}
		handleOperatorsQuery(l, s, &nm)
		require.Equal(t, api2.TypeOperator, nm.Msg.Type)
	...
	t.Run("query non-existing index", func(t *testing.T) {
		nm := api2.NetworkMessage{
			Msg: api2.Message{
				Type:   api2.TypeOperator,
				Filter: api2.MessageFilter{From: 100, To: 101},
			},
			Err:  nil,
			Conn: nil,
		}
	...
	t.Run("query by pubKey", func(t *testing.T) {
		nm := api2.NetworkMessage{
			Msg: api2.Message{
				Type:   api2.TypeOperator,
				Filter: api2.MessageFilter{PublicKey: "03030303"},
			},
			Err:  nil,
			Conn: nil,
		}
...
func TestHandleValidatorsQuery(t *testing.T) {
	...
	t.Run("query by index", func(t *testing.T) {
		...
		require.Equal(t, api2.TypeValidator, nm.Msg.Type)
...
// TODO: un-lint
//nolint
func newDecidedAPIMsg(pk string, from, to int64) *api2.NetworkMessage {
	return &api2.NetworkMessage{
		Msg: api2.Message{
			Type: api2.TypeDecided,
			Filter: api2.MessageFilter{
				PublicKey: pk,
				From:      from,
				To:        to,
				Role:      api2.RoleAttester,
			},
		},
		Err:  nil,
		Conn: nil,
