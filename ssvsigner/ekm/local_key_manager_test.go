...
// TestRemoveSharePreservesSlashingProtection verifies that slashing protection data
// is preserved when a share is removed and re-added, preventing potential slashing events.
//
// Test flow:
//  1. Add a new validator share to the key manager
//  2. Sign an attestation and a block to establish slashing protection history
//  3. Verify the slashing protection data was properly recorded
//  4. Remove the share from the key manager
//  5. Verify the account is removed but slashing protection data remains in the database
//  6. Re-add the same share to simulate a validator being re-activated
//  7. Verify the previous slashing protection history is still intact
//  8. Attempt to sign the same attestation and block - both should fail due to slashing protection
//  9. Verify the re-added share can still sign new non-slashable messages
//
// This test ensures that removing a validator doesn't create a vulnerability where
// re-adding the same validator could lead to accidental double-signing and slashing.
func TestRemoveSharePreservesSlashingProtection(t *testing.T) {
	require.NoError(t, bls.Init(bls.BLS12_381))
	...
	sk := &bls.SecretKey{}
	sk.SetByCSPRNG()
	pubKey := phase0.BLSPubKey(sk.GetPublicKey().Serialize())

	encryptedPrivKey, err := operatorPrivateKey.Public().Encrypt([]byte(sk.SerializeToHexStr()))
	require.NoError(t, err)

	// Add the share
	require.NoError(t, km.AddShare(t.Context(), encryptedPrivKey, pubKey))

	// Sign an attestation to establish slashing protection history
	currentSlot := network.Beacon.EstimatedCurrentSlot()
	...
	// Sign a block to establish proposal history
	blockSlot := currentSlot + 10
	...
	// Verify slashing protection was recorded
	highestAtt, found, err := km.RetrieveHighestAttestation(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.NotNil(t, highestAtt)
	require.Equal(t, currentEpoch+1, highestAtt.Target.Epoch)

	highestProp, found, err := km.RetrieveHighestProposal(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, blockSlot, highestProp)

	// Remove the share
	require.NoError(t, km.RemoveShare(t.Context(), pubKey))

	// Verify account is removed
	accounts, err := km.(*LocalKeyManager).ListAccounts()
	require.NoError(t, err)
	for _, acc := range accounts {
		require.NotEqual(t, pubKey[:], acc.ValidatorPublicKey())
	}

	// Verify slashing protection history is PRESERVED
	highestAttAfterRemove, found, err := km.RetrieveHighestAttestation(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.NotNil(t, highestAttAfterRemove)
	require.Equal(t, highestAtt.Target.Epoch, highestAttAfterRemove.Target.Epoch)

	highestPropAfterRemove, found, err := km.RetrieveHighestProposal(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, highestProp, highestPropAfterRemove)

	// Re-add the same share
	require.NoError(t, km.AddShare(t.Context(), encryptedPrivKey, pubKey))

	// Verify slashing protection is still intact after re-adding
	highestAttAfterReAdd, found, err := km.RetrieveHighestAttestation(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.NotNil(t, highestAttAfterReAdd)
	require.Equal(t, highestAtt.Target.Epoch, highestAttAfterReAdd.Target.Epoch)

	highestPropAfterReAdd, found, err := km.RetrieveHighestProposal(pubKey)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, highestProp, highestPropAfterReAdd)

	// Try to sign the same attestation again - should fail due to slashing protection
	_, _, err = km.SignBeaconObject(
		t.Context(),
		attestationData,
		phase0.Domain{},
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "slashable attestation")

	// Try to sign the same block again - should fail due to slashing protection
	_, _, err = km.SignBeaconObject(
		t.Context(),
		block,
		phase0.Domain{},
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "slashable proposal")

	// Verify we can sign non-slashable messages
	newAttestationData := &phase0.AttestationData{
		Slot:            currentSlot + 100,
		Index:           1,
		BeaconBlockRoot: phase0.Root{10, 11, 12},
		Source: &phase0.Checkpoint{
			Epoch: currentEpoch + 1,
			Root:  phase0.Root{13, 14, 15},
		},
		Target: &phase0.Checkpoint{
			Epoch: currentEpoch + 2,
			Root:  phase0.Root{16, 17, 18},
		},
	}
