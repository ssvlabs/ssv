...
func (km *RemoteKeyManager) ArchiveSlashingProtection(r basedb.Reader, validatorPubKey []byte, sharePubKey []byte) error {
	if storage, ok := km.slashingProtector.(*SlashingProtector); ok {
		return storage.signerStore.ArchiveSlashingProtection(r, validatorPubKey, sharePubKey)
	}
...
func (km *RemoteKeyManager) applyArchivedSlashingProtectionInternal(store Storage, validatorPubKey []byte, sharePubKey phase0.BLSPubKey) error {
