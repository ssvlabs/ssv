...
func (gc *GoClient) GetValidatorData(
	...
	recordRequest(ctx, gc.log, "Validators", http.MethodPost, gc.multiClient.Address(), true, time.Since(reqStart), err)
	if err != nil {
...
func (gc *GoClient) SubmitValidatorRegistrations(ctx context.Context, registrations []*api.VersionedSignedValidatorRegistration) error {
	for chunk := range slices.Chunk(registrations, 500) {
		...
		recordRequest(ctx, gc.log, "SubmitValidatorRegistrations", http.MethodPost, gc.multiClient.Address(), true, time.Since(reqStart), err)
		if err != nil {
