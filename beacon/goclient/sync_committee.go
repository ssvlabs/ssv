...
func (gc *GoClient) SyncCommitteeDuties(ctx context.Context, epoch phase0.Epoch, validatorIndices []phase0.ValidatorIndex) ([]*eth2apiv1.SyncCommitteeDuty, error) {
	...
	recordRequest(ctx, gc.log, "SyncCommitteeDuties", http.MethodPost, gc.multiClient.Name(), true, time.Since(reqStart), err)
	if err != nil {
...
func (gc *GoClient) SubmitSyncMessages(ctx context.Context, msgs []*altair.SyncCommitteeMessage) error {
	...
	recordRequest(ctx, gc.log, "SubmitSyncCommitteeMessages", http.MethodPost, gc.multiClient.Name(), true, time.Since(reqStart), err)
	if err != nil {
