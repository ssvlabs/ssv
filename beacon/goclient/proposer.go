...
func (gc *GoClient) selectBestProposal(
	...
	case p := <-proposalsWithFeeRecipient:
		...
		return p, nil
	case p := <-proposals:
		// Got a proposal with no fee recipient. We'd prefer a proposal with fee recipient
		// but settle for the 1st one with no fee recipient otherwise.
	...
	// Fine, "soft deadline" has passed, and we still have no proposal. Any proposal will be
