...
func (gc *GoClient) fetchWeightedAttestationData(
	...
	recordRequest(ctx, logger, "AttestationData", client.Address(), http.MethodGet, false, time.Since(reqStart), err)
	if err != nil {
...
func (gc *GoClient) multiClientSubmit(
	...
	for _, client := range gc.clients {
		p.Go(func(ctx context.Context) error {
			...
			recordRequest(ctx, gc.log, routeName, client.Address(), http.MethodPost, false, time.Since(start), err)
			if err != nil {
