...
func Test_ApplyPinnedPeersSettings_PersistsAddrsAndProtects(t *testing.T) {
	h, err := libp2p.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = h.Close() })
	...
	n := &p2pNetwork{
		host:                  h,
		libConnManager:        h.ConnManager(),
		pinnedPeers:           hashmap.New[peer.ID, *peer.AddrInfo](),
		pinnedStabilizers:     hashmap.New[peer.ID, *time.Timer](),
		pinnedRedialScheduler: retry.NewScheduler(retry.BackoffConfig{Initial: 5 * time.Millisecond, Max: 5 * time.Millisecond}),
		logger:                zaptest.NewLogger(t),
	}
	t.Cleanup(func() { n.pinnedRedialScheduler.Close() })

	n.pinnedPeers.Set(id, &peer.AddrInfo{ID: id, Addrs: []ma.Multiaddr{addr}})
...
func Test_HandlePinnedConnected_SetsStabilizationTimer(t *testing.T) {
	id := genPeerID(t)
	n := &p2pNetwork{
		pinnedPeers:           hashmap.New[peer.ID, *peer.AddrInfo](),
		pinnedStabilizers:     hashmap.New[peer.ID, *time.Timer](),
	...
	n.handlePinnedConnected(id)
	if _, ok := n.pinnedStabilizers.Get(id); !ok {
		t.Fatalf("expected stabilization timer set for %s", id)
	}
}

func Test_HandlePinnedDisconnected_ClearsTimer(t *testing.T) {
	id := genPeerID(t)
	n := &p2pNetwork{
		pinnedPeers:           hashmap.New[peer.ID, *peer.AddrInfo](),
		pinnedStabilizers:     hashmap.New[peer.ID, *time.Timer](),
	...
	n.pinnedStabilizers.Set(id, tmr)

	n.handlePinnedDisconnected(id)
	if _, ok := n.pinnedStabilizers.Get(id); ok {
