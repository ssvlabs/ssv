...
func BenchmarkChoosePeersToTrim_150Peers(b *testing.B) {
	...
	for subnet := range topicCount {
		topicNames = append(topicNames, commons.Subnet(subnet).AlanTopic())
	}
	...
	for i := 0; i < peerCount; i++ {
		...
		subnet := commons.Subnet(i % topicCount)
		topicsByName[subnet.AlanTopic()] = append(topicsByName[subnet.AlanTopic()], peerID)
...
func subnetsFor(subnets ...uint64) commons.Subnets {
	...
	for _, subnet := range subnets {
		result.Set(commons.Subnet(subnet))
	}
	return result
}
