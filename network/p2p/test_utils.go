...
func (ln *LocalNet) WithBootnode(ctx context.Context, logger *zap.Logger) error {
	...
	bn, err := discovery.NewBootnode(ctx, logger, networkconfig.TestSSVConfig, &discovery.BootnodeOptions{
...
func (ln *LocalNet) NewTestP2pNetwork(ctx context.Context, nodeIndex uint64, keys testing.NodeKeys, logger *zap.Logger, options LocalNetOptions) (network.P2PNetwork, error) {
	...
	nodeStorage, err := storage.NewNodeStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), logger, db)
	if err != nil {
		return nil, err
	}
