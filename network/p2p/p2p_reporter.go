...
func (n *p2pNetwork) ReportValidation(msg *spectypes.SSVMessage, res protocolp2p.MsgValidationResult) {
	if !n.isReady() {
		return
	}
	...
	for _, pi := range peers {
		...
		if err != nil {
			n.logger.Warn("could not score peer", logging.PeerID(pi), zap.Error(err))
			continue
		}
	}
}
