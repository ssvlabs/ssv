...
func (n *p2pNetwork) applyPinnedPeersSettings() {
	...
	n.host.Network().Notify(&network.NotifyBundle{
		ConnectedF: func(_ network.Network, c network.Conn) {
			...
			// Stop current backoff timer but DO NOT reset attempts yet. Start a stabilization
			// window; only if the connection stays up beyond it do we reset the backoff.
			...
			t := time.AfterFunc(pinnedStabilizeWindow, func() {
				// If still connected after the window, reset backoff attempts.
		...
		DisconnectedF: func(_ network.Network, c network.Conn) {
			...
			// Cancel any stabilization timer for this peer; keep backoff attempts so
			// the next schedule continues backing off.
			if t, ok := n.pinnedStabilizers.Get(pid); ok && t != nil {
				t.Stop()
				n.pinnedStabilizers.Delete(pid)
			}
			// Schedule redial with exponential backoff until connected.
