...
func TestTopicScoreParams(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			func() *Options {
				validators := uint64(0)
				opts := NewSubnetTopicOpts(networkconfig.TestingNetworkConfig, validators, 128, []*storage.Committee{})
				return opts
			},
			nil,
		},
		{
			...
			func() *Options {
				...
				opts := NewSubnetTopicOpts(networkconfig.TestingNetworkConfig, validators, 128, createTestingSingleCommittees(validators))
				return opts
			},
			nil,
		},
...
func TestPeerScoreParams(t *testing.T) {
	peerScoreParams := PeerScoreParams(networkconfig.TestingNetworkConfig, 550*(time.Millisecond*700), false)
