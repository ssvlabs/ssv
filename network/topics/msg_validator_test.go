...
func TestMsgValidator(t *testing.T) {
	...
	ns, err := operatorstorage.NewNodeStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), logger, db)
	require.NoError(t, err)
	...
	slot := networkconfig.TestBeaconConfig.EstimatedCurrentSlot()
...
func dummySSVConsensusMsg(dutyExecutorID []byte, height qbft.Height) (*spectypes.SSVMessage, error) {
	id := spectypes.NewMsgID(networkconfig.TestSSVConfig.DomainType, dutyExecutorID, spectypes.RoleCommittee)
