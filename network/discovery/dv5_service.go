...
func (dvs *DiscV5Service) checkPeer(logger *zap.Logger, e PeerEvent) error {
	...
	if nodeDomainType != dvs.networkConfig.Domain {
...
func (dvs *DiscV5Service) createLocalNode(logger *zap.Logger, discOpts *Options, ipAddr net.IP) (*enode.LocalNode, error) {
	...
	err = DecorateNode(
		...
		DecorateWithDomainType(dvs.networkConfig.Domain),
	...
	logger.Debug("node record is ready", fields.ENRLocalNode(localNode), fields.Domain(dvs.networkConfig.Domain), fields.Subnets(opts.Subnets))
