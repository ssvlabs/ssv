...
if [[ -d .k8/yamls-v3-prod/ ]]; then
  for file in $(ls -A1 .k8/yamls-v3/); do
   sed -i -e "s|REPLACE_NAMESPACE|${NAMESPACE}|g" \
	  ...
	  -e "s|REPLACE_IMAGETAG|${IMAGETAG}|g" ".k8/yamls-v3-prod/${file}" || exit 1
  done
fi
...
#deploy
kubectl --context=$K8S_CONTEXT apply -f .k8/yamls-v3-prod/ssv-node-1-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/yamls-v3-prod/ssv-node-2-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/yamls-v3-prod/ssv-node-3-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/yamls-v3-prod/ssv-node-4-deployment.yml || exit 1
