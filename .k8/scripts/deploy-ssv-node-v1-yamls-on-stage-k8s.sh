...
if [[ -d .k8/yamls-stage/ ]]; then
  for file in $(ls -A1 .k8/yamls-stage-v1/); do
   sed -i -e "s|REPLACE_NAMESPACE|${NAMESPACE}|g" \
	  ...
	  -e "s|REPLACE_IMAGETAG|${IMAGETAG}|g" ".k8/yamls-stage-v1/${file}" || exit 1
  done
fi
...
#deploy
kubectl --context=$K8S_CONTEXT apply -f .k8/yamls-stage-v1/ssv-node-v2-1-deployment.yml || exit 1
