...
if [[ -d .k8/production/prater/ ]]; then
  for file in $(ls -A1 .k8/production/prater/); do
   sed -i -e "s|REPLACE_NAMESPACE|${NAMESPACE}|g" \
	  ...
	  -e "s|REPLACE_IMAGETAG|${IMAGETAG}|g" ".k8/production/prater/${file}" || exit 1
  done
fi
...
#deploy
kubectl --context=$K8S_CONTEXT apply -f .k8/production/prater/ssv-node-prater-1-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/production/prater/ssv-node-prater-2-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/production/prater/ssv-node-prater-3-deployment.yml || exit 1
kubectl --context=$K8S_CONTEXT apply -f .k8/production/prater/ssv-node-prater-4-deployment.yml || exit 1
