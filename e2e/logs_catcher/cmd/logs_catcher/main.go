...
func main() {
	...
	flag.Var(Approvers, "approver", " Logs to collect for approval on, format as json fields { 'message': 'good attestation', 'slot': 1 }")
	...
	cfg.ApproverFunc = logs_catcher.DefaultApprover(len(allDockers)) // todo should probably make sure its one for each docker
