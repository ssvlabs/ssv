...
// F1MultiRoundScenario is the f1 speedup scenario name
const F1MultiRoundScenario = "F1MultiRound"
...
func (r *f1MultiRoundScenario) PostExecution(ctx *runner.ScenarioContext) error {
	for i := range ctx.Stores[:len(ctx.Stores)-1] {
		msgs, err := ctx.Stores[i].GetDecided(message.NewIdentifier(r.share.PublicKey.Serialize(), spectypes.BNRoleAttester), specqbft.Height(1), specqbft.Height(4))
		if err != nil {
			return err
		}
		if len(msgs) < 1 {
