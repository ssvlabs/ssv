...
// ChangeRoundSpeedupScenario is the change round speedup scenario name
const ChangeRoundSpeedupScenario = "ChangeRoundSpeedup"
...
func (r *changeRoundSpeedupScenario) PostExecution(ctx *runner.ScenarioContext) error {
	...
	for i := range ctx.Stores[:len(ctx.Stores)-1] {
		msgs, err := ctx.Stores[i].GetDecided(message.NewIdentifier(r.share.PublicKey.Serialize(), spectypes.BNRoleAttester), specqbft.Height(1), specqbft.Height(1))
		if err != nil {
			return err
		}
		if len(msgs) < 1 {
