...
func TestMigration6ExitEpochField(t *testing.T) {
	...
	storageSharesKey := storage.SharesDBPrefix(opstorage.OperatorStoragePrefix)
	...
	t.Run("successfully migrates all shares", func(t *testing.T) {
		...
		seededShares, err := seedDatabase(initialDBCapacity, options.Db, storageSharesKey)
		require.NoError(t, err)
		...
		err = options.Db.GetAll(storageSharesKey, func(i int, o basedb.Obj) error {
			share := &storage.Share{}
		...
		for _, seededShare := range seededShares {
			...
			if !found {
				t.Fatalf("one of the seeded share was not found after migration")
			}
		}
	})
}
