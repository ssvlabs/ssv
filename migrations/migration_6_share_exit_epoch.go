...
var migration_6_share_exit_epoch = Migration{
	...
	Run: func(ctx context.Context, l *zap.Logger, opt Options, key []byte, completed CompletedFunc) error {
		var (
			storagePrefix   = storage.SharesDBPrefix(opstorage.OperatorStoragePrefix) //TODO: figure out if that's correct prefix
			oldShares       = make(map[string]*migration_6_OldStorageShare)
			sharesToPersist []basedb.Obj
		)
		logger := l.
			With(zap.String("migration_name", migration_6_Name)).
			With(zap.String("storage_prefix", string(storagePrefix)))

		logger.Debug("fetching and converting all shares that needs to be migrated")
		...
		err = opt.Db.SetMany(storagePrefix, len(sharesToPersist), func(i int) (basedb.Obj, error) {
