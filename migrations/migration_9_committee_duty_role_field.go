...
func convertSignerDataV1(in []*migration_9_SignerData) []*exporter.SignerData {
	out := make([]*exporter.SignerData, 0, len(in))
	for _, item := range in {
		if item == nil {
			continue
		}
		...
		out = append(out, &exporter.SignerData{
			Signer:       item.Signer,
...
func convertDecidedsV1(in []*migration_9_DecidedTrace) []*exporter.DecidedTrace {
	out := make([]*exporter.DecidedTrace, 0, len(in))
	for _, item := range in {
		if item == nil {
			continue
		}
		signers := convertOperatorIDsV1(item.Signers)
		out = append(out, &exporter.DecidedTrace{
			Round:        item.Round,
...
func convertQBFTTraceV1(in *migration_9_QBFTTrace) *exporter.QBFTTrace {
	if in == nil {
		return nil
	}
	return &exporter.QBFTTrace{
		Round:        in.Round,
...
func convertQBFTTracesV1(in []*migration_9_QBFTTrace) []*exporter.QBFTTrace {
	out := make([]*exporter.QBFTTrace, 0, len(in))
	for _, item := range in {
		if item == nil {
			continue
		}
...
func convertRoundChangeV1(in *migration_9_RoundChangeTrace) *exporter.RoundChangeTrace {
	if in == nil {
		return nil
	}
...
func convertRoundChangesV1(in []*migration_9_RoundChangeTrace) []*exporter.RoundChangeTrace {
	out := make([]*exporter.RoundChangeTrace, 0, len(in))
	for _, item := range in {
		if item == nil {
			continue
		}
...
func convertProposalTraceV1(in *migration_9_ProposalTrace) *exporter.ProposalTrace {
	if in == nil {
		return nil
	}
...
func convertRoundTraceV1(in *migration_9_RoundTrace) *exporter.RoundTrace {
	if in == nil {
		return nil
	}
	return &exporter.RoundTrace{
...
func convertRoundsV1(in []*migration_9_RoundTrace) []*exporter.RoundTrace {
	out := make([]*exporter.RoundTrace, 0, len(in))
	for _, item := range in {
		if item == nil {
			continue
		}
