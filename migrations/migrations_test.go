...
func Test_RunNotMigratingTwice(t *testing.T) {
	...
	require.NoError(t, migrations.Run(ctx, logger, opt))
	require.NoError(t, migrations.Run(ctx, logger, opt))
...
func Test_Rollback(t *testing.T) {
	...
	err = Migrations{fakeMigration(migrationKey, fakeError)}.Run(ctx, logger, opt)
	...
	err = Migrations{fakeMigration(migrationKey, nil)}.Run(ctx, logger, opt)
	require.NoError(t, err)
...
func Test_NextMigrationNotExecutedOnFailure(t *testing.T) {
	...
	err = migrations.Run(ctx, logger, opt)
	require.Error(t, err)
