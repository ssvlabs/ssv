...
// peekPeer returns a copy of the state recorded for messages from peerID, without adding one: an unknown peer
// reads as an empty state, so a message that fails verification leaves no trace here.
