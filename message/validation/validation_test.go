...
func Test_ValidateSSVMessage(t *testing.T) {
	...
	t.Run("happy flow", func(t *testing.T) {
		validator := NewMessageValidator(netCfg, WithShareStorage(ns.Shares())).(*messageValidator)
	...
	t.Run("no proposal duties", func(t *testing.T) {
		...
		validator := NewMessageValidator(netCfg, WithShareStorage(ns.Shares()), WithDutyStore(dutyStore)).(*messageValidator)
...
func BenchmarkHerumiBLSVerify(b *testing.B) {
...
func BenchmarkVerifyPKCS1v15Fast(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
