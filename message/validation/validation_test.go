...
func Test_ValidateSSVMessage(t *testing.T) {
	...
	t.Run("message counts", func(t *testing.T) {
		...
		signedSSVMessage.FullData = nil
		_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, receivedAt.Add(netCfg.BeaconConfig.SlotDuration()))
		require.NoError(t, err)
	...
	t.Run("too many duties", func(t *testing.T) {
		...
		// First duty.
		topicID := commons.CommitteeTopicID(committeeID)[0]
		_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, netCfg.BeaconConfig.GetSlotStartTime(slot))
		require.NoError(t, err)

		// Second duty.
		signedSSVMessage = generateSignedMessage(ks, identifier, slot+4)
		_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, netCfg.BeaconConfig.GetSlotStartTime(slot+4))
		require.NoError(t, err)
		...
		_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, netCfg.BeaconConfig.GetSlotStartTime(slot+8))
		require.NoError(t, err)
			...
			for role, msgTypes := range tests {
				for _, msgType := range msgTypes {
					...
					t.Run(subtestName, func(t *testing.T) {
						...
						receivedAt := netCfg.BeaconConfig.GetSlotStartTime(spectestingutils.TestingDutySlot)
	...
	t.Run("late message", func(t *testing.T) {
		...
		tests := map[spectypes.RunnerRole]time.Time{
			spectypes.RoleCommittee:                 netCfg.BeaconConfig.GetSlotStartTime(slot + 35),
			spectypes.RoleAggregator:                netCfg.BeaconConfig.GetSlotStartTime(slot + 35),
			spectypes.RoleProposer:                  netCfg.BeaconConfig.GetSlotStartTime(slot + 4),
			spectypes.RoleSyncCommitteeContribution: netCfg.BeaconConfig.GetSlotStartTime(slot + 4),
		}
	...
	t.Run("round already advanced", func(t *testing.T) {
		...
		receivedAt := netCfg.BeaconConfig.GetSlotStartTime(slot).Add(5 * roundtimer.QuickTimeout)
		topicID := commons.CommitteeTopicID(spectypes.CommitteeID(signedSSVMessage.SSVMessage.GetID().GetDutyExecutorID()[16:]))[0]
		_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, receivedAt)
		require.NoError(t, err)
	...
	t.Run("round too high", func(t *testing.T) {
		...
		for role, round := range tests {
			...
			t.Run(message.RunnerRoleToString(role), func(t *testing.T) {
				...
				receivedAt := netCfg.BeaconConfig.GetSlotStartTime(slot).Add(sinceSlotStart)
				_, err = validator.handleSignedSSVMessage(signedSSVMessage, topicID, receivedAt)
	...
	// Receive an unknown message type from an operator that is not myself should receive an error
