...
func TestMessageValidator_currentEstimatedRound(t *testing.T) {
	...
	tt := []struct {
		...
		{
			name:         "DefaultProposerQuickTimeout/2 - expected first round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout / 2,
			want:         specqbft.FirstRound,
		},
		{
			name:         "DefaultProposerQuickTimeout - expected first+1 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout,
			want:         specqbft.FirstRound + 1,
		},
		{
			name:         "DefaultProposerQuickTimeout*2 - expected first+2 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 2,
			want:         specqbft.FirstRound + 2,
		},
		{
			name:         "DefaultProposerQuickTimeout*3 - expected first+3 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 3,
			want:         specqbft.FirstRound + 3,
		},
		{
			name:         "DefaultProposerQuickTimeout*4 - expected first+4 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 4,
			want:         specqbft.FirstRound + 4,
		},
		{
			name:         "DefaultProposerQuickTimeout*5 - expected first+5 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 5,
			want:         specqbft.FirstRound + 5,
		},
		{
			name:         "DefaultProposerQuickTimeout*6 - expected first+6 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 6,
			want:         specqbft.FirstRound + 6,
		},
		{
			name:         "DefaultProposerQuickTimeout*7 - expected first+7 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 7,
			want:         specqbft.FirstRound + 7,
		},
		{
			name:         "DefaultProposerQuickTimeout*8 - expected first+8 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * 8,
			want:         specqbft.FirstRound + 8,
		},
		{
			name:         "DefaultProposerQuickTimeout*9 - expected first+8 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * time.Duration(roundtimer.QuickTimeoutThreshold+1),
			want:         roundtimer.QuickTimeoutThreshold + 1,
		},
		{
			name:         "DefaultProposerQuickTimeout*10 - expected first+8 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout * time.Duration(roundtimer.QuickTimeoutThreshold+2),
			want:         roundtimer.QuickTimeoutThreshold + 1,
		},
		{
			name:         "(DefaultProposerQuickTimeout*8 + SlowTimeout) - expected first+9 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout*time.Duration(roundtimer.QuickTimeoutThreshold) + roundtimer.SlowTimeout,
			want:         roundtimer.QuickTimeoutThreshold + 2,
		},
		{
			name:         "(DefaultProposerQuickTimeout*8 + SlowTimeout*2) - expected first+10 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout*time.Duration(roundtimer.QuickTimeoutThreshold) + roundtimer.SlowTimeout*2,
			want:         roundtimer.QuickTimeoutThreshold + 3,
		},
		{
			name:         "(DefaultProposerQuickTimeout*8 + SlowTimeout*3) - expected first+11 round",
			role:         spectypes.RoleProposer,
			timeIntoSlot: roundtimer.DefaultProposerQuickTimeout*time.Duration(roundtimer.QuickTimeoutThreshold) + roundtimer.SlowTimeout*3,
