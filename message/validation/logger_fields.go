...
func (mv *messageValidator) addDutyIDField(lf *LoggerFields) {
	if lf.Role == spectypes.RoleCommittee {
		...
		if ok {
			lf.DutyID = fields.FormatCommitteeDutyID(c.Operators, mv.netCfg.BeaconConfig.EstimatedEpochAtSlot(lf.Slot), lf.Slot)
		}
	} else {
		// get the validator index from the msgid
		v, ok := mv.validatorStore.Validator(lf.DutyExecutorID)
		if ok {
			lf.DutyID = fields.FormatDutyID(mv.netCfg.BeaconConfig.EstimatedEpochAtSlot(lf.Slot), lf.Slot, lf.Role.String(), v.ValidatorIndex)
		}
	}
}
