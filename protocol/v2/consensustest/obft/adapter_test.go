...
// TestAdapter_BFTStart_BoundaryBehavior pins the BFTStart-aware
// broadcast-schedule clamp `T_broadcast_max_k = max(BFTStart, T_commit
// − B_k)` per the spec at obft/base/types.go §Setting. Three regimes:
//
//  1. BFTStart well below the L_0 fetch-clamp boundary
//     (`T_commit − B_0 − fetchBuffer` ≈ 3475ms at BTT=100ms, K=4)
//     — clamp dormant; schedule and decision time bit-identical to
//     BFTStart=0. Underpins the UI's close-to-ground-truth
//     approximation for picker values ≤ BFT_START_APPROX_BOUNDARY.
//  2. BFTStart above the L_0 fetch-clamp boundary but below T_commit
//     — clamp engages; the primary's broadcast deadline floors at
//     BFTStart (and backups, whose `B_k = T_commit`, always floor to
//     BFTStart). Cluster still decides when the compressed window
//     fits propagation.
//  3. BFTStart ≥ T_commit — every layer's deadline collapses past the
//     σ-or-NR commit point; cluster MISSes with a recorded reason.
func TestAdapter_BFTStart_BoundaryBehavior(t *testing.T) {
	...
	// tCommit = 4000 − 100 − 50 − 50 − 100 = 3700ms; B_0 = 2·BTT = 200ms;
	// fetchBuffer = BTT/4 = 25ms (default, MaxMEVFetch=false). The
	// L_0 fetch-clamp engages at BFTStart > tCommit − B_0 − fetchBuffer
	// = 3475ms; the DecidingBroadcastTime clamp engages at BFTStart >
	// tCommit − B_0 = 3500ms (no fetchBuffer in that path).

	// Case 1: BFTStart well below both clamp boundaries — bit-identical
	// to BFT=0 (covers the UI's [0, 1600] approximation range and then
	// some).
	cfg0 := baseCfg(0)
	out0, err := obftadapter.Protocol{}.Run(cfg0)
	require.NoError(t, err)
	require.True(t, out0.Decided, "BFTStart=0 must decide at healthy mesh")

	for _, bft := range []time.Duration{500, 1600, 2800, 3400} {
		cfgLow := baseCfg(bft * time.Millisecond)
		outLow, err := obftadapter.Protocol{}.Run(cfgLow)
		...
		require.Equalf(t, out0.DecisionTime, outLow.DecisionTime,
			"BFTStart=%dms < clamp boundary must produce bit-identical DecisionTime (schedule unchanged)", bft)
		require.Equalf(t, out0.DecidingBroadcastTime, outLow.DecidingBroadcastTime,
			"BFTStart=%dms < clamp boundary must produce bit-identical DecidingBroadcastTime (clamp dormant)", bft)
	}

	// Case 2: BFTStart above the L_0 clamp boundary but below T_commit
	// — clamp engages; DecidingBroadcastTime floors to BFTStart;
	// cluster still decides because the compressed window (T_commit −
	// BFTStart = 100ms) fits one BTT of prod-profile propagation.
	cfgClamped := baseCfg(3600 * time.Millisecond)
	outClamped, err := obftadapter.Protocol{}.Run(cfgClamped)
	require.NoError(t, err)
	require.True(t, outClamped.Decided,
		"BFTStart=3600ms (clamp engaged, 100ms residual window) must still decide on prod profile")
	require.GreaterOrEqual(t, outClamped.DecidingBroadcastTime, 3600*time.Millisecond,
		"BFTStart=3600ms must floor DecidingBroadcastTime to ≥ BFTStart per the spec clamp")

	// Case 3: BFTStart ≥ T_commit — no room for any broadcast before
	// the σ-or-NR commit point; cluster MISSes.
	cfgPastCommit := baseCfg(4000 * time.Millisecond)
	outPastCommit, err := obftadapter.Protocol{}.Run(cfgPastCommit)
	require.NoError(t, err)
	require.False(t, outPastCommit.Decided,
		"BFTStart=4000ms ≥ T_commit must MISS (no broadcast window before commit)")
	require.NotEmpty(t, outPastCommit.MissReason,
...
func TestAdapter_NoRefloodDelay_VariantBehavior(t *testing.T) {
	...
	cfgAdv := ct.DefaultProposerDutyConfig(200 * time.Millisecond)
	cfgAdv.Byz = ct.ByzPattern{Kind: ct.ByzSigmaRefusal, ByzOperators: []ct.OperatorID{4}}
	// cfgAdv.RefloodDelay stays at the zero default — adversarial
	// scenarios never opt into the Healthy 700ms cushion.
