...
func p2pBaselineSweep(scenarios []Scenario, protocols []Protocol, iters Iterations, n, k int, profiles []string) Sweep {
	...
	return Sweep{
		...
		Description: "Calibrated empirical baseline across (n, K, BTT, profile, instability). Profile selects a per-hop latency mixture fitted to real SSV gossipsub telemetry: `prod` / `stage1` / `stage2` are mainnet + staging clusters; `slow`, `heavy_tail`, `slow_heavy_tail` are derived from prod (latency ×80 / outlier frequency ×24 / both). Per-hop ≈ cluster-wide at n=4 (cluster ops typically share a gossipsub mesh), so cfg.Network and cfg.Mesh.HopDelay both use the selected profile; larger-n cells are an extrapolation. Important: under empirical profiles, the BTT axis is a PROTOCOL-BUDGET axis, not a network-speed axis — the network model is the profile (≈ 1-10 ms in prod), while BTT (100-500 ms here) drives the protocol's internal timing budgets. Framework-side budgets per consensustest's intentional conservatism: OBFT Δ_2 = 2·BTT (vs tightened spec recommendation 1·BTT); OBFT primary B_0 = 2·BTT + scenario-set RefloodDelay (Healthy opts into 700ms, matching SSV's libp2p heartbeat; adversarial scenarios keep the default 0); QBFT computed-RT phase budget = 1·BTT post-tightening (RT = 6×phaseBudget = 6·BTT); 2abOBFT Δ_2a = Δ_2b = 2·BTT framework-side (vs tightened spec Δ_2b = 1·BTT + ε_proc). The instability axis applies only to Baseline-group scenarios (Healthy); non-Baseline rows show their level=none stats regardless of picker. Each `make stresstest` run contributes one (n, K) slice; reruns compose into the same data.js.",
