...
// p2pBaselineSweepDescription is the report-UI tooltip for the
// p2p_baseline sweep. Broken into prose chunks via Go string
// concatenation; the runtime value is one continuous string (no
// embedded newlines), so the JSON payload and report UI see exactly
...
const p2pBaselineSweepDescription = "" +
	...
	"Framework-side budgets (spec-aligned, post-tightening): OBFT Δ_2 = 1·BTT (spec recommendation, reflood absorbed by B_0); OBFT/2abOBFT/QBFT each sweep a fixed cushion ladder X-0/X-300/X-500/X-700 (SafetyBuffer folded into OBFT's B_0 = 2·BTT + cushion and into 2abOBFT's resolveBudget; per-round RT cushion in QBFT) — the non-700 rungs run on Baseline (Healthy) scenarios only, while adversarial scenarios run just the canonical X-700 rung per family; QBFT pristine per-round RT = 3·BTT (R1) / 4·BTT (R≥2), QBFT-SSV flat 2s; 2abOBFT Δ_2a = 2·BTT structural minimum, Δ_2b = 1·BTT spec-aligned (Phase-2 total 3·BTT). " +
	"Every cell runs at BFT_start=0; the report UI derives later BFT_start values post-hoc. " +
	"The instability axis applies only to Baseline-group scenarios (Healthy); non-Baseline rows show their level=none stats regardless of picker. " +
	"Each `make stresstest` run contributes one (n, K) slice; reruns compose into the same data.js."
...
func p2pBaselineSweep(scenarios []Scenario, protocols []Protocol, iters Iterations, n, k int, profiles []string, bttValues []time.Duration) Sweep {
	...
	return Sweep{
		...
		Description: p2pBaselineSweepDescription,
