...
// L_0 (see ValueMsg.L0Partial); NoValueMsg carries no L_0 partial. Commit
// at Phase 2b carries only the NR-direction partial in one of two sides:
//
...
//     fires (the NoValue→Commit-NR sequence).
...
//     ValueMsg/NoValueMsg entirely (the Commit-NRDirect-alone sequence).
//
// The wire kind set has no "Signed" side — KindValue itself IS the σ-side
// terminal emission at L_0. The 0x01 CommitSide value is reserved/removed;
// receivers MUST reject any commit carrying it at validation time.
//
...
const (
	...
	// 0x01 is reserved and carries no σ-side meaning — KindValue carries the
	// σ partial directly, so there is no "Signed" commit side. Decoders MUST
	// reject 0x01 to surface cluster-wide wire-version drift loudly (do not
	// reuse 0x01 for a new constant).
...
// or Phase-2a NRDirect). All valid Commit sides ARE NR-direction — but the
// method is kept for call-site clarity vs IsNRDirect.
...
// σ-pool[V_Layer][leader] with the leader's head-start partial.
//
...
// (where that colocated V is present), per the "root plus σ-colocation"
...
// ValueMsg is the σ-direction Phase-2 emission at L_0 — the TERMINAL
// σ-side emission at L_0. KindValue itself carries the emitter's plaintext
// σ partial on V (the L0Partial field): receivers observing KindValue
// immediately verify+pool the σ partial into σ-pool[V_0_root], so the
// cluster reaches σ-quorum in 1·BTT post-emission. There is no separate
// later σ commit — KindValue is the operator's entire σ-side contribution.
//
// Emission paths:
...
//     σ-lock at L_0 on V at this moment.
//   - Upgrade path (NoValueMsg-path op later receives V + host valid) →
//     emit upgrade KindValue with same wire shape. The upgrade KindValue
//     IS the σ-commit; there is no follow-up σ message.
//
// **V binding to leader**: the Witnesses field carries forwarded leader
// σ-witnesses, one per layer the emitter is σ-side on. The Layer-0 entry
// is the L_0 leader's σ partial on this KindValue's V (the peer-reflood-V
// vector); deeper-layer entries carry the L_k leader's σ partial on V_k,
// with V_k riding in this op's SigmaChained LayerEntry at that layer (root
// plus σ-colocation — see LayerWitness). Receivers verify each witness
// against the layer-leader's pubKeyShare on the colocated V; on success V
// is harvested into retention as if the receiver had observed that leader's
// Phase-1 bundle directly (enabling peer-reflood-V as the
// HV1SelectiveDelivery recovery vector at every fall-through layer). On
// verify failure that witness is silently dropped (anti-leader-framing —
// the emitter's envelope doesn't cryptographically attribute the bytes to
// the leader, so Rule 5 against the leader would be unsound). The
// leader-signed-garbage attack stays covered in ObservePhase1Bundle, where
// the bundle's outer envelope binds the bytes to the leader.
//
// **L0Partial verify failure** fires Rule 5 against the EMITTER (not the
// leader): the L0Partial is the emitter's OWN signing artifact, verifiable
// against pubKeyShares[v.OperatorID] on v.V. A failed verify is unambiguous
// emitter misbehavior — by BLS-share unforgeability the emitter either
// presented random bytes or deliberately signed garbage. No framing-attack
// symmetry with the forwarded Witnesses.
type ValueMsg struct {
	...
	ValueRoot [32]byte
	// Witnesses carries forwarded leader σ-witnesses: one LayerWitness
	...
	// Required and non-empty. Receivers verify on observation and pool into
	// σ-pool[L_0][ValueRoot(V)][emitter] on success; verify failure fires
	// Rule 5 (fake plaintext σ) against the emitter (NOT the leader — see
	// Witnesses vs L0Partial attribution distinction in the type-level
	// docstring above).
	//
	// EKM lock at L_0 is acquired when L0Partial is signed (at KindValue
	// emit time, ~1·BTT into the slot, enlarging the crash-loss window).
	// The structural cost is that an operator that emits KindValue with its
	// σ partial cannot later pivot to KindCommit-NR (σ-XOR-NR). See
	// docs/2abOBFT.md §Phase 2a.
	L0Partial Signature
...
// op later emits a ValueMsg upgrade (the NoValue→Value sequence), the
// receiver moves that op from novalue_pool to value_pool.
...
// Commit is the Phase-2b binding envelope. Commit is NR-side only —
// KindValue carries the σ partial directly at Phase 2a, so there is no
// "Signed" commit. Each operator emits at most one Commit per (slot); at
// L_k>0 the per-layer commitment is already on the wire from Phase 2a (in
...
//     this is a Phase-2b NR commit following an earlier KindNoValue — the
...
//     Phase 2a (equivocation observed pre-emission — the
//     Commit-NRDirect-alone sequence). This is the only Commit kind that
//     carries LayerEntries.
//
...
// IBE-share level. The σ-lock is acquired at KindValue emit time; a bug
// that requested σ-then-NR or NR-then-σ at the same layer is caught by
// transitionToSigma / transitionToNR in the Instance before any wire bytes
// leave the build path.
type Commit struct {
	ClusterID  [32]byte
	OperatorID OperatorID
	Height     Height
	// Side discriminates the L_0 commitment shape. Valid values are NR /
	// NRDirect only — there is no Signed side.
	Side CommitSide
...
// (§Phase 2a Upgrade in docs/2abOBFT.md).
