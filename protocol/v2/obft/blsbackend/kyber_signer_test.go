...
func kyberSignFor(t *testing.T, ks *kyberKeyset, id uint64, msg []byte) obft.Signature {
	t.Helper()
...
func TestKyberSigner_RoundTrip_n7(t *testing.T) {
	...
	for id := uint64(1); id <= uint64(ks.q); id++ {
		partials[obft.OperatorID(id)] = kyberSignFor(t, ks, id, msg)
	}
...
func TestKyberSigner_AnyQuorumSubsetYieldsSameAggregate(t *testing.T) {
	...
	all := make(map[obft.OperatorID]obft.Signature, ks.n)
	for id := uint64(1); id <= uint64(ks.n); id++ {
		all[obft.OperatorID(id)] = kyberSignFor(t, ks, id, msg)
	}
