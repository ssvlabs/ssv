...
func kyberSignTag(t *testing.T, shares map[uint64]*bls.SecretKey, id uint64, tag []byte) obft.Signature {
	t.Helper()
...
func TestTLockIBE_RoundTripWithHerumiShares(t *testing.T) {
	...
	for id := uint64(1); id <= q; id++ {
		partials[obft.OperatorID(id)] = kyberSignTag(t, shares, id, tag)
	}
...
func TestTLockIBE_DifferentSubsetsDecryptIdentically(t *testing.T) {
	...
	partials := make(map[obft.OperatorID]obft.Signature, n)
...
func TestTLockIBE_WrongTagKeyFails(t *testing.T) {
	...
	partials := make(map[obft.OperatorID]obft.Signature, q)
	for id := uint64(1); id <= q; id++ {
		partials[obft.OperatorID(id)] = kyberSignTag(t, shares, id, []byte("wrong-tag"))
	}
