...
func (k *keyset) signWith(t *testing.T, id uint64, msg []byte) obft.Signature {
	t.Helper()
	share := k.shares[id]
...
func TestSigner_RoundTrip_n7(t *testing.T) {
	...
	// Operators 1..q sign.
	partials := make(map[obft.OperatorID]obft.Signature, ks.q)
	for id := uint64(1); id <= uint64(ks.q); id++ {
		partials[obft.OperatorID(id)] = ks.signWith(t, id, msg)
	}
...
func TestSigner_AnyQuorumSubsetYieldsSameAggregate(t *testing.T) {
	...
	allPartials := make(map[obft.OperatorID]obft.Signature, ks.n)
	for id := uint64(1); id <= uint64(ks.n); id++ {
		allPartials[obft.OperatorID(id)] = ks.signWith(t, id, msg)
	}
...
func TestSigner_AggregateRejectsCrossMessagePartials(t *testing.T) {
	...
	partials := map[obft.OperatorID]obft.Signature{
		obft.OperatorID(1): ks.signWith(t, 1, msgA),
		obft.OperatorID(2): ks.signWith(t, 2, msgA),
		obft.OperatorID(3): ks.signWith(t, 3, msgB), // wrong message
	}
...
func TestSigner_EmptyInputs(t *testing.T) {
	...
	_, err = verifier.AggregatePartials(map[obft.OperatorID]obft.Signature{})
	require.Error(t, err)
}

// ---- helpers -------------------------------------------------------------

func mapSubset(m map[obft.OperatorID]obft.Signature, ids ...uint64) map[obft.OperatorID]obft.Signature {
	out := make(map[obft.OperatorID]obft.Signature, len(ids))
	for _, id := range ids {
		opID := obft.OperatorID(id)
		v, ok := m[opID]
		if !ok {
