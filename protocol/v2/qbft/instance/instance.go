	...
	i.startOnce.Do(func() {
		...
		if proposerID == i.State.Share.OperatorID {
			...
			if err != nil {
				...
				// nolint
				logger = logger.With(fields.Root(proposalMsg.Root))
...
func allSigners(all []*spectypes.SignedSSVMessage) []spectypes.OperatorID {
	...
	for _, m := range all {
		signers = append(signers, m.GetOperatorIDs()...)
	}
	return signers
}
...
func (i *Instance) BaseMsgValidation(signedMsg *spectypes.SignedSSVMessage) error {
	...
	case specqbft.RoundChangeMsgType:
		return validRoundChangeForData(i.State, i.config, signedMsg, i.State.Height, msg.Round, signedMsg.FullData)
	default:
		return errors.New("signed message type not supported")
	}
}
