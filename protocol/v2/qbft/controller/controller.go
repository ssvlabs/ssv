...
type Controller struct {
	Identifier func() spectypes.DomainType `json:"-"`
...
func (c *Controller) BaseMsgValidation(msg *specqbft.ProcessingMessage) error {
	...
	if !bytes.Equal(c.GetIdentifier(), msg.QBFTMessage.Identifier) {
...
func (c *Controller) InstanceForHeight(logger *zap.Logger, height specqbft.Height) *instance.Instance {
	...
	storedInst, err := c.config.GetStorage().GetInstance(c.GetIdentifier(), height)
	if err != nil {
	...
	inst := instance.NewInstance(c.config, c.CommitteeMember, c.GetIdentifier(), storedInst.State.Height, c.OperatorSigner)
...
func (c *Controller) addAndStoreNewInstance() *instance.Instance {
	i := instance.NewInstance(c.GetConfig(), c.CommitteeMember, c.GetIdentifier(), c.Height, c.OperatorSigner)
