...
// BuilderPreferencesEntry is one entry in the beacon-APIs#630 submitBuilderPreferences body: the
// ahead-of-time per-builder preference a proposer asks its beacon node to forward. The beacon node
// routes it by URL to that builder's submitBuilderPreferences for ProposerPubKey, so the builder holds
// MaxExecutionPayment (authenticated by Auth) before the bid request arrives. JSON-encoded on the wire,
// with uint64 as a decimal string per the beacon-API convention.
