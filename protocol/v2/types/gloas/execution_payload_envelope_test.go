...
// SignedExecutionPayloadEnvelope.Blinded carries the signature unchanged onto the blinded envelope (whose
// message root equals the full one's), and the resulting signed blinded envelope — the §6 publication body
// on the blinded path — round-trips through SSZ.
func TestSignedExecutionPayloadEnvelopeBlindedRoundTrip(t *testing.T) {
	...
	signedBlinded, err := full.Blinded()
	require.NoError(t, err)
	require.Equal(t, full.Signature, signedBlinded.Signature, "signature must be carried unchanged")

	// The blinded message hashes to the same root as the full envelope, so the carried signature is valid.
	fullMsgRoot, err := full.Message.HashTreeRoot()
	require.NoError(t, err)
	blindedMsgRoot, err := signedBlinded.Message.HashTreeRoot()
	require.NoError(t, err)
	require.Equal(t, fullMsgRoot, blindedMsgRoot)
