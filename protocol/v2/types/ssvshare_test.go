...
func TestSSVShare_IsParticipating(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			result := tc.Share.IsParticipating(networkconfig.TestNetwork.BeaconConfig, tc.Epoch)
			require.Equal(t, tc.Expected, result)
		})
	}
}
...
func TestIsSyncCommitteeEligible(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			result := tc.Share.IsSyncCommitteeEligible(networkconfig.TestNetwork.BeaconConfig, tc.Epoch)
			require.Equal(t, tc.Expected, result)
		})
	}
}
