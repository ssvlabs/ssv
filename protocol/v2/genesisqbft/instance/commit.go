...
*/
func CreateCommit(state *genesisspecqbft.State, config genesisqbft.IConfig, root [32]byte) (*genesisspecqbft.SignedMessage, error) {
...
func BaseCommitValidation(
	...
	if config.VerifySignatures() {
		if err := genesisqbfttypes.VerifyByOperators(signedCommit.Signature, signedCommit, config.GetSignatureDomainType(), genesisspectypes.QBFTSignatureType, operators); err != nil {
