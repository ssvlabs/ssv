...
func (r *AggregatorRunner) ProcessPreConsensus(ctx context.Context, logger *zap.Logger, signedMsg *spectypes.PartialSignatureMessages) error {
	...
	if err != nil {
		return observability.Errorf(span, "committee_index: %d validator_index: %d failed to submit aggregate and proof: %w", duty.CommitteeIndex, duty.ValidatorIndex, err)
	}
