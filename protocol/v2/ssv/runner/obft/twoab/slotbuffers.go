...
// validation slot-window so any envelope that could pass slot-window admission
// is checked against the ended-set.
...
//
// Duplicated from the bare-OBFT runner's pendingBuffer (Approach 2: separate
// runners; the PendingEnvelope shape differs — twoab carries ValueMsg /
// NoValueMsg too). Not thread-safe: the owning Controller serializes all
// access under its mutex (the buffer participates in cross-concern checks
// against the ended-ring under that single lock).
...
// consults it to refuse post-teardown buffering. Eviction is FIFO at
// MaxEndedSlots. Not thread-safe: serialized by the owning Controller's mutex.
