...
// submit time for parent_root and block-content validation. Strict
// per-receiver parent_root validation against a stable head snapshot
// would catch byzantine-fork blocks but also reject many honest re-orgs
// where one operator's head moved relative to the fetcher's — splitting
// honest verdicts and triggering the assumption-3 validity-divergence
// path more often than it prevents byzantine-fork acceptance. SSV's
// strategic choice (shared with QBFT) is the looser gate, explicitly
// acknowledged by the spec at docs/OBFT.md §Head-change handling
// ("SSV's implementation choice (proposer duty, OBFT and QBFT)") — the
// trade-off lives at the relay/beacon-node submit boundary instead.
//
