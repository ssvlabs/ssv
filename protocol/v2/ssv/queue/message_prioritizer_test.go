...
var messagePriorityTests = []struct {
...
}{
	{
		...
		messages: []mockMessage{
			...
			// 2.2. Pre-consensus
			mockNonConsensusMessage{Slot: 100, Type: ssvtypes.SelectionProofPartialSig},
			// 2.3. Post-consensus
			...
			// 3.2. Pre-consensus
			mockNonConsensusMessage{Slot: 101, Type: ssvtypes.SelectionProofPartialSig},
			// 3.3. Consensus
			...
			// 4.3. Pre-consensus
			mockNonConsensusMessage{Slot: 99, Type: ssvtypes.SelectionProofPartialSig},
		},
	},
	{
...
func TestMessagePrioritizer_LowerHeightCommitOutranksLowerSlotPreConsensusAtAdvancedRound(t *testing.T) {
	...
	lowerSlotPreConsensus, err := DecodeSignedSSVMessage(
		mockNonConsensusMessage{Slot: 99, Type: ssvtypes.SelectionProofPartialSig}.ssvMessage(state),
	)
	require.NoError(t, err)
...
func (m mockTimeoutMessage) ssvMessage(state *State) *spectypes.SignedSSVMessage {
	td := ssvtypes.TimeoutData{Slot: m.Slot}
