...
func ReconstructSignatures(signatures map[message.OperatorID][]byte, pub string) (*bls.Sign, error) {
	...
	for index, signature := range signatures {
		logger.Debug("reconstructing signature", zap.String("pubKey", pub),
			zap.String("index", fmt.Sprintf("%d", index)), zap.String("signature", fmt.Sprintf("%x", signature)))
		blsID := bls.ID{}
