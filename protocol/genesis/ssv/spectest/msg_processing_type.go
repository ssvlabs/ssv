...
type MsgProcessingSpecTest struct {
	...
	Messages                []*genesisspectypes.SSVMessage
...
func (test *MsgProcessingSpecTest) RunAsPartOfMultiTest(t *testing.T, logger *zap.Logger) {
	...
	v.Network = test.Runner.GetNetwork().(genesisspecqbft.Network) // TODO need to align
	...
	for _, msg := range test.Messages {
		dmsg, err := genesisqueue.DecodeGenesisSSVMessage(msg)
		if err != nil {
			lastErr = err
			continue
		}
...
func (test *MsgProcessingSpecTest) compareOutputMsgs(t *testing.T, v *validator.Validator) {
	filterPartialSigs := func(messages []*genesisspectypes.SSVMessage) []*genesisspectypes.SSVMessage {
		ret := make([]*genesisspectypes.SSVMessage, 0)
	...
	net := v.Network.(genesisspecssv.Network)
