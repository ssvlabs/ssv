...
func newRunnerDutySpecTestFromMap(t *testing.T, m map[string]interface{}) *StartNewRunnerDutySpecTest {
	...
	outputMsgs := make([]*genesisspectypes.SignedPartialSignatureMessage, 0)
	if v, ok := m["OutputMessages"].([]interface{}); ok {
		for _, msg := range v {
			byts, _ = json.Marshal(msg)
			typedMsg := &genesisspectypes.SignedPartialSignatureMessage{}
			require.NoError(t, json.Unmarshal(byts, typedMsg))
			outputMsgs = append(outputMsgs, typedMsg)
		}
	}

	ks := testingutils.KeySetForShare(&genesisspectypes.Share{Quorum: uint64(baseRunnerMap["Share"].(map[string]interface{})["Quorum"].(float64))})
...
func msgProcessingSpecTestFromMap(t *testing.T, m map[string]interface{}) *MsgProcessingSpecTest {
	...
	msgs := make([]*genesisspectypes.SSVMessage, 0)
	for _, msg := range m["Messages"].([]interface{}) {
		byts, _ = json.Marshal(msg)
		typedMsg := &genesisspectypes.SSVMessage{}
		require.NoError(t, json.Unmarshal(byts, typedMsg))
...
func fixControllerForRun(t *testing.T, logger *zap.Logger, runner runner.Runner, contr *controller.Controller, ks *testingutils.TestKeySet) *controller.Controller {
	config := qbfttesting.TestingConfig(logger, ks, genesisspectypes.BNRoleAttester)
...
func fixInstanceForRun(t *testing.T, inst *instance.Instance, contr *controller.Controller, share *genesisspectypes.Share) *instance.Instance {
...
func baseRunnerForRole(logger *zap.Logger, role genesisspectypes.BeaconRole, base *runner.BaseRunner, ks *testingutils.TestKeySet) runner.Runner {
	switch role {
