...
type Subscriber interface {
	...
	Unsubscribe(logger *zap.Logger, pk genesisspectypes.ValidatorPK) error
...
type RequestHandler func(*genesisspectypes.SSVMessage) (*genesisspectypes.SSVMessage, error)
...
func CombineRequestHandlers(handlers ...RequestHandler) RequestHandler {
	return func(ssvMessage *genesisspectypes.SSVMessage) (res *genesisspectypes.SSVMessage, err error) {
...
func (s SyncResults) String() string {
	...
	for _, m := range s {
		...
		if m.Msg.MsgType == genesisspectypes.SSVConsensusMsgType {
...
type ValidationReporting interface {
	...
	ReportValidation(logger *zap.Logger, message *genesisspectypes.SSVMessage, res MsgValidationResult)
}
