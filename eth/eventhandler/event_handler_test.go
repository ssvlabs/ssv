...
func TestHandleBlockEventsStream(t *testing.T) {
	...
	t.Run("test ClusterLiquidated event handle", func(t *testing.T) {
		...
		highestAttestation, found, err := eh.keyManager.RetrieveHighestAttestation(nil, phase0.BLSPubKey(sharePubKey))
		require.NoError(t, err)
		...
		highestProposal, found, err := eh.keyManager.RetrieveHighestProposal(nil, phase0.BLSPubKey(sharePubKey))
		require.NoError(t, err)
		require.True(t, found)
...
func requireKeyManagerDataToExist(t *testing.T, eh *EventHandler, expectedAccounts int, validatorData *testValidatorData) {
	...
	_, found, err = eh.keyManager.RetrieveHighestProposal(nil, phase0.BLSPubKey(sharePubKey))
	require.NoError(t, err)
	require.True(t, found)
}
