...
func TestHandleDecidedQuery(t *testing.T) {
	...
	for _, role := range roles {
		...
		ssvConfig := networkconfig.TestSSVConfig
...
func newStorageForTest(db basedb.Database, logger *zap.Logger, roles ...spectypes.BeaconRole) (storage.Storage, *qbftstorage.ParticipantStores) {
	sExporter, err := storage.NewNodeStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), logger, db)
	if err != nil {
		panic(err)
	}
