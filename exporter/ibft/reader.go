...
func NewIbftReadOnly(opts ReaderOptions) Reader {
	r := reader{
		logger:         opts.Logger.With(zap.String("pubKey", opts.ValidatorShare.PublicKey.SerializeToHexStr())),
...
func (r *reader) Sync() error {
	...
	hs := ibftsync.NewHistorySync(r.logger, r.validatorShare.PublicKey.Serialize(), identifier,
