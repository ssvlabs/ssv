...
// Hash: 8b96e2ad90ff76b0dc3ac251b6b58d7c99f2e39cddbdae053c453e97728ccef5
// Version: 0.1.3
package exporter
...
func (v *ValidatorDutyTrace) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o2:o3]
		num, err := ssz.DivideInt2(len(buf), 49, 13)
		if err != nil {
			return err
		}
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = v.Pre[ii].UnmarshalSSZ(buf[ii*49 : (ii+1)*49]); err != nil {
				return err
			}
		}
	}
	...
	// Field (3) 'Post'
	{
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = v.Post[ii].UnmarshalSSZ(buf[ii*49 : (ii+1)*49]); err != nil {
				return err
			}
		}
	}
...
func (c *CommitteeDutyTrace) UnmarshalSSZ(buf []byte) error {
	...
	// Field (2) 'Post'
	{
		...
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if c.Post[indx] == nil {
				c.Post[indx] = new(CommitteeMessageTrace)
			}
...
func (c *CommitteeMessageTrace) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Validators'
	{
		...
		tempValidators := make([]uint64, num)
		for ii := 0; ii < num; ii++ {
			tempValidators[ii] = ssz.UnmarshallUint64(buf[ii*8 : (ii+1)*8])
		}
		c.Validators = make([]phase0.ValidatorIndex, num)
		for ii, val := range tempValidators {
...
func (r *RoundTrace) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (3) 'Prepares'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.Prepares) * 49

	// Offset (4) 'Commits'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.Commits) * 49
...
func (r *RoundTrace) UnmarshalSSZ(buf []byte) error {
	...
	// Field (3) 'Prepares'
	{
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = r.Prepares[ii].UnmarshalSSZ(buf[ii*49 : (ii+1)*49]); err != nil {
				return err
			}
		}
	}
	...
	// Field (4) 'Commits'
	{
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = r.Commits[ii].UnmarshalSSZ(buf[ii*49 : (ii+1)*49]); err != nil {
				return err
			}
		}
	}
...
func (r *RoundTrace) SizeSSZ() (size int) {
	...
	// Field (3) 'Prepares'
	size += len(r.Prepares) * 49

	// Field (4) 'Commits'
	size += len(r.Commits) * 49
...
func (r *RoundChangeTrace) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'Round'
	dst = ssz.MarshalUint8(dst, r.Round)
	...
	dst = ssz.MarshalUint8(dst, r.PreparedRound)

	// Offset (5) 'PrepareMessages'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.PrepareMessages) * 49
...
func (r *RoundChangeTrace) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Round'
	r.Round = ssz.UnmarshallUint8(buf[0:1])

	// Field (1) 'BeaconRoot'
	copy(r.BeaconRoot[:], buf[1:33])

	// Field (2) 'Signer'
	r.Signer = spectypes.OperatorID(ssz.UnmarshallUint64(buf[33:41]))

	// Field (3) 'ReceivedTime'
	r.ReceivedTime = ssz.UnmarshallUint64(buf[41:49])

	// Field (4) 'PreparedRound'
	r.PreparedRound = ssz.UnmarshallUint8(buf[49:50])

	// Offset (5) 'PrepareMessages'
	if o5 = ssz.ReadOffset(buf[50:54]); o5 > size {
		return ssz.ErrOffset
	}
	...
	// Field (5) 'PrepareMessages'
	{
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = r.PrepareMessages[ii].UnmarshalSSZ(buf[ii*49 : (ii+1)*49]); err != nil {
				return err
			}
		}
	}
...
func (m *MessageTrace) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'Round'
	dst = ssz.MarshalUint8(dst, m.Round)
...
func (m *MessageTrace) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Round'
	m.Round = ssz.UnmarshallUint8(buf[0:1])

	// Field (1) 'BeaconRoot'
	copy(m.BeaconRoot[:], buf[1:33])

	// Field (2) 'Signer'
	m.Signer = spectypes.OperatorID(ssz.UnmarshallUint64(buf[33:41]))

	// Field (3) 'ReceivedTime'
	m.ReceivedTime = ssz.UnmarshallUint64(buf[41:49])
