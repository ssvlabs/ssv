...
func TestSharesStorage(t *testing.T) {
	...
	t.Run("KV_reuse_works", func(t *testing.T) {
		storageDuplicate, _, err := NewSharesStorage(networkconfig.TestNetwork.BeaconConfig, storage.db, []byte("test"))
		require.NoError(t, err)
...
func (t *testStorage) open(logger *zap.Logger) error {
	var err error
	t.Shares, t.ValidatorStore, err = NewSharesStorage(networkconfig.TestNetwork.BeaconConfig, t.db, []byte("test"))
	if err != nil {
		return err
	}
