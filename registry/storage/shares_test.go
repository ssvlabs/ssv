...
func TestValidatorStoreThroughSharesStorage(t *testing.T) {
	...
	for _, withReopen := range []bool{true, false} {
		t.Run(fmt.Sprintf("withReopen=%t", withReopen), func(t *testing.T) {
			...
			require.Equal(t, updatedMetadata.Index, updatedShare.ValidatorIndex)
			require.Equal(t, updatedMetadata.Balance, updatedShare.Balance)
			require.Equal(t, updatedMetadata.Status, updatedShare.Status)
			require.Equal(t, updatedMetadata.ActivationEpoch, updatedShare.ActivationEpoch)
