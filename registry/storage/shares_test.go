...
func TestSharesStorage(t *testing.T) {
	...
	t.Run("KV_reuse_works", func(t *testing.T) {
		storageDuplicate, _, err := NewSharesStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), storage.db, []byte("test"))
		require.NoError(t, err)
...
func generateRandomValidatorStorageShare(splitKeys map[uint64]*bls.SecretKey) *Share {
	...
	return &Share{
		...
		DomainType:          networkconfig.TestSSVConfig.DomainType,
...
func (t *testStorage) open(logger *zap.Logger) error {
	var err error
	t.Shares, t.ValidatorStore, err = NewSharesStorage(networkconfig.NewNetwork(networkconfig.TestBeaconConfig, networkconfig.TestSSVConfig), t.db, []byte("test"))
	if err != nil {
		return err
	}
