...
func init() {
	threshold.Init()
	genesistypes.SetDefaultDomain(genesisspectypes.DomainType(networkconfig.TestNetwork.DomainType()))
}
...
func TestShareDeletionHandlesValidatorStoreCorrectly(t *testing.T) {
	...
	testWithStorageReopen(t, func(t *testing.T, storage *testStorage, reopen func(t *testing.T)) {
		...
		require.NoError(t, storage.Shares.Save(nil, validatorShare))
		validatorShare.DomainType = spectypes.DomainType(genesistypes.GetDefaultDomain())
		reopen(t)
		...
		requireValidatorStoreIntegrity(t, storage.ValidatorStore, []*types.SSVShare{validatorShare})
		...
		requireValidatorStoreIntegrity(t, storage.ValidatorStore, []*types.SSVShare{})
	})
...
func TestValidatorStoreThroughSharesStorage(t *testing.T) {
	testWithStorageReopen(t, func(t *testing.T, storage *testStorage, reopen func(t *testing.T)) {
		...
		requireValidatorStoreIntegrity(t, storage.ValidatorStore, []*types.SSVShare{testShare})
...
func TestShareStorage_MultipleCommittees(t *testing.T) {
	testWithStorageReopen(t, func(t *testing.T, storage *testStorage, reopen func(t *testing.T)) {
		shares := map[phase0.ValidatorIndex]*types.SSVShare{}
		saveAndVerify := func(s ...*types.SSVShare) {
		...
		deleteAndVerify := func(share *types.SSVShare) {
...
func requireEqualShare(t *testing.T, expected, actual *types.SSVShare, msgAndArgs ...any) {
...
func requireEqualShares(t *testing.T, expected, actual []*types.SSVShare, msgAndArgs ...any) {
	require.Equal(t, len(expected), len(actual), msgAndArgs...)

	// Sort shares by validator pubkey.
	expectedSorted := make([]*types.SSVShare, len(expected))
	copy(expectedSorted, expected)
	slices.SortFunc(expectedSorted, func(a, b *types.SSVShare) int {
		return strings.Compare(string(a.ValidatorPubKey[:]), string(b.ValidatorPubKey[:]))
	})
	actualSorted := make([]*types.SSVShare, len(actual))
	copy(actualSorted, actual)
	slices.SortFunc(actual, func(a, b *types.SSVShare) int {
		return strings.Compare(string(a.ValidatorPubKey[:]), string(b.ValidatorPubKey[:]))
	})
...
func generateRandomShare(splitKeys map[uint64]*bls.SecretKey) (*ssvtypes.SSVShare, *bls.SecretKey) {
...
func fakeNonParticipatingShare(index phase0.ValidatorIndex, pk spectypes.ValidatorPK) *ssvtypes.SSVShare {
	share := fakeParticipatingShare(index, pk, []uint64{1, 2, 3, 4})
	share.Metadata.BeaconMetadata.Status = eth2apiv1.ValidatorStatePendingQueued
	share.Metadata.BeaconMetadata.ActivationEpoch = math.MaxUint64
