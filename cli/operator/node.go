...
var StartNodeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		networkCfg := networkconfig.NewNetwork(beaconCfg, ssvCfg)
		...
		nodeStorage, err := operatorstorage.NewNodeStorage(networkCfg, logger, db)
		if err != nil {
		...
		if usingSSVSigner {
			...
			localKeyManager, err := ekm.NewLocalKeyManager(logger, db, networkCfg, operatorPrivKey)
			if err != nil {
		...
		cfg.SSVOptions.ValidatorOptions.NetworkConfig = networkCfg
		...
		slotTickerProvider := func() slotticker.SlotTicker {
			return slotticker.New(logger, slotticker.Config{
				SlotDuration: networkCfg.SlotDuration(),
				GenesisTime:  networkCfg.GenesisTime(),
			})
		}
