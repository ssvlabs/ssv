...
//
//		networkPrivateKey := cfg.P2pNetworkConfig.NetworkPrivateKey
//		pubKey, err := p2pcommons.ECDSAPrivToInterface(networkPrivateKey)
//		if err != nil {
//			panic(err)
//		}
...
//					err = p2pNetwork.Broadcast(msg.SSVMessage)
//					logger.Info("send>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
//					if err != nil {
//						println("here>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
//						println(err.Error())
