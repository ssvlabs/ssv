...
func roundChangeValidator() func(t *testing.T, committee int, actual *protocolstorage.StoredInstance) {
	return func(t *testing.T, committee int, actual *protocolstorage.StoredInstance) {
		...
		require.Equal(t, qbft.Round(2), actual.State.Round, "round not matching")
		...
		require.Equal(t, spectypes.OperatorID(2), actual.State.ProposeContainer.Msgs[2][0].Signers[0]) //on second round proposer is 2,
	}
}
