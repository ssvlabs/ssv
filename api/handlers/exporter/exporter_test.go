...
func TestExporterTraceDecideds(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not found case - GetCommitteeDecideds returns empty result + not found error",
			request: map[string]any{
				"from":    100,
				"to":      100,
				"roles":   []string{"ATTESTER"},
			...
			validateResp: func(t *testing.T, rec *httptest.ResponseRecorder) {
				...
				require.Contains(t, resp.Errors, dutytracer.ErrNotFound.Error())
			},
		},
		{
...
func TestExporterCommitteeTraces(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not found case - GetCommitteeDuty returns empty result + not found error",
			request: map[string]any{
				"from":         100,
				"to":           100,
				"committeeIDs": []string{"0eb9655577d1af04ff5d382848be15d1454b04838713bfb1ac209808fe3e9f7f"},
		...
		{
			name: "not found case - GetCommitteeDuties returns empty result + not found error",
			request: map[string]any{
				"from": 100,
				"to":   100,
			},
			...
			validateResp: func(t *testing.T, rec *httptest.ResponseRecorder) {
				...
				require.Contains(t, resp.Errors, dutytracer.ErrNotFound.Error(), fmt.Sprintf("errors: %+v", resp.Errors))
				require.Contains(t, resp.Errors, estore.ErrNotFound.Error())
			},
		},
		{
...
func TestExporterValidatorTraces(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not found case - validator duty returns not found",
			request: map[string]any{
				"from":    100,
				"to":      200,
				"roles":   []string{"PROPOSER"},
			...
			expectedStatus: http.StatusOK,
			validateResp: func(t *testing.T, rec *httptest.ResponseRecorder) {
				// Unknown indices produce empty data and a not-found error entry
				var resp validatorTraceResponse
				require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
				require.Len(t, resp.Data, 0)
				require.Contains(t, resp.Errors, dutytracer.ErrNotFound.Error())
