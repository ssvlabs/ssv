...
func (e *Exporter) TraceDecideds(w http.ResponseWriter, r *http.Request) error {
	...
	// if we don't have a single valid participant and we have at least one meaningful error, return an error
	if len(participants) == 0 && hasErrorsOtherThanNotFound(errs) {
