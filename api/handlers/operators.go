package handlers

import (
	"github.com/ssvlabs/ssv/api"
	"github.com/ssvlabs/ssv/message/crawler"
	"net/http"
)

type OperatorPeer struct {
	PeerID     string `json:"peer_id"`
	OperatorID uint64 `json:"operator_id"`
	Version    string `json:"version"`
	Subnets    string `json:"subnets"`
}

type OperatorToPeerJSON struct {
	Data []*OperatorPeer `json:"data"`
}

type Operators struct {
}

func (h Operators) List(w http.ResponseWriter, r *http.Request) error {
	opt := &OperatorToPeerJSON{make([]*OperatorPeer, 0)}
	crawler.PeerIDtoSignerMtx.Lock()
	defer crawler.PeerIDtoSignerMtx.Unlock()
	for k, v := range crawler.PeerIDtoSigner {
		opt.Data = append(opt.Data, &OperatorPeer{PeerID: k.String(), OperatorID: v.OpID, Version: v.Version, Subnets: v.Subnets})
	}
	return api.Render(w, r, opt)
}
